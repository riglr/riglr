use riglr_cross_chain_tools::{
    bridge::execute_cross_chain_bridge,
...
async fn mock_fetch_bridge_quote(
    ...
    if from_chain == "solana" && !from_address.chars().all(|c| c.is_alphanumeric()) {
        return Err(CrossChainError::InvalidRoute("Invalid Solana address".to_string()));
    }
    
    if from_chain != "solana" && !from_address.starts_with("0x") {
        return Err(CrossChainError::InvalidRoute("Invalid EVM address".to_string()));
    }
    
    // Validate amount
    let _amount_parsed: u64 = amount.parse()
...
async fn mock_bridge_transaction(
    ...
    if from_chain == "solana" {
        ...
        signer.sign_and_send_evm_transaction(tx_request).await
            .map_err(|e| CrossChainError::BridgeExecutionError(e.to_string()))
    }
}
