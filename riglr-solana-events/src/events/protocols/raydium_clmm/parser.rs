...
impl RaydiumClmmEventParser {
    ...
    fn parse_swap_instruction(
        ...
        let amount = read_u64_le(data, 0).ok()?;
        let other_amount_threshold = read_u64_le(data, 8).ok()?;
        let sqrt_price_limit_x64 = read_u128_le(data, 16).ok()?;
        let is_base_input = read_u8_le(data, 32).ok()? == 1;
    ...
    fn parse_create_pool_instruction(
        ...
        let sqrt_price_x64 = read_u128_le(data, 0).ok()?;
        let _open_time = read_u64_le(data, 16).ok()?; // Not used in event
    ...
    fn parse_open_position_v2_instruction(
        ...
        Some(Box::new(RaydiumClmmOpenPositionV2Event {
            ...
            tick_array_upper_start_index: read_i32_le(data, 12).ok()?,
            liquidity: read_u128_le(data, 16).ok()?,
            amount0_max: read_u64_le(data, 32).ok()?,
            amount1_max: read_u64_le(data, 40).ok()?,
            with_metadata: read_u8_le(data, 48).ok()? == 1,
            base_flag: read_option_bool(data, &mut 49).ok()?,
            payer: accounts[0],
    ...
    fn parse_increase_liquidity_v2_instruction(
        ...
        let liquidity = read_u128_le(data, 0).ok()?;
        let amount0_max = read_u64_le(data, 16).ok()?;
        let amount1_max = read_u64_le(data, 24).ok()?;
        let base_flag = read_option_bool(data, &mut 32).ok()?;
    ...
    fn parse_decrease_liquidity_v2_instruction(
        ...
        let liquidity = read_u128_le(data, 0).ok()?;
        let amount0_min = read_u64_le(data, 16).ok()?;
        let amount1_min = read_u64_le(data, 24).ok()?;
