...
impl RaydiumAmmV4EventParser {
    ...
    fn parse_swap_base_input_instruction(
        ...
        let amount_in = read_u64_le(data, 0).ok()?;
        let minimum_amount_out = read_u64_le(data, 8).ok()?;
    ...
    fn parse_swap_base_output_instruction(
        ...
        let max_amount_in = read_u64_le(data, 0).ok()?;
        let amount_out = read_u64_le(data, 8).ok()?;
    ...
    fn parse_deposit_instruction(
        ...
        let max_coin_amount = read_u64_le(data, 0).ok()?;
        let max_pc_amount = read_u64_le(data, 8).ok()?;
        let base_side = read_u64_le(data, 16).ok()?;
    ...
    fn parse_initialize2_instruction(
        ...
        let nonce = read_u8_le(data, 0).ok()?;
        let open_time = read_u64_le(data, 1).ok()?;
        let init_pc_amount = read_u64_le(data, 9).ok()?;
        let init_coin_amount = read_u64_le(data, 17).ok()?;
