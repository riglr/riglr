...
async fn test_validation_pipeline() {
    ...
    let has_duplicate_error = results[2].errors.iter().any(|e| {
        matches!(e, ValidationError::Duplicate { .. })
    });
...
fn test_batch_parser_with_mixed_protocols() {
    ...
    rt.block_on(async {
        ...
        let protocols: std::collections::HashSet<_> = events.iter()
            .map(|e| e.protocol_type())
            .collect();
...
fn test_zero_copy_event_data_access() {
    ...
    assert_eq!(owned_event.get_parsed_data::<TestData>().unwrap(), &test_data);
...
fn test_parser_performance_characteristics() {
    ...
    rt.block_on(async {
        ...
        for size in sizes {
            ...
            let result = raydium_parser.parse_from_slice(&data, metadata.clone()).unwrap();
