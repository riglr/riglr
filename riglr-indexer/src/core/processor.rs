...
use crate::core::{ServiceContext, ServiceLifecycle, EventProcessing, ProcessingStats, ComponentHealth, HealthStatus};
...
impl EventProcessor {
    ...
    pub async fn new(
        ...
        info!("Initializing event processor with {} workers", config.workers);

        let worker_semaphore = Arc::new(Semaphore::new(config.workers));

        // Create batch processor
        let batch_processor = BatchProcessor::new(
            config.batch_config.max_size,
            config.batch_config.max_age,
        );
            ...
            match self.store_event(&*event).await {
                ...
                Err(e) => {
                    ...
                    if let Some(delay) = backoff.next_backoff() {
                        warn!("Retrying event processing in {:?} (attempt {}/{})", delay, attempts, max_attempts);
                        sleep(delay).await;
                    } else {
                        error!("Backoff timer expired, giving up after {} attempts", attempts);
    ...
    async fn store_event(&self, event: &dyn Event) -> IndexerResult<()> {
        ...
        if matches!(self.context.config.storage.cache.backend, crate::config::CacheBackend::Redis) {
            if let Err(e) = self.context.store.cache_event(event.id(), &storage_event, Some(Duration::from_secs(3600))).await {
...
impl ServiceLifecycle for EventProcessor {
    async fn start(&mut self) -> IndexerResult<()> {
        ...
        info!("Event processor started with {} workers", self.config.workers);
        Ok(())
    }
    ...
    async fn stop(&mut self) -> IndexerResult<()> {
        ...
        while *self.active_workers.read().await > 0 {
            if start_time.elapsed() > timeout_duration {
                warn!("Timeout waiting for workers to complete, {} workers still active", 
                      *self.active_workers.read().await);
                break;
            }
    ...
    async fn health(&self) -> IndexerResult<HealthStatus> {
        ...
        let message = if healthy {
            format!("Processor healthy: {:.2} events/sec, {:.1}ms avg latency", 
                   stats.events_per_second, stats.avg_latency_ms)
        } else {
            format!("Processor unhealthy: {:.2}% error rate", 
        ...
        components.insert("processor".to_string(), if healthy {
...
impl EventProcessing for EventProcessor {
    ...
    async fn process_batch(&self, events: Vec<Box<dyn Event>>) -> IndexerResult<()> {
        ...
        let processing_time = start_time.elapsed();
        info!("Batch processing completed in {:?}: {} success, {} errors", 
        ...
        self.context.metrics.record_histogram(
            "indexer_batch_size",
            (success_count + error_count) as f64,
        );

        if error_count > 0 {
            self.context.metrics.increment_counter("indexer_batch_errors");
            
            // If more than half the batch failed, consider it a batch failure
            if error_count > success_count {
                return Err(IndexerError::Processing(ProcessingError::ValidationFailed {
                    reason: format!("Batch processing failed: {}/{} events failed", 
