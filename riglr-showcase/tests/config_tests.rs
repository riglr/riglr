...
fn test_config_from_env_with_custom_values() {
    ...
    assert_eq!(config.network.get_rpc_url(1), Some("https://custom.ethereum.com".to_string()));
...
fn test_config_empty_env_values() {
    ...
    assert!(validation_result.is_ok(), "Validation should pass with empty strings");
...
fn test_config_special_characters_in_env() {
    ...
    assert_eq!(config.network.get_rpc_url(1), Some("wss://ethereum.com/ws".to_string()));
    assert_eq!(config.providers.twitter_bearer_token, Some("Bearer abc123!@#$%".to_string()));
    assert_eq!(config.database.neo4j_url.as_deref(), Some("neo4j+s://user:pass@neo4j.com:7687"));
    assert_eq!(config.database.redis_url, "redis://user:pass@redis.com:6379/0");
    assert_eq!(config.providers.openai_api_key.as_deref(), Some("sk-123abc!@#"));
...
fn test_config_localhost_urls() {
    ...
    assert_eq!(config.network.get_rpc_url(1), Some("http://127.0.0.1:8545".to_string()));
    assert_eq!(config.database.neo4j_url.as_deref(), Some("bolt://localhost:7687"));
...
fn test_config_network_specific_urls() {
    ...
    assert_eq!(config.network.solana_rpc_url, "https://api.devnet.solana.com");
    assert_eq!(config.network.get_rpc_url(1), Some("https://rpc.ankr.com/eth_goerli".to_string()));
