...
fn interactive_setup(project_name: Option<String>) -> Result<ProjectConfig> {
    ...
    let name = if let Some(name) = project_name {
        ...
        Input::with_theme(&theme)
            ...
            .validate_with(validation::validate_project_name)
            .interact_text()?
    };
    ...
    let mut chains = vec![];
    for (idx, &selected) in selected_chains.iter().enumerate() {
        if selected {
            chains.push(match idx {
                0 => "solana".to_string(),
    ...
    // Author information
    let author_name = Input::with_theme(&theme)
        .with_prompt("Author name")
        .default(whoami::realname())
        .interact_text()?;

    let author_email = Input::with_theme(&theme)
        .with_prompt("Author email")
        .validate_with(validation::validate_email)
        .interact_text()?;
    ...
    Ok(ProjectConfig {
        ...
        include_tests: selected_features[9],
        include_docs: selected_features[11],
    })
}
