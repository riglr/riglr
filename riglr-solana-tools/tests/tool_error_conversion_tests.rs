...
fn test_rpc_error_conversions() {
    ...
    assert_eq!(tool_error.to_string(), "Retriable error: connection timeout occurred");
    ...
    assert!(tool_error.is_rate_limited());
    assert!(tool_error.is_retriable());
    assert_eq!(tool_error.to_string(), "Rate limited: 429 Too Many Requests");
...
fn test_invalid_address_conversion() {
    ...
    assert!(!tool_error.is_retriable());
    assert!(!tool_error.is_rate_limited());
    assert_eq!(tool_error.to_string(), "Permanent error: Invalid base58 encoding");
}

#[test]
fn test_transaction_error_conversions() {
    // Test insufficient funds becomes permanent
    let insufficient_funds = SolanaToolError::Transaction("insufficient funds for transaction".to_string());
    let tool_error: ToolError = insufficient_funds.into();
    assert!(!tool_error.is_retriable());
    assert!(!tool_error.is_rate_limited());
    assert_eq!(tool_error.to_string(), "Permanent error: insufficient funds for transaction");
...
fn test_generic_error_conversion() {
    ...
    assert!(tool_error.is_retriable());
    assert!(!tool_error.is_rate_limited());
    assert_eq!(tool_error.to_string(), "Retriable error: Something went wrong");
}
...
fn test_case_insensitive_rate_limit_detection() {
    ...
    for case in test_cases {
        ...
        assert!(tool_error.is_rate_limited(), "Case '{}' should be rate limited", case);
        assert!(tool_error.is_retriable(), "Rate limited errors should also be retriable");
    }
}
...
fn test_case_insensitive_permanent_error_detection() {
    ...
    for case in test_cases {
        ...
        assert!(!tool_error.is_retriable(), "Case '{}' should not be retriable", case);
        assert!(!tool_error.is_rate_limited(), "Case '{}' should not be rate limited", case);
    }
}
...
fn test_retriable_error_detection() {
    ...
    for (error_type, message) in test_cases {
        ...
        let tool_error: ToolError = solana_error.into();
        assert!(tool_error.is_retriable(), "{} error '{}' should be retriable", error_type, message);
        assert!(!tool_error.is_rate_limited(), "{} error '{}' should not be rate limited", error_type, message);
    }
}
