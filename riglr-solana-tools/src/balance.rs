...
pub use __riglr_tool_get_sol_balance::GetSolBalanceTool;
pub use __riglr_tool_get_spl_token_balance::GetSplTokenBalanceTool;

// Manual Tool trait implementations for balance tools
#[async_trait::async_trait]
impl riglr_core::Tool for GetSolBalanceTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid address parameter: {}", e))
        })?;

        let result = get_sol_balance(address, &*self.context).await?;
...
#[async_trait::async_trait]
impl riglr_core::Tool for GetSplTokenBalanceTool {
    type Args = serde_json::Value;
    type Output = riglr_core::JobResult;
    type Error = riglr_core::ToolError;
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid mintAddress parameter: {}", e))
        })?;

        let result = get_spl_token_balance(owner_address, mint_address, &*self.context).await?;
        riglr_core::JobResult::success(&result).map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Failed to serialize result: {}", e))
        })
    }
...
impl riglr_core::Tool for GetMultipleBalancesTool {
        ...
        serde_json::json!({
            ...
            "properties": {
                "addresses": {
                    ...
                    "description": "Array of Solana wallet addresses to check"
                }
            },
            "required": ["addresses"]
        })
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid addresses parameter: {}", e))
        })?;
