...
pub use __riglr_tool_perform_jupiter_swap::PerformJupiterSwapTool;

// Manual Tool trait implementations for swap tools
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "inputMint": {
                    "type": "string",
                    "description": "Source token mint address to swap from"
                },
                "outputMint": {
                    "type": "string",
                    "description": "Destination token mint address to swap to"
                },
                "amount": {
                    "type": "integer",
                ...
                "onlyDirectRoutes": {
                    "type": "boolean",
                    "description": "If true, only consider direct swap routes"
                },
                "jupiterApiUrl": {
                    "type": ["string", "null"],
                    "description": "Optional custom Jupiter API endpoint URL"
                }
            },
            "required": ["inputMint", "outputMint", "amount", "slippageBps", "onlyDirectRoutes"]
        })
    }

    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        let input_mint: String = serde_json::from_value(
            args.get("inputMint")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid inputMint parameter: {}", e))
        })?;
        let output_mint: String = serde_json::from_value(
            args.get("outputMint")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid outputMint parameter: {}", e))
        })?;
        ...
        let slippage_bps: u16 = serde_json::from_value(
            args.get("slippageBps")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid slippageBps parameter: {}", e))
        })?;
        let only_direct_routes: bool = serde_json::from_value(
            args.get("onlyDirectRoutes")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid onlyDirectRoutes parameter: {}",
                e
            ))
        })?;
        let jupiter_api_url: Option<String> = serde_json::from_value(
            args.get("jupiterApiUrl")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid jupiterApiUrl parameter: {}",
                e
            ))
        })?;
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "baseMint": {
                    "type": "string",
                    "description": "Token address to price (the token being quoted)"
                },
                "quoteMint": {
                    "type": "string",
                    "description": "Token address to price against (usually USDC or SOL)"
                },
                "jupiterApiUrl": {
                    "type": ["string", "null"],
                    "description": "Optional custom Jupiter API endpoint URL"
            ...
            "required": ["baseMint", "quoteMint"]
        })
    }

    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        let base_mint: String = serde_json::from_value(
            args.get("baseMint")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid baseMint parameter: {}", e))
        })?;
        let quote_mint: String = serde_json::from_value(
            args.get("quoteMint")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid quoteMint parameter: {}", e))
        })?;
        let jupiter_api_url: Option<String> = serde_json::from_value(
            args.get("jupiterApiUrl")
                .cloned()
        ...
        let result =
            get_token_price(base_mint, quote_mint, jupiter_api_url, &*self.context).await?;
        riglr_core::JobResult::success(&result).map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Failed to serialize result: {}", e))
        })
    }
...
#[async_trait::async_trait]
impl riglr_core::Tool for PerformJupiterSwapTool {
    type Args = serde_json::Value;
    type Output = riglr_core::JobResult;
    type Error = riglr_core::ToolError;
        ...
        serde_json::json!({
            ...
            "properties": {
                ...
                "useVersionedTransaction": {
                    "type": "boolean",
                    "description": "Whether to use versioned transactions"
                }
            },
            "required": ["inputMint", "outputMint", "amount", "slippageBps", "useVersionedTransaction"]
        })
    }
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let use_versioned_transaction: bool = serde_json::from_value(
            args.get("useVersionedTransaction")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid useVersionedTransaction parameter: {}",
                e
            ))
        })?;
