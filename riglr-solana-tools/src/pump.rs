...
pub use __riglr_tool_buy_pump_token::BuyPumpTokenTool;
pub use __riglr_tool_deploy_pump_token::DeployPumpTokenTool;
pub use __riglr_tool_get_pump_token_info::GetPumpTokenInfoTool;
pub use __riglr_tool_get_trending_pump_tokens::GetTrendingPumpTokensTool;
pub use __riglr_tool_sell_pump_token::SellPumpTokenTool;

// Manual Tool trait implementations for pump tools (similar to balance.rs)
#[async_trait::async_trait]
impl riglr_core::Tool for DeployPumpTokenTool {
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "initialBuySol": {
                    "type": "number",
                    "description": "Optional initial buy amount in SOL"
                }
            },
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let symbol: String = serde_json::from_value(
            args.get("symbol")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid symbol parameter: {}", e))
        })?;
        ...
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid description parameter: {}", e))
        })?;
        let image_url: Option<String> = args
            .get("imageUrl")
            .and_then(|v| serde_json::from_value(v.clone()).ok());
        let initial_buy_sol: Option<f64> = args
            .get("initialBuySol")
            .and_then(|v| serde_json::from_value(v.clone()).ok());
...
#[async_trait::async_trait]
impl riglr_core::Tool for BuyPumpTokenTool {
    type Args = serde_json::Value;
    type Output = riglr_core::JobResult;
    type Error = riglr_core::ToolError;
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        let token_mint: String = serde_json::from_value(
            args.get("tokenMint")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid tokenMint parameter: {}", e))
        })?;
        let sol_amount: f64 = serde_json::from_value(
            args.get("solAmount")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid solAmount parameter: {}", e))
        })?;
        let slippage_percent: Option<f64> = args
            .get("slippagePercent")
            .and_then(|v| serde_json::from_value(v.clone()).ok());

        let result =
            buy_pump_token(token_mint, sol_amount, slippage_percent, &*self.context).await?;
        riglr_core::JobResult::success(&result).map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Failed to serialize result: {}", e))
        })
    }
...
#[async_trait::async_trait]
impl riglr_core::Tool for SellPumpTokenTool {
    type Args = serde_json::Value;
    type Output = riglr_core::JobResult;
    type Error = riglr_core::ToolError;
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let token_amount: u64 = serde_json::from_value(
            args.get("tokenAmount")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Invalid tokenAmount parameter: {}", e))
        })?;
        let slippage_percent: Option<f64> = args
            .get("slippagePercent")
            .and_then(|v| serde_json::from_value(v.clone()).ok());
        ...
        let result =
            sell_pump_token(token_mint, token_amount, slippage_percent, &*self.context).await?;
        riglr_core::JobResult::success(&result).map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Failed to serialize result: {}", e))
        })
    }
...
#[async_trait::async_trait]
impl riglr_core::Tool for GetPumpTokenInfoTool {
    type Args = serde_json::Value;
    type Output = riglr_core::JobResult;
    type Error = riglr_core::ToolError;
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let result = get_pump_token_info(token_mint, &*self.context).await?;
        riglr_core::JobResult::success(&result).map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Failed to serialize result: {}", e))
        })
    }
...
#[async_trait::async_trait]
impl riglr_core::Tool for GetTrendingPumpTokensTool {
    type Args = serde_json::Value;
    type Output = riglr_core::JobResult;
    type Error = riglr_core::ToolError;
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "limit": {
                    "type": "number",
                    "description": "Optional limit for number of tokens to return"
                }
            }
        })
    }
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        let limit: Option<u32> = args
            .get("limit")
            .and_then(|v| serde_json::from_value(v.clone()).ok());

        let result = get_trending_pump_tokens(limit, &*self.context).await?;
        riglr_core::JobResult::success(&result).map_err(|e| {
            riglr_core::ToolError::permanent_string(format!("Failed to serialize result: {}", e))
        })
    }
