...
use riglr_streams::core::{
    EventHandler, HandlerExecutionMode, MetricsCollector, StreamHealth, StreamManager,
};
...
impl EventHandler for MockHandler {
    ...
    async fn handle(
        &self,
        _event: Arc<dyn std::any::Any + Send + Sync>,
...
async fn test_metrics_collection() {
    ...
    metrics
        .record_handler_execution("test-handler", 5.0, true)
        .await;
    metrics
        .record_handler_execution("test-handler", 10.0, false)
        .await;
...
async fn test_metrics_percentiles() {
    ...
    for i in 1..=100 {
        metrics
            .record_stream_event("test-stream", i as f64, 100)
            .await;
    }
    ...
    // Check percentiles
    assert!(
        stream_metrics.p50_processing_time_ms >= 49.0
            && stream_metrics.p50_processing_time_ms <= 51.0
    );
    assert!(
        stream_metrics.p95_processing_time_ms >= 94.0
            && stream_metrics.p95_processing_time_ms <= 96.0
    );
    assert!(
        stream_metrics.p99_processing_time_ms >= 98.0
            && stream_metrics.p99_processing_time_ms <= 100.0
    );
