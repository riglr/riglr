...
use riglr_streams::external::binance::{BinanceStream, BinanceConfig, BinanceEventData};
        ...
        Ok(Ok(event)) => {
            ...
            match &event.data {
                ...
                BinanceEventData::OrderBook(book) => {
                    println!("Order book: {} with {} bids, {} asks", 
                ...
                if let BinanceEventData::OrderBook(book) = &event.data {
                    ...
                    if book.bids.len() > 0 && book.asks.len() > 0 {
                        ...
                        println!("BTC Order Book - Bid: {} @ {}, Ask: {} @ {}", 
                                 best_bid[1], best_bid[0], 
                                 best_ask[1], best_ask[0]);
                    }
                }
            }
            Ok(Err(e)) => println!("Error receiving event: {:?}", e),
...
async fn test_binance_stream_metrics() {
    ...
    println!("Binance stream metrics - Events processed: {}, Errors: {}", 
        ...
        match timeout(Duration::from_secs(2), rx.recv()).await {
            Ok(Ok(event)) => {
                if let BinanceEventData::Kline(kline) = &event.data {
                    klines_received += 1;
                    println!("BTC 1m Kline - Open: {}, High: {}, Low: {}, Close: {}", 
        ...
        match timeout(Duration::from_millis(500), rx.recv()).await {
            Ok(Ok(event)) => {
                if let BinanceEventData::Trade(trade) = &event.data {
                    trades_received += 1;
                    println!("BTC Trade - Price: {}, Quantity: {}, Buyer is maker: {}", 
