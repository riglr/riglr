...
async fn test_multiple_protocols() {
    ...
    let config = MockConfig {
        ...
        event_kinds: vec![EventKind::Swap, EventKind::Liquidity, EventKind::Transfer],
        max_events: Some(15),
    };
...
async fn test_stream_restart() {
    ...
    assert!(
        events_after_restart > 0,
        "Should receive events after restart"
    );
...
async fn test_infinite_stream() {
    ...
    assert!(
        (15..=25).contains(&events),
        "Should receive approximately 20 events, got {}",
        events
    );
