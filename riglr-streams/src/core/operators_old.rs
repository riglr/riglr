    ...
    E: Event + Clone + 'static,
{
    ...
    fn metadata_mut(
        ...
        // BatchEvent is a composite and doesn't have its own mutable metadata
        Err(riglr_events_core::error::EventError::Generic {
            message:
                "BatchEvent: mutable metadata not supported - use individual events within batch"
                    .to_string(),
        })
    }
    ...
    fn to_json(&self) -> riglr_events_core::error::EventResult<serde_json::Value> {
        let events_json: Result<Vec<_>, _> = self.events.iter().map(|e| e.to_json()).collect();
    ...
    E: Event + Clone + 'static,
{
    ...
    fn metadata_mut(
        ...
        Err(riglr_events_core::error::EventError::Generic {
            message: "metadata_mut not supported for ScanEvent".to_string(),
        })
    }
...
impl Event for TypeErasedEvent {
    ...
    fn metadata_mut(
        ...
        Err(riglr_events_core::error::EventError::Generic {
            message: "metadata_mut not supported for TypeErasedEvent".to_string(),
        })
    }
    ...
    E2: Event + Clone + 'static,
{
    ...
    fn metadata_mut(
        ...
        if let Some(ref mut first) = self.first {
            ...
            Err(riglr_events_core::error::EventError::Generic {
                message: "metadata_mut not supported for CombinedEvent with no events".to_string(),
            })
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_batch_event_metadata_mut_when_called_should_return_error() {
        ...
        let result = batch.metadata_mut();
        assert!(result.is_err());
        if let Err(e) = result {
            assert!(e
                .to_string()
                .contains("BatchEvent: mutable metadata not supported"));
        }
    }
    ...
    #[test]
    fn test_scan_event_metadata_mut_when_called_should_return_error() {
        ...
        let result = scan_event.metadata_mut();
        assert!(result.is_err());
        if let Err(e) = result {
            assert!(e
                .to_string()
                .contains("metadata_mut not supported for ScanEvent"));
        }
    }
    ...
    #[test]
    fn test_type_erased_event_metadata_mut_when_called_should_return_error() {
        let event = Arc::new(TestEvent::new("test".to_string(), 42)) as Arc<dyn Event>;
        let mut erased = TypeErasedEvent::new(event, "test_stream".to_string());

        let result = erased.metadata_mut();
        assert!(result.is_err());
        if let Err(e) = result {
            assert!(e
                .to_string()
                .contains("metadata_mut not supported for TypeErasedEvent"));
        }
    }
