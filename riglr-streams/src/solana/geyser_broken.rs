...
use crate::core::{StreamMetadata, StreamedEvent, IntoStreamedEvent};
...
pub struct SolanaGeyserStream {
    ...
    event_tx: broadcast::Sender<Arc<StreamedEvent<Box<dyn UnifiedEvent>>>>,
...
pub type SolanaStreamEvent = StreamedEvent<Box<dyn UnifiedEvent>>;

// SolanaStreamEvent is now a type alias, no need for manual trait implementations
...
impl Stream for SolanaGeyserStream {
    type Event = StreamedEvent<Box<dyn UnifiedEvent>>;
...
impl SolanaGeyserStream {
        ...
        tokio::spawn(async move {
            ...
            while running.load(Ordering::Relaxed) {
                ...
                while retry_count < max_retries && !connected {
                    info!("Attempting to connect to Solana WebSocket: {} (attempt {}/{})", 
                    ...
                    match connect_async(&url).await {
                        Ok((ws_stream, _)) => {
                            ...
                            // Send subscription message for each program ID
                            for (idx, program_id) in program_ids.iter().enumerate() {
                                ...
                                if let Err(e) = write.send(Message::Text(subscribe_msg.to_string())).await {
                                    error!("Failed to send subscription for program {}: {}", program_id, e);
                                    retry_count += 1;
                        ...
                        Err(e) => {
                            ...
                            if retry_count < max_retries {
                                let delay = tokio::time::Duration::from_secs(2_u64.pow(retry_count));
    ...
    fn parse_websocket_message_to_events(
        ...
        sequence_number: u64
    ) -> Option<Vec<StreamedEvent<Box<dyn UnifiedEvent>>>> {
        ...
        if let Some(transaction_data) = json.get("params")?.get("result")?.get("transaction") {
