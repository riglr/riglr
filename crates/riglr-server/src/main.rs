...
use riglr_server::{start_axum, Config as ServerConfig};
...
impl SignerFactory for DevSignerFactory {
    async fn create_signer(
        ...
        let config = riglr_config::SolanaNetworkConfig::new(network, rpc_url);
...
mod tests {
    ...
    async fn test_echo_agent_prompt_stream_when_valid_input_should_return_stream() {
        ...
        assert_eq!(chunks.first().unwrap().as_ref().unwrap(), "start:");
        assert_eq!(chunks.get(1).unwrap().as_ref().unwrap(), " ");
        assert_eq!(chunks.get(2).unwrap().as_ref().unwrap(), "test");
    }
    ...
    async fn test_echo_agent_prompt_stream_when_empty_input_should_return_stream() {
        ...
        assert_eq!(chunks.first().unwrap().as_ref().unwrap(), "start:");
        assert_eq!(chunks.get(1).unwrap().as_ref().unwrap(), " ");
        assert_eq!(chunks.get(2).unwrap().as_ref().unwrap(), "");
    }
    ...
    async fn test_echo_agent_prompt_stream_when_special_characters_should_return_stream() {
        ...
        assert_eq!(chunks.first().unwrap().as_ref().unwrap(), "start:");
        assert_eq!(chunks.get(1).unwrap().as_ref().unwrap(), " ");
        assert_eq!(chunks.get(2).unwrap().as_ref().unwrap(), "hello\nworld");
    }
