...
// expect() usage in test context is acceptable with descriptive messages
...
async fn test_3_1_solana_devnet_to_indexer_pipeline() -> Result<()> {
    ...
    let database_available = env::var(DATABASE_URL).is_ok();
    ...
    let simulated_rpc_available = env::var(SOLANA_RPC_URL).is_ok();
    ...
    let test_program =
        Pubkey::from_str("11111111111111111111111111111111").expect("Valid test program ID");
    ...
    println!(
        ...
        stream_manager.list_streams().len()
    );

    // Create a channel for receiving events
    let (tx, mut rx) = mpsc::channel::<GenericEvent>(100);
    ...
    assert!(
        event_json
            ...
            .as_str()
            .expect("transaction_hash should be string")
            == test_tx_signature
    );
    assert!(
        event_json
            ...
            .unwrap_or(&serde_json::Value::Null)
            .as_str()
            .expect("amount should be string")
            == "1000000"
    );
...
/// Create a mock transaction log for testing
const fn create_mock_transaction_log() -> &'static str {
    r#"{
...
/// Extract transfer information from parsed transaction
fn extract_transaction_info(parsed_tx: &serde_json::Value) -> (String, u64, i64) {
    let signature = parsed_tx
        .get("signature")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown")
        .to_string();
...
/// Create an event from parsed transaction data
fn create_event_from_transaction(
    parsed_tx: &serde_json::Value,
    signature: &str,
    slot: u64,
    block_time: i64,
) -> GenericEvent {
    GenericEvent::with_source(
        uuid::Uuid::new_v4().to_string(),
        EventKind::Transfer,
        ...
        serde_json::json!({
            ...
            "accounts": parsed_tx.get("accounts").unwrap_or(&serde_json::Value::Null),
            "logs": parsed_tx.get("logMessages").unwrap_or(&serde_json::Value::Null),
            "status": parsed_tx.get("confirmationStatus").unwrap_or(&serde_json::Value::Null),
        }),
    )
}

/// Add enrichment metadata to an event
fn add_enrichment_metadata(event: &mut GenericEvent) -> Result<()> {
...
fn verify_basic_event_properties(event: &GenericEvent) -> Result<()> {
    ...
    assert_eq!(
        event_json
            ...
            .and_then(|data| data.get("transaction_hash"))
            .and_then(|v| v.as_str()),
        Some("TestSignature123")
    );
    assert_eq!(
        event_json
            .get("data")
            .and_then(|data| data.get("block_number"))
            .and_then(serde_json::Value::as_u64),
        Some(12_345_678)
    );
    assert_eq!(
        event_json
            .get("data")
            .and_then(|data| data.get("blockchain"))
            .and_then(|v| v.as_str()),
        Some("solana")
    );
    assert!(event
...
/// Test additional enrichment functionality
fn test_additional_enrichment(event: &GenericEvent) -> Result<()> {
...
async fn test_3_2_event_parsing_and_enrichment() -> Result<()> {
    ...
    // Create and parse mock transaction log
    let mock_log = create_mock_transaction_log();
    let parsed_tx: serde_json::Value = serde_json::from_str(mock_log)?;

    // Extract transaction information
    let (signature, slot, block_time) = extract_transaction_info(&parsed_tx);

    // Create event from parsed data
    let mut event = create_event_from_transaction(&parsed_tx, &signature, slot, block_time);

    // Add enrichment metadata
    add_enrichment_metadata(&mut event)?;

    // Verify basic properties
    verify_basic_event_properties(&event)?;

    // Test additional enrichment
    test_additional_enrichment(&event)?;
...
async fn test_3_3_stream_error_handling_and_recovery() -> Result<()> {
    ...
    let initial_streams = stream_manager.list_streams();
    ...
    let final_streams = stream_manager.list_streams();
    assert_eq!(
        initial_streams.len(),
        final_streams.len(),
