...
use riglr_streams::core::{
    ...
    ComposableStream, IntoDynamicStreamed, Stream,
};
use std::sync::Arc;
...
#[tokio::main]
async fn main() -> Result<(), Box<dyn CoreError>> {
    // Initialize logging
    ...
    let trading_pipeline = build_trading_pipeline();
...
fn build_trading_pipeline() -> TradingPipeline {
    ...
    let price_feed = mock_stream
        .map(|_event| {
            let price_data = PriceData::new("SOL/USDT".to_string(), 100.0, 1_000_000.0);
...
async fn monitor_pipeline(pipeline: TradingPipeline) -> Result<(), Box<dyn CoreError>> {
        ...
        loop {
            ...
            if let Ok(event) = recv_result {
                ...
                if let Some(transaction) = event.inner().as_any().downcast_ref::<Transaction>() {
                    execute_trades(vec![transaction.clone()]);
                }
            } else {
                break;
            }
    ...
    // Keep running
    let ctrl_c_result = ctrl_c().await;
    ctrl_c_result?;
...
impl RiskState {
    fn _evaluate_opportunity(&mut self, opp: ArbitrageOpportunity) -> TradingDecision {
        // Risk checks
...
fn _prepare_transaction(decision: &TradingDecision) -> Transaction {
    Transaction {
        _to: "dex_address".to_string(),
...
fn execute_trades(batch: Vec<Transaction>) {
...
fn log_dex_activity(event: &DexTrade) {
    info!(
...
fn update_dashboard(data: &PriceData) {
    info!(
...
impl DexTrade {
    fn new(protocol: &ProtocolType, pair: String, price: f64, volume: f64) -> Self {
        Self {
            metadata: EventMetadata::new(pair.clone(), EventKind::Swap, format!("{protocol:?}")),
            pair,
            price,
