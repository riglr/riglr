...
impl CircuitBreaker {
    ...
    #[must_use]
    pub const fn with_failure_threshold(mut self, threshold: u64) -> Self {
    ...
    #[must_use]
    pub const fn with_success_threshold(mut self, threshold: u64) -> Self {
...
impl RetryPolicy {
    ...
    pub fn calculate_delay(&self, attempt: u32) -> Duration {
        let mut delay = match self.backoff {
            BackoffStrategy::Exponential { factor } => {
                // Cast is safe for reasonable retry attempts (< 2^31)
                #[expect(
                    clippy::cast_possible_wrap,
                    reason = "Cast is safe for reasonable retry attempts (< 2^31)"
                )]
                let power = attempt as i32;
                self.initial_delay.mul_f64(factor.powi(power))
            }
            BackoffStrategy::Fixed => self.initial_delay,
            BackoffStrategy::Linear { increment } => {
                #[expect(
                    clippy::arithmetic_side_effects,
                    reason = "Arithmetic overflow not expected in normal retry scenarios"
                )]
                let result = self.initial_delay + increment.mul_f64(f64::from(attempt));
                result
            }
        };
    ...
    /// - The underlying operation returns an error that cannot be retried
    /// - Backoff delays are interrupted by system shutdown
    #[expect(clippy::arithmetic_side_effects)]
...
#[cfg(test)]
#[expect(clippy::unwrap_used, clippy::expect_used, clippy::float_cmp)]
mod tests {
    use super::*;
    ...
    fn test_retry_policy_default() {
        ...
        match policy.backoff {
            ...
            _ => unreachable!("Expected exponential backoff"),
        }
    }
    ...
    async fn test_retry_policy_execute_failure_after_max_retries() {
        ...
        assert_eq!(
            result.expect_err("Expected failure after max retries"),
            "persistent failure"
        );
