...
impl HealthMonitor {
    ...
    fn check_component_health(
        ...
        #[expect(
            ...
            reason = "Precision loss is acceptable for metric approximation"
        )]
...
mod tests {
    ...
    fn test_health_monitor_with_thresholds() {
        ...
        let monitor = HealthMonitor::new(stream_manager).with_thresholds(custom_thresholds);
    ...
    fn test_check_component_health_when_not_connected_should_return_down() {
        ...
        assert_eq!(alerts.first().unwrap().component, "test_stream");
        assert_eq!(alerts.first().unwrap().message, "Stream is not connected");
    }
    ...
    fn test_check_component_health_when_old_events_should_return_degraded() {
        ...
        assert!(alerts.first().unwrap().message.contains("No events for"));
    }
    ...
    fn test_check_component_health_when_high_error_count_should_return_unhealthy() {
        ...
        assert_eq!(alerts.len(), 1);
        assert_eq!(alerts.first().unwrap().severity, AlertSeverity::Error);
        assert!(alerts
            .first()
            .unwrap()
            .message
            .contains("High error count: 10"));
    }
    ...
    fn test_check_component_health_when_duration_since_fails_should_use_zero() {
        ...
        assert_eq!(
            component_health.metrics.get("event_age_seconds").unwrap(),
            &0.0
        );
    }
    ...
    fn test_health_snapshot_serialization() {
        ...
        let _serialized = serde_json::to_string(&snapshot).unwrap();
    }
    ...
    fn test_component_health_serialization() {
        ...
        let _serialized = serde_json::to_string(&component).unwrap();
    }
    ...
    fn test_health_alert_serialization() {
        ...
        let _serialized = serde_json::to_string(&alert).unwrap();
    }
    ...
    fn test_copy_implementations() {
        ...
        let status = HealthStatus::Healthy;
        let _ = status;
        let _ = status; // Should work due to Copy trait

        let severity = AlertSeverity::Warning;
        let _ = severity;
        let _ = severity; // Should work due to Copy trait
    }
}
