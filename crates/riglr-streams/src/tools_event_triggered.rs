...
use crate::core::{DynamicStreamed, EventHandler, StreamManager};
...
pub trait StreamingTool: Send + Sync {
    /// Execute the tool
    async fn execute(&self, event: &dyn Event) -> Result<(), Box<dyn CoreError + Send + Sync>>;
...
impl<T: StreamingTool> Debug for EventTriggeredTool<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        f.debug_struct("EventTriggeredTool")
            .field("tool", &format!("Arc<{}>", type_name::<T>()))
            .field(
                "conditions",
...
impl<T: StreamingTool + 'static> EventTriggeredTool<T> {
    ...
    #[must_use]
    pub const fn with_combinator(mut self, combinator: ConditionCombinator) -> Self {
...
impl<T: StreamingTool> Debug for EventTriggerBuilder<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        f.debug_struct("EventTriggerBuilder")
            .field("tool", &type_name::<T>().to_string())
            .field("name", &self.name)
            .field(
                "conditions",
                &format!("[{} conditions]", self.conditions.len()),
...
impl<T: StreamingTool + 'static> EventTriggerBuilder<T> {
    ...
    /// Set combinator
    #[must_use]
    pub const fn combinator(mut self, combinator: ConditionCombinator) -> Self {
        self.combinator = combinator;
        self
    }
...
// Test code uses expect() and expect_err() for descriptive error messages in test assertions following established project patterns
mod tests {
    use super::*;
    ...
    impl StreamingTool for MockStreamingTool {
        async fn execute(&self, _event: &dyn Event) -> Result<(), Box<dyn StdError + Send + Sync>> {
    ...
    #[derive(Debug)]
    struct NonEventType {
        #[expect(dead_code)] // Used in negative test cases
        data: String,
    }
    ...
    async fn test_execute_with_event_when_tool_fails_should_return_error() {
        ...
        assert!(result
            .expect_err("Expected tool execution to fail in test")
            .to_string()
            .contains("Mock tool failed"));
    }
    ...
    async fn test_execution_count_increment_with_concurrent_executions() {
        ...
        for handle in handles {
            assert!(handle
                .await
                .expect("Expected concurrent execution to complete successfully")
                .is_ok());
        }
