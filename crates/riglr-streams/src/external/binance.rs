...
impl BinanceStream {
            ...
            let connector = ResilientWebSocketBuilder::new(
                ...
                move || {
                    ...
                    Box::pin(async move {
                        ...
                        let final_result = connect_result.map(|(ws, _)| ws).map_err(|e| {
                ...
                |_write| Box::pin(async move { Ok::<(), Box<dyn Error + Send + Sync>>(()) }),
                |text: String, sequence_number: u64| {
                    serde_json::from_str::<serde_json::Value>(&text)
                        .map_or_else(|_| None, |json| Self::parse_message(&json, sequence_number))
                },
            )
    ...
    fn parse_message(json: &serde_json::Value, sequence_number: u64) -> Option<DynamicStreamed> {
        // Check event type
        ...
        let metadata = EventMetadata::new(id, kind, "binance-ws".to_string());
        ...
        Some(DynamicStreamed::from_event(
...
mod tests {
    ...
    fn test_binance_config_custom() {
        ...
        assert_eq!(config.streams.first().unwrap(), "btcusdt@ticker");
        assert_eq!(config.streams.get(1).unwrap(), "ethusdt@depth");
        assert!(config.testnet);
    ...
    async fn test_binance_stream_start_when_already_running_should_error() {
        ...
        let error = result.expect_err("Expected AlreadyRunning error result to be an error");
        match error {
    ...
    fn test_binance_event_data_unknown_serialization() {
        ...
        let event_data = BinanceEventData::Unknown(unknown_json);
        ...
        if let Some(binance_event) = event.inner().as_any().downcast_ref::<BinanceStreamEvent>() {
            match binance_event.data {
                BinanceEventData::Ticker(ref ticker) => {
                    ...
                    assert_eq!(ticker.event_time, 1_234_567_890);
                }
                _ => unreachable!("Expected Ticker event"),
            }
        } else {
            unreachable!("Expected BinanceStreamEvent");
        }
        ...
        if let Some(binance_event) = event.inner().as_any().downcast_ref::<BinanceStreamEvent>() {
            match binance_event.data {
                BinanceEventData::OrderBook(ref order_book) => {
                    ...
                    assert_eq!(order_book.event_time, 1_234_567_890);
                }
                _ => unreachable!("Expected OrderBook event"),
            }
        } else {
            unreachable!("Expected BinanceStreamEvent");
        }
        ...
        if let Some(binance_event) = event.inner().as_any().downcast_ref::<BinanceStreamEvent>() {
            match binance_event.data {
                BinanceEventData::Trade(ref trade) => {
                    ...
                    assert_eq!(trade.trade_time, 1_234_567_890);
                    assert!(trade.is_buyer_maker);
                }
                _ => unreachable!("Expected Trade event"),
            }
        } else {
            unreachable!("Expected BinanceStreamEvent");
        }
        ...
        if let Some(binance_event) = event.inner().as_any().downcast_ref::<BinanceStreamEvent>() {
            match binance_event.data {
                BinanceEventData::Kline(ref kline) => {
                    ...
                    assert_eq!(kline.kline.open_time, 1_234_567_890);
                ...
                _ => unreachable!("Expected Kline event"),
            }
        } else {
            unreachable!("Expected BinanceStreamEvent");
        }
    ...
    fn test_parse_message_unknown_event() {
        ...
        let event = BinanceStream::parse_message(&json, 5).unwrap();
        ...
        if let Some(binance_event) = event.inner().as_any().downcast_ref::<BinanceStreamEvent>() {
            match binance_event.data {
                ...
                _ => unreachable!("Expected Unknown event"),
            }
        } else {
            unreachable!("Expected BinanceStreamEvent");
        }
