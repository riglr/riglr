...
impl EventKindMatcher {
    ...
    #[must_use]
    pub const fn new(event_kinds: Vec<EventKind>) -> Self {
        Self { event_kinds }
    }
...
impl EventCondition for EventKindMatcher {
    async fn matches(&self, event: &(dyn Any + Send + Sync)) -> bool {
        as_event(event).is_some_and(|event_ref| self.event_kinds.contains(event_ref.kind()))
    }
...
impl EventCondition for SourceMatcher {
    async fn matches(&self, event: &(dyn Any + Send + Sync)) -> bool {
        as_event(event)
            .is_some_and(|event_ref| self.sources.contains(&event_ref.source().to_string()))
    }
...
impl TimestampRangeMatcher {
    ...
    #[must_use]
    pub const fn new(min_timestamp: Option<SystemTime>, max_timestamp: Option<SystemTime>) -> Self {
        Self {
            min_timestamp,
            max_timestamp,
        }
    ...
    #[must_use]
    pub const fn after(timestamp: SystemTime) -> Self {
        Self {
    ...
    #[must_use]
    pub const fn before(timestamp: SystemTime) -> Self {
        Self {
            min_timestamp: None,
...
impl<F> Debug for CustomCondition<F>
where
    F: Fn(&(dyn Any + Send + Sync)) -> bool + Send + Sync,
{
...
pub trait EventMatcher {
    ...
    #[must_use]
    fn timestamp_range(min: Option<SystemTime>, max: Option<SystemTime>) -> Box<dyn EventCondition>
    where
        Self: Sized,
    {
...
mod tests {
    ...
    fn create_test_event(
        ...
        // Safe cast: u64 seconds is clamped to i64::MAX to prevent wrap-around
        #[expect(clippy::arithmetic_side_effects, clippy::cast_possible_wrap)]
        let timestamp = chrono::Utc::now()
            - chrono::Duration::seconds(timestamp_offset_secs.min(i64::MAX as u64) as i64);
        let metadata =
    ...
    async fn test_event_kind_matcher_description_when_multiple_kinds_should_format_correctly() {
        ...
        let matcher = EventKindMatcher::new(kinds);
    ...
    async fn test_source_matcher_description_when_multiple_sources_should_format_correctly() {
        let sources = vec!["source1".to_string(), "source2".to_string()];
        let matcher = SourceMatcher::new(sources);
        let description = matcher.description();
