...
impl StreamHealth {
    ...
    /// Healthy stream health constant
...
impl<S: Stream + Debug> fmt::Debug for DynamicStreamWrapper<S> {
...
mod tests {
    ...
    impl TestStream {
        ...
        fn send_event(
            &self,
            event: MockEvent,
        ) -> Result<usize, broadcast::error::SendError<Arc<DynamicStreamed>>> {
            ...
            let dynamic_event = DynamicStreamed::from_event(Box::new(event), metadata);
    ...
    fn test_stream_event_stream_metadata_when_some_should_return_metadata() {
        ...
        assert_eq!(
            metadata
                .expect("Expected metadata to be Some in test")
                .stream_source,
            "test_stream"
        );
    }
    ...
    fn test_stream_event_received_at_when_metadata_exists_should_return_time() {
        ...
        assert!(received_at.expect("Expected received_at to be Some in test") <= SystemTime::now());
    }
