...
use crate::core::{
    DynamicStreamed, IntoDynamicStreamed, Stream, StreamError, StreamHealth, StreamMetadata,
};
...
impl MockStream {
    ...
    fn start_event_generation(&self) {
            ...
            while running.load(Ordering::Relaxed) {
                ...
                if let Some(event) = generated_event {
...

                    {
                        ...
                        events_generated = events_generated.saturating_add(1);
                    }
...
mod tests {
    ...
    async fn test_mock_stream_generation() {
        ...
        loop {
            let recv_result = timeout(Duration::from_secs(2), rx.recv()).await;
        ...
        stream.stop().await.expect("Failed to stop mock stream");
    }
    ...
    fn test_generate_mock_event_with_single_kind() {
        ...
        let event = event.expect("Expected mock event to be generated");
        assert_eq!(event.kind(), &EventKind::Transaction);
        assert!(event.id().contains("Transaction"));
        assert!(event.id().contains("42"));

        let stream_meta = event.stream_metadata().expect("Expected stream metadata");
    ...
    fn test_mock_event_metadata_mut() {
        let mut event = MockEvent::new("test-id".to_string(), EventKind::Transaction);
        {
            let metadata = event
                .metadata_mut()
                .expect("Failed to get mutable metadata");
    ...
    fn test_mock_event_to_json() {
        ...
        let json = json_result.expect("Failed to serialize event to JSON");
    ...
    async fn test_mock_stream_with_none_max_events() {
        ...
        loop {
            let recv_result = timeout(Duration::from_millis(50), rx.recv()).await;
            if let Ok(Ok(_event)) = recv_result {
                events_received = events_received.saturating_add(1);
    ...
    async fn test_mock_stream_high_event_rate() {
        ...
        loop {
            let recv_result = timeout(Duration::from_secs(1), rx.recv()).await;
            if let Ok(Ok(_event)) = recv_result {
                events_received = events_received.saturating_add(1);
    ...
    async fn test_mock_stream_low_event_rate() {
        ...
        assert!(result.is_ok());
        assert!(result.expect("Timeout waiting for event").is_ok());
        ...
        loop {
            ...
            if let Ok(Ok(event)) = recv_result {
                if let Some(seq) = event
                    .stream_metadata()
                    .expect("Expected stream metadata")
                    .sequence_number
                {
