            ...
            loop {
                ...
                if let Ok(event) = recv_result {
                    ...
                    if let Ok(json) = json_result {
                        if let Some(price) = json.get("price").and_then(serde_json::Value::as_f64) {
...
pub struct VWAPStream<S> {
    ...
    price_volume_pairs: Arc<RwLock<Vec<(f64, f64, Instant)>>>,
}
                ...
                if let Ok(event) = recv_result {
                    ...
                    if let Ok(json) = json_result {
                        if let (Some(price), Some(volume)) = (
                            json.get("price").and_then(serde_json::Value::as_f64),
                            json.get("volume").and_then(serde_json::Value::as_f64),
                        ) {
                            let now = Instant::now();
