...
pub struct GroupedStream<S, F, K> {
    inner: S,
    key_fn: Arc<F>,
    groups: Arc<DashMap<K, broadcast::Sender<Arc<DynamicStreamed>>>>,
}
...
where
    S: Stream,
    F: Fn(&DynamicStreamed) -> K + Send + Sync + 'static,
    K: Clone + Eq + Hash + Send + Sync + 'static,
{
    ...
    pub fn get_group(&self, key: &K) -> Option<broadcast::Receiver<Arc<DynamicStreamed>>> {
...
pub struct WindowedStream<S> {
    ...
    windows: Arc<RwLock<Vec<Arc<DynamicStreamed>>>>,
}
