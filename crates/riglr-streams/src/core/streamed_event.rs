...
impl<T: Event + Clone> StreamedEvent<T> {
    ...
    #[must_use]
    pub const fn new(inner: T, stream_metadata: StreamMetadata) -> Self {
        Self {
            inner,
            stream_metadata,
        }
    ...
    #[must_use]
    pub const fn stream_meta(&self) -> &StreamMetadata {
...
impl fmt::Debug for DynamicStreamed {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DynamicStreamed")
...
// Implement Event by delegating to the inner event
impl Event for DynamicStreamed {
    fn id(&self) -> &str {
        self.inner.id()
    }
...
// Implement StreamEvent to provide streaming-specific functionality
impl StreamEvent for DynamicStreamed {
    fn stream_metadata(&self) -> Option<&StreamMetadata> {
        Some(&self.stream_metadata)
    }
}
...
/// Helper trait for creating `DynamicStreamed` from Box<dyn Event>
pub trait IntoDynamicStreamed {
    /// Wrap this boxed event with streaming metadata
    #[must_use]
    fn with_stream_metadata(self, metadata: StreamMetadata) -> DynamicStreamed;

    /// Wrap this boxed event with default streaming metadata
    #[must_use]
    fn with_default_stream_metadata(self, source: impl Into<String>) -> DynamicStreamed;
}

impl IntoDynamicStreamed for Box<dyn Event> {
    fn with_stream_metadata(self, metadata: StreamMetadata) -> DynamicStreamed {
        DynamicStreamed::from_event(self, metadata)
    }
...
mod tests {
    ...
    fn test_streamed_event_inner_mut() {
        ...
        inner_mut
            .metadata_mut()
            .expect("metadata_mut should succeed for mock event in test")
    ...
    fn test_streamed_event_metadata_mut() {
        ...
        streamed
            .metadata_mut()
            .expect("metadata_mut should succeed for streamed event in test")
            .id = "new_id".to_string();
    ...
    fn test_dynamic_streamed_event_from_event() {
        ...
        let dynamic = DynamicStreamed::from_event(boxed_event, metadata.clone());
    ...
    fn test_dynamic_streamed_event_inner() {
        ...
        let boxed_event: Box<dyn Event> = Box::new(event.clone());
        let dynamic = DynamicStreamed::from_event(boxed_event, metadata);
    ...
    fn test_dynamic_streamed_event_debug() {
        ...
        assert!(debug_str.contains("DynamicStreamed"));
    ...
    fn test_dynamic_streamed_event_metadata_mut() {
        ...
        let boxed_event: Box<dyn Event> = Box::new(event);
        let mut dynamic = DynamicStreamed::from_event(boxed_event, metadata);

        // Modify metadata through Event trait
        dynamic
            .metadata_mut()
            .expect("metadata_mut should succeed for dynamic event in test")
            .id = "modified_dynamic".to_string();
    ...
    fn test_dynamic_streamed_event_as_any() {
        ...
        // Should be able to downcast back to DynamicStreamed
        assert!(any_ref.downcast_ref::<DynamicStreamed>().is_some());
    }
    ...
    fn test_dynamic_streamed_event_as_any_mut() {
        ...
        // Should be able to downcast back to mutable DynamicStreamed
        assert!(any_mut.downcast_mut::<DynamicStreamed>().is_some());
    }
