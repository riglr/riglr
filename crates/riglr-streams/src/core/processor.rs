...
use crate::core::config::{Backpressure, BackpressureStrategy, Batch};
...
impl<E> Window<E> {
    ...
    #[must_use]
    const fn new(id: u64, start_time: Instant) -> Self {
        Self {
            end_time: None,
            events: Vec::new(),
            id,
...
impl<E> WindowManager<E> {
    ...
    pub fn add_event(&mut self, event: E) -> Vec<Window<E>> {
        ...
        match self.window_type {
            ...
            WindowType::Session { timeout } => {
                self.handle_session_window(event, timeout, now, &mut completed_windows);
            }
            WindowType::Count { size } => {
                self.handle_count_window(event, size, now, &mut completed_windows);
            }
        }
    ...
    /// Handle tumbling window event processing
    fn handle_tumbling_window(
        &mut self,
        event: E,
        duration: Duration,
        now: Instant,
        system_now: SystemTime,
        completed_windows: &mut Vec<Window<E>>,
    ) {
        let window_id =
            system_now
                ...
                .map_or(0, |duration_since_epoch| {
                    ...
                    let window_id = duration_since_epoch.as_secs() / duration.as_secs();
                    window_id
                });
    ...
    /// Handle sliding window event processing
    fn handle_sliding_window(
        &mut self,
        event: E,
        size: Duration,
        step: Duration,
        ...
        let window_start =
            system_now
                ...
                .map_or(0, |duration| {
                    #[expect(clippy::arithmetic_side_effects)]
                    let result = (duration.as_secs() / step_secs) * step_secs;
                    result
                });
        ...
        let cutoff = now.checked_sub(size).unwrap_or(now);
        let expired_ids: Vec<_> = self
            .active_windows
            .iter()
            .filter(|&(_, window)| window.start_time < cutoff)
    ...
    /// Handle session window event processing
    fn handle_session_window(
        ...
        let window_id = self.next_windowid;
        let window = self.active_windows.entry(window_id).or_insert_with(|| {
            self.next_windowid = self.next_windowid.saturating_add(1);
        ...
        let inactive_cutoff = now.checked_sub(timeout).unwrap_or(now);
        let inactive_ids: Vec<_> = self
            .active_windows
            .iter()
            .filter(|&(_, window)| window.events.is_empty() || window.start_time < inactive_cutoff)
            .map(|(&id, _)| id)
            .collect();
    ...
    /// Handle count-based window event processing
    fn handle_count_window(
        &mut self,
        event: E,
        size: usize,
        now: Instant,
    ...
    fn cleanup_old_windows(&mut self) {
        ...
        let cutoff = cleanup_start
            .checked_sub(Duration::from_secs(3600))
            .unwrap_or(cleanup_start); // Keep windows for 1 hour max
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    /// # Errors
    ///
    /// Returns error if checkpointing to durable storage fails
    pub fn checkpoint(&self) -> StreamResult<()> {
    ...
    #[must_use]
    pub fn remove_state(&self, key: &K) -> Option<S> {
        self.state_store.remove(key).map(|(_, value)| value)
    }

    /// Updates the state for the given key using the provided function
    pub fn update_state<F, R>(&self, key: K, update_fn: F) -> R
    where
        F: FnOnce(Option<&S>) -> (S, R),
    {
        let current_value = self.state_store.get(&key).map(|r| r.value().clone());
        let (new_state, result) = update_fn(current_value.as_ref());
...
impl FlowController {
    ...
    #[must_use]
    pub fn new(config: Backpressure) -> Self {
        let semaphore = Arc::new(Semaphore::new(config.channel_size));

        Self {
            config,
            drop_count: Arc::new(RwLock::new(0)),
    ...
    /// # Errors
    ///
    /// Returns error if acquiring a semaphore permit fails or if backpressure limits are exceeded
        ...
        match self.config.strategy {
            ...
            BackpressureStrategy::Drop => {
                if current_size >= self.config.high_watermark() {
                    let current_drop_count = {
                        // Scope to ensure RwLockWriteGuard is dropped early
                        let mut drop_count = self.drop_count.write().await;
                        *drop_count = drop_count.saturating_add(1);
                        *drop_count // Extract value before dropping guard
                    }; // RwLockWriteGuard is automatically dropped here

                    if current_drop_count % 100 == 0 {
                        warn!("Dropped {} events due to backpressure", current_drop_count);
                    }
            ...
            BackpressureStrategy::Retry {
                ...
                while attempts < max_attempts {
                    ...
                    let delay =
                        Duration::from_millis(base_wait_ms.saturating_mul(1 << attempts.min(5))); // Exponential backoff
                    sleep(delay).await;
                }
                ...
                Err(StreamError::Backpressure {
                    message: format!("Failed after {max_attempts} retry attempts"),
                })
            }
            ...
            BackpressureStrategy::Adaptive => {
                ...
                if let Ok(Ok(permit)) =
                    timeout(Duration::from_millis(10), self.semaphore.acquire()).await
                {
                    permit.forget();
                    return Ok(());
                }
    ...
    pub async fn update_queue_size(&self, delta: i32) {
        let mut size = self.queue_size.write().await;
        if delta > 0 {
            if let Ok(delta_usize) = usize::try_from(delta) {
                *size = size.saturating_add(delta_usize);
            }
        } else if delta < 0 {
            #[expect(clippy::arithmetic_side_effects)]
            if let Ok(decrease) = usize::try_from(-delta) {
                *size = size.saturating_sub(decrease);
            }
        }
    }
    ...
    E: Clone,
{
    ...
    #[must_use]
    pub const fn new(patterns: Vec<EventPattern<E>>, max_history: usize) -> Self {
        Self {
            patterns,
    ...
    fn matches_pattern(&self, pattern: &EventPattern<E>, current_event: &E, now: Instant) -> bool {
        match pattern {
            ...
            EventPattern::Sequence(predicates) => {
                ...
                for &(ref event, _timestamp) in &self.event_history {
            ...
            &EventPattern::Within {
                ...
                let recent_events: Vec<_> = self
                    ...
                    .map(|&(ref event, timestamp)| (event.clone(), timestamp))
                    .collect();

                // Create a temporary matcher with recent events
                let mut temp_matcher = Self {
                    patterns: vec![pattern.as_ref().clone()],
            ...
            &EventPattern::Any(ref patterns) => patterns
                .iter()
                .any(|p| self.matches_pattern(p, current_event, now)),

            &EventPattern::All(ref patterns) => patterns
                .iter()
...
mod tests {
    ...
    async fn test_stateful_processor() {
        ...
        let state = processor.get_state(&"key1".to_string());
    ...
    fn test_batch_processor() {
        ...
        assert_eq!(
            batch
                .expect("Batch should be created when reaching batch size")
                .len(),
            3
        );
    }
    ...
    async fn test_stateful_processor_remove_state() {
        ...
        // Insert some state
        processor.update_state("key1".to_string(), |_| (42, ()));

        // Remove and verify
        let removed = processor.remove_state(&"key1".to_string());
    ...
    async fn test_stateful_processor_checkpoint_not_ready() {
        ...
        let result = processor.checkpoint();
        assert!(result.is_ok());
    }
    ...
    fn test_batch_processor_disabled() {
        ...
        assert_eq!(
            result
                .expect("Single event should be returned when batching is disabled")
                .len(),
            1
        );
    }
    ...
    fn test_batch_processor_timeout_flush() {
        ...
        assert_eq!(
            result
                .expect("Batch should be flushed due to timeout")
                .len(),
            2
        );
    }
    ...
    fn test_pattern_matcher_single() {
        let patterns = vec![EventPattern::Single(|e: &TestEvent| e.id == 1)];
        ...
        let match_results = matcher.match_event(event1);
        assert_eq!(match_results, vec![0]);

        let event2 = TestEvent {
            id: 2,
            event_type: "test".into(),
        };
        let match_results = matcher.match_event(event2);
    ...
    fn test_pattern_matcher_sequence_empty() {
        ...
        let match_results = matcher.match_event(event);
        assert!(match_results.is_empty());
    }
    ...
    fn test_pattern_matcher_any() {
        let patterns = vec![EventPattern::Any(vec![
            EventPattern::Single(|e: &TestEvent| e.id == 1),
            EventPattern::Single(|e: &TestEvent| e.id == 2),
        ])];
        let mut matcher = PatternMatcher::new(patterns, 10);
        ...
        let match_results = matcher.match_event(event3);
        assert!(match_results.is_empty());
    }
    ...
    fn test_pattern_matcher_all() {
        let patterns = vec![EventPattern::All(vec![
            EventPattern::Single(|e: &TestEvent| e.id > 0),
            EventPattern::Single(|e: &TestEvent| e.event_type == "test"),
        ])];
        let mut matcher = PatternMatcher::new(patterns, 10);
    ...
    fn test_pattern_matcher_within() {
        let inner_pattern = EventPattern::Single(|e: &TestEvent| e.id == 1);
