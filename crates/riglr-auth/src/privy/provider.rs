...
pub struct PrivyProvider {
    ...
    config: PrivyConfig,
    #[cfg(feature = "caching")]
    user_cache: Arc<Mutex<LruCache<String, (PrivyUserData, Instant)>>>,
}
...
impl PrivyProvider {
    ...
    fn create_client(config: &PrivyConfig) -> reqwest::Client {
        ...
        if let Ok(app_id_header) = config.app_id.parse() {
            headers.insert("privy-app-id", app_id_header);
        }
        if let Ok(content_type_header) = "application/json".parse() {
            headers.insert("Content-Type", content_type_header);
        }
        if let Ok(auth_header) = basic.parse() {
    ...
    async fn fetch_user_data(&self, user_id: &str) -> AuthResult<PrivyUserData> {
        #[cfg(feature = "caching")]
        {
            ...
            if self.config.enable_cache {
                if let Ok(mut cache) = self.user_cache.lock() {
                    if let Some(&(ref data, timestamp)) = cache.get(user_id) {
        ...
        if !response.status().is_success() {
            ...
            return Err(AuthError::ApiError(format!(
                "Failed to fetch user data: {status} - {body}"
            )));
        }
        ...
        #[cfg(feature = "caching")]
        {
            ...
            if self.config.enable_cache {
                if let Ok(mut cache) = self.user_cache.lock() {
                    cache.put(user_id.to_string(), (user_data.clone(), Instant::now()));
                }
            }
        }
    ...
    fn get_verification_key() -> AuthResult<String> {
        // In production, fetch from JWKS endpoint
        // For now, get from environment
        let env_result = env::var(PRIVY_VERIFICATION_KEY);
        env_result.map_err(|_| AuthError::ConfigError("Missing PRIVY_VERIFICATION_KEY".to_string()))
    }

    /// Create a new Privy provider
    ///
    /// # Panics
    /// This function will panic if the LRU cache size cannot be created (should never happen with valid size 1000)
    #[must_use]
    ...
    fn verify_token(&self, token: &str) -> AuthResult<PrivyClaims> {
        ...
        #[cfg(feature = "solana")]
        {
            if let Some(sol_wallet) = user_data.solana_wallet() {
                ...
                let rpc_url = match network_name.as_str() {
                    ...
                    _ => "https://api.mainnet-beta.solana.com".to_string(), // Default for mainnet and mainnet-beta
                };
        ...
        #[cfg(feature = "evm")]
        {
            if let Some(evm_wallet) = user_data.evm_wallet() {
                ...
                let (chain_id, rpc_url) = match auth_data.network.as_str() {
                    ...
                    _ => (1u64, "https://eth.llamarpc.com".to_string()), // Default for ethereum and mainnet
                };
...
/// # Errors
///
/// Returns error if required environment variables are missing or Privy configuration is invalid
...
mod tests {
    use super::*;
    use crate::privy::config::{PRIVY_APP_ID, PRIVY_APP_SECRET, PRIVY_AUTH_URL};
    ...
    async fn test_get_verification_key_when_env_var_set_should_return_key() {
        ...
        let result = PrivyProvider::get_verification_key();
        assert!(result.is_ok());
        assert_eq!(result.expect("Expected success"), "test_key");
    ...
    async fn test_get_verification_key_when_env_var_missing_should_return_error() {
        ...
        let error = result.expect_err("Expected error result");
        match error {
            AuthError::ConfigError(msg) => assert_eq!(msg, "Missing PRIVY_VERIFICATION_KEY"),
            _ => panic!("Expected ConfigError, got: {error:?}"),
        }
    }
    ...
    async fn test_verify_token_with_invalid_key_format_should_return_error() {
        ...
        let result = provider.verify_token("invalid_token");
        assert!(result.is_err());
        match result.expect_err("Expected error result") {
    ...
    async fn test_refresh_token_should_return_unsupported_error() {
        ...
        match error {
            ...
            _ => panic!("Expected UnsupportedOperation error, got: {error:?}"),
        }
    }
