...
impl ProviderConfig for PrivyConfig {
    fn from_env() -> Result<Self, AuthError> {
        ...
        if let Ok(api_url) = env::var(PRIVY_API_URL) {
            config.api_url = api_url;
        }
        if let Ok(auth_url) = env::var(PRIVY_AUTH_URL) {
            config.auth_url = auth_url;
        }
        if let Ok(jwks_url) = env::var(PRIVY_JWKS_URL) {
            config.jwks_url = jwks_url;
        }
        if let Ok(cache) = env::var(PRIVY_ENABLE_CACHE) {
...
mod tests {
    ...
    fn test_validate_when_empty_app_id_should_return_err() {
        let config = PrivyConfig::new(String::new(), "test_secret".to_string());
        let result = config.validate();

        assert!(result.is_err());
        match result.expect_err("Expected error result for empty app ID validation") {
            AuthError::ConfigError(msg) => assert_eq!(msg, "Privy app ID is required"),
            _ => panic!("Expected ConfigError for empty app ID validation"),
        }
    }
    ...
    fn test_validate_when_empty_app_secret_should_return_err() {
        let config = PrivyConfig::new("test_app_id".to_string(), String::new());
        let result = config.validate();

        assert!(result.is_err());
        match result.expect_err("Expected error result for empty app secret validation") {
            AuthError::ConfigError(msg) => assert_eq!(msg, "Privy app secret is required"),
            _ => panic!("Expected ConfigError for empty app secret validation"),
        }
    }
    ...
    fn test_from_env_when_required_vars_present_should_return_ok() {
        ...
        let config = result.expect("Expected successful config creation from env vars");
        assert_eq!(config.app_id, "test_app_id");
        assert_eq!(config.app_secret, "test_secret");
        assert_eq!(config.api_url, "https://api.privy.io");
        assert_eq!(config.auth_url, "https://auth.privy.io");
    ...
    fn test_from_env_when_app_id_missing_should_return_err() {
        ...
        match result.expect_err("Expected error result when PRIVY_APP_ID is missing") {
            AuthError::ConfigError(msg) => assert_eq!(msg, "PRIVY_APP_ID not found"),
            _ => panic!("Expected ConfigError when PRIVY_APP_ID is missing"),
        }
    }
    ...
    fn test_from_env_when_app_secret_missing_should_return_err() {
        ...
        match result.expect_err("Expected error result when PRIVY_APP_SECRET is missing") {
            AuthError::ConfigError(msg) => assert_eq!(msg, "PRIVY_APP_SECRET not found"),
            _ => panic!("Expected ConfigError when PRIVY_APP_SECRET is missing"),
        }
    ...
    fn test_from_env_with_custom_api_url_should_override_default() {
        ...
        let config = result.expect("Expected successful config creation with custom API URL");
        assert_eq!(config.api_url, "https://custom.api.url");
    ...
    fn test_from_env_with_custom_auth_url_should_override_default() {
        ...
        let config = result.expect("Expected successful config creation with custom auth URL");
        assert_eq!(config.auth_url, "https://custom.auth.url");
    ...
    fn test_from_env_with_custom_jwks_url_should_override_default() {
        ...
        let config = result.expect("Expected successful config creation with custom JWKS URL");
    ...
    fn test_from_env_with_cache_true_should_enable_cache() {
        ...
        let config = result.expect("Expected successful config creation with cache enabled");
        assert!(config.enable_cache);
    ...
    fn test_from_env_with_cache_false_should_disable_cache() {
        ...
        let config = result.expect("Expected successful config creation with cache disabled");
        assert!(!config.enable_cache);
    ...
    fn test_from_env_with_invalid_cache_value_should_default_to_true() {
        ...
        let config = result.expect("Expected successful config creation with invalid cache value");
    ...
    fn test_from_env_with_all_optional_overrides_should_use_all_custom_values() {
        ...
        let config = result.expect("Expected successful config creation with all custom values");
        assert_eq!(config.app_id, "test_app_id");
        assert_eq!(config.app_secret, "test_secret");
        assert_eq!(config.api_url, "https://custom.api.url");
        assert_eq!(config.auth_url, "https://custom.auth.url");
    ...
    fn test_from_env_when_validation_fails_should_return_err() {
        ...
        match result.expect_err("Expected error result when validation fails for empty app ID") {
            AuthError::ConfigError(msg) => assert_eq!(msg, "Privy app ID is required"),
            _ => panic!("Expected ConfigError when validation fails for empty app ID"),
        }
    ...
    fn test_serde_deserialization_should_work() {
        ...
        let config = config.expect("Expected successful serde deserialization");
        assert_eq!(config.app_id, "test_app_id");
        assert_eq!(config.app_secret, "test_secret");
