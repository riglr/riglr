...
mod tests {
    ...
    use riglr_core::signer::{
        Chain, EvmClient, EvmSigner, SignerBase, SignerError, SolanaClient, SolanaSigner,
        UnifiedSigner,
    };
    ...
    impl SolanaClient for MockSolanaClient {
        async fn get_balance(&self, _pubkey: &str) -> Result<String, Box<dyn SignerError>> {
            return Ok("1000000000".to_string()); // 1 SOL in lamports
        }
        async fn get_latest_blockhash(&self) -> Result<String, Box<dyn SignerError>> {
            Ok("mock_blockhash".to_string())
        }
        ...
        async fn get_fee_for_transaction(
            ...
            return Ok("5000".to_string()); // 5000 lamports
        }
        async fn get_minimum_balance_for_rent_exemption(
            &self,
            _data_len: usize,
        ) -> Result<String, Box<dyn SignerError>> {
            return Ok("890880".to_string()); // Typical rent exemption amount
        }
    ...
    impl SignerBase for MockSolanaSigner {
        ...
        fn user_id(&self) -> String {
            self.user_id.as_deref().unwrap_or("mock_user").to_string()
        }
    }
    ...
    impl SolanaSigner for MockSolanaSigner {
        fn client(&self) -> &dyn SolanaClient {
            // For test mocking, we'll use a placeholder - this would normally be a real client
            static MOCK_CLIENT: MockSolanaClient = MockSolanaClient;
            &MOCK_CLIENT
        }
    ...
    impl EvmSigner for MockEvmSigner {
        ...
        fn client(&self) -> &dyn EvmClient {
            static MOCK_CLIENT: MockEvmClient = MockEvmClient;
            &MOCK_CLIENT
        }
        async fn sign_and_send_transaction(
            &self,
    ...
    impl EvmClient for MockEvmClient {
        ...
        async fn get_balance(&self, _address: &str) -> Result<String, Box<dyn SignerError>> {
            return Ok("1000000000000000000".to_string()); // 1 ETH in wei
        }
        ...
        async fn get_gas_price(&self) -> Result<String, Box<dyn SignerError>> {
            return Ok("20000000000".to_string()); // 20 gwei
        }
    ...
    mod auth_provider_type_tests {
        ...
        fn test_auth_provider_type_as_str_when_custom_empty_should_return_empty() {
            let provider_type = AuthProviderType::Custom(String::new());
    ...
    impl SignerFactory for MockSignerFactory {
        ...
        ) -> Result<Box<dyn UnifiedSigner>, Box<dyn CoreError + Send + Sync>> {
            if self.should_fail {
                return Err("Mock creation error".into());
            }

            if auth_data.credentials.get("token") == Some(&"invalid".to_string()) {
                return Err("Invalid token".into());
            }
            if auth_data.credentials.get("username") == Some(&"invalid".to_string()) {
                return Err("Invalid credentials".into());
            }

            let address = auth_data
                .credentials
                .get("address")
    ...
    mod auth_provider_tests {
        ...
        fn test_auth_provider_auth_type_when_custom_empty_should_return_empty() {
            let mock_factory = Box::new(MockSignerFactory::new(vec![String::new()]));
            let provider = AuthProvider::new(AuthProviderType::Custom(String::new()), mock_factory);
        ...
        async fn test_auth_provider_create_signer_when_valid_token_should_succeed() {
            ...
            let signer = result.expect("Expected successful signer creation with valid token");
        ...
        async fn test_auth_provider_create_signer_when_invalid_token_should_fail() {
            ...
            let error = result.expect_err("Expected error with invalid token");
        ...
        async fn test_auth_provider_create_signer_when_valid_credentials_should_succeed() {
            ...
            let signer =
                result.expect("Expected successful signer creation with valid credentials");
            // For mock signer, just verify it supports the expected blockchain types
            assert!(signer.supports_solana() || signer.supports_evm());
        }
        ...
        async fn test_auth_provider_create_signer_when_invalid_credentials_should_fail() {
            ...
            let result = provider.create_signer(auth_data).await;
            assert!(result.is_err());
            let error = result.expect_err("Expected error with invalid credentials");
        ...
        async fn test_auth_provider_create_signer_when_custom_auth_data_should_succeed() {
            ...
            let signer = result.expect("Expected successful signer creation with custom auth data");
            // For mock signer, just verify it supports the expected blockchain types
            assert!(signer.supports_solana() || signer.supports_evm());
        }
        ...
        async fn test_auth_provider_create_signer_when_factory_fails_should_fail() {
            ...
            let result = provider.create_signer(auth_data).await;
            assert!(result.is_err());
            let error = result.expect_err("Expected mock creation error when factory fails");
            assert!(error.to_string().contains("Mock creation error"));
        }
