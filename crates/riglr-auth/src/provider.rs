...
#[cfg(feature = "magic")]
use crate::magic::MagicConfig;
#[cfg(feature = "privy")]
use crate::privy::PrivyConfig;
#[cfg(feature = "web3auth")]
use crate::web3auth::Web3AuthConfig;
...
impl AuthProvider {
    ...
    #[cfg(feature = "magic")]
    #[must_use]
    pub fn magic(config: MagicConfig) -> Self {
        use crate::magic::MagicProvider;
        Self::new(
            AuthProviderType::Magic,
            Box::new(MagicProvider::new(config)),
        )
    }
    ...
    #[cfg(feature = "privy")]
    #[must_use]
    pub fn privy(config: PrivyConfig) -> Self {
        use crate::privy::PrivyProvider;
        Self::new(
            AuthProviderType::Privy,
            Box::new(PrivyProvider::new(config)),
        )
    }
    ...
    #[cfg(feature = "web3auth")]
    #[must_use]
    pub fn web3auth(config: Web3AuthConfig) -> Self {
        use crate::web3auth::Web3AuthProvider;
        Self::new(
            AuthProviderType::Web3Auth,
            Box::new(Web3AuthProvider::new(config)),
        )
    }
}
...
mod tests {
    use super::*;
    use async_trait::async_trait;
    use riglr_core::signer::{Chain, EvmSigner, SignerBase, SolanaSigner, UnifiedSigner};
    use std::collections::HashMap;
    ...
    impl AuthenticationProvider for MockAuthProvider {
        async fn validate_token(&self, _token: &str) -> AuthResult<UserInfo> {
            Ok(UserInfo {
    ...
    fn test_user_info_with_all_values_should_work() {
        ...
        assert_eq!(
            user_info.email.as_ref().map_or("", |v| v),
            "admin@example.com"
        );
        assert_eq!(
            user_info.solana_address.as_ref().map_or("", |v| v),
            "So11111111111111111111111111111111111111112"
        );
        assert_eq!(
            user_info.evm_address.as_ref().map_or("", |v| v),
    ...
    async fn test_authentication_provider_refresh_token_default_should_return_unsupported_error() {
        ...
        if let Err(error) = result {
            match error {
                ...
                _ => {
                    panic!("Expected UnsupportedOperation error, got: {error:?}");
                }
            }
        } else {
    ...
    async fn test_authentication_provider_validate_token_should_work() {
        ...
        assert!(result.is_ok());
        let user_info =
            result.unwrap_or_else(|e| panic!("validate_token should succeed, got error: {e:?}"));
