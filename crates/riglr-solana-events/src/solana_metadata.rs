...
impl SolanaEventMetadata {
    ...
    #[must_use]
    #[inline]
    pub const fn core(&self) -> &EventMetadata {
        &self.core
    }

    /// Get a mutable reference to the core `EventMetadata`
    #[inline]
    pub const fn core_mut(&mut self) -> &mut EventMetadata {
        &mut self.core
    }
...
mod tests {
    ...
    fn test_new_when_valid_inputs_should_create_instance() {
        ...
        assert_eq!(metadata.program_received_time_ms, 1_234_567_890);
    ...
    fn test_create_metadata_when_valid_inputs_should_create_instance() {
        ...
        assert_eq!(metadata.program_received_time_ms, 9_876_543_210);
    ...
    fn test_create_metadata_when_none_block_time_should_create_instance() {
        ...
        assert_eq!(metadata.program_received_time_ms, 1_111_111_111);
    }
    ...
    fn test_serde_serialization_when_serialized_should_succeed() {
        ...
        let json = serialized.expect("Failed to serialize metadata to JSON");
    ...
    fn test_serde_deserialization_when_valid_json_should_succeed() {
        ...
        let json = serde_json::to_string(&original)
            .expect("Failed to serialize original metadata to JSON");
        let deserialized: Result<SolanaEventMetadata, _> = serde_json::from_str(&json);

        assert!(deserialized.is_ok());
        let metadata = deserialized.expect("Failed to deserialize metadata from JSON");
    ...
    fn test_borsh_serialization_when_serialized_should_succeed() {
        ...
        assert!(serialized.is_ok());
        assert!(!serialized
            .expect("Failed to serialize metadata with Borsh")
            .is_empty());
    }
    ...
    fn test_borsh_deserialization_when_valid_data_should_succeed() {
        ...
        let serialized =
            borsh::to_vec(&original).expect("Failed to serialize original metadata with Borsh");
        let deserialized: Result<SolanaEventMetadata, _> = borsh::from_slice(&serialized);

        assert!(deserialized.is_ok());
        let metadata = deserialized.expect("Failed to deserialize metadata from Borsh data");
        assert_eq!(metadata.signature, "sig");
        assert_eq!(metadata.slot, 100);
        assert_eq!(metadata.event_type, EventType::Swap);
        assert_eq!(metadata.protocol_type, ProtocolType::Jupiter);
    ...
    fn test_borsh_skip_core_when_serialized_should_exclude_core() {
        ...
        let serialized =
            borsh::to_vec(&metadata).expect("Failed to serialize metadata for core skip test");
        let deserialized: SolanaEventMetadata = borsh::from_slice(&serialized)
            .expect("Failed to deserialize metadata in core skip test");
