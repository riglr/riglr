...
pub mod parsing {
    ...
    pub const U8_RANGE: Range<usize> = 0..U8_SIZE;
    /// Byte range for parsing a single u16 value
    pub const U16_RANGE: Range<usize> = 0..U16_SIZE;
    /// Byte range for parsing a single u32 value
    pub const U32_RANGE: Range<usize> = 0..U32_SIZE;
    /// Byte range for parsing a single u64 value
    pub const U64_RANGE: Range<usize> = 0..U64_SIZE;
    /// Byte range for parsing a single u128 value
    pub const U128_RANGE: Range<usize> = 0..U128_SIZE;
    /// Byte range for parsing a single public key value
    pub const PUBKEY_RANGE: Range<usize> = 0..PUBKEY_SIZE;

    /// Second u64 range (for parsing two consecutive u64 values)
    pub const SECOND_U64_RANGE: Range<usize> = U64_SIZE..(U64_SIZE * 2);

    /// Third u64 range (for parsing three consecutive u64 values)
    pub const THIRD_U64_RANGE: Range<usize> = (U64_SIZE * 2)..(U64_SIZE * 3);
...
pub mod instruction_sizes {
    use super::parsing::{ANCHOR_DISCRIMINATOR_SIZE, PUBKEY_SIZE, U32_SIZE, U64_SIZE};
...
mod tests {
    ...
    fn test_instruction_sizes_should_have_logical_ordering() {
        ...
        // Note: This assertion may be optimized out if condition is always true
    ...
    fn test_range_properties_should_be_valid() {
        ...
        assert!(!parsing::U8_RANGE.is_empty());
        assert!(!parsing::U16_RANGE.is_empty());
        assert!(!parsing::U32_RANGE.is_empty());
        assert!(!parsing::U64_RANGE.is_empty());
        assert!(!parsing::U128_RANGE.is_empty());
        assert!(!parsing::PUBKEY_RANGE.is_empty());
        assert!(!parsing::SECOND_U64_RANGE.is_empty());
        assert!(!parsing::THIRD_U64_RANGE.is_empty());
    }
    ...
    fn test_instruction_sizes_all_include_discriminator() {
        ...
        {
            assert!(
                instruction_sizes::MIN_SWAP_INSTRUCTION_SIZE >= parsing::ANCHOR_DISCRIMINATOR_SIZE
            );
            assert!(
                instruction_sizes::MIN_LIQUIDITY_INSTRUCTION_SIZE
                    >= parsing::ANCHOR_DISCRIMINATOR_SIZE
