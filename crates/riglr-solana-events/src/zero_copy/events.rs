...
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct ZeroCopyEvent<'data_lifetime> {
    /// Event-specific data as JSON (lazily computed)
    pub json_data: Option<serde_json::Value>,
    /// Event metadata (owned)
    pub metadata: EventMetadata,
    /// Protocol-specific parsed data (lazily computed)
    pub parsed_data: Option<Arc<dyn Any + Send + Sync>>,
    /// Raw instruction or log data (borrowed)
    pub raw_data: Cow<'data_lifetime, [u8]>,
}

impl<'data_lifetime> ZeroCopyEvent<'data_lifetime> {
    /// Get JSON representation, computing if necessary
    #[must_use]
    #[inline]
    pub const fn get_json_data(&self) -> Option<&serde_json::Value> {
    ...
    #[must_use]
    #[inline]
    pub fn get_parsed_data<T: Any + Send + Sync>(&self) -> Option<&T> {
    ...
    #[must_use]
    #[inline]
    pub fn new_borrowed(metadata: EventMetadata, raw_data: &'data_lifetime [u8]) -> Self {
        Self {
            json_data: None,
            metadata,
            parsed_data: None,
    ...
    #[inline]
    pub fn set_parsed_data<T: Any + Send + Sync>(&mut self, data: T) {
...
impl ZeroCopyEvent<'_> {
    ...
    pub fn timestamp(&self) -> SystemTime {
        ...
        SystemTime::from(self.metadata.core.timestamp)
    }
}
...
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct ZeroCopySwapEvent<'data_lifetime> {
    /// Amount in (parsed lazily)
    pub amount_in: Option<u64>,
    /// Amount out (parsed lazily)
    pub amount_out: Option<u64>,
    /// Base event data
    pub base: ZeroCopyEvent<'data_lifetime>,
...
impl<'data_lifetime> ZeroCopySwapEvent<'data_lifetime> {
    /// Get amount in, parsing if necessary
    #[inline]
    pub const fn amount_in(&mut self) -> Option<u64> {
    ...
    #[inline]
    pub const fn amount_out(&mut self) -> Option<u64> {
    ...
    #[inline]
    pub const fn input_mint(&mut self) -> Option<Pubkey> {
    ...
    #[must_use]
    #[inline]
    pub const fn new(base: ZeroCopyEvent<'data_lifetime>) -> Self {
        Self {
            amount_in: None,
            amount_out: None,
            base,
    ...
    #[inline]
    pub const fn output_mint(&mut self) -> Option<Pubkey> {
    ...
    /// Protocol-specific parsing method for amount in
    const fn parse_amount_in() -> Option<u64> {
        // This would be implemented based on the protocol
        None
    }

    /// Protocol-specific parsing method for amount out
    const fn parse_amount_out() -> Option<u64> {
        // This would be implemented based on the protocol
        None
    }

    /// Protocol-specific parsing method for input mint
    const fn parse_input_mint() -> Option<Pubkey> {
        // This would be implemented based on the protocol
        None
    }

    /// Protocol-specific parsing method for output mint
    const fn parse_output_mint() -> Option<Pubkey> {
        // This would be implemented based on the protocol
        None
    }
}
...
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct ZeroCopyLiquidityEvent<'data_lifetime> {
    /// Base event data
    pub base: ZeroCopyEvent<'data_lifetime>,
...
impl<'data_lifetime> ZeroCopyLiquidityEvent<'data_lifetime> {
    /// Get LP token amount, parsing if necessary
    #[inline]
    pub const fn lp_amount(&mut self) -> Option<u64> {
    ...
    /// Protocol-specific parsing method for LP amount
    const fn parse_lp_amount() -> Option<u64> {
        None
    }

    /// Protocol-specific parsing method for pool address
    const fn parse_pool_address() -> Option<Pubkey> {
        None
    }

    /// Protocol-specific parsing method for token A amount
    const fn parse_token_a_amount() -> Option<u64> {
        None
    }

    /// Protocol-specific parsing method for token B amount
    const fn parse_token_b_amount() -> Option<u64> {
        None
    }

    /// Get pool address, parsing if necessary
    #[inline]
    pub const fn pool_address(&mut self) -> Option<Pubkey> {
    ...
    #[inline]
    pub const fn token_a_amount(&mut self) -> Option<u64> {
    ...
    #[inline]
    pub const fn token_b_amount(&mut self) -> Option<u64> {
...
mod tests {
    ...
    fn test_zerocopy_event_new_borrowed_should_create_with_borrowed_data() {
        ...
        let event = ZeroCopyEvent::new_borrowed(metadata, raw_data);
    ...
    fn test_zerocopy_event_set_and_get_parsed_data_should_work_with_valid_type() {
        ...
        assert_eq!(
            retrieved.expect("Retrieved parsed data should be Some"),
            &test_data
        );
    }
    ...
    fn test_zerocopy_event_set_and_get_json_data_should_work() {
        ...
        assert!(retrieved.is_some());
        assert_eq!(
            retrieved.expect("Retrieved JSON data should be Some"),
            &json_value
        );
    }
    ...
    fn test_zerocopy_swap_event_parse_methods_should_return_none() {
        ...
        assert!(ZeroCopySwapEvent::parse_input_mint().is_none());
        assert!(ZeroCopySwapEvent::parse_output_mint().is_none());
        assert!(ZeroCopySwapEvent::parse_amount_in().is_none());
        assert!(ZeroCopySwapEvent::parse_amount_out().is_none());
    }
    ...
    fn test_zerocopy_liquidity_event_parse_methods_should_return_none() {
        // Test all private parse methods through reflection of their behavior
        assert!(ZeroCopyLiquidityEvent::parse_pool_address().is_none());
        assert!(ZeroCopyLiquidityEvent::parse_lp_amount().is_none());
        assert!(ZeroCopyLiquidityEvent::parse_token_a_amount().is_none());
        assert!(ZeroCopyLiquidityEvent::parse_token_b_amount().is_none());
    }
