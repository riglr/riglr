...
// Re-export specific items instead of glob imports
pub use events::{
    ZeroCopyEvent as Event, ZeroCopyLiquidityEvent as LiquidityEvent,
    ZeroCopySwapEvent as SwapEvent,
};

// Re-export original names for backward compatibility
// These have allow attributes to suppress module name repetition warnings
#[expect(clippy::module_name_repetitions)]
pub use events::ZeroCopyEvent;
#[expect(clippy::module_name_repetitions)]
pub use events::ZeroCopyLiquidityEvent;
#[expect(clippy::module_name_repetitions)]
pub use events::ZeroCopySwapEvent;
pub use parsers::{
    BatchEventParser, BatchParserStats, ByteSliceEventParser, CustomDeserializer,
    MemoryMappedParser, ParseError, RpcConnectionPool, SIMDPatternMatcher,
};
...
mod tests {
    ...
    impl ByteSliceEventParser for MockParser {
        ...
        fn parse_from_slice<'data>(
            ...
            if data.is_empty() {
                return Err(ParseError::InvalidInstructionData("Empty data".to_owned()));
            }
    ...
    #[test]
    fn zero_copy_event_new_borrowed() {
    ...
    #[test]
    fn zero_copy_event_new_owned() {
        let data = b"test data".to_vec();
        let metadata = create_test_metadata();
        let event = ZeroCopyEvent::new_owned(metadata, data.clone());
    ...
    #[test]
    fn zero_copy_event_set_and_get_parsed_data() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let mut event = ZeroCopyEvent::new_borrowed(metadata, data);

        // Test setting and getting parsed data
        let test_string = "parsed data".to_owned();
        event.set_parsed_data(test_string.clone());

        let retrieved = event
            .get_parsed_data::<String>()
            .expect("Failed to retrieve parsed data as String");
    ...
    #[test]
    fn zero_copy_event_get_parsed_data_when_none() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let event = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn zero_copy_event_set_and_get_json_data() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let mut event = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn zero_copy_event_get_json_data_when_none() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let event = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn zero_copy_event_to_owned() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let mut event = ZeroCopyEvent::new_borrowed(metadata, data);

        // Set some data
        event.set_parsed_data("test".to_owned());
    ...
    #[test]
    fn zero_copy_event_helper_methods() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let event = ZeroCopyEvent::new_borrowed(metadata, data);
        ...
        assert_eq!(
            event.block_number().expect("Block number should exist"),
            12345
        );
    }

    #[test]
    fn zero_copy_event_timestamp() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let event = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn zero_copy_swap_event_lazy_parsing() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let base = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn zero_copy_liquidity_event_new() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let base = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn zero_copy_liquidity_event_lazy_parsing() {
        let data = b"test data";
        let metadata = create_test_metadata();
        let base = ZeroCopyEvent::new_borrowed(metadata, data);
    ...
    #[test]
    fn parse_error_invalid_instruction_data() {
        let error = ParseError::InvalidInstructionData("test error".to_owned());
        assert!(error
            .to_string()
    ...
    #[test]
    fn parse_error_unknown_discriminator() {
    ...
    #[test]
    fn parse_error_memory_map_error() {
        let error = ParseError::MemoryMapError("mmap failed".to_owned());
        assert!(error.to_string().contains("Memory map error: mmap failed"));
    }

    // Tests for MemoryMappedParser
    #[test]
    fn memory_mapped_parser_from_file_nonexistent() {
        let result = MemoryMappedParser::from_file("/nonexistent/file.dat");
        result.unwrap_err();
    }

    #[test]
    fn memory_mapped_parser_from_file_success() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        file.write_all(b"test data for mmap")
            .expect("Failed to write test data to file");

        let parser =
            MemoryMappedParser::from_file(file_path.to_str().expect("Path should be valid UTF-8"))
                .expect("Failed to create memory mapped parser");
        assert_eq!(parser.size(), 18); // "test data for mmap" is 18 bytes
    }

    #[test]
    fn memory_mapped_parser_add_parser() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        ...
        let _events = parser.parse_all().expect("Failed to parse all events");
        // If no error occurred, the parser was likely added successfully
    }

    #[test]
    fn memory_mapped_parser_parse_all_no_parsers() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        file.write_all(b"test data")
        ...
        let parser =
            MemoryMappedParser::from_file(file_path.to_str().expect("Path should be valid UTF-8"))
                .expect("Failed to create memory mapped parser");
        let events = parser.parse_all().expect("Failed to parse all events");
        assert!(events.is_empty());
    }

    #[test]
    fn memory_mapped_parser_parse_all_with_parser() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        file.write_all(b"test data")
    ...
    #[test]
    fn memory_mapped_parser_parse_all_parser_cannot_parse() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        file.write_all(b"test data")
    ...
    #[test]
    fn memory_mapped_parser_data_slice_valid() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        file.write_all(b"test data")
        ...
        let parser =
            MemoryMappedParser::from_file(file_path.to_str().expect("Path should be valid UTF-8"))
                .expect("Failed to create memory mapped parser");
        let slice = parser.data_slice(0, 4).expect("Failed to get data slice");
        assert_eq!(slice, b"test");
    }

    #[test]
    fn memory_mapped_parser_data_slice_invalid() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
    ...
    #[test]
    fn memory_mapped_parser_data_slice_offset_out_of_bounds() {
        let dir = tempdir().expect("Failed to create temporary directory");
        let file_path = dir.path().join("test.dat");
        let mut file = File::create(&file_path).expect("Failed to create test file");
        file.write_all(b"test")
    ...
    #[test]
    fn simd_pattern_matcher_default() {
        let pattern_matcher = SIMDPatternMatcher::default();
        ...
        assert!(pattern_matcher.find_matches(&test_data).is_empty());
        assert_eq!(pattern_matcher.match_discriminator(&test_data), None);
    }

    #[test]
    fn simd_pattern_matcher_add_pattern() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);

        // patterns and protocols fields are private
        // We can test behavior indirectly by checking that the pattern works
        let test_data = vec![0x01, 0x02, 0x03];
        let match_results = pattern_matcher.find_matches(&test_data);
        assert_eq!(match_results.len(), 1);
        assert_eq!(match_results.first().unwrap(), &(0, ProtocolType::Jupiter));
    }

    #[test]
    fn simd_pattern_matcher_find_matches_empty_data() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);

        let match_results = pattern_matcher.find_matches(&[]);
        assert!(match_results.is_empty());
    }

    #[test]
    fn simd_pattern_matcher_find_matches_no_matches() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);

        let data = vec![0x03, 0x04, 0x05];
        let match_results = pattern_matcher.find_matches(&data);
        assert!(match_results.is_empty());
    }

    #[test]
    fn simd_pattern_matcher_find_matches_single_match() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);
        ...
        let match_results = pattern_matcher.find_matches(&data);
        assert_eq!(match_results.len(), 1);
        assert_eq!(match_results.first().unwrap(), &(1, ProtocolType::Jupiter));
    }

    #[test]
    fn simd_pattern_matcher_find_matches_multiple_matches() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01], ProtocolType::Jupiter);

        let data = vec![0x01, 0x02, 0x01, 0x03];
        let match_results = pattern_matcher.find_matches(&data);
        assert_eq!(match_results.len(), 2);
        assert_eq!(match_results.first().unwrap(), &(0, ProtocolType::Jupiter));
        assert_eq!(match_results.get(1).unwrap(), &(2, ProtocolType::Jupiter));
    }

    #[test]
    fn simd_pattern_matcher_find_matches_overlapping_patterns() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);
        pattern_matcher.add_pattern(vec![0x02, 0x03], ProtocolType::RaydiumAmmV4);
    ...
    #[test]
    fn simd_pattern_matcher_find_matches_insufficient_data() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02, 0x03], ProtocolType::Jupiter);
    ...
    #[test]
    fn simd_pattern_matcher_match_discriminator_match() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);

        let data = vec![0x01, 0x02, 0x03, 0x04];
        let result = pattern_matcher.match_discriminator(&data);
        assert_eq!(result, Some(ProtocolType::Jupiter));
    }

    #[test]
    fn simd_pattern_matcher_match_discriminator_no_match() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);
    ...
    #[test]
    fn simd_pattern_matcher_match_discriminator_insufficient_data() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);
    ...
    #[test]
    fn simd_pattern_matcher_match_discriminator_multiple_patterns() {
        let mut pattern_matcher = SIMDPatternMatcher::default();
        pattern_matcher.add_pattern(vec![0x01, 0x02], ProtocolType::Jupiter);
        pattern_matcher.add_pattern(vec![0x03, 0x04], ProtocolType::RaydiumAmmV4);
    ...
    #[test]
    fn custom_deserializer_read_u8_success() {
        let data = vec![0x42, 0x43, 0x44];
        let mut deserializer = CustomDeserializer::new(&data);

        let value = deserializer
            .read_u8()
            .expect("Failed to read u8 from deserializer");
    ...
    #[test]
    fn custom_deserializer_read_u8_insufficient_data() {
        let data = vec![];
        let mut deserializer = CustomDeserializer::new(&data);

        let result = deserializer.read_u8();
        assert!(result.is_err());
        match result.expect_err("Expected error but got success") {
    ...
    #[test]
    fn custom_deserializer_read_u32_le_success() {
        let data = vec![0x01, 0x02, 0x03, 0x04, 0x05];
        let mut deserializer = CustomDeserializer::new(&data);

        let value = deserializer
            .read_u32_le()
            .expect("Failed to read u32 from deserializer");
        assert_eq!(value, 0x0403_0201_u32); // Little endian
        assert_eq!(deserializer.position(), 4);
        assert_eq!(deserializer.remaining(), 1);
    }

    #[test]
    fn custom_deserializer_read_u32_le_insufficient_data() {
    ...
    #[test]
    fn custom_deserializer_read_u64_le_success() {
        let data = vec![0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09];
        let mut deserializer = CustomDeserializer::new(&data);

        let value = deserializer
            .read_u64_le()
            .expect("Failed to read u64 from deserializer");
        assert_eq!(value, 0x0807_0605_0403_0201_u64); // Little endian
        assert_eq!(deserializer.position(), 8);
        assert_eq!(deserializer.remaining(), 1);
    }

    #[test]
    fn custom_deserializer_read_u64_le_insufficient_data() {
    ...
    #[test]
    fn custom_deserializer_read_pubkey_success() {
        let mut data = vec![0; 32];
        if let Some(v) = data.get_mut(0) {
            *v = 0x01; // Make it non-zero to be a valid pubkey
        }
        let mut deserializer = CustomDeserializer::new(&data);

        let pubkey = deserializer
            .read_pubkey()
            .expect("Failed to read pubkey from deserializer");
        ...
        assert_eq!(pubkey.to_bytes().first().unwrap(), &0x01);
    }

    #[test]
    fn custom_deserializer_read_pubkey_insufficient_data() {
    ...
    #[test]
    fn custom_deserializer_read_bytes_success() {
        let data = vec![0x01, 0x02, 0x03, 0x04, 0x05];
        let mut deserializer = CustomDeserializer::new(&data);

        let bytes = deserializer
            .read_bytes(3)
            .expect("Failed to read bytes from deserializer");
    ...
    #[test]
    fn custom_deserializer_read_bytes_insufficient_data() {
    ...
    #[test]
    fn custom_deserializer_skip_success() {
        let data = vec![0x01, 0x02, 0x03, 0x04, 0x05];
        let mut deserializer = CustomDeserializer::new(&data);

        deserializer
            .skip(3)
            .expect("Failed to skip bytes in deserializer");
        assert_eq!(deserializer.position(), 3);
        assert_eq!(deserializer.remaining(), 2);
    }

    #[test]
    fn custom_deserializer_skip_insufficient_data() {
        let data = vec![0x01, 0x02];
        let mut deserializer = CustomDeserializer::new(&data);
    ...
    #[test]
    fn custom_deserializer_remaining_data() {
        let data = vec![0x01, 0x02, 0x03, 0x04, 0x05];
        let mut deserializer = CustomDeserializer::new(&data);
    ...
    #[test]
    fn custom_deserializer_remaining_data_empty() {
        let data = vec![0x01, 0x02];
        let mut deserializer = CustomDeserializer::new(&data);
    ...
    #[test]
    fn batch_event_parser_add_parser_raydium() {
    ...
    #[test]
    fn batch_event_parser_add_parser_jupiter() {
        let mut parser = BatchEventParser::new(100);
        let mock_parser = Arc::new(MockParser {
            protocol: ProtocolType::Jupiter,
            can_parse_result: true,
    ...
    #[test]
    fn batch_event_parser_add_parser_other() {
        let mut parser = BatchEventParser::new(100);
        let mock_parser = Arc::new(MockParser {
            protocol: ProtocolType::Other("Custom".to_owned()),
            can_parse_result: true,
        });
    ...
    #[test]
    fn batch_event_parser_parse_batch_empty() {
        let parser = BatchEventParser::new(100);
        let batch: &[&[u8]] = &[];
        let metadatas = vec![];

        let result = parser
            .parse_batch(batch, metadatas)
            .expect("Failed to parse batch");
        assert!(result.is_empty());
    }

    #[test]
    fn batch_event_parser_parse_batch_exceeds_max_size() {
    ...
    #[test]
    fn batch_event_parser_parse_batch_no_matching_parser() {
        let parser = BatchEventParser::new(100);
        let data = b"test data";
    ...
    #[test]
    fn batch_event_parser_parse_batch_with_matching_parser() {
        let mut parser = BatchEventParser::new(100);
        let mock_parser = Arc::new(MockParser {
            protocol: ProtocolType::RaydiumAmmV4,
            can_parse_result: true,
    ...
    #[test]
    fn batch_event_parser_parse_batch_parser_error() {
        #[derive(Debug)]
        struct ErrorParser;
    ...
    #[test]
    fn rpc_connection_pool_new() {
        let urls = vec!["http://localhost:8899".to_owned()];
    ...
    #[test]
    fn rpc_connection_pool_new_empty() {
        let urls = vec![];
        let pool = RpcConnectionPool::new(urls);
    ...
    #[test]
    fn rpc_connection_pool_new_multiple() {
        let urls = vec![
            "http://localhost:8899".to_owned(),
            "http://localhost:8900".to_owned(),
            "http://localhost:8901".to_owned(),
        ];
        let pool = RpcConnectionPool::new(urls);
    ...
    #[test]
    fn rpc_connection_pool_get_client_empty_pool() {
    ...
    #[test]
    fn rpc_connection_pool_get_client_single() {
        let urls = vec!["http://localhost:8899".to_owned()];
        let pool = RpcConnectionPool::new(urls);

        let client1 = pool.get_client().expect("Failed to get client from pool");
        let client2 = pool.get_client().expect("Failed to get client from pool");
    ...
    #[test]
    fn rpc_connection_pool_get_client_round_robin() {
        let urls = vec![
            "http://localhost:8899".to_owned(),
            "http://localhost:8900".to_owned(),
        ];
        ...
        let client1 = pool.get_client().expect("Failed to get client from pool");
        let client2 = pool.get_client().expect("Failed to get client from pool");
        let client3 = pool.get_client().expect("Failed to get client from pool");
    ...
    #[test]
    fn mock_parser_parse_from_slice_empty_data() {
    ...
    fn mock_parser_can_parse() {
        ...
        let parser_can_parse = MockParser {
            protocol: ProtocolType::Jupiter,
            can_parse_result: true,
        };

        assert!(parser_can_parse.can_parse(b"any data"));
    }

    #[test]
    fn mock_parser_protocol_type() {
        let parser = MockParser {
            protocol: ProtocolType::RaydiumAmmV4,
            can_parse_result: true,
        };
    ...
    #[test]
    fn parse_error_from_borsh_error() {
        use borsh::io::{Error as BorshError, ErrorKind};
        let borsh_error = BorshError::new(ErrorKind::UnexpectedEof, "test borsh error");
    ...
    #[test]
    fn memory_mapped_parser_from_file_io_error() {
        // Test with a directory instead of a file to trigger an IO error
        let dir = tempdir().expect("Failed to create temporary directory");
        let result =
            MemoryMappedParser::from_file(dir.path().to_str().expect("Path should be valid UTF-8"));
        result.unwrap_err();
    }

    // Test remaining coverage for CustomDeserializer position tracking
    #[test]
    fn custom_deserializer_position_tracking() {
        let data = vec![0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09];
        let mut deserializer = CustomDeserializer::new(&data);
