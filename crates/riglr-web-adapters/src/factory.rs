...
pub trait SignerFactory: Send + Sync + fmt::Debug {
...
impl fmt::Debug for CompositeSignerFactory {
...
impl CompositeSignerFactory {
    ...
    pub fn add_factory(&mut self, auth_type: String, factory: Arc<dyn SignerFactory>) {
...
mod tests {
    ...
    async fn test_unsupported_auth_type() {
        ...
        let error_msg = result
            .expect_err("Expected error for unsupported auth type")
            .to_string();
    ...
    fn test_add_factory_method() {
        let mut composite = CompositeSignerFactory::default();
        let mock_factory = Arc::new(MockSignerFactory);
    ...
    async fn test_supported_auth_types_with_multiple_factories() {
        ...
        composite.add_factory("multi".to_string(), Arc::new(MultiAuthFactory));
        composite.add_factory("mock".to_string(), Arc::new(MockSignerFactory));
    ...
    async fn test_composite_factory_with_no_registered_factories() {
        ...
        assert!(result.is_err());
        let error_msg = result
            .expect_err("Expected error for nonexistent auth type")
            .to_string();
    ...
    async fn test_composite_factory_with_multiple_registered_factories() {
        ...
        composite.add_factory("factory2".to_string(), Arc::new(MockSignerFactory));
