...
mod tests {
    ...
    use {
        riglr_config::SolanaNetworkConfig,
        riglr_core::provider::ApplicationContext,
        riglr_solana_tools::{
            common_conversions::system_transfer_v3, signer::Local, transaction::transfer_sol,
            utils::transaction::send_transaction_with_retry,
        },
    ...
    async fn test_solana_transfer_integration() {
        ...
        let solana_config = SolanaNetworkConfig::new("test-local", rpc_url.clone());
        let signer =
            Local::new(&payer.to_base58_string(), solana_config).expect("Failed to create signer");

        // Set up application context
        let config = (*riglr_config::Config::from_env()).clone();
    ...
    async fn test_solana_transaction_retry() {
        ...
        let recipient = Keypair::new();
        let instruction = system_transfer_v3(
            &payer.pubkey(),
            &recipient.pubkey(),
            500_000_000, // 0.5 SOL
        );
        ...
        use riglr_solana_tools::utils::Config;
