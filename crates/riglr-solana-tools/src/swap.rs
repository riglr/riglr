//! Jupiter DEX integration for token swaps on Solana
//!
//! This module provides tools for interacting with the Jupiter aggregator,
//! enabling token swaps with optimal routing across multiple DEXs.

use crate::clients::{Clients, JupiterClient};
use crate::transaction::Status;
use crate::utils::send;
use core::str::FromStr as _;
use core::{future::Future, marker::Send, pin::Pin};
use riglr_core::provider::ApplicationContext;
use riglr_core::signer::SolanaSignerHandle;
use riglr_core::{SignerContext, ToolError};
use riglr_macros::tool;
use schemars::{r#gen::SchemaGenerator, JsonSchema};
use serde::{Deserialize, Serialize};
use serde_json::json;
use solana_sdk::{pubkey::Pubkey, transaction::Transaction as SolanaTransaction};
use std::sync::Arc;
use tracing::{debug, info};

/// Route plan step in a Jupiter swap
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct RoutePlanStep {
    /// Percentage of amount to route through this step
    pub percent: u32,
    /// DEX name for this step
    pub swap_info: Info,
}

/// Swap information for a route step
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct Info {
    /// Name of the AMM/DEX
    pub amm_key: String,
    /// Fee amount
    pub fee_amount: String,
    /// Fee mint
    pub fee_mint: String,
    /// Expected input amount
    pub in_amount: String,
    /// Input mint for this step
    pub input_mint: String,
    /// Human-readable label
    pub label: Option<String>,
    /// Expected output amount
    pub out_amount: String,
    /// Output mint for this step
    pub output_mint: String,
}

/// Jupiter quote response
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct Quote {
    /// Quote freshness metadata
    pub context_slot: Option<u64>,
    /// Input amount
    pub in_amount: String,
    /// Input token mint address
    pub input_mint: String,
    /// Minimum output amount after slippage
    pub other_amount_threshold: String,
    /// Expected output amount
    pub out_amount: String,
    /// Output token mint address
    pub output_mint: String,
    /// Price impact as percentage
    pub price_impact_pct: String,
    /// Routing plan through DEXs
    pub route_plan: Vec<RoutePlanStep>,
    /// Time taken to compute quote
    pub time_taken: Option<f64>,
}

/// Result of a Jupiter swap transaction
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct Transaction {
    /// Optional idempotency key for deduplication
    pub idempotency_key: Option<String>,
    /// Actual input amount
    pub in_amount: String,
    /// Input token mint
    pub input_mint: String,
    /// Actual output amount
    pub out_amount: String,
    /// Output token mint
    pub output_mint: String,
    /// Price impact as percentage
    pub price_impact_pct: f64,
    /// Transaction signature
    pub signature: String,
    /// Transaction status
    pub status: Status,
}

/// Token price information
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct PriceInfo {
    /// Base token mint address (being priced)
    pub base_mint: String,
    /// Token mint address
    pub mint: String,
    /// Price in USD
    pub price: f64,
    /// Price impact as percentage
    pub price_impact_pct: f64,
    /// Quote token mint address (pricing against)
    pub quote_mint: String,
    /// Data source
    pub source: String,
    /// Symbol/ticker
    pub symbol: String,
}

/// Get a quote from Jupiter for swapping tokens
///
/// This tool queries the Jupiter aggregator to find the best swap route between two SPL tokens
/// and returns detailed pricing information without executing any transaction. Jupiter aggregates
/// liquidity from multiple DEXs to provide optimal pricing.
///
/// # Arguments
///
/// * `input_mint` - Source token mint address to swap from
/// * `output_mint` - Destination token mint address to swap to
/// * `amount` - Input amount in token's smallest unit (e.g., lamports for SOL)
/// * `slippage_bps` - Maximum acceptable slippage in basis points (e.g., 50 = 0.5%)
/// * `only_direct_routes` - If true, only consider direct swap routes (no intermediate tokens)
/// * `jupiter_api_url` - Optional custom Jupiter API endpoint URL
///
/// # Returns
///
/// Returns `Quote` containing:
/// - `input_mint` and `output_mint`: Token addresses
/// - `in_amount` and `out_amount`: Expected input and output amounts
/// - `other_amount_threshold`: Minimum output after slippage
/// - `price_impact_pct`: Price impact as percentage
/// - `route_plan`: Detailed routing through DEXs
/// - `context_slot` and `time_taken`: Quote freshness metadata
///
/// # Errors
///
/// * `ToolError::Permanent` - When token addresses are invalid or no routes exist
/// * `ToolError::Retriable` - When Jupiter API is temporarily unavailable
///
/// # Examples
///
/// ```rust,ignore
/// use riglr_solana_tools::swap::get_jupiter_quote;
/// use riglr_core::provider::ApplicationContext;
///
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// let context = ApplicationContext::from_env();
/// // Get quote for swapping 1 SOL to USDC
/// let quote = get_jupiter_quote(
///     "So11111111111111111111111111111111111111112".to_string(), // SOL mint
///     "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(), // USDC mint
///     1_000_000_000, // 1 SOL in lamports
///     50, // 0.5% slippage
///     false, // Allow multi-hop routes
///     None, // Use default Jupiter API
///     &context,
/// ).await?;
///
/// println!("Quote: {} SOL -> {} USDC", quote.in_amount, quote.out_amount);
/// println!("Price impact: {:.2}%", quote.price_impact_pct);
/// # Ok(())
/// # }
/// ```
/// Validate swap parameters and prepare context
fn validate_quote_params(
    input_mint: &str,
    output_mint: &str,
    context: &ApplicationContext,
) -> Result<Clients, ToolError> {
    // Validate mint addresses
    let _input_pubkey = Pubkey::from_str(input_mint).map_err(|parse_err| {
        ToolError::permanent_string(format!("Invalid input mint: {parse_err}"))
    })?;
    let _output_pubkey = Pubkey::from_str(output_mint).map_err(|parse_err| {
        ToolError::permanent_string(format!("Invalid output mint: {parse_err}"))
    })?;

    // Get API clients from context
    let api_clients = context
        .get_extension::<Arc<Clients>>()
        .ok_or_else(|| ToolError::permanent_string("Clients not found in context"))?;

    Ok((**api_clients).clone())
}

/// Build quote request URL with parameters
fn build_quote_url(
    input_mint: &str,
    output_mint: &str,
    amount: u64,
    slippage_bps: u16,
    only_direct_routes: bool,
    api_url: &str,
) -> String {
    let url = format!("{api_url}/quote");
    let mut params = vec![
        format!("inputMint={}", input_mint),
        format!("outputMint={}", output_mint),
        format!("amount={}", amount),
        format!("slippageBps={}", slippage_bps),
    ];

    if only_direct_routes {
        params.push("onlyDirectRoutes=true".to_owned());
    }

    format!("{}?{}", url, params.join("&"))
}

/// Request quote from Jupiter API
async fn request_jupiter_quote(
    url: &str,
    jupiter_client: &JupiterClient,
) -> Result<JupiterQuoteResponse, ToolError> {
    debug!("Requesting quote from: {}", url);

    let response = jupiter_client
        .http_client()
        .get(url)
        .send()
        .await
        .map_err(|request_err| {
            ToolError::retriable_string(format!("Failed to request quote: {request_err}"))
        })?;

    if !response.status().is_success() {
        let error_text = response
            .text()
            .await
            .unwrap_or_else(|_| "Unknown error".to_owned());
        return Err(ToolError::permanent_string(format!(
            "Jupiter API error: {error_text}"
        )));
    }

    let quote_response: JupiterQuoteResponse = response.json().await.map_err(|parse_err| {
        ToolError::permanent_string(format!("Failed to parse quote response: {parse_err}"))
    })?;

    Ok(quote_response)
}

/// Get a price quote from Jupiter aggregator for token swaps
///
/// # Errors
///
/// Returns an error if:
/// - Jupiter API request fails or returns non-success status
/// - Invalid input parameters provided
/// - Network connection issues
/// - JSON parsing of response fails
#[tool]
#[inline]
pub async fn get_jupiter_quote(
    input_mint: String,
    output_mint: String,
    amount: u64,
    slippage_bps: u16,
    only_direct_routes: bool,
    jupiter_api_url: Option<String>,
    context: &ApplicationContext,
) -> Result<Quote, ToolError> {
    debug!(
        "Getting Jupiter quote for {} {} -> {}",
        amount, input_mint, output_mint
    );

    // Validate parameters and get context
    let api_clients = validate_quote_params(&input_mint, &output_mint, context)?;

    // Determine API URL
    let api_url =
        jupiter_api_url.unwrap_or_else(|| format!("{}/v6", api_clients.jupiter.api_url()));

    // Build quote request URL
    let url = build_quote_url(
        &input_mint,
        &output_mint,
        amount,
        slippage_bps,
        only_direct_routes,
        &api_url,
    );

    // Request quote from API
    let quote_response = request_jupiter_quote(&url, &api_clients.jupiter).await?;

    // Calculate price impact
    let price_impact = calculate_price_impact(&quote_response);

    info!(
        "Jupiter quote: {} {} -> {} {} (price impact: {:.2}%)",
        amount,
        input_mint,
        quote_response.out_amount,
        output_mint,
        price_impact * 100.0_f64
    );

    Ok(Quote {
        context_slot: quote_response.context_slot,
        in_amount: quote_response.in_amount.to_string(),
        input_mint,
        other_amount_threshold: quote_response.other_amount_threshold.to_string(),
        out_amount: quote_response.out_amount.to_string(),
        output_mint,
        price_impact_pct: (price_impact * 100.0_f64).to_string(),
        route_plan: quote_response.route_plan.clone(),
        time_taken: quote_response.time_taken,
    })
}

// Manual Tool trait implementation for GetJupiterQuoteTool
impl riglr_core::Tool for GetJupiterQuoteTool {
    type Args = serde_json::Value;
    type Error = riglr_core::ToolError;
    type Output = riglr_core::JobResult;

    fn call<'life0, 'async_trait>(
        &'life0 self,
        args: Self::Args,
    ) -> Pin<Box<dyn Future<Output = Result<Self::Output, Self::Error>> + Send + 'async_trait>>
    where
        'life0: 'async_trait,
        Self: 'async_trait,
    {
        Box::pin(async move {
            #[derive(serde::Deserialize)]
            #[serde(rename_all = "camelCase")]
            struct ParsedArgs {
                amount: u64,
                input_mint: String,
                jupiter_api_url: Option<String>,
                only_direct_routes: bool,
                output_mint: String,
                slippage_bps: u16,
            }

            let parsed: ParsedArgs = serde_json::from_value(args).map_err(|e| {
                riglr_core::ToolError::permanent_string(format!("Invalid arguments: {e}"))
            })?;

            let result = get_jupiter_quote(
                parsed.input_mint,
                parsed.output_mint,
                parsed.amount,
                parsed.slippage_bps,
                parsed.only_direct_routes,
                parsed.jupiter_api_url,
                &self.context,
            )
            .await?;

            Ok(riglr_core::JobResult::Success {
                value: serde_json::to_value(result).map_err(|e| {
                    riglr_core::ToolError::permanent_string(format!("Serialization error: {e}"))
                })?,
                tx_hash: None,
            })
        })
    }

    fn description(&self) -> &'static str {
        "Get a price quote from Jupiter aggregator for token swaps"
    }

    fn name(&self) -> &'static str {
        "get_jupiter_quote"
    }

    fn schema(&self) -> serde_json::Value {
        #[derive(schemars::JsonSchema, serde::Serialize, serde::Deserialize)]
        #[serde(rename_all = "camelCase")]
        struct Args {
            amount: u64,
            input_mint: String,
            jupiter_api_url: Option<String>,
            only_direct_routes: bool,
            output_mint: String,
            slippage_bps: u16,
        }

        let schema = SchemaGenerator::default().into_root_schema_for::<Args>();
        serde_json::to_value(schema)
            .unwrap_or_else(|_| serde_json::Value::Object(serde_json::Map::new()))
    }
}

/// Prepare swap context with signer and API clients
fn prepare_swap_context(
    context: &ApplicationContext,
    jupiter_api_url: Option<String>,
) -> Result<(SolanaSignerHandle, Pubkey, Clients, String), ToolError> {
    // Get signer from context
    let signer_context = SignerContext::current_as_solana().map_err(|signer_err| {
        ToolError::permanent_string(format!("No Solana signer context: {signer_err}"))
    })?;

    let signer_pubkey =
        signer_context
            .signer()
            .pubkey()
            .parse::<Pubkey>()
            .map_err(|parse_err| {
                ToolError::permanent_string(format!("Invalid signer pubkey: {parse_err}"))
            })?;

    // Get the API clients from context
    let api_clients = context
        .get_extension::<Arc<Clients>>()
        .ok_or_else(|| ToolError::permanent_string("Clients not found in context"))?;

    let api_url =
        jupiter_api_url.unwrap_or_else(|| format!("{}/v6", api_clients.jupiter.api_url()));

    Ok((
        signer_context,
        signer_pubkey,
        (**api_clients).clone(),
        api_url,
    ))
}

/// Build swap request payload from quote
fn build_swap_request(
    signer_pubkey: &Pubkey,
    quote: &Quote,
    use_versioned_transaction: bool,
) -> serde_json::Value {
    json!({
        "userPublicKey": signer_pubkey,
        "quoteResponse": {
            "inputMint": quote.input_mint,
            "outputMint": quote.output_mint,
            "inAmount": quote.in_amount.to_string(),
            "outAmount": quote.out_amount.to_string(),
            "otherAmountThreshold": quote.other_amount_threshold.to_string(),
            "routePlan": quote.route_plan,
            "contextSlot": quote.context_slot,
        },
        "wrapAndUnwrapSol": true,
        "useSharedAccounts": true,
        "prioritizationFeeLamports": "auto",
        "asLegacyTransaction": !use_versioned_transaction,
    })
}

/// Request swap transaction from Jupiter API
async fn request_jupiter_swap_transaction(
    api_url: &str,
    swap_request: &serde_json::Value,
) -> Result<JupiterSwapResponse, ToolError> {
    debug!("Requesting swap transaction from Jupiter");

    let reqwest_client = reqwest::Client::new();
    let response = reqwest_client
        .post(format!("{api_url}/swap"))
        .json(swap_request)
        .send()
        .await
        .map_err(|request_err| {
            ToolError::retriable_string(format!(
                "Failed to request swap transaction: {request_err}"
            ))
        })?;

    if !response.status().is_success() {
        let error_text = response
            .text()
            .await
            .unwrap_or_else(|_| "Unknown error".to_owned());
        return Err(ToolError::permanent_string(format!(
            "Jupiter swap API error: {error_text}"
        )));
    }

    let swap_response: JupiterSwapResponse = response.json().await.map_err(|parse_err| {
        ToolError::permanent_string(format!("Failed to parse swap response: {parse_err}"))
    })?;

    Ok(swap_response)
}

/// Process and execute swap transaction
async fn execute_swap_transaction(
    swap_response: &JupiterSwapResponse,
    input_mint: &str,
    output_mint: &str,
) -> Result<String, ToolError> {
    // Deserialize and sign the transaction
    use base64::{engine::general_purpose, Engine as _};
    let transaction_bytes = general_purpose::STANDARD
        .decode(&swap_response.swap_transaction)
        .map_err(|decode_err| {
            ToolError::permanent_string(format!("Failed to decode transaction: {decode_err}"))
        })?;

    let mut transaction: SolanaTransaction =
        bincode::deserialize(&transaction_bytes).map_err(|deserialize_err| {
            ToolError::permanent_string(format!(
                "Failed to deserialize transaction: {deserialize_err}"
            ))
        })?;

    // Send transaction with retry logic
    let signature = send(
        &mut transaction,
        &format!("Jupiter Swap ({input_mint} -> {output_mint})"),
    )
    .await?;

    Ok(signature)
}

/// Execute a token swap using Jupiter
///
/// This tool executes an actual token swap using the Jupiter aggregator. It automatically
/// gets a fresh quote, constructs the swap transaction, signs it with the current signer context,
/// and submits it to the Solana network. The swap uses optimal routing across multiple DEXs.
///
/// # Arguments
///
/// * `input_mint` - Source token mint address to swap from
/// * `output_mint` - Destination token mint address to swap to
/// * `amount` - Input amount in token's smallest unit
/// * `slippage_bps` - Maximum acceptable slippage in basis points (e.g., 50 = 0.5%)
/// * `jupiter_api_url` - Optional custom Jupiter API endpoint URL
/// * `use_versioned_transaction` - Whether to use versioned transactions (recommended for lower fees)
///
/// # Returns
///
/// Returns `Transaction` containing:
/// - `signature`: Transaction signature for tracking
/// - `input_mint` and `output_mint`: Token addresses involved
/// - `in_amount` and `out_amount`: Actual swap amounts
/// - `price_impact_pct`: Price impact percentage experienced
/// - `status`: Current transaction status (initially Pending)
///
/// # Errors
///
/// * `ToolError::Permanent` - When addresses invalid, signer unavailable, or swap construction fails
/// * `ToolError::Retriable` - When Jupiter API unavailable or network issues occur
///
/// # Examples
///
/// ```rust,ignore
/// use riglr_solana_tools::swap::perform_jupiter;
/// use riglr_core::SignerContext;
///
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// // Swap 0.1 SOL to USDC
/// let result = perform_jupiter(
///     "So11111111111111111111111111111111111111112".to_string(), // SOL mint
///     "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(), // USDC mint
///     100_000_000, // 0.1 SOL in lamports
///     100, // 1% slippage tolerance
///     None, // Use default Jupiter API
///     true, // Use versioned transactions
/// ).await?;
///
/// println!("Swap executed! Signature: {}", result.signature);
/// println!("Swapped {} for {} tokens", result.in_amount, result.out_amount);
/// # Ok(())
/// # }
/// ```
#[tool]
#[inline]
pub async fn perform_jupiter(
    input_mint: String,
    output_mint: String,
    amount: u64,
    slippage_bps: u16,
    jupiter_api_url: Option<String>,
    use_versioned_transaction: bool,
    context: &ApplicationContext,
) -> Result<Transaction, ToolError> {
    debug!(
        "Executing Jupiter swap: {} {} -> {}",
        amount, input_mint, output_mint
    );

    // Prepare swap context
    let (_signer_context, signer_pubkey, _api_clients, api_url) =
        prepare_swap_context(context, jupiter_api_url)?;

    // Get quote
    let quote = get_jupiter_quote(
        input_mint.clone(),
        output_mint.clone(),
        amount,
        slippage_bps,
        false,
        Some(api_url.clone()),
        context,
    )
    .await?;

    // Build swap request
    let swap_request = build_swap_request(&signer_pubkey, &quote, use_versioned_transaction);

    // Request swap transaction from API
    let swap_response = request_jupiter_swap_transaction(&api_url, &swap_request).await?;

    // Execute the swap transaction
    let signature = execute_swap_transaction(&swap_response, &input_mint, &output_mint).await?;

    info!(
        "Jupiter swap executed: {} {} -> {} {} (expected), signature: {}",
        quote.in_amount, input_mint, quote.out_amount, output_mint, signature
    );

    Ok(Transaction {
        signature,
        input_mint,
        output_mint,
        in_amount: quote.in_amount,
        out_amount: quote.out_amount,
        price_impact_pct: quote.price_impact_pct.parse().unwrap_or(0.0),
        status: Status::Pending,
        idempotency_key: None,
    })
}

// Manual Tool trait implementation for PerformJupiterTool
impl riglr_core::Tool for PerformJupiterTool {
    type Args = serde_json::Value;
    type Error = riglr_core::ToolError;
    type Output = riglr_core::JobResult;

    fn call<'life0, 'async_trait>(
        &'life0 self,
        args: Self::Args,
    ) -> Pin<Box<dyn Future<Output = Result<Self::Output, Self::Error>> + Send + 'async_trait>>
    where
        'life0: 'async_trait,
        Self: 'async_trait,
    {
        Box::pin(async move {
            #[derive(serde::Deserialize)]
            #[serde(rename_all = "camelCase")]
            struct ParsedArgs {
                amount: u64,
                input_mint: String,
                jupiter_api_url: Option<String>,
                output_mint: String,
                slippage_bps: u16,
                use_versioned_transaction: bool,
            }

            let parsed: ParsedArgs = serde_json::from_value(args).map_err(|e| {
                riglr_core::ToolError::permanent_string(format!("Invalid arguments: {e}"))
            })?;

            let result = perform_jupiter(
                parsed.input_mint,
                parsed.output_mint,
                parsed.amount,
                parsed.slippage_bps,
                parsed.jupiter_api_url,
                parsed.use_versioned_transaction,
                &self.context,
            )
            .await?;

            Ok(riglr_core::JobResult::Success {
                value: serde_json::to_value(&result).map_err(|e| {
                    riglr_core::ToolError::permanent_string(format!("Serialization error: {e}"))
                })?,
                tx_hash: Some(result.signature),
            })
        })
    }

    fn description(&self) -> &'static str {
        "Execute a token swap using Jupiter"
    }

    fn name(&self) -> &'static str {
        "perform_jupiter"
    }

    fn schema(&self) -> serde_json::Value {
        #[derive(schemars::JsonSchema, serde::Serialize, serde::Deserialize)]
        #[serde(rename_all = "camelCase")]
        struct Args {
            amount: u64,
            input_mint: String,
            jupiter_api_url: Option<String>,
            output_mint: String,
            slippage_bps: u16,
            use_versioned_transaction: bool,
        }

        let schema = SchemaGenerator::default().into_root_schema_for::<Args>();
        serde_json::to_value(schema)
            .unwrap_or_else(|_| serde_json::Value::Object(serde_json::Map::new()))
    }
}

/// Get the current price of a token pair
///
/// This tool fetches the current exchange rate between two SPL tokens by requesting
/// a small test quote from Jupiter. This provides real-time pricing without executing trades.
///
/// # Arguments
///
/// * `base_mint` - Token address to price (the token being quoted)
/// * `quote_mint` - Token address to price against (usually USDC or SOL)
/// * `jupiter_api_url` - Optional custom Jupiter API endpoint URL
///
/// # Returns
///
/// Returns `PriceInfo` containing:
/// - `base_mint` and `quote_mint`: Token addresses used
/// - `price`: Exchange rate (how much `quote_mint` per 1 `base_mint`)
/// - `price_impact_pct`: Price impact for small test trade
///
/// # Errors
///
/// * `ToolError::Permanent` - When tokens are invalid or no liquidity exists
/// * `ToolError::Retriable` - When Jupiter API is temporarily unavailable
///
/// # Examples
///
/// ```rust,ignore
/// use riglr_solana_tools::swap::get_token_price;
///
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// // Get SOL price in USDC
/// let price_info = get_token_price(
///     "So11111111111111111111111111111111111111112".to_string(), // SOL mint
///     "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(), // USDC mint
///     None, // Use default Jupiter API
/// ).await?;
///
/// println!("1 SOL = {} USDC", price_info.price);
/// println!("Price impact: {:.3}%", price_info.price_impact_pct);
/// # Ok(())
/// # }
/// ```
#[tool]
#[inline]
pub async fn get_token_price(
    base_mint: String,
    quote_mint: String,
    jupiter_api_url: Option<String>,
    context: &ApplicationContext,
) -> Result<PriceInfo, ToolError> {
    debug!("Getting price for {} in terms of {}", base_mint, quote_mint);

    // Validate mint addresses first (for better error messages in tests)
    let _base_pubkey = Pubkey::from_str(&base_mint).map_err(|parse_err| {
        ToolError::permanent_string(format!("Invalid input mint: {parse_err}"))
    })?;
    let _quote_pubkey = Pubkey::from_str(&quote_mint).map_err(|parse_err| {
        ToolError::permanent_string(format!("Invalid output mint: {parse_err}"))
    })?;

    // Get the API clients from context
    let api_clients = context
        .get_extension::<Arc<Clients>>()
        .ok_or_else(|| ToolError::permanent_string("Clients not found in context"))?;

    let api_url =
        jupiter_api_url.unwrap_or_else(|| format!("{}/v6", api_clients.jupiter.api_url()));

    // Get a small quote to determine price
    let amount = 1_000_000; // 1 token with 6 decimals
    let quote = get_jupiter_quote(
        base_mint.clone(),
        quote_mint.clone(),
        amount,
        50, // 0.5% slippage
        false,
        Some(api_url),
        context,
    )
    .await?;

    // Calculate price with precision loss warning suppression
    // Note: For price calculations, we need f64 representation despite potential precision loss
    // since prices are typically displayed to users and don't require exact integer precision.
    // For large token amounts (>2^52), some precision may be lost, but this is acceptable for price display.
    let out_amount: f64 = quote.out_amount.parse().map_err(|parse_err| {
        ToolError::permanent_string(format!("Invalid out_amount in quote: {parse_err}"))
    })?;
    let in_amount: f64 = quote.in_amount.parse().map_err(|parse_err| {
        ToolError::permanent_string(format!("Invalid in_amount in quote: {parse_err}"))
    })?;
    let price = if in_amount == 0.0 {
        0.0
    } else {
        out_amount / in_amount
    };

    Ok(PriceInfo {
        mint: base_mint.clone(),
        base_mint,
        quote_mint,
        price,
        symbol: "UNKNOWN".to_string(), // Could be enhanced to fetch actual symbol
        source: "Jupiter".to_string(),
        price_impact_pct: quote.price_impact_pct.parse().unwrap_or(0.0),
    })
}

// Manual Tool trait implementation for GetTokenPriceTool
impl riglr_core::Tool for GetTokenPriceTool {
    type Args = serde_json::Value;
    type Error = riglr_core::ToolError;
    type Output = riglr_core::JobResult;

    fn call<'life0, 'async_trait>(
        &'life0 self,
        args: Self::Args,
    ) -> Pin<Box<dyn Future<Output = Result<Self::Output, Self::Error>> + Send + 'async_trait>>
    where
        'life0: 'async_trait,
        Self: 'async_trait,
    {
        Box::pin(async move {
            #[derive(serde::Deserialize)]
            #[serde(rename_all = "camelCase")]
            struct ParsedArgs {
                base_mint: String,
                jupiter_api_url: Option<String>,
                quote_mint: String,
            }

            let parsed: ParsedArgs = serde_json::from_value(args).map_err(|e| {
                riglr_core::ToolError::permanent_string(format!("Invalid arguments: {e}"))
            })?;

            let result = get_token_price(
                parsed.base_mint,
                parsed.quote_mint,
                parsed.jupiter_api_url,
                &self.context,
            )
            .await?;

            Ok(riglr_core::JobResult::Success {
                value: serde_json::to_value(result).map_err(|e| {
                    riglr_core::ToolError::permanent_string(format!("Serialization error: {e}"))
                })?,
                tx_hash: None,
            })
        })
    }

    fn description(&self) -> &'static str {
        "Get the current price of a token pair"
    }

    fn name(&self) -> &'static str {
        "get_token_price"
    }

    fn schema(&self) -> serde_json::Value {
        #[derive(schemars::JsonSchema, serde::Serialize, serde::Deserialize)]
        #[serde(rename_all = "camelCase")]
        struct Args {
            base_mint: String,
            jupiter_api_url: Option<String>,
            quote_mint: String,
        }

        let schema = SchemaGenerator::default().into_root_schema_for::<Args>();
        serde_json::to_value(schema)
            .unwrap_or_else(|_| serde_json::Value::Object(serde_json::Map::new()))
    }
}

/// Jupiter quote response
#[derive(Debug, Clone, Serialize, Deserialize)]
struct JupiterQuoteResponse {
    /// Solana slot context for quote freshness
    pub context_slot: Option<u64>,
    /// Input amount in smallest token units
    pub in_amount: u64,
    /// Minimum output amount after slippage
    pub other_amount_threshold: u64,
    /// Output amount in smallest token units
    pub out_amount: u64,
    /// Price impact percentage for the swap
    pub price_impact_pct: Option<f64>,
    /// Detailed routing plan through DEXs
    pub route_plan: Vec<RoutePlanStep>,
    /// Time taken to compute the quote in seconds
    pub time_taken: Option<f64>,
}

/// Jupiter swap response
#[derive(Debug, Clone, Serialize, Deserialize)]
struct JupiterSwapResponse {
    /// Last valid block height for transaction expiry
    pub last_valid_block_height: u64,
    /// Optional prioritization fee in lamports
    pub prioritization_fee: Option<u64>,
    /// Base64 encoded transaction ready for signing
    pub swap_transaction: String,
}

// Re-export the macro-generated tool structs for public API
pub use __riglr_tool_get_jupiter_quote::GetJupiterQuoteTool;
pub use __riglr_tool_get_token_price::GetTokenPriceTool;
pub use __riglr_tool_perform_jupiter::PerformJupiterTool;

/// Calculate price impact from Jupiter quote response
#[inline]
fn calculate_price_impact(quote: &JupiterQuoteResponse) -> f64 {
    // Jupiter provides price impact in the response
    // This is a simplified calculation
    quote.price_impact_pct.unwrap_or(0.0)
}

// Re-exports for public API (maintaining backward compatibility)
pub use Info as JupiterInfo;
pub use Quote as JupiterQuote;
pub use Transaction as JupiterResult;

// Function alias for backward compatibility
pub use perform_jupiter as execute;

#[cfg(test)]
#[expect(clippy::float_cmp, clippy::expect_used, clippy::unwrap_used)]
mod tests {
    use super::*;
    use crate::clients::JUPITER_API_URL;

    // Jupiter API URL tests
    #[test]
    fn test_default_jupiter_api_url() {
        use std::env;
        let jupiter_api_url =
            env::var(JUPITER_API_URL).unwrap_or_else(|_| "https://quote-api.jup.ag".to_string());
        assert!(jupiter_api_url.contains("jup.ag") || jupiter_api_url.contains("jupiter"));
    }

    // calculate_price_impact tests
    #[test]
    fn test_calculate_price_impact_with_price_impact() {
        let quote = JupiterQuoteResponse {
            in_amount: 1_000_000,
            out_amount: 990_000,
            other_amount_threshold: 980_000,
            route_plan: vec![],
            context_slot: Some(123_456),
            time_taken: Some(0.1),
            price_impact_pct: Some(1.5),
        };
        let impact = calculate_price_impact(&quote);
        assert_eq!(impact, 1.5);
    }

    #[test]
    fn test_calculate_price_impact_with_none_returns_zero() {
        let quote = JupiterQuoteResponse {
            in_amount: 1_000_000,
            out_amount: 990_000,
            other_amount_threshold: 980_000,
            route_plan: vec![],
            context_slot: None,
            time_taken: None,
            price_impact_pct: None,
        };
        let impact = calculate_price_impact(&quote);
        assert_eq!(impact, 0.0);
    }

    // Serialization/Deserialization tests
    #[test]
    fn test_swap_quote_serialization() {
        let quote = Quote {
            input_mint: "So11111111111111111111111111111111111111112".to_string(),
            output_mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            in_amount: "1000000000".to_string(),
            out_amount: "50000000".to_string(),
            other_amount_threshold: "49500000".to_string(),
            price_impact_pct: "0.5".to_string(),
            route_plan: vec![],
            context_slot: Some(123_456),
            time_taken: Some(0.123),
        };

        let json = serde_json::to_string(&quote).expect("Failed to serialize Quote to JSON");
        assert!(json.contains("input_mint"));
        assert!(json.contains("1000000000"));

        let deserialized: Quote =
            serde_json::from_str(&json).expect("Failed to deserialize Quote from JSON");
        assert_eq!(deserialized.input_mint, quote.input_mint);
        assert_eq!(deserialized.out_amount, quote.out_amount);
    }

    #[test]
    fn test_swap_result_serialization() {
        let result = Transaction {
            signature: "test_signature".to_string(),
            input_mint: "So11111111111111111111111111111111111111112".to_string(),
            output_mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            in_amount: "1000000000".to_string(),
            out_amount: "50000000".to_string(),
            price_impact_pct: 0.5,
            status: Status::Pending,
            idempotency_key: Some("test_key".to_string()),
        };

        let json = serde_json::to_string(&result).expect("Failed to serialize Transaction to JSON");
        assert!(json.contains("test_signature"));

        let deserialized: Transaction =
            serde_json::from_str(&json).expect("Failed to deserialize Transaction from JSON");
        assert_eq!(deserialized.signature, result.signature);
        assert_eq!(deserialized.idempotency_key, result.idempotency_key);
    }

    #[test]
    fn test_price_info_serialization() {
        let price_info = PriceInfo {
            mint: "So11111111111111111111111111111111111111112".to_string(),
            base_mint: "So11111111111111111111111111111111111111112".to_string(),
            quote_mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            price: 50.5,
            symbol: "SOL".to_string(),
            source: "Jupiter".to_string(),
            price_impact_pct: 0.1,
        };

        let json =
            serde_json::to_string(&price_info).expect("Failed to serialize PriceInfo to JSON");
        assert!(json.contains("50.5"));

        let deserialized: PriceInfo =
            serde_json::from_str(&json).expect("Failed to deserialize PriceInfo from JSON");
        {
            assert_eq!(deserialized.price, price_info.price);
        }
        assert_eq!(deserialized.base_mint, price_info.base_mint);
    }

    #[test]
    fn test_route_plan_step_serialization() {
        let swap_info = Info {
            amm_key: "test_amm_key".to_string(),
            label: Some("Raydium".to_string()),
            input_mint: "So11111111111111111111111111111111111111112".to_string(),
            output_mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            in_amount: "1000000".to_string(),
            out_amount: "50000".to_string(),
            fee_amount: "1000".to_string(),
            fee_mint: "So11111111111111111111111111111111111111112".to_string(),
        };

        let route_step = RoutePlanStep {
            swap_info,
            percent: 100,
        };

        let json =
            serde_json::to_string(&route_step).expect("Failed to serialize RoutePlanStep to JSON");
        assert!(json.contains("test_amm_key"));
        assert!(json.contains("Raydium"));

        let deserialized: RoutePlanStep =
            serde_json::from_str(&json).expect("Failed to deserialize RoutePlanStep from JSON");
        assert_eq!(deserialized.percent, 100);
        assert_eq!(deserialized.swap_info.label, Some("Raydium".to_string()));
    }

    #[test]
    fn test_swap_info_with_none_label() {
        let swap_info = Info {
            amm_key: "test_amm_key".to_string(),
            label: None,
            input_mint: "So11111111111111111111111111111111111111112".to_string(),
            output_mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            in_amount: "1000000".to_string(),
            out_amount: "50000".to_string(),
            fee_amount: "1000".to_string(),
            fee_mint: "So11111111111111111111111111111111111111112".to_string(),
        };

        let json = serde_json::to_string(&swap_info).expect("Failed to serialize Info to JSON");
        let deserialized: Info =
            serde_json::from_str(&json).expect("Failed to deserialize Info from JSON");
        assert_eq!(deserialized.label, None);
        assert_eq!(deserialized.amm_key, "test_amm_key");
    }

    #[test]
    fn test_jupiter_quote_response_serialization() {
        let response = JupiterQuoteResponse {
            in_amount: 1_000_000,
            out_amount: 990_000,
            other_amount_threshold: 980_000,
            route_plan: vec![],
            context_slot: Some(123_456),
            time_taken: Some(0.5),
            price_impact_pct: Some(1.0),
        };

        let json = serde_json::to_string(&response)
            .expect("Failed to serialize JupiterQuoteResponse to JSON");
        let deserialized: JupiterQuoteResponse = serde_json::from_str(&json)
            .expect("Failed to deserialize JupiterQuoteResponse from JSON");
        assert_eq!(deserialized.in_amount, 1_000_000);
        assert_eq!(deserialized.price_impact_pct, Some(1.0));
    }

    #[test]
    fn test_jupiter_swap_response_serialization() {
        let response = JupiterSwapResponse {
            swap_transaction: "base64_transaction_data".to_string(),
            last_valid_block_height: 123_456_789,
            prioritization_fee: Some(5000),
        };

        let json = serde_json::to_string(&response)
            .expect("Failed to serialize JupiterSwapResponse to JSON");
        let deserialized: JupiterSwapResponse = serde_json::from_str(&json)
            .expect("Failed to deserialize JupiterSwapResponse from JSON");
        assert_eq!(deserialized.swap_transaction, "base64_transaction_data");
        assert_eq!(deserialized.last_valid_block_height, 123_456_789);
        assert_eq!(deserialized.prioritization_fee, Some(5000));
    }

    #[test]
    fn test_jupiter_swap_response_with_none_fee() {
        let response = JupiterSwapResponse {
            swap_transaction: "base64_transaction_data".to_string(),
            last_valid_block_height: 123_456_789,
            prioritization_fee: None,
        };

        let json = serde_json::to_string(&response)
            .expect("Failed to serialize JupiterSwapResponse to JSON");
        let deserialized: JupiterSwapResponse = serde_json::from_str(&json)
            .expect("Failed to deserialize JupiterSwapResponse from JSON");
        assert_eq!(deserialized.prioritization_fee, None);
    }

    // Debug and Clone trait tests
    #[test]
    fn test_struct_debug_implementations() {
        let swap_info = Info {
            amm_key: "test".to_string(),
            label: Some("Test".to_string()),
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "900".to_string(),
            fee_amount: "10".to_string(),
            fee_mint: "fee".to_string(),
        };

        let route_step = RoutePlanStep {
            swap_info: swap_info.clone(),
            percent: 50,
        };

        let quote = Quote {
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "900".to_string(),
            other_amount_threshold: "890".to_string(),
            price_impact_pct: "0.5".to_string(),
            route_plan: vec![route_step.clone()],
            context_slot: Some(123),
            time_taken: Some(0.1),
        };

        let result = Transaction {
            signature: "sig".to_string(),
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "900".to_string(),
            price_impact_pct: 0.5,
            status: Status::Pending,
            idempotency_key: None,
        };

        let price_info = PriceInfo {
            mint: "base".to_string(),
            base_mint: "base".to_string(),
            quote_mint: "quote".to_string(),
            price: 1.5,
            symbol: "TEST".to_string(),
            source: "Jupiter".to_string(),
            price_impact_pct: 0.1,
        };

        // Test Debug implementations
        let debug_swap_info = format!("{swap_info:?}");
        let debug_route_step = format!("{route_step:?}");
        let debug_quote = format!("{quote:?}");
        let debug_result = format!("{result:?}");
        let debug_price_info = format!("{price_info:?}");

        assert!(debug_swap_info.contains("Info"));
        assert!(debug_route_step.contains("RoutePlanStep"));
        assert!(debug_quote.contains("Quote"));
        assert!(debug_result.contains("Transaction"));
        assert!(debug_price_info.contains("PriceInfo"));

        // Test Clone implementations
        let cloned_swap_info = swap_info.clone();
        let cloned_route_step = route_step.clone();
        let cloned_quote = quote.clone();
        let cloned_result = result.clone();
        let cloned_price_info = price_info.clone();

        assert_eq!(cloned_swap_info.amm_key, swap_info.amm_key);
        assert_eq!(cloned_route_step.percent, route_step.percent);
        assert_eq!(cloned_quote.in_amount, quote.in_amount);
        assert_eq!(cloned_result.signature, result.signature);
        {
            assert_eq!(cloned_price_info.price, price_info.price);
        }
    }

    // Edge case tests for struct field variations
    #[test]
    fn test_swap_quote_with_empty_route_plan() {
        let quote = Quote {
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "900".to_string(),
            other_amount_threshold: "890".to_string(),
            price_impact_pct: "0.0".to_string(),
            route_plan: vec![],
            context_slot: None,
            time_taken: None,
        };

        assert!(quote.route_plan.is_empty());
        assert_eq!(quote.context_slot, None);
        assert_eq!(quote.time_taken, None);
        assert_eq!(quote.price_impact_pct, "0.0");
    }

    #[test]
    fn test_swap_result_with_none_idempotency_key() {
        let result = Transaction {
            signature: "sig".to_string(),
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "900".to_string(),
            price_impact_pct: 0.5,
            status: Status::Confirmed,
            idempotency_key: None,
        };

        assert_eq!(result.idempotency_key, None);
        assert!(matches!(result.status, Status::Confirmed));
    }

    #[test]
    fn test_route_plan_step_with_zero_percent() {
        let swap_info = Info {
            amm_key: "test".to_string(),
            label: None,
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "0".to_string(),
            out_amount: "0".to_string(),
            fee_amount: "0".to_string(),
            fee_mint: "fee".to_string(),
        };

        let route_step = RoutePlanStep {
            swap_info,
            percent: 0,
        };

        assert_eq!(route_step.percent, 0);
        assert_eq!(route_step.swap_info.in_amount, "0");
    }

    #[test]
    fn test_price_info_with_zero_price() {
        let price_info = PriceInfo {
            mint: "base".to_string(),
            base_mint: "base".to_string(),
            quote_mint: "quote".to_string(),
            price: 0.0,
            symbol: "TEST".to_string(),
            source: "Jupiter".to_string(),
            price_impact_pct: 0.0,
        };

        {
            assert_eq!(price_info.price, 0.0);
            assert_eq!(price_info.price_impact_pct, 0.0);
        }
    }

    #[test]
    fn test_price_info_with_negative_price_impact() {
        let price_info = PriceInfo {
            mint: "base".to_string(),
            base_mint: "base".to_string(),
            quote_mint: "quote".to_string(),
            price: 1.0,
            symbol: "TEST".to_string(),
            source: "Jupiter".to_string(),
            price_impact_pct: -0.1, // Negative price impact (beneficial)
        };

        {
            assert_eq!(price_info.price_impact_pct, -0.1);
        }
    }

    #[test]
    fn test_jupiter_quote_response_with_all_none_optionals() {
        let response = JupiterQuoteResponse {
            in_amount: 1000,
            out_amount: 900,
            other_amount_threshold: 890,
            route_plan: vec![],
            context_slot: None,
            time_taken: None,
            price_impact_pct: None,
        };

        assert_eq!(response.context_slot, None);
        assert_eq!(response.time_taken, None);
        assert_eq!(response.price_impact_pct, None);
    }

    #[test]
    fn test_swap_info_with_empty_strings() {
        let swap_info = Info {
            amm_key: String::new(),
            label: Some(String::new()),
            input_mint: String::new(),
            output_mint: String::new(),
            in_amount: String::new(),
            out_amount: String::new(),
            fee_amount: String::new(),
            fee_mint: String::new(),
        };

        assert_eq!(swap_info.amm_key, "");
        assert_eq!(swap_info.label, Some(String::new()));
        assert_eq!(swap_info.input_mint, "");
    }

    #[test]
    fn test_large_numeric_values() {
        let quote = Quote {
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: u64::MAX.to_string(),
            out_amount: (u64::MAX - 1).to_string(),
            other_amount_threshold: (u64::MAX - 2).to_string(),
            price_impact_pct: f64::MAX.to_string(),
            route_plan: vec![],
            context_slot: Some(u64::MAX),
            time_taken: Some(f64::MAX),
        };

        assert_eq!(quote.in_amount, u64::MAX.to_string());
        assert_eq!(quote.out_amount, (u64::MAX - 1).to_string());
        assert_eq!(quote.price_impact_pct, f64::MAX.to_string());
    }

    // Test struct field access and manipulation
    #[test]
    fn test_mutable_struct_modifications() {
        let mut quote = Quote {
            input_mint: "old".to_string(),
            output_mint: "old".to_string(),
            in_amount: "100".to_string(),
            out_amount: "90".to_string(),
            other_amount_threshold: "85".to_string(),
            price_impact_pct: "1.0".to_string(),
            route_plan: vec![],
            context_slot: Some(123),
            time_taken: Some(0.1),
        };

        quote.input_mint = "new".to_string();
        quote.in_amount = "200".to_string();
        quote.price_impact_pct = "2.0".to_string();

        assert_eq!(quote.input_mint, "new");
        assert_eq!(quote.in_amount, "200");
        assert_eq!(quote.price_impact_pct, "2.0");
    }

    // Helper function to create a test context with Clients
    fn create_test_context() -> ApplicationContext {
        // Load .env.test for test environment
        dotenvy::from_filename(".env.test").ok();

        let config = riglr_config::Config::from_env();
        let context = ApplicationContext::from_config(&Arc::new(config.clone()));

        // Create and inject Clients
        let api_clients = Clients::new(&config.providers);
        context.set_extension(Arc::new(api_clients));

        context
    }

    // Tests for async function input validation
    #[tokio::test]
    async fn test_get_jupiter_quote_invalid_input_mint() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "invalid_mint".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            50,
            false,
            None,
            &context,
        )
        .await;

        assert!(result.is_err());
        let error = result.unwrap_err();
        assert!(error.to_string().contains("Invalid input mint"));
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_invalid_output_mint() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "So11111111111111111111111111111111111111112".to_string(),
            "invalid_mint".to_string(),
            1_000_000,
            50,
            false,
            None,
            &context,
        )
        .await;

        assert!(result.is_err());
        let error = result.unwrap_err();
        assert!(error.to_string().contains("Invalid output mint"));
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_empty_input_mint() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            String::new(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            50,
            false,
            None,
            &context,
        )
        .await;

        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_empty_output_mint() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "So11111111111111111111111111111111111111112".to_string(),
            String::new(),
            1_000_000,
            50,
            false,
            None,
            &context,
        )
        .await;

        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_zero_amount() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "So11111111111111111111111111111111111111112".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            0,
            50,
            false,
            None,
            &context,
        )
        .await;

        // Zero amount should pass validation but fail on Jupiter API
        // This tests that our validation allows zero (Jupiter will handle the business logic)
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_max_slippage() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "So11111111111111111111111111111111111111112".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            u16::MAX,
            false,
            None,
            &context,
        )
        .await;

        // Should pass validation but fail on Jupiter API due to network issues
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_with_custom_api_url() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "So11111111111111111111111111111111111111112".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            50,
            false,
            Some("https://custom.api.com".to_string()),
            &context,
        )
        .await;

        // Should pass validation but fail on network request
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_jupiter_quote_only_direct_routes_true() {
        let context = create_test_context();
        let result = get_jupiter_quote(
            "So11111111111111111111111111111111111111112".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            50,
            true, // only_direct_routes
            None,
            &context,
        )
        .await;

        // Should pass validation but fail on network request
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_get_token_price_invalid_base_mint() {
        let context = create_test_context();
        let result = get_token_price(
            "invalid_mint".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            None,
            &context,
        )
        .await;

        assert!(result.is_err());
        let error = result.unwrap_err();
        // The error message should indicate invalid mint format
        let error_str = error.to_string();
        assert!(
            error_str.contains("Invalid input mint") || error_str.contains("Invalid base58"),
            "Error was: {error_str}"
        );
    }

    #[tokio::test]
    async fn test_get_token_price_invalid_quote_mint() {
        let context = create_test_context();
        let result = get_token_price(
            "So11111111111111111111111111111111111111112".to_string(),
            "invalid_mint".to_string(),
            None,
            &context,
        )
        .await;

        assert!(result.is_err());
        let error = result.unwrap_err();
        // The error message should indicate invalid mint format
        let error_str = error.to_string();
        assert!(
            error_str.contains("Invalid output mint") || error_str.contains("Invalid base58"),
            "Error was: {error_str}"
        );
    }

    #[tokio::test]
    async fn test_get_token_price_with_custom_api() {
        let context = create_test_context();
        let result = get_token_price(
            "So11111111111111111111111111111111111111112".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            Some("https://custom.api.com".to_string()),
            &context,
        )
        .await;

        // Should pass validation but fail on network request
        assert!(result.is_err());
    }

    // Test perform_jupiter function error paths
    #[tokio::test]
    async fn test_perform_jupiter_invalid_input_mint() {
        let context = create_test_context();
        let result = perform_jupiter(
            "invalid_mint".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            50,
            None,
            true,
            &context,
        )
        .await;

        assert!(result.is_err());
        let error = result.expect_err("Expected error for missing signer context");
        // Without a signer context, the error will be about missing signer
        assert!(error.to_string().contains("No signer context available"));
    }

    #[tokio::test]
    async fn test_perform_jupiter_invalid_output_mint() {
        let context = create_test_context();
        let result = perform_jupiter(
            "So11111111111111111111111111111111111111112".to_string(),
            "invalid_mint".to_string(),
            1_000_000,
            50,
            None,
            false,
            &context,
        )
        .await;

        assert!(result.is_err());
        let error = result.expect_err("Expected error for missing signer context");
        // Without a signer context, the error will be about missing signer
        assert!(error.to_string().contains("No signer context available"));
    }

    #[tokio::test]
    async fn test_perform_jupiter_versioned_transaction_false() {
        let context = create_test_context();
        let result = perform_jupiter(
            "So11111111111111111111111111111111111111112".to_string(),
            "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            1_000_000,
            50,
            None,
            false, // use_versioned_transaction = false
            &context,
        )
        .await;

        // Should pass validation but fail due to no signer context
        assert!(result.is_err());
    }

    // Additional edge case tests
    #[test]
    fn test_route_plan_step_max_percent() {
        let swap_info = Info {
            amm_key: "test".to_string(),
            label: Some("Test".to_string()),
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "900".to_string(),
            fee_amount: "10".to_string(),
            fee_mint: "fee".to_string(),
        };

        let route_step = RoutePlanStep {
            swap_info,
            percent: 255, // u8::MAX
        };

        assert_eq!(route_step.percent, 255);
    }

    #[test]
    fn test_jupiter_quote_response_debug_clone() {
        let response = JupiterQuoteResponse {
            in_amount: 1000,
            out_amount: 900,
            other_amount_threshold: 890,
            route_plan: vec![],
            context_slot: Some(123),
            time_taken: Some(0.1),
            price_impact_pct: Some(1.0),
        };

        let cloned = response.clone();
        let debug_str = format!("{response:?}");

        assert_eq!(cloned.in_amount, response.in_amount);
        assert!(debug_str.contains("JupiterQuoteResponse"));
    }

    #[test]
    fn test_jupiter_swap_response_debug_clone() {
        let response = JupiterSwapResponse {
            swap_transaction: "test".to_string(),
            last_valid_block_height: 123,
            prioritization_fee: Some(100),
        };

        let cloned = response.clone();
        let debug_str = format!("{response:?}");

        assert_eq!(cloned.swap_transaction, response.swap_transaction);
        assert!(debug_str.contains("JupiterSwapResponse"));
    }

    // Test extreme numeric values
    #[test]
    fn test_jupiter_quote_response_with_zero_values() {
        let response = JupiterQuoteResponse {
            in_amount: 0,
            out_amount: 0,
            other_amount_threshold: 0,
            route_plan: vec![],
            context_slot: Some(0),
            time_taken: Some(0.0),
            price_impact_pct: Some(0.0),
        };

        assert_eq!(response.in_amount, 0);
        assert_eq!(response.out_amount, 0);
        assert_eq!(response.other_amount_threshold, 0);
    }

    #[test]
    fn test_jupiter_swap_response_with_zero_block_height() {
        let response = JupiterSwapResponse {
            swap_transaction: String::new(),
            last_valid_block_height: 0,
            prioritization_fee: Some(0),
        };

        assert_eq!(response.last_valid_block_height, 0);
        assert_eq!(response.prioritization_fee, Some(0));
    }

    // Test complex route plan
    #[test]
    fn test_complex_route_plan_serialization() {
        let swap_info1 = Info {
            amm_key: "key1".to_string(),
            label: Some("DEX1".to_string()),
            input_mint: "mint1".to_string(),
            output_mint: "mint2".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "500".to_string(),
            fee_amount: "5".to_string(),
            fee_mint: "mint1".to_string(),
        };

        let swap_info2 = Info {
            amm_key: "key2".to_string(),
            label: None,
            input_mint: "mint2".to_string(),
            output_mint: "mint3".to_string(),
            in_amount: "500".to_string(),
            out_amount: "450".to_string(),
            fee_amount: "2".to_string(),
            fee_mint: "mint2".to_string(),
        };

        let route_plan = vec![
            RoutePlanStep {
                swap_info: swap_info1,
                percent: 60,
            },
            RoutePlanStep {
                swap_info: swap_info2,
                percent: 40,
            },
        ];

        let quote = Quote {
            input_mint: "mint1".to_string(),
            output_mint: "mint3".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "450".to_string(),
            other_amount_threshold: "440".to_string(),
            price_impact_pct: "1.5".to_string(),
            route_plan,
            context_slot: Some(12345),
            time_taken: Some(0.25),
        };

        let json =
            serde_json::to_string(&quote).expect("Failed to serialize complex Quote to JSON");
        let deserialized: Quote =
            serde_json::from_str(&json).expect("Failed to deserialize complex Quote from JSON");

        assert_eq!(deserialized.route_plan.len(), 2);
        assert_eq!(
            deserialized
                .route_plan
                .first()
                .expect("Should have first route")
                .percent,
            60
        );
        assert_eq!(
            deserialized
                .route_plan
                .get(1)
                .expect("Should have second route")
                .percent,
            40
        );
        assert_eq!(
            deserialized
                .route_plan
                .first()
                .expect("Should have first route")
                .swap_info
                .label,
            Some("DEX1".to_string())
        );
        assert_eq!(
            deserialized
                .route_plan
                .get(1)
                .expect("Should have second route")
                .swap_info
                .label,
            None
        );
    }

    // Test struct field boundary values
    #[test]
    fn test_price_info_with_infinity_values() {
        let price_info = PriceInfo {
            mint: "base".to_string(),
            base_mint: "base".to_string(),
            quote_mint: "quote".to_string(),
            price: f64::INFINITY,
            symbol: "TEST".to_string(),
            source: "Jupiter".to_string(),
            price_impact_pct: f64::NEG_INFINITY,
        };

        {
            assert_eq!(price_info.price, f64::INFINITY);
            assert_eq!(price_info.price_impact_pct, f64::NEG_INFINITY);
        }
    }

    #[test]
    fn test_price_info_with_nan_values() {
        let price_info = PriceInfo {
            mint: "base".to_string(),
            base_mint: "base".to_string(),
            quote_mint: "quote".to_string(),
            price: f64::NAN,
            symbol: "TEST".to_string(),
            source: "Jupiter".to_string(),
            price_impact_pct: f64::NAN,
        };

        assert!(price_info.price.is_nan());
        assert!(price_info.price_impact_pct.is_nan());
    }

    // Test swap result with different transaction statuses
    #[test]
    fn test_swap_result_with_failed_status() {
        let result = Transaction {
            signature: "failed_sig".to_string(),
            input_mint: "input".to_string(),
            output_mint: "output".to_string(),
            in_amount: "1000".to_string(),
            out_amount: "0".to_string(), // No output due to failure
            price_impact_pct: 100.0,     // High impact
            status: Status::Failed("test failure".to_string()),
            idempotency_key: Some("retry_key".to_string()),
        };

        assert!(matches!(result.status, Status::Failed(_)));
        assert_eq!(result.out_amount, "0");
        assert_eq!(result.idempotency_key, Some("retry_key".to_string()));
    }
}
