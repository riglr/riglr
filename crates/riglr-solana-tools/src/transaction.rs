...
use crate::utils::transaction::send;
use core::mem;
...
/// Memo program ID constant for adding memos to transactions
...
/// Result of a SOL transfer operation
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[non_exhaustive]
...
/// # Returns
///
/// Returns `SolTransferResult` containing:
...
pub async fn transfer_sol(
...
) -> Result<SolTransferResult, ToolError> {
    debug!(
    ...
    // Validate inputs
    if amount_sol <= 0.0_f64 {
        return Err(ToolError::permanent_string("Amount must be positive"));
    }

    let to_pubkey = to_address
        .parse::<Pubkey>()
        .map_err(|e| ToolError::permanent_string(format!("Invalid recipient address: {e}")))?;

    // Convert SOL to lamports with proper validation
    #[expect(clippy::cast_precision_loss)]
    let lamports_f64 = amount_sol * (LAMPORTS_PER_SOL as f64);
    #[expect(clippy::cast_precision_loss)]
    let max_check = u64::MAX as f64;
    if lamports_f64 < 0.0 || lamports_f64 > max_check {
        return Err(ToolError::permanent_string("Amount out of valid range"));
    }
    #[expect(clippy::cast_sign_loss, clippy::cast_possible_truncation)]
    let lamports = lamports_f64.round() as u64;
    ...
    let signature = send(
        &mut transaction,
        &format!("SOL Transfer ({amount_sol} SOL)"),
    )
    .await?;
    ...
    Ok(SolTransferResult {
        signature,
        from: from_pubkey.to_string(),
        to: to_address,
        amount: lamports,
        amount_display: format!("{amount_sol} SOL"),
...
pub async fn transfer_spl_token(
    ...
    let signature = send(
        &mut transaction,
        &format!("SPL Token Transfer ({ui_amount:.9} tokens)"),
    )
    .await?;
    ...
    Ok(TokenTransferResult {
        ...
        ui_amount,
        decimals,
        amount_display: format!("{ui_amount:.9}"),
        status: Status::Pending,
        idempotency_key: None,
    })
}
...
pub async fn create_spl_token_mint(
    ...
    let payer_pubkey_str = signer_context.signer().pubkey();
    ...
    let mint_account_size = mem::size_of::<Mint>();
    let rent = client
    ...
    let signature = send(
        &mut transaction,
        &format!("Create SPL Token Mint ({decimals} decimals)"),
    )
    .await
...
// Manual Tool trait implementations for transaction tools
...
impl riglr_core::Tool for TransferSolTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        let to_address: String = serde_json::from_value(
            args.get("toAddress")
                .cloned()
        ...
        .map_err(|json_error| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid toAddress parameter: {json_error}"
            ))
        })?;
        let amount_sol: f64 = serde_json::from_value(
            args.get("amountSol")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|json_error| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid amountSol parameter: {json_error}"
            ))
        })?;
        let memo: Option<String> = args.get("memo").and_then(|v| v.as_str()).map(String::from);
        let priority_fee: Option<u64> = args.get("priorityFee").and_then(serde_json::Value::as_u64);
        let result =
            transfer_sol(to_address, amount_sol, memo, priority_fee, &self.context).await?;
    ...
    fn description(&self) -> &'static str {
        "Transfer SOL from the current signer to another address"
    }
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "amountSol": {
                    "type": "number",
                    "description": "Amount to transfer in SOL (decimal format, e.g., 0.1 for 100M lamports)"
                },
                "memo": {
                    "type": "string",
                ...
                "priorityFee": {
                    "type": "integer",
                    "description": "Optional priority fee in lamports to speed up transaction processing"
                }
            },
...
#[async_trait::async_trait]
impl riglr_core::Tool for TransferSplTokenTool {
    type Args = serde_json::Value;
    type Error = riglr_core::ToolError;
    type Output = riglr_core::JobResult;
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let amount: u64 = serde_json::from_value(
            args.get("amount")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|json_error| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid amount parameter: {json_error}"
            ))
        })?;
        ...
        let decimals: u8 = args
            ...
            .unwrap_or(9); // Default to 9 decimals if not provided
        let create_ata_if_needed: bool = args
            .get("createAtaIfNeeded")
    ...
    #[inline]
    fn description(&self) -> &'static str {
        "Transfer SPL tokens from the current signer to another address"
    }
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "amount": {
                    "type": "integer",
                    "description": "Amount to transfer in base units (raw token amount)"
                },
                "decimals": {
                    "type": "integer",
                    "description": "Number of decimal places for the token (0-255, commonly 6 or 9)"
                },
                "createAtaIfNeeded": {
                    "type": "boolean",
                    "description": "Whether to create the Associated Token Account if it doesn't exist (default: true)"
                },
                "priorityFee": {
                    "type": "integer",
                    "description": "Optional priority fee in lamports to speed up transaction processing"
...
#[async_trait::async_trait]
impl riglr_core::Tool for CreateSplTokenMintTool {
    type Args = serde_json::Value;
    type Error = riglr_core::ToolError;
    type Output = riglr_core::JobResult;
    ...
    #[inline]
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        let decimals: u8 = serde_json::from_value(
            args.get("decimals")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|json_error| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid decimals parameter: {json_error}"
            ))
        })?;
        let initial_supply: u64 = serde_json::from_value(
            args.get("initialSupply")
                .cloned()
                .unwrap_or(serde_json::Value::Null),
        )
        .map_err(|json_error| {
            riglr_core::ToolError::permanent_string(format!(
                "Invalid initialSupply parameter: {json_error}"
            ))
        })?;
        let freezable: bool = args
            .get("freezable")
            .and_then(serde_json::Value::as_bool)
            .unwrap_or(false);
        let result =
            create_spl_token_mint(decimals, initial_supply, freezable, &self.context).await?;
        return riglr_core::JobResult::success(&result).map_err(|serialization_error| {
            riglr_core::ToolError::permanent_string(format!(
                "Failed to serialize result: {serialization_error}"
            ))
    ...
    #[inline]
    fn description(&self) -> &'static str {
        "Create a new SPL token mint with the current signer as authority"
    }
    ...
    fn schema(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "decimals": {
                    "type": "integer",
                    "description": "Number of decimal places for the token (0-9, commonly 6 or 9)"
                },
                "initialSupply": {
                    "type": "integer",
                    "description": "Initial supply to mint in base units"
                },
                "freezable": {
                    "type": "boolean",
                    "description": "Whether the mint should support freezing accounts (default: false)"
                }
            },
            "required": ["decimals", "initialSupply"]
        })
    }
}

// Re-export tool structs generated by the #[tool] macro
pub use __riglr_tool_create_spl_token_mint::CreateSplTokenMintTool;
pub use __riglr_tool_transfer_sol::TransferSolTool;
pub use __riglr_tool_transfer_spl_token::TransferSplTokenTool;
...
mod tests {
    ...
    fn test_transaction_status_serialization() {
        ...
        let status = Status::Failed("error".to_string());
        let json = serde_json::to_string(&status).expect("Status should serialize to JSON");
    ...
    fn test_transaction_status_deserialization() {
        let json = "\"Pending\"";
        let status: Status =
            serde_json::from_str(json).expect("Valid JSON should deserialize to Status");
    ...
    fn test_transaction_result_creation() {
        ...
        matches!(result.status, Status::Pending);
    }
    ...
    fn test_transaction_result_without_memo() {
        ...
        matches!(result.status, Status::Confirmed);
    }

    #[test]
    // Float comparisons are exact in test context
    fn test_token_transfer_result_creation() {
        ...
        matches!(result.status, Status::Finalized);
    }
    ...
    fn test_create_mint_result_creation() {
        ...
        assert_eq!(result.initial_supply, 1_000_000_000);
    ...
    fn test_serialization_roundtrip_transaction_result() {
        let original = SolTransferResult {
            signature: "sig123".to_string(),
            from: "from123".to_string(),
            to: "to123".to_string(),
            amount: 500_000,
            amount_display: "0.0005 SOL".to_string(),
            status: Status::Failed("Network error".to_string()),
            memo: Some("payment".to_string()),
            idempotency_key: Some("key123".to_string()),
        };

        let json =
            serde_json::to_string(&original).expect("Original struct should serialize to JSON");
        let deserialized: SolTransferResult = serde_json::from_str(&json)
            .expect("Valid JSON should deserialize to SolTransferResult");
        ...
        if let (Status::Failed(orig_msg), Status::Failed(deser_msg)) =
    ...
    fn test_serialization_roundtrip_token_transfer_result() {
        ...
        let json =
            serde_json::to_string(&original).expect("Original struct should serialize to JSON");
        let deserialized: TokenTransferResult = serde_json::from_str(&json)
            .expect("Valid JSON should deserialize to TokenTransferResult");
        ...
        assert_eq!(original.amount_display, deserialized.amount_display);
        assert_eq!(original.idempotency_key, deserialized.idempotency_key);
        matches!(deserialized.status, Status::Pending);
    }
    ...
    fn test_serialization_roundtrip_create_mint_result() {
        ...
        let json =
            serde_json::to_string(&original).expect("Original struct should serialize to JSON");
        let deserialized: CreateMintResult =
            serde_json::from_str(&json).expect("Valid JSON should deserialize to CreateMintResult");
    ...
    fn test_debug_formatting() {
        ...
        let status = Status::Failed("test error".to_string());
        let debug_str = format!("{status:?}");
        ...
        assert!(debug_str.contains("SolTransferResult"));
    }

    #[test]
    fn test_clone_functionality() {
        let original_status = Status::Failed("clone test".to_string());
        let cloned_status = original_status.clone();
        if let (Status::Failed(orig), Status::Failed(cloned)) = (&original_status, &cloned_status) {
            assert_eq!(orig, cloned);
        }

        let original_result = SolTransferResult {
    ...
    async fn test_transfer_sol_empty_address() {
        let context = create_test_context();
        let result = transfer_sol(String::new(), 0.001, None, None, &context).await;
    ...
    async fn test_transfer_spl_token_empty_addresses() {
        let context = create_test_context();
        let result =
            transfer_spl_token(String::new(), String::new(), 1_000_000, 6, false, &context).await;
    ...
    async fn test_create_spl_token_mint_decimals_boundary_valid() {
        ...
        let result_0 = create_spl_token_mint(0, 1_000_000, true, &context).await;
        let result_9 = create_spl_token_mint(9, 1_000_000, false, &context).await;
    ...
    fn test_ui_amount_calculation() {
        ...
        let decimals = 6u8;
        #[expect(clippy::cast_precision_loss)]
        let ui_amount = amount as f64 / 10_f64.powi(i32::from(decimals));
        assert_eq!(ui_amount, 1.0);

        let amount = 1_500_000_u64;
        let decimals = 6u8;
        #[expect(clippy::cast_precision_loss)]
        let ui_amount = amount as f64 / 10_f64.powi(i32::from(decimals));
        ...
        #[expect(clippy::cast_precision_loss)]
        let ui_amount = amount as f64 / 10_f64.powi(i32::from(decimals));
        assert_eq!(ui_amount, 0.000_000_001);
    ...
    fn test_lamports_conversion() {
        ...
        let lamports = lamports_f64 as u64;
        ...
        let amount_sol = 0.000_000_001_f64; // 1 lamport
        #[expect(clippy::cast_precision_loss)]
        let lamports_f64 = amount_sol * (LAMPORTS_PER_SOL as f64);
        #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
        let lamports = lamports_f64 as u64;
    ...
    fn test_memo_program_pubkey() {
        ...
        let memo_pubkey = "MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr"
            .parse::<Pubkey>()
            .expect("Valid memo program ID should parse to Pubkey");
        assert_eq!(
    ...
    fn test_address_validation_edge_cases() {
        // Test various invalid address formats that parse::<Pubkey> should reject

        // Empty string
        let result = "".parse::<Pubkey>();
        assert!(result.is_err());

        // Too short
        let result = "short".parse::<Pubkey>();
        assert!(result.is_err());

        // Invalid characters
        let result = "InvalidChars!@#$%^&*()".parse::<Pubkey>();
        assert!(result.is_err());

        // Too long
        let result =
            "ThisIsWayTooLongToBeAValidSolanaPubkeyAddressAndShouldFail123456789".parse::<Pubkey>();
        assert!(result.is_err());

        // Valid address should work
        let result = "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM".parse::<Pubkey>();
        assert!(result.is_ok());
    }

    #[test]
    fn test_transaction_status_variants_coverage() {
        // Test all variants of Status
        let pending = Status::Pending;
        let confirmed = Status::Confirmed;
        let finalized = Status::Finalized;
        let failed = Status::Failed("test error".to_string());
    ...
    fn test_result_structs_field_access() {
        ...
        let tx_result = SolTransferResult {
            signature: "test_signature".to_string(),
