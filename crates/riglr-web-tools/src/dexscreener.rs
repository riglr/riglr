...
use crate::{
    client::WebClient,
    dexscreener_api::{
        aggregate_token_info, get_latest_token_boosts,
        get_latest_token_profiles as api_get_latest_token_profiles, get_token_orders,
        get_top_token_boosts, BoostsResponse, DexScreenerResponse, DexScreenerResponseRaw, Order,
        TokenProfile,
    },
    error::WebToolError,
};
...
/// or no pairs are found for the given token address.
#[tool]
pub async fn get_token_info(
    ...
    let response = client.get(&url).await.map_err(|e| {
        if e.to_string().contains("timeout") || e.to_string().contains("connection") {
            return WebToolError::Network(format!("Failed to fetch token info: {e}"));
        }
...
pub async fn search_tokens(
    ...
    limit: Option<u32>,
) -> Result<TokenSearchResult, WebToolError> {
    ...
    let result = TokenSearchResult {
        ...
        metadata: SearchMetadata {
            result_count: u32::try_from(tokens.len().min(u32::MAX as usize)).unwrap_or(0),
...
pub async fn get_trending_tokens(
    ...
    limit: Option<u32>,
) -> Result<Vec<TokenInfo>, WebToolError> {
    debug!(
...
/// # Errors
///
/// Returns an error if token information cannot be retrieved or parsed.
#[tool]
pub async fn analyze_token_market(
    context: &ApplicationContext,
    token_address: String,
    chain_id: Option<String>,
    #[expect(unused_variables)] include_technical: Option<bool>,
    include_risk: Option<bool>,
) -> Result<MarketAnalysis, WebToolError> {
...
pub async fn get_top_pairs(
    ...
    min_liquidity: Option<f64>,
    limit: Option<u32>,
) -> Result<Vec<TokenPair>, WebToolError> {
    debug!(
...
pub async fn get_latest_token_profiles(
    _context: &ApplicationContext,
    limit: Option<u32>,
) -> Result<Vec<TokenProfile>, WebToolError> {
    debug!("Fetching latest token profiles");

    let profiles = api_get_latest_token_profiles()
        .await
...
pub async fn get_latest_boosted_tokens(
    _context: &ApplicationContext,
    limit: Option<u32>,
) -> Result<Vec<BoostsResponse>, WebToolError> {
    debug!("Fetching latest boosted tokens");

    let boosts = get_latest_token_boosts()
        .await
...
pub async fn get_top_boosted_tokens(
    ...
    let boosts = get_top_token_boosts()
        .await
...
pub async fn check_token_orders(
    ...
    chain_id: String,
    token_address: String,
) -> Result<Vec<Order>, WebToolError> {
    debug!(
        "Checking orders for token {} on chain {}",
        token_address, chain_id
    );

    let orders = get_token_orders(&chain_id, &token_address)
        .await
...
fn parse_token_response(
    ...
    let dex_response_raw: DexScreenerResponseRaw = serde_json::from_str(response)
        .map_err(|e| WebToolError::Parsing(format!("Failed to parse DexScreener response: {e}")))?;

    // Convert raw to clean types
    let dex_response: DexScreenerResponse = dex_response_raw.into();
    ...
    let token_info_opt = aggregate_token_info(dex_response.pairs, token_address);
    ...
    Err(WebToolError::Api(format!(
        "No pairs found for token address: {token_address}"
    )))
}
...
pub fn get_native_token(chain_id: &str) -> String {
    match chain_id {
        ...
        "ethereum" | "arbitrum" | "optimism" => "ETH".to_string(),
...
fn parse_search_results(response: &str) -> Result<Vec<TokenInfo>, WebToolError> {
    // Parse into raw response and convert to clean types
    let dex_response_raw: DexScreenerResponseRaw =
        serde_json::from_str(response).map_err(|e| WebToolError::Parsing(e.to_string()))?;
    let dex_response: DexScreenerResponse = dex_response_raw.into();
    ...
    for pair in dex_response.pairs {
        ...
        let entry = tokens_map.entry(token_address.clone()).or_insert_with(|| {
            TokenInfo {
        ...
        entry.pairs.push(TokenPair {
            ...
            txns_24h: TransactionStats {
                buys: pair.txns.as_ref().and_then(|t| {
                    t.h24.as_ref().and_then(|h| {
                        h.buys
                            .map(|b| u32::try_from(b.clamp(0, u64::from(u32::MAX))).unwrap_or(0))
                    })
                }),
                sells: pair.txns.as_ref().and_then(|t| {
                    t.h24.as_ref().and_then(|h| {
                        h.sells
        ...
        {
            entry.pair_count = entry.pair_count.saturating_add(1);
        }
    }
    Ok(tokens_map.into_values().collect())
}
fn parse_trending_response(response: &str) -> Result<Vec<TokenInfo>, WebToolError> {
    // Same as search results for now
    parse_search_results(response)
}

/// Parse trading pairs response
fn parse_pairs_response(response: &str) -> Result<Vec<TokenPair>, WebToolError> {
    // Parse into raw response and convert to clean types
    let dex_response_raw: DexScreenerResponseRaw =
        serde_json::from_str(response).map_err(|e| WebToolError::Parsing(e.to_string()))?;
    let dex_response: DexScreenerResponse = dex_response_raw.into();
...
fn analyze_price_trends(token: &TokenInfo) -> TrendAnalysis {
    ...
    let strength = price_change_24h.map_or(5, |change| {
        let rounded = ((change.abs() / 10.0).clamp(1.0, 10.0)).round();
        #[expect(clippy::cast_possible_truncation)]
        {
            u32::try_from(rounded as i32).unwrap_or(5)
        }
    }); // Default to medium strength if no data

    // Calculate momentum and velocity only if we have data
    let momentum = match (price_change_1h, price_change_24h) {
        (Some(h1), Some(h24)) => h1.mul_add(24.0, -h24), // Acceleration
        _ => 0.0,
    };
    ...
    let (support_levels, resistance_levels) = token.price_usd.map_or_else(
        || (vec![], vec![]),
        |price| (vec![price * 0.95], vec![price * 1.05]),
    );
...
fn analyze_volume_patterns(token: &TokenInfo) -> VolumeAnalysis {
...
fn analyze_liquidity(token: &TokenInfo) -> LiquidityAnalysis {
    ...
    let depth_score = if total_liquidity > 1_000_000.0 {
        85
    } else {
        60
    };
...
fn analyze_price_levels(token: &TokenInfo) -> PriceLevelAnalysis {
    ...
    let (high_24h, low_24h, range_position) = token.price_usd.map_or((None, None, None), |price| {
        token
            .price_change_24h
            .map_or((None, None, None), |change_pct| {
                // Rough estimate: if price went up X%, low was price/(1+X/100)
                let change_factor = 1.0 + (change_pct / 100.0);
                if change_pct > 0.0 {
                    let low = price / change_factor;
                    return (Some(price), Some(low), Some(1.0)); // Currently at high
                }
...
fn assess_token_risks(token: &TokenInfo) -> RiskAssessment {
    let mut risk_factors = vec![];
    let mut total_risk: i32 = 0;

    // Check liquidity risk
    let total_liquidity: f64 = token
        .pairs
        .iter()
        .map(|p| p.liquidity_usd.unwrap_or(0.0))
        .sum();

    let liquidity_score = if total_liquidity < 100_000.0 {
    ...
    {
        total_risk = total_risk.saturating_add(liquidity_score);
    }
    ...
    {
        total_risk = total_risk.saturating_add(contract_score);
    }
    ...
    {
        total_risk = total_risk.saturating_add(volatility_score);
    }
    ...
    RiskAssessment {
        risk_level,
        risk_factors,
        liquidity_risk: u32::try_from(liquidity_score.max(0)).unwrap_or(0),
        volatility_risk: u32::try_from(volatility_score.max(0)).unwrap_or(0),
        contract_risk: u32::try_from(contract_score.max(0)).unwrap_or(0),
    }
}
...
mod tests {
    ...
    fn test_token_info_serialization() {
        let token = TokenInfo {
            ...
            total_supply: Some(10_000_000.0),
            pair_count: 1,
            pairs: vec![],
            chain: ChainInfo {
    ...
    async fn test_analyze_price_trends_when_bullish_data_should_return_bullish_trend() {
        ...
        let result = analyze_price_trends(&token);

        assert_eq!(result.direction, "Bullish");
        assert_eq!(result.strength, 1); // 10.0 / 10.0 = 1.0, clamped to 1
        {
            assert!((result.momentum - 38.0).abs() < f64::EPSILON);
        } // 2.0 * 24.0 - 10.0 = 48.0 - 10.0
        {
            assert!((result.velocity - (10.0 / 24.0)).abs() < f64::EPSILON);
        }
    ...
    async fn test_analyze_price_trends_when_bearish_data_should_return_bearish_trend() {
        ...
        assert_eq!(result.strength, 1); // 10.0 / 10.0 = 1.0, clamped to 1
        {
            assert!((result.momentum - (-14.0)).abs() < f64::EPSILON);
        } // -1.0 * 24.0 - (-10.0)
        {
            assert!((result.velocity - (-10.0 / 24.0)).abs() < f64::EPSILON);
        }
    ...
    async fn test_analyze_price_trends_when_neutral_data_should_return_neutral_trend() {
        ...
        {
            assert!((result.momentum - 10.0).abs() < f64::EPSILON);
        } // 0.5 * 24.0 - 2.0
    }
    ...
    async fn test_analyze_price_trends_when_no_data_should_return_unknown_trend() {
        ...
        {
            assert!((result.velocity - 0.0).abs() < f64::EPSILON);
        }
        assert_eq!(result.support_levels.len(), 1); // Has price, so basic levels
        assert_eq!(result.resistance_levels.len(), 1);
    }
    ...
    async fn test_analyze_volume_patterns_when_valid_data_should_calculate_ratio() {
        ...
        token.market_cap = Some(1_000_000.0);

        let result = analyze_volume_patterns(&token);
    ...
    async fn test_analyze_liquidity_when_multiple_pairs_should_aggregate_liquidity() {
        let mut token = create_test_token_info(Some(5.0), Some(1.0), Some(1.0));
        token.pairs = vec![
            create_test_token_pair("dex1", 100_000.0),
            create_test_token_pair("dex2", 200_000.0),
        ];

        let result = analyze_liquidity(&token);

        {
            assert!((result.total_liquidity_usd - 300_000.0).abs() < f64::EPSILON);
        }
        assert_eq!(result.dex_distribution.len(), 2);
        assert_eq!(result.dex_distribution.get("dex1"), Some(&100_000.0));
        assert_eq!(result.dex_distribution.get("dex2"), Some(&200_000.0));
    ...
    async fn test_analyze_liquidity_when_high_liquidity_should_return_high_depth_score() {
        let mut token = create_test_token_info(Some(5.0), Some(1.0), Some(1.0));
        token.pairs = vec![create_test_token_pair("dex1", 2_000_000.0)];

        let result = analyze_liquidity(&token);

        {
            assert!((result.total_liquidity_usd - 2_000_000.0).abs() < f64::EPSILON);
        }
    ...
    async fn test_analyze_liquidity_when_no_pairs_should_return_zero_liquidity() {
        ...
        {
            assert!((result.total_liquidity_usd - 0.0).abs() < f64::EPSILON);
        }
    ...
    async fn test_analyze_price_levels_when_positive_change_should_estimate_at_high() {
        ...
        let result = analyze_price_levels(&token);

        assert_eq!(result.high_24h, Some(100.0));
        assert!(result.low_24h.is_some());
        assert!(result.low_24h.expect("Low 24h should be available") < 100.0);
    ...
    async fn test_analyze_price_levels_when_negative_change_should_estimate_at_low() {
        ...
        assert!(result.high_24h.expect("High 24h should be available") > 90.0);
    ...
    async fn test_assess_token_risks_when_low_liquidity_should_add_liquidity_risk() {
        ...
        let result = assess_token_risks(&token);
        ...
        assert_eq!(result.risk_level, "Medium"); // (75+20+30)/3 = 41.67
    }

    #[tokio::test]
    async fn test_assess_token_risks_when_high_liquidity_should_have_low_liquidity_risk() {
        let mut token = create_test_token_info(Some(5.0), Some(1.0), Some(1.0));
        token.pairs = vec![create_test_token_pair("dex1", 500_000.0)]; // High liquidity
    ...
    async fn test_assess_token_risks_when_unverified_contract_should_add_contract_risk() {
        ...
        token.pairs = vec![create_test_token_pair("dex1", 500_000.0)];
    ...
    async fn test_parse_search_results_when_invalid_json_should_return_error() {
        ...
        let result = parse_search_results(invalid_json);

        assert!(result.is_err());
        if let Err(WebToolError::Parsing(msg)) = result {
            assert!(msg.contains("expected"));
        }
    }

    #[test]
    fn test_parse_trending_response_when_invalid_json_should_return_error() {
        let invalid_json = "invalid json";
        let result = parse_trending_response(invalid_json);
    ...
    async fn test_parse_pairs_response_when_invalid_json_should_return_error() {
        let invalid_json = "invalid json";
        let result = parse_pairs_response(invalid_json);
    ...
    async fn test_parse_token_response_when_invalid_json_should_return_parsing_error() {
        let invalid_json = "invalid json";
        let result = parse_token_response(invalid_json, "0x123", "ethereum", Some(true));
    ...
    async fn test_parse_token_response_when_no_pairs_found_should_return_api_error() {
        ...
        let result = parse_token_response(valid_json_no_pairs, "0x123", "ethereum", Some(true));

        assert!(result.is_err());
        if let Err(WebToolError::Api(msg)) = result {
    ...
    async fn test_parse_token_response_when_no_valid_pairs_should_return_api_error() {
        ...
        let result =
            parse_token_response(json_with_unmatched_pairs, "0x123", "ethereum", Some(true));
    ...
    async fn test_parse_token_response_when_valid_data_should_return_token_info() {
        ...
        let result = parse_token_response(&valid_json, "0x123", "ethereum", Some(true));

        assert!(result.is_ok());
        let token = result.expect("Token parsing should succeed");
    ...
    async fn test_parse_token_response_when_multiple_pairs_should_use_highest_liquidity() {
        ...
        let result =
            parse_token_response(&json_with_multiple_pairs, "0x123", "ethereum", Some(true));

        assert!(result.is_ok());
        let token = result.expect("Token parsing should succeed");
        assert_eq!(token.pair_count, 2);
        // Should have aggregated volume from both pairs
        assert!(token.volume_24h.expect("Volume 24h should be available") > 10000.0);
    ...
    async fn test_parse_token_response_when_case_insensitive_address_should_match() {
        let valid_json = create_valid_dexscreener_response();
        let result = parse_token_response(&valid_json, "0X123", "ethereum", Some(true)); // Uppercase
    ...
    async fn test_parse_token_response_when_no_liquidity_data_should_handle_gracefully() {
        ...
        let result = parse_token_response(&json_no_liquidity, "0x123", "ethereum", Some(true));

        assert!(result.is_ok());
        let token = result.expect("Token parsing should succeed");
        assert_eq!(token.pairs.first().unwrap().liquidity_usd, None);
    }
    ...
    async fn test_parse_token_response_when_transaction_data_available_should_calculate_totals() {
        ...
        let result = parse_token_response(&json_with_txns, "0x123", "ethereum", Some(true));

        assert!(result.is_ok());
        let token = result.expect("Token parsing should succeed");
        let pair = token.pairs.first().unwrap();
    ...
    async fn test_parse_token_response_when_unknown_chain_should_use_default_formatting() {
        let valid_json = create_valid_dexscreener_response();
        let result = parse_token_response(&valid_json, "0x123", "unknown-chain", Some(true));
    ...
    async fn test_parse_token_response_when_price_parsing_fails_should_handle_gracefully() {
        ...
        let result = parse_token_response(&json_invalid_price, "0x123", "ethereum", Some(true));
