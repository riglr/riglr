...
impl Config {
    // Note: from_context method removed as it was unused
}
...
pub async fn search_web_with_context(
...
) -> Result<SearchOperation, WebToolError> {
    debug!(
    ...
    if let Some(date) = date_filter.as_ref() {
    ...
    let response = client
        ...
        .map_err(|e| {
            if e.to_string().contains("timeout") || e.to_string().contains("connection") {
                return WebToolError::Network(format!("Web search request failed: {e}"));
            }
            WebToolError::Config(format!("Web search request failed: {e}"))
        })?;

    // Parse search results
    let parse_result = parse_exa_search_response(&response, &query);
    ...
    let search_result = SearchOperation {
        query: query.clone(),
        search_type: "semantic".to_string(),
        results: results.clone(),
        metadata: SearchMetadata {
            total_results: u32::try_from(results.len()).unwrap_or(0),
            returned_results: u32::try_from(results.len()).unwrap_or(0),
...
pub async fn search_web(
    ...
    return search_web_with_context(
        query,
        max_results,
        include_content,
        domain_filter,
...
///
/// Returns an error if:
/// - `EXA_API_KEY` is not configured in the application context
/// - Network request to the API fails
/// - Response parsing fails
#[tool]
pub async fn find_similar_pages(
...
) -> Result<SimilarPagesResult, WebToolError> {
    ...
    // Parse results
    let parse_result = parse_similar_pages_response(&response);
...
///
/// Returns an error if:
/// - Network requests to any of the provided URLs fail
/// - Content extraction or parsing fails for any URL
/// - HTML parsing encounters malformed content
#[tool]
pub async fn summarize_web_content(
...
) -> Result<Vec<ContentSummary>, WebToolError> {
    debug!("Summarizing content from {} URLs", urls.len());

    // Note: include_quotes parameter is reserved for future quote extraction functionality
    let _ = include_quotes;
    ...
    let exa_api_key = context
        ...
        .unwrap_or_else(|| env::var(EXA_API_KEY).unwrap_or_else(|_| String::default()));
    ...
    // Process each URL
    for url in urls {
        let extract_result = extract_and_summarize_page(
            &client,
            &url,
            summary_length.as_ref(),
            focus_topics.as_ref(),
        )
        .await;
...
/// Returns an error if:
/// - `EXA_API_KEY` is not configured in the application context
/// - Network request to the news API fails
/// - Response parsing fails
/// - Invalid time window parameter is provided
#[tool]
pub async fn search_recent_news(
    ...
    let parse_result = parse_exa_search_response(&response, &topic);
    ...
    let search_result = SearchOperation {
        ...
        metadata: SearchMetadata {
            ...
            filtered: true,
            related_queries: generate_related_queries(&topic),
            top_domains: extract_top_domains(&results),
        },
        insights,
        searched_at: Utc::now(),
...
fn parse_exa_search_response(
    ...
    for r in results {
        ...
        let words = content.as_ref().map_or(0, |c| {
            u32::try_from(c.split_whitespace().count()).unwrap_or(0)
        });
        let reading_time = if words > 0 {
            Some(
                #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
                {
                    (f64::from(words) / 200.0)
                        .ceil()
        ...
        let content_type = ContentType {
            ...
            is_paywalled: None,
            #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
            quality_score: Some(((score * 100.0).clamp(0.0, f64::from(u32::MAX)) as u32).min(100)),
            length_category,
        };
...
fn parse_similar_pages_response(response: &str) -> Result<Vec<SearchResult>, WebToolError> {
    // Reuse the general Exa parser without query context
    parse_exa_search_response(response, "")
}
...
async fn extract_and_summarize_page(
    ...
    focus_topics: Option<&Vec<String>>,
) -> Result<ContentSummary, WebToolError> {
    ...
    let n = match summary_length.map(String::as_str) {
        Some("comprehensive") => 8_usize,
        Some("detailed") => 5_usize,
        _ => 3_usize,
    };

    let topic_set: HashSet<String> = focus_topics
    ...
    #[expect(clippy::unwrap_used)] // Hardcoded valid regex pattern
    ...
    let mut confidence = (clean_text.len().min(8000) as f64 / 8000.0).mul_add(0.6, 0.3);
...
fn extract_main_content(
    ...
    // Prefer og:title
    #[expect(clippy::unwrap_used)] // Hardcoded valid CSS selector
...
fn extract_text_from_node(root: ElementRef<'_>) -> (String, Vec<String>) {
    ...
    #[expect(clippy::unwrap_used)] // Hardcoded valid CSS selectors
...
fn rank_sentences(
    ...
    // Normalize
    let max_tf = tf.values().copied().fold(1.0, f64::max);
    ...
    let mut scored: Vec<(String, f64)> = sentences
        ...
        .map(|(i, s)| {
            ...
            #[expect(clippy::cast_precision_loss)]
            let position_boost = 0.15 * (1.0 / ((i.saturating_add(1)) as f64).sqrt());
            {
...
fn analyze_search_results(results: &[SearchResult]) -> SearchInsights {
    ...
    for result in results {
        ...
        // Count languages
        if let Some(lang) = result.language.as_ref() {
            {
                let count = languages.entry(lang.clone()).or_insert(0u32);
                *count = count.saturating_add(1u32);
            }
        }
        ...
        if let Some(pub_date) = result.published_date {
            let days_ago = Utc::now().signed_duration_since(pub_date).num_days();
    ...
    let avg_quality_score = if quality_scores.is_empty() {
        ...
        #[expect(clippy::cast_precision_loss)]
        {
            Some(f64::from(quality_scores.iter().sum::<u32>()) / quality_scores.len() as f64)
        }
    };
...
fn analyze_news_results(results: &[SearchResult]) -> SearchInsights {
...
fn analyze_similarity(results: &[SearchResult]) -> SimilarityMetadata {
    #[expect(clippy::cast_precision_loss)]
    let avg_similarity =
    ...
    for result in results {
        {
            let count = domain_counts
                .entry(result.domain.name.clone())
                .or_insert(0u32);
            *count = count.saturating_add(1u32);
        }
    }
...
fn format_date_filter(window: &str) -> String {
    let days_ago = match window {
        ...
        "month" => 30,
        "year" => 365,
        _ => 7, // includes "week" and other values
    };
...
fn extract_topics_from_text(text: &str) -> Vec<String> {
    ...
    for w in text.split(|c: char| !c.is_alphanumeric()) {
        ...
        {
            let count = counts.entry(w).or_insert(0u32);
            *count = count.saturating_add(1u32);
        }
    }
...
mod tests {
    ...
    fn test_search_result_serialization() {
        ...
        let json =
            serde_json::to_string(&result).expect("Failed to serialize SearchResult to JSON");
