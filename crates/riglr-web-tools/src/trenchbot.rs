...
pub async fn get_bundle_info(
    ...
    token: String,
) -> Result<BundleResponse, WebToolError> {
...
fn calculate_wallet_categories(
    ...
    let mut sniper_count = 0i32;
    let mut regular_count = 0i32;
    let mut sniper_tokens = 0i64;
    let mut total_tokens = 0i64;

    for bundle in bundles.values() {
        if let Some(ref categories) = bundle.wallet_categories {
            for category in categories.values() {
                match category.as_str() {
                    "sniper" => sniper_count = sniper_count.saturating_add(1),
                    "regular" => regular_count = regular_count.saturating_add(1),
                    _ => {}
                }
            }
        }

        if let Some(ref wallet_info) = bundle.wallet_info {
            for (addr, info) in wallet_info {
                if let Some(tokens) = info.tokens {
                    total_tokens = total_tokens.saturating_add(tokens);
                    if let Some(ref categories) = bundle.wallet_categories {
                        if categories.get(addr).is_some_and(|c| c == "sniper") {
                            sniper_tokens = sniper_tokens.saturating_add(tokens);
                        }
                    }
                }
            }
...
fn collect_warnings(bundle_data: &BundleResponse, sniper_percentage: f64) -> Vec<String> {
    ...
    if let Some(ref creator) = bundle_data.creator_analysis {
        if let Some(ref flags) = creator.warning_flags {
...
/// # Errors
///
/// Returns an error if the bundle information cannot be retrieved or processed.
#[tool]
pub async fn analyze_token_bundles(
    context: &ApplicationContext,
    token: String,
) -> Result<BundleAnalysisResult, WebToolError> {
    ...
    let (sniper_count, regular_count, sniper_percentage, primary_category) =
        bundle_data.bundles.as_ref().map_or_else(
            || (0, 0, 0.0, "regular".to_string()),
            calculate_wallet_categories,
        );
...
/// # Errors
///
/// Returns an error if the bundle information cannot be retrieved.
#[tool]
pub async fn check_bundle_risk(
    context: &ApplicationContext,
    token: String,
) -> Result<BundleRiskCheck, WebToolError> {
    ...
    let message = format!(
        "{risk_level} risk: {bundle_percentage:.1}% of supply is bundled across {bundle_count} bundles"
    );
...
pub async fn analyze_creator_risk(
    context: &ApplicationContext,
    token: String,
) -> Result<CreatorAnalysisResult, WebToolError> {
    ...
    if let Some(creator) = bundle_data.creator_analysis {
        ...
        // Check for red flags
        if let Some(ref history) = creator.history {
        ...
        return Ok(CreatorAnalysisResult {
            token,
...
mod tests {
    ...
    fn test_risk_level_serialization() {
        ...
        let json = serde_json::to_string(&risk).expect("Failed to serialize RiskLevel to JSON");
        assert_eq!(json, "\"HIGH\"");

        let risk: RiskLevel =
            serde_json::from_str("\"MEDIUM\"").expect("Failed to deserialize RiskLevel from JSON");
    ...
    fn test_bundle_response_deserialization() {
        ...
        let response: BundleResponse =
            serde_json::from_str(json).expect("Failed to deserialize BundleResponse from JSON");
    ...
    fn test_creator_analysis_deserialization() {
        ...
        let creator: CreatorAnalysis =
            serde_json::from_str(json).expect("Failed to deserialize CreatorAnalysis from JSON");
        ...
        let history = creator
            .history
            .expect("Expected history field to be present in test data");
