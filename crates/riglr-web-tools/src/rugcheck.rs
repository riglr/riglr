...
///
/// Returns an error if:
/// - Network request to `RugCheck` API fails
/// - API response cannot be parsed as valid JSON
/// - API returns an error response
#[tool]
pub async fn get_token_report(
    _context: &ApplicationContext,
    mint: String,
) -> Result<TokenCheck, WebToolError> {
...
const fn calculate_risk_level(risk_score: i32) -> RiskLevel {
    match risk_score {
        0..=20 => RiskLevel::Safe,
        21..=40 => RiskLevel::Low,
...
fn analyze_insider_activity(report: &TokenCheck) -> Option<InsiderAnalysis> {
    ...
    Some(InsiderAnalysis {
        networks_detected: i32::try_from(networks.len()).unwrap_or(0),
...
///
/// Returns an error if:
/// - The underlying `get_token_report` call fails
/// - Network or parsing errors occur during data retrieval
#[tool]
pub async fn analyze_token_risks(
    context: &ApplicationContext,
    mint: String,
) -> Result<RiskAnalysis, WebToolError> {
    ...
    let (critical_risks, high_risks, medium_risks, low_risks) = report.risks.as_ref().map_or_else(
        || (Vec::new(), Vec::new(), Vec::new(), Vec::new()),
        |risks| categorize_risks(risks),
    );
...
pub async fn check_if_rugged(
    ...
    let risk_score = report.score.unwrap_or(0);
    let risk_count = report.risks.as_ref().map_or(0, Vec::len);
    ...
    Ok(RugCheckResult {
        ...
        is_rugged,
        mint,
        risk_factors: i32::try_from(risk_count).unwrap_or(0),
        risk_score,
...
mod tests {
    ...
    fn test_risk_level_serialization() {
        ...
        let json =
            serde_json::to_string(&risk).expect("Failed to serialize RiskLevel::Critical to JSON");
        assert_eq!(json, "\"critical\"");

        let risk: RiskLevel =
            serde_json::from_str("\"high\"").expect("Failed to deserialize RiskLevel from JSON");
    ...
    fn test_token_check_deserialization() {
        ...
        let token: TokenCheck =
            serde_json::from_str(json).expect("Failed to deserialize TokenCheck from JSON");
    ...
    #[test]
    fn test_categorize_risks() {
        let risks = vec![
            Risk {
                name: "Critical issue".to_string(),
                description: "Critical desc".to_string(),
            ...
            Risk {
                ...
                description: "High desc".to_string(),
            ...
            Risk {
                ...
                description: "Medium desc".to_string(),
                level: "medium".to_string(),
                score: 50,
                value: None,
            },
            Risk {
                name: "Caution issue".to_string(),
                description: "Caution desc".to_string(),
                level: "caution".to_string(),
                score: 45,
                value: None,
            },
            Risk {
                name: "Low issue".to_string(),
                description: "Low desc".to_string(),
                level: "low".to_string(),
                score: 20,
                value: None,
            },
            Risk {
                name: "Unknown issue".to_string(),
                description: "Unknown desc".to_string(),
        ...
        let (critical, high, medium, low) = categorize_risks(&risks);

        // Check critical risks
        assert_eq!(critical.len(), 1);
        assert_eq!(critical.first().unwrap().level, "critical");

        // Check high risks
        assert_eq!(high.len(), 1);
        assert_eq!(high.first().unwrap().level, "high");

        // Check medium risks (should include "medium", "caution", and unknown levels)
        assert_eq!(medium.len(), 3);
        let medium_levels: Vec<&str> = medium.iter().map(|r| r.level.as_str()).collect();
        assert!(medium_levels.contains(&"medium"));
        assert!(medium_levels.contains(&"caution"));
        assert!(medium_levels.contains(&"unknown"));

        // Check low risks
        assert_eq!(low.len(), 1);
        assert_eq!(low.first().unwrap().level, "low");
    }
}
