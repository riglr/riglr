...
use riglr_core::sentiment::AnalyzerMarker;
...
impl AnalyzerMarker for LexiconSentimentAnalyzer {}
...
impl LexiconSentimentAnalyzer {
    ...
    fn analyze_sentiment_impl(
        ...
        for &(word, weight) in &self.positive_words {
            ...
            word_count = word_count.saturating_add(count);
        }
        ...
        let overall_score = if word_count > 0 {
            #[expect(clippy::cast_precision_loss)]
            let normalized = sentiment_score / word_count as f64;
        ...
        #[expect(clippy::cast_precision_loss)]
        let confidence = (word_count as f64 / 10.0)
            .min(1.0)
            .mul_add(0.5, (full_text.len() as f64 / 500.0).min(1.0) * 0.5)
            .clamp(0.3, 0.95);
    ...
    fn calculate_emotions(text_lower: &str) -> EmotionalIndicators {
    ...
    fn extract_key_phrases(full_text: &str) -> Vec<SentimentPhrase> {
    ...
    fn calculate_topic_sentiments(text_lower: &str, overall_score: f64) -> HashMap<String, f64> {
        ...
        for topic in &topics {
            if text_lower.contains(topic) {
                ...
                let topic_score = if text_lower.contains(&format!("{topic} surge"))
                    || text_lower.contains(&format!("{topic} rally"))
                {
                    0.5
                } else if text_lower.contains(&format!("{topic} crash"))
                    || text_lower.contains(&format!("{topic} plunge"))
                {
                    -0.5
                } else {
...
impl Config {
    /// Create `Config` from `ApplicationContext`
...
///
/// Returns an error if:
/// - API configuration is invalid or missing credentials
/// - Network requests to news sources fail
/// - News parsing or sentiment analysis fails
#[tool]
pub async fn get_crypto_articles(
...
) -> Result<AggregationResult, WebToolError> {
    debug!(
    ...
    let (all_articles, sources_queried) =
        query_all_news_sources(&client, &config, &topic, time_window.as_deref()).await;

    let filtered_articles = apply_filters(all_articles, source_types, min_credibility, &config);
    let articles = deduplicate_articles(filtered_articles);

    let insights = generate_insights_if_requested(&articles, include_analysis)?;
    let trending_topics = extract_trending_topics(&articles);

    let result = build_aggregation_result(
        topic.clone(),
        &articles,
        sources_queried,
        insights,
...
/// Validate news configuration has required API keys
fn validate_news_config(config: &Config) -> Result<(), WebToolError> {
...
/// Query all available news sources
async fn query_all_news_sources(
    client: &WebClient,
    config: &Config,
    topic: &str,
    ...
    (all_articles, sources_queried)
}

/// Apply filters to article collection
fn apply_filters(
    ...
    if let Some(types) = source_types {
        articles.retain(|article| types.contains(&article.source.category.to_lowercase()));
    }

    // Filter by minimum credibility
    let min_cred = min_credibility.unwrap_or(config.min_credibility_score);
    articles.retain(|article| article.source.credibility_score >= min_cred);

    articles
}

/// Generate insights if analysis is requested
#[expect(clippy::unnecessary_wraps)]
fn generate_insights_if_requested(
    articles: &[Article],
    include_analysis: Option<bool>,
) -> Result<Insights, WebToolError> {
    if include_analysis.unwrap_or(true) {
        Ok(analyze_news_collection(articles))
    } else {
        Ok(Insights {
...
fn build_aggregation_result(
    ...
    AggregationResult {
        ...
        metadata: AggregationMetadata {
            total_articles: u32::try_from(articles.len()).unwrap_or(0),
            returned_articles: u32::try_from(articles.len()).unwrap_or(0),
            sources_queried,
            avg_credibility: calculate_avg_credibility(articles),
            time_range_hours: parse_time_window(time_window.unwrap_or("24h")),
...
pub async fn get_trending_articles(
    ...
    let articles: Vec<Article> = trending_articles
        .into_iter()
        .take(limit.unwrap_or(30) as usize)
        .collect();

    // Analyze trending patterns
    let insights = analyze_trending_patterns(&articles)?;
    let trending_topics = extract_trending_topics(&articles);
...
/// - API configuration is invalid or missing credentials
/// - Network requests to news sources fail
/// - Alert generation or filtering fails
#[tool]
pub async fn monitor_breaking_alerts(
    context: &ApplicationContext,
    keywords: Vec<String>,
    severity_threshold: Option<String>, // "Critical", "High", "Medium"
    impact_threshold: Option<u32>,      // 0-100
    #[expect(unused_variables)] alert_channels: Option<Vec<String>>, // "webhook", "email", "slack"
) -> Result<Vec<BreakingNewsAlert>, WebToolError> {
...
/// - API configuration is invalid or missing credentials
/// - Network requests to news sources fail
/// - Sentiment analysis or market impact calculation fails
#[tool]
...
pub async fn analyze_market_sentiment(
    ...
    #[expect(unused_variables)] source_weights: Option<HashMap<String, f64>>, // Weight different sources
    #[expect(unused_variables)] include_social: Option<bool>,
) -> Result<Insights, WebToolError> {
    debug!(
    ...
    let recent_news =
        gather_news_for_sentiment_analysis(context, asset_filter.as_ref(), time_window.as_deref())
            .await;

    let insights = analyze_news_collection(&recent_news);
...
/// Gather news articles for sentiment analysis based on asset filters
async fn gather_news_for_sentiment_analysis(
    context: &ApplicationContext,
    asset_filter: Option<&Vec<String>>,
    time_window: Option<&str>,
) -> Vec<Article> {
    if let Some(assets) = asset_filter {
        gather_asset_specific_news(context, assets, time_window).await
    } else {
        gather_general_market_news(context, time_window.map(ToString::to_string)).await
    }
}

/// Gather news for specific assets
async fn gather_asset_specific_news(
    context: &ApplicationContext,
    assets: &[String],
    time_window: Option<&str>,
) -> Vec<Article> {
    ...
    for asset in assets {
        let news_result = get_crypto_articles(
            context,
            asset.clone(),
            time_window.map(ToString::to_string),
            None,
...
/// Gather general market news
async fn gather_general_market_news(
    context: &ApplicationContext,
    time_window: Option<String>,
) -> Vec<Article> {
    match get_trending_articles(context, time_window, None, Some(50), Some(100)).await {
...
async fn query_newsapi(
    ...
    topic: &str,
    time_window: Option<&str>,
) -> Result<Vec<Article>, WebToolError> {
    // Build URL and params for NewsAPI /v2/everything
    let url = format!("{}/everything", config.base_url);
    let default_window = format!("{}h", config.freshness_hours);
    let window = time_window.unwrap_or(&default_window);
    let hours = i64::from(parse_time_window(window));
    let from = Utc::now()
    ...
    let mut articles_out: Vec<Article> = Vec::new();
    if let Some(arr) = json.get("articles").and_then(|a| a.as_array()) {
        for a in arr {
            ...
            let sentiment = analyze_sentiment(&title, description.as_deref(), content.as_deref());
...
async fn query_cryptopanic(
    ...
    let default_window = "24h".to_string();
    let window = time_window.unwrap_or(&default_window);
...
fn analyze_news_collection(articles: &[Article]) -> Insights {
    #[expect(clippy::cast_precision_loss)]
    ...
    for article in articles {
        ...
        for entity in &article.entities {
            let entry = entity_mentions
                ...
                .or_insert((0, 0.0));
            entry.0 = entry.0.saturating_add(entity.mention_count);
        ...
        for geo in &article.category.geographic_scope {
            let count = geo_distribution.entry(geo.clone()).or_insert(0u32);
    ...
    let top_entities: Vec<EntityMention> = entity_mentions
        ...
        .map(|(name, (count, sentiment))| EntityMention {
            name,
            mention_count: count,
            avg_sentiment: sentiment / f64::from(count),
    ...
    let unique_sources_count = u32::try_from(unique_sources.len()).unwrap_or(0);
...
fn extract_trending_topics(articles: &[Article]) -> Vec<TrendingTopic> {
    ...
    for article in articles {
        for tag in &article.category.tags {
            *topic_counts.entry(tag.clone()).or_insert(0) = topic_counts
                .entry(tag.clone())
                .or_insert(0)
                .saturating_add(1);
    ...
    let trending_topics: Vec<TrendingTopic> = topic_counts
        ...
        .map(|(topic, count)| TrendingTopic {
            topic: topic.clone(),
            article_count: count,
            velocity: f64::from(count) / 24.0, // Articles per hour (assuming 24h window)
            sentiment: topic_sentiments.get(&topic).unwrap_or(&0.0) / f64::from(count),
...
/// Helper functions
#[expect(clippy::cast_precision_loss)]
fn calculate_avg_credibility(articles: &[Article]) -> f64 {
    if articles.is_empty() {
        return 0.0;
    }
    articles
        .iter()
        .map(|a| f64::from(a.source.credibility_score))
        .sum::<f64>()
        / articles.len() as f64
}
...
fn parse_time_window(window: &str) -> u32 {
    match window {
        ...
        "6h" => 6,
        "week" => 168,
        _ => 24, // Default to 24 hours, including "24h"
    }
}

#[expect(clippy::cast_precision_loss)]
fn determine_sentiment_trend(articles: &[Article]) -> String {
                ...
                if let Some(arts) = json.get("articles").and_then(|v| v.as_array()) {
                    for a in arts {
                        ...
                        let category = Category {
                            primary: "Trending".to_string(),
                            sub_category: None,
                            tags: extract_tags_from_text(&title, description.as_deref()),
                            geographic_scope: vec!["Global".to_string()],
                            target_audience: "Retail".to_string(),
                        };
                        ...
                        out.push(Article {
                            ...
                            related_assets: extract_crypto_mentions(
                                &title,
                                description.as_deref(),
                                content.as_deref(),
                            ),
...
#[expect(clippy::unnecessary_wraps)]
fn analyze_trending_patterns(articles: &[Article]) -> Result<Insights, WebToolError> {
...
async fn detect_breaking_news(
    ...
    if !config.newsapi_key.is_empty() {
        if let Ok(mut a) = query_newsapi(client, config, keyword, Some("1h")).await {
            articles.append(&mut a);
        }
    }
    if !config.cryptopanic_key.is_empty() {
        let time_window = "1h".to_string();
        if let Ok(mut a) = query_cryptopanic(client, config, keyword, Some(&time_window)).await {
            articles.append(&mut a);
        }
    }
    ...
    let now = Utc::now();
    let mut grouped: Vec<Article> = Vec::new();
    for a in articles {
        if now.signed_duration_since(a.published_at) <= chrono::Duration::hours(2) {
            let hay = format!(
                "{} {} {}",
                a.title,
    ...
    if !grouped.is_empty() {
        ...
        let alert = BreakingNewsAlert {
            ...
            created_at: now,
            expires_at: now.checked_add_signed(chrono::Duration::hours(4)),
        };
        alerts.push(alert);
    }
...
fn analyze_sentiment(title: &str, description: Option<&str>, content: Option<&str>) -> Sentiment {
    // Use the default analyzer for now
    let analyzer = LexiconSentimentAnalyzer::default();
    analyzer.analyze_sentiment_impl(title, description, content)
}
...
fn calculate_market_impact(
    ...
    let sentiment_impact = (sentiment.overall_score.abs() * 100.0 * sentiment.confidence)
        .clamp(0.0, f64::from(u32::MAX)) as u32;

    // Adjust for source credibility
    let credibility_factor = f64::from(source.credibility_score) / 100.0;
    #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let base_score =
        (f64::from(sentiment_impact) * credibility_factor).clamp(0.0, f64::from(u32::MAX)) as u32;
    ...
    #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let impact_score = (f64::from(base_score) * category_multiplier)
        .min(100.0)
        .clamp(0.0, f64::from(u32::MAX)) as u32;
...
fn extract_entities_from_text(
        ...
        for cap in re.captures_iter(&full_text) {
            if let Some(matched) = cap.get(0) {
                ...
                let entry = entity_map
                    ...
                    .or_insert_with(|| ("Cryptocurrency".to_string(), 0));
    ...
    if let Ok(re) = Regex::new(company_pattern) {
        for cap in re.captures_iter(&full_text) {
            if let Some(matched) = cap.get(0) {
                ...
                let entry = entity_map
                    .entry(name.to_string())
                    .or_insert_with(|| ("Company".to_string(), 0));
                entry.1 = entry.1.saturating_add(1);
            }
        }
    }
    ...
    if let Ok(re) = Regex::new(person_pattern) {
        for cap in re.captures_iter(&full_text) {
            if let Some(matched) = cap.get(0) {
                ...
                let entry = entity_map
                    .entry(name.to_string())
                    .or_insert_with(|| ("Person".to_string(), 0));
                entry.1 = entry.1.saturating_add(1);
            }
        }
    }
    ...
    if let Ok(re) = Regex::new(protocol_pattern) {
        for cap in re.captures_iter(&full_text) {
            if let Some(matched) = cap.get(0) {
                ...
                let entry = entity_map
                    .entry(name.to_string())
                    .or_insert_with(|| ("Protocol".to_string(), 0));
                entry.1 = entry.1.saturating_add(1);
            }
        }
    }
    ...
    for (name, (entity_type, count)) in entity_map {
        let relevance_score = (f64::from(count) / 10.0).min(1.0);
...
fn extract_tags_from_text(title: &str, description: Option<&str>) -> Vec<String> {
    let full_text = format!(
        "{} {}",
        title.to_lowercase(),
        description.map_or("", |s| s).to_lowercase()
    );
...
fn extract_crypto_mentions(
    ...
    let full_text = format!(
        ...
        title.to_lowercase(),
        description.map_or("", |s| s).to_lowercase(),
        content.map_or("", |s| s).to_lowercase()
    );
...
fn calculate_quality_metrics(
    ...
    let _has_content = content.as_ref().is_some_and(|c| !c.is_empty());

    // Content depth based on length and structure
    let content_length = content.as_ref().map_or(0, |s| s.len());
    ...
    // Overall score
    #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let overall_score = f64::from(citation_quality)
        .mul_add(
            0.2,
            f64::from(writing_quality).mul_add(
                0.2,
                f64::from(source_credibility).mul_add(0.3, f64::from(depth_score) * 0.3),
            ),
        )
        .clamp(0.0, f64::from(u32::MAX)) as u32;
...
fn calculate_market_impact_from_content(
    ...
    #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let impact_score = ((f64::from(base_score) * sentiment.overall_score.abs().mul_add(0.3, 1.0))
        .clamp(0.0, f64::from(u32::MAX)) as u32)
        .min(100);
...
mod tests {
    ...
    fn test_calculate_avg_credibility() {
        ...
        let empty_articles: Vec<Article> = vec![];
        assert!((calculate_avg_credibility(&empty_articles) - 0.0).abs() < f64::EPSILON);

        // Test with single article
        let single_article = vec![create_test_article_with_credibility(75)];
        assert!((calculate_avg_credibility(&single_article) - 75.0).abs() < f64::EPSILON);
        ...
        assert!((calculate_avg_credibility(&multiple_articles) - 70.0).abs() < f64::EPSILON);
    }
    ...
    fn test_analyze_sentiment_comprehensive() {
        ...
        let negative_sentiment = analyze_sentiment(
            ...
            Some("Major decline and concerns about future"),
            Some("The drop causes risk and threats to vulnerable markets"),
        );
        ...
        let greed_content =
            analyze_sentiment("Moon lambo rich massive explosive gains profit", None, None);
        ...
        assert!(bitcoin_surge.topic_sentiments.get("bitcoin").unwrap() > &0.0);
        ...
        assert!(ethereum_crash.topic_sentiments.get("ethereum").unwrap() < &0.0);
    }
    ...
    fn test_analyze_sentiment_edge_cases() {
        ...
        assert!((empty_sentiment.overall_score - 0.0).abs() < f64::EPSILON);
        assert_eq!(empty_sentiment.classification, "Neutral");

        // Test content with no sentiment words
        let neutral_words =
            analyze_sentiment("The weather is nice today", Some("Random content"), None);
        assert!((neutral_words.overall_score - 0.0).abs() < f64::EPSILON);
    ...
    fn test_calculate_market_impact() {
        ...
        let breaking_category = Category {
        ...
        let regulation_category = Category {
        ...
        let fearful_sentiment = Sentiment {
            overall_score: -0.5,
            confidence: 0.8,
    ...
    fn test_extract_entities_from_text() {
        ...
        let entities = extract_entities_from_text(
            crypto_text,
            Some("BTC and ETH analysis"),
            Some("Solana SOL also mentioned"),
            "crypto",
        );
        ...
        let protocol_entities = extract_entities_from_text(protocol_text, None, None, "protocols");
        ...
        assert_eq!(no_entities.first().unwrap().name, "default_topic");
        assert_eq!(no_entities.first().unwrap().entity_type, "Topic");

        // Test sorting by relevance
        let multi_mention_text = "Bitcoin Bitcoin Bitcoin Ethereum";
        let sorted_entities = extract_entities_from_text(multi_mention_text, None, None, "crypto");
        // Bitcoin should be first due to higher mention count
        assert_eq!(sorted_entities.first().unwrap().name, "Bitcoin");
        assert!(
            sorted_entities.first().unwrap().mention_count
                > sorted_entities.get(1).unwrap().mention_count
        );
    }
    ...
    fn test_extract_tags_from_text() {
        ...
        let nft_tags = extract_tags_from_text(nft_text, Some("NFT content"));
    ...
    fn test_extract_crypto_mentions() {
        ...
        let cryptos = extract_crypto_mentions(
            crypto_text,
            Some("Solana SOL analysis"),
            Some("Cardano ADA update"),
        );
    ...
    fn test_calculate_quality_metrics() {
        ...
        let long_content_high = "a".repeat(2500);
        let high_quality = calculate_quality_metrics(
            "Comprehensive Analysis of Market Trends",
            Some("Detailed description of market conditions"),
            Some(&long_content_high), // Long content > 2000 chars
        ...
        let medium_content = "a".repeat(1500);
        let medium_quality = calculate_quality_metrics(
            "Market Update",
            Some("Brief description"),
            Some(&medium_content), // Medium content length
            70,
        );
        ...
        let edge_case = calculate_quality_metrics(
            ...
            Some("Description present"),
            Some(&long_content), // Exactly 500 chars
            75,
        );
    ...
    fn test_calculate_market_impact_from_content() {
        ...
        let critical_content = calculate_market_impact_from_content(
            ...
            Some("Criminal fraud investigation"),
            Some("SEC enforcement action bankruptcy"),
        );
        ...
        let high_content = calculate_market_impact_from_content(
            ...
            Some("Major adoption integration"),
            Some("New launch acquisition"),
        );
        ...
        let low_content =
            calculate_market_impact_from_content("Regular news", Some("Standard content"), None);
    ...
    fn create_test_article_with_credibility(credibility: u32) -> Article {
        Article {
            id: "test".to_string(),
    ...
    fn create_test_article_with_url(url: &str) -> Article {
        let mut article = create_test_article_with_credibility(70);
        article.url = url.to_string();
        article
    }

    fn create_test_article_with_sentiment(sentiment_score: f64) -> Article {
        let mut article = create_test_article_with_credibility(70);
