...
impl Default for Config {
    fn default() -> Self {
        Self {
            api_key: env::var(FASTER100X_API_KEY).unwrap_or_default(),
...
fn convert_raw_concentration_risk(raw: &api_types::ConcentrationRiskRaw) -> ConcentrationRisk {
    ConcentrationRisk {
        ...
        risk_score: u8::try_from(raw.score.unwrap_or(50)).unwrap_or(50),
...
fn create_faster100x_client_with_context(
...
fn normalize_token_address(address: &str) -> Result<String, WebToolError> {
    ...
    if address.len() != 42 || !address.to_lowercase().starts_with("0x") {
...
///
/// # Errors
/// Returns `WebToolError::Config` if token address is invalid or chain is unsupported
/// Returns `WebToolError::Parsing` if API response is malformed
pub async fn analyze_token_holders(
    ...
    let client = create_faster100x_client_with_context(context)?;
...
///
/// # Errors
/// Returns `WebToolError::Config` if token address is invalid or timeframe is unsupported
/// Returns `WebToolError::Parsing` if API response is malformed
...
///
/// # Errors
/// Returns `WebToolError::Config` if token address is invalid or period is unsupported
/// Returns `WebToolError::Parsing` if API response is malformed
pub async fn get_holder_trends(
    ...
    let trend_strength = u8::try_from(data.trend_strength.unwrap_or(50)).unwrap_or(50);
...
mod tests {
    ...
    fn test_normalize_token_address() {
        ...
        assert!(result.is_ok());
        assert_eq!(
            result.expect("Token address normalization should succeed for valid address"),
    ...
    fn test_token_holder_analysis_serialization() {
        ...
        let serialized = serde_json::to_string(&analysis)
            .expect("TokenHolderAnalysis should serialize to JSON successfully");
    ...
    fn test_normalize_token_address_with_whitespace() {
        ...
        assert!(result.is_ok());
        assert_eq!(
            result.expect("Token address normalization should succeed for address with whitespace"),
            "0x1234567890123456789012345678901234567890"
        );
    }
    ...
    fn test_normalize_token_address_uppercase_to_lowercase() {
        ...
        assert!(result.is_ok());
        assert_eq!(
            result.expect("Token address normalization should succeed for uppercase address"),
    ...
    fn test_normalize_token_address_invalid_length_too_long() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should return error for too-long address")
            .to_string()
    ...
    fn test_normalize_token_address_invalid_length_too_short() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should return error for too-short address")
            .to_string()
            .contains("Invalid token address format"));
    }
    ...
    fn test_normalize_token_address_missing_0x_prefix() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should return error for address missing 0x prefix")
            .to_string()
            .contains("Invalid token address format"));
    }
    ...
    fn test_normalize_token_address_wrong_prefix() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should return error for address with wrong prefix")
            .to_string()
            .contains("Invalid token address format"));
    }
    ...
    fn test_normalize_token_address_empty_string() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should return error for empty string")
            .to_string()
            .contains("Invalid token address format"));
    }
    ...
    fn test_normalize_token_address_exact_42_chars() {
        ...
        assert_eq!(
            result
                .expect("Token address normalization should succeed for 42-char address")
                .len(),
            42
        );
    }
    ...
    fn test_whale_activity_serialization() {
        ...
        let serialized = serde_json::to_string(&whale_activity)
            .expect("WhaleActivity should serialize to JSON successfully");
    ...
    fn test_holder_trends_serialization() {
        ...
        let serialized = serde_json::to_string(&holder_trends)
            .expect("HolderTrends should serialize to JSON successfully");
    ...
    fn test_holder_distribution_default_values() {
        ...
        assert!((distribution.top_1_percent - 0.0).abs() < f64::EPSILON);
        assert!((distribution.gini_coefficient - 0.0).abs() < f64::EPSILON);
    ...
    fn test_wallet_holding_creation() {
        ...
        assert!((wallet.token_amount - 1000.0).abs() < f64::EPSILON);
    ...
    fn test_holder_activity_negative_change() {
        ...
        assert!((activity.holder_growth_rate_7d - (-2.5)).abs() < f64::EPSILON);
    }
    ...
    fn test_whale_transaction_values() {
        ...
        assert!((tx.usd_value - 100_000.0).abs() < f64::EPSILON);
        assert!((tx.price_impact - 2.5).abs() < f64::EPSILON);
    }
    ...
    fn test_holder_trend_point_creation() {
        ...
        assert!((point.token_price - 2.5).abs() < f64::EPSILON);
    }
