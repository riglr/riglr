...
use std::{fs, io};
use wiremock::{
    matchers::{self, method, path, query_param},
...
impl MockApiServer {
    ...
    pub const fn verify_request_made(_path_pattern: &str) -> bool {
...
pub fn load_fixture(file_name: &str) -> Result<String, io::Error> {
...
#[expect(dead_code)]
pub fn load_json_fixture(file_name: &str) -> Result<Value, Box<dyn Error>> {
...
mod tests {
    ...
    async fn test_mock_dexscreener_response() {
        ...
        assert!(MockApiServer::verify_request_made("/tokens/ethereum/0x123"));
    }
}
