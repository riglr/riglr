...
use riglr_evm_tools::{get_uniswap_quote, swap::Parameters};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match get_uniswap_quote(
        Parameters {
            ...
            decimals_out: 18,              // WETH decimals
            fee_tier: Some(3000),          // 0.3% fee tier
            slippage: Some(50),            // 0.5% slippage
    ...
    match get_uniswap_quote(
        Parameters {
            ...
            amount_in: "1".to_string(), // 1 WETH
            decimals_in: 18,            // WETH decimals
            decimals_out: 6,            // USDC decimals
            fee_tier: Some(3000),       // 0.3% fee tier
            slippage: Some(50),         // 0.5% slippage
            chain_id: Some(1),          // Ethereum mainnet
        },
    ...
    match get_uniswap_quote(
        Parameters {
            token_in: dai.clone(),
            token_out: usdc.clone(),
            amount_in: "1000".to_string(), // 1000 DAI
            decimals_in: 18,               // DAI decimals
            decimals_out: 6,               // USDC decimals
            fee_tier: Some(500),           // 0.05% fee tier for stablecoins
            slippage: Some(10),            // 0.1% slippage for stablecoins
            chain_id: Some(1),             // Ethereum mainnet
        },
        &context,
    )
