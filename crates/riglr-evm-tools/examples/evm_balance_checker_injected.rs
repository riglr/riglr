...
use riglr_evm_tools::{
    balance::{check_erc20_token, check_eth},
    Balance, Token,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let wallet_address = if let Some(addr) = addresses.first() {
        (*addr).to_string()
    } else {
...
fn print_eth_balance(balance: &Balance) {
...
fn print_token_balance(balance: &Token, token_name: &str) {
    println!("  📍 Token: {token_name}");
    println!("  💰 Balance: {}", balance.balance_formatted);
    if let Some(ref symbol) = balance.token_symbol {
