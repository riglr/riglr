...
use alloy::dyn_abi::{DynSolType, DynSolValue, JsonAbiExt as _};
use alloy::json_abi::Function;
use alloy::primitives::{self, Address, Bytes, FixedBytes, I256, U256};
...
use riglr_core::retry::{retry_async, ErrorClass, RetryConfig};
use riglr_core::{SignerContext, ToolError};
use riglr_macros::tool;
use tracing::{debug, info};

// Import balance helper functions
...
pub async fn call_contract_read(
    ...
    let contract_addr = contract_address.parse::<Address>().map_err(|error| {
        EvmToolError::InvalidAddress(format!("Invalid contract address: {error}"))
    })?;
    ...
    let result = retry_async(
        || async {
            client.call(&tx).await.map_err(|error| {
                EvmToolError::ProviderError(format!("Contract call failed: {error}"))
            })
        },
        |error| {
            // Classify errors - network/RPC errors are retriable
            match error {
                &EvmToolError::ProviderError(_) => ErrorClass::Retryable,
                _ => ErrorClass::Permanent,
            }
        },
        &RetryConfig::new(3, 100, 2000, 2.0, true),
        "contract_read",
    )
    .await
    ...
    decode_contract_result(&result, &output_types).map_err(|error| {
        EvmToolError::ContractError(format!("Failed to decode result: {error}")).into()
    })
}
...
pub async fn call_contract_write(
    ...
    let signer = SignerContext::current_as_evm()
        .map_err(|error| ToolError::SignerContext(format!("No EVM signer context: {error}")))?;

    // Get signer address
    let from_addr_str = signer.signer().address();
    let from_addr = from_addr_str.parse::<Address>().map_err(|error| {
        EvmToolError::InvalidAddress(format!("Invalid signer address: {error}"))
    })?;
    ...
    let (calldata, _) =
        encode_function_call_with_types(&function_selector, &params).map_err(|error| {
            EvmToolError::InvalidParameter(format!("Failed to encode function call: {error}"))
        })?;
    ...
    let tx_hash = retry_async(
        || async {
            ...
            signer
                ...
                .await
                .map_err(|error| {
                    EvmToolError::Generic(format!("Failed to send transaction: {error}"))
                })
        },
        |error| {
            // Classify errors - network/gas errors are retriable, others are permanent
            match *error {
                EvmToolError::Generic(ref msg)
        ...
        &RetryConfig::new(3, 500, 5000, 2.0, true),
        "contract_write",
    )
    .await
    .map_err(ToolError::from)?;
...
pub async fn read_erc20_info(
    ...
    let token_addr = token_address
        .parse::<Address>()
        .map_err(|error| EvmToolError::InvalidAddress(format!("Invalid token address: {error}")))?;
    ...
    let total_supply = {
        ...
        let selector = Bytes::from(hex::decode("18160ddd").map_err(|error| {
            EvmToolError::InvalidParameter(format!("selector decode: {error}"))
        })?);
        let tx = TransactionRequest::default()
            .to(token_addr)
            .input(selector.into());
        (client.call(&tx).await).map_or_else(
            |_| String::default(),
            |bytes| {
                U256::try_from_be_slice(bytes.as_ref())
...
fn encode_function_call_with_types(
    ...
    params: &[String],
) -> Result<(Bytes, Vec<String>), Box<dyn CoreError + Send + Sync>> {
    ...
    if function_or_selector.starts_with("0x") && function_or_selector.len() == 10 {
        ...
        let selector_bytes =
            hex::decode(sel).map_err(|e| format!("Invalid function selector: {e}"))?;
        ...
        for p in params {
            ...
            let encoded = if p.starts_with("0x") && p.len() == 42 {
                ...
                return Err(format!(
                    "Unsupported param format: {p}. Use 0x-prefixed hex or decimal number."
                )
                .into());
            };
...
fn parse_param_to_dyn_sol_value(
    param: &str,
    expected_type: &str,
) -> Result<DynSolValue, Box<dyn CoreError + Send + Sync>> {
...
fn parse_address(param: &str) -> Result<DynSolValue, Box<dyn CoreError + Send + Sync>> {
    let addr = Address::from_str(param).map_err(|e| format!("Invalid address '{param}': {e}"))?;
    Ok(DynSolValue::Address(addr))
}

/// Parse a boolean parameter
fn parse_bool(param: &str) -> Result<DynSolValue, Box<dyn CoreError + Send + Sync>> {
    let val = param
        .parse::<bool>()
        .or_else(|_| {
            if param == "1" {
        ...
        .map_err(|()| format!("Invalid bool value '{param}'"))?;
    Ok(DynSolValue::Bool(val))
}

/// Parse a bytes parameter
fn parse_bytes(param: &str) -> Result<DynSolValue, Box<dyn CoreError + Send + Sync>> {
...
fn parse_uint(param: &str, bits: usize) -> Result<DynSolValue, Box<dyn CoreError + Send + Sync>> {
    ...
    let max = if bits == 256 {
        ...
        U256::from(1_u64)
            .checked_shl(bits)
            .unwrap_or(U256::MAX)
            .saturating_sub(U256::from(1_u64))
    };
    if value > max {
        return Err(EvmToolError::ContractError(format!(
            "Value {value} exceeds max for uint{bits}"
        ))
        .into());
    }
...
fn parse_int(param: &str, bits: usize) -> Result<DynSolValue, Box<dyn CoreError + Send + Sync>> {
    let value = if let Some(abs_str) = param.strip_prefix('-') {
        ...
        I256::try_from(abs_val)
            .map_err(|e| format!("Invalid negative int: {e}"))
            .and_then(|i| i.checked_neg().ok_or_else(|| "Negation overflow".into()))?
    } else {
        // Positive number
...
fn parse_tuple(
    ...
    let values = if param.starts_with('[') && param.ends_with(']') {
        ...
        // Parentheses tuple
        param
            .get(1..param.len().saturating_sub(1))
            .unwrap_or_default()
            .split(',')
...
fn decode_contract_result(
    ...
    if output_types.is_empty()
        || (output_types.len() == 1 && output_types.first().is_some_and(|t| t == "bytes"))
    {
    ...
    if decoded_values.len() == 1 {
        return Ok(decoded_values
            .first()
            .cloned()
            .unwrap_or_else(|| format!("0x{}", hex::encode(result))));
    }
    Ok(serde_json::to_string(&decoded_values)
        .unwrap_or_else(|_| format!("0x{}", hex::encode(result))))
}
...
fn decode_single_type(
        ...
        *sol_type,
        ...
        if data.len() >= offset.saturating_add(32) {
            let chunk = data
                ...
                .ok_or_else(|| EvmToolError::ContractError("Invalid data range".to_string()))?;
            *offset = offset.saturating_add(32);
            ...
            match *sol_type {
                ...
                DynSolType::Address => {
                    let addr = Address::from_slice(chunk.get(12..).ok_or_else(|| {
                        EvmToolError::ContractError("Invalid address data".to_string())
                    })?);
                    return Ok(DynSolValue::Address(addr));
                }
                DynSolType::Bool => {
                    let value = chunk.get(31).ok_or_else(|| {
                        EvmToolError::ContractError("Invalid bool data".to_string())
                    })? != &0;
                    return Ok(DynSolValue::Bool(value));
                }
                _ => {
                    return Err(Box::new(EvmToolError::ContractError(
    ...
    Err(Box::new(EvmToolError::ContractError(format!(
        "Complex type decoding not fully implemented for: {sol_type:?}"
    ))))
}
...
fn format_decoded_value(value: &DynSolValue) -> String {
    match *value {
        DynSolValue::Address(ref addr) => format!("0x{}", hex::encode(addr.as_slice())),
        DynSolValue::Uint(ref v, _) => v.to_string(),
        DynSolValue::Int(ref v, _) => v.to_string(),
        DynSolValue::Bool(ref b) => b.to_string(),
        DynSolValue::String(ref s) => s.clone(),
        DynSolValue::Bytes(ref b) => format!("0x{}", hex::encode(b)),
        DynSolValue::FixedBytes(ref b, _) => format!("0x{}", hex::encode(b.as_slice())),
        DynSolValue::Array(ref arr) => {
            let items: Vec<String> = arr.iter().map(format_decoded_value).collect();
            format!("[{}]", items.join(", "))
        }
        DynSolValue::Tuple(ref vals) => {
...
mod tests {
    ...
    fn test_decode_address() {
        ...
        assert!(result.is_ok());
        assert!(result.expect("Test should pass").starts_with("0x"));
    }
    ...
    fn test_encode_function_call_with_types_when_function_signature_should_encode_correctly() {
        ...
        assert!(result.is_ok());
        let (calldata, output_types) = result.expect("Test should pass");
    ...
    fn test_parse_address_when_valid_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::Address(addr) = result.expect("Test should pass") {
            assert_eq!(
                format!("{addr:?}"),
    ...
    fn test_parse_bool_when_true_should_return_ok() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.expect("Test should pass"), DynSolValue::Bool(true));
    }
    ...
    fn test_parse_bool_when_false_should_return_ok() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.expect("Test should pass"), DynSolValue::Bool(false));
    }
    ...
    fn test_parse_bytes_when_hex_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::Bytes(bytes) = result.expect("Test should pass") {
    ...
    fn test_parse_uint_when_decimal_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::Uint(val, bits) = result.expect("Test should pass") {
    ...
    fn test_parse_uint_when_hex_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::Uint(val, _) = result.expect("Test should pass") {
            assert_eq!(val, U256::from(1000u64));
        }
    }
    ...
    fn test_parse_int_when_positive_decimal_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::Int(val, bits) = result.expect("Test should pass") {
            assert_eq!(val, I256::try_from(1000u64).expect("Test should pass"));
            assert_eq!(bits, 256);
        }
    }
    ...
    fn test_parse_int_when_negative_decimal_should_return_ok() {
        let result = parse_int("-1000", 256);
        assert!(result.is_ok());
        if let DynSolValue::Int(val, _) = result.expect("Test should pass") {
            assert_eq!(
                val,
                I256::ZERO - I256::try_from(1000u64).expect("Test should pass")
            );
        }
    }
    ...
    fn test_parse_array_when_json_format_should_return_ok() {
        let inner = DynSolType::Uint(256);
        let result = parse_array("[\"1\", \"2\", \"3\"]", &inner);
        assert!(result.is_ok());
        if let DynSolValue::Array(arr) = result.expect("Test should pass") {
            assert_eq!(arr.len(), 3);
        }
    }
    ...
    fn test_parse_fixed_array_when_correct_size_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::FixedArray(arr) = result.expect("Test should pass") {
            assert_eq!(arr.len(), 2);
        }
    }
    ...
    fn test_parse_tuple_when_json_format_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::Tuple(vals) = result.expect("Test should pass") {
    ...
    fn test_parse_param_to_dyn_sol_value_when_string_should_return_ok() {
        ...
        assert!(result.is_ok());
        if let DynSolValue::String(s) = result.expect("Test should pass") {
            assert_eq!(s, "hello");
        }
    }
    ...
    fn test_decode_contract_result_when_empty_types_should_return_hex() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.expect("Test should pass"), "0x1234");
    }
    ...
    fn test_decode_contract_result_when_multiple_types_should_return_json() {
        ...
        assert!(result.is_ok());
        let result_str = result.expect("Test should pass");
    ...
    fn test_decode_contract_result_when_decode_fails_should_fallback_to_hex() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.expect("Test should pass"), "0x12");
    }
    ...
    fn test_decode_single_type_when_bool_should_return_ok() {
        ...
        data.push(1);
        let mut offset = 0;
        let sol_type = DynSolType::Bool;

        let result = decode_single_type(&data, &mut offset, &sol_type);
        assert!(result.is_ok());
        if let DynSolValue::Bool(val) = result.expect("Test should pass") {
            assert!(val);
        }
    }
    ...
    fn test_decode_single_type_when_complex_type_should_return_err() {
        let data = vec![0u8; 32];
        let mut offset = 0;
        let sol_type = DynSolType::String;
    ...
    fn test_format_decoded_value_when_int_should_format_decimal() {
        let val = DynSolValue::Int(I256::try_from(1000u64).expect("Test should pass"), 256);
        let formatted = format_decoded_value(&val);
        assert_eq!(formatted, "1000");
    }
    ...
    fn test_format_decoded_value_when_fixed_bytes_should_format_hex() {
        let val = DynSolValue::FixedBytes(FixedBytes::from_slice(&[0x12, 0x34]), 2);
        let formatted = format_decoded_value(&val);
        assert_eq!(formatted, "0x1234");
    }
    ...
    fn test_decode_single_type_when_int256_should_return_err() {
        let data = vec![0u8; 32];
        let mut offset = 0;
        let sol_type = DynSolType::Int(256);
    ...
    async fn test_call_contract_read_uses_unified_retry() {
        ...
        let retry_config = RetryConfig::new(3, 100, 2000, 2.0, true);
    ...
    async fn test_call_contract_write_uses_unified_retry() {
        ...
        let retry_config = RetryConfig::new(3, 500, 5000, 2.0, true);
