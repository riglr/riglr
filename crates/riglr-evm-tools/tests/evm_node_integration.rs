...
mod tests {
    ...
    use {
        ...
        riglr_evm_tools::{transaction::send_eth, EvmLocalClient},
        std::str::FromStr,
        std::sync::Arc,
    ...
    async fn get_balance(provider_url: &str, address: Address) -> U256 {
        let url = provider_url.parse().expect("Failed to parse provider URL");
    ...
    async fn test_evm_transfer_integration() {
        ...
        let signer = EvmLocalClient::new(&private_key, config).expect("Failed to create signer");
    ...
    async fn test_evm_contract_deployment() {
        ...
        SignerContext::with_signer(unified_signer, async move {
            ...
            let receipt = provider
                .get_transaction_receipt(deployment_hash.parse().expect("Valid hash"))
                .await
    ...
    async fn test_evm_multiple_transfers() {
        ...
        let config = EvmNetworkConfig::new("local", 31337, rpc_url.clone());
        let _signer = EvmLocalClient::new(&private_key, config).expect("Failed to create signer");
        ...
        for recipient_str in &recipients {
            let recipient = Address::from_str(recipient_str).expect("Valid address");
    ...
    async fn test_evm_insufficient_funds_error() {
        ...
        let _signer = EvmLocalClient::new(unfunded_key, config).expect("Failed to create signer");
        ...
        assert!(
            ...
            "Error should indicate insufficient funds: {error_msg}"
        );
    }
    ...
    async fn test_evm_gas_estimation() {
        ...
        SignerContext::with_signer(unified_signer, async move {
            ...
            let evm_signer = SignerContext::current_as_evm().expect("Failed to get signer");
            ...
            let url = rpc_url.parse().expect("Failed to parse RPC URL");

            let provider = ProviderBuilder::new().on_http(url);

            let receipt = provider
                .get_transaction_receipt(tx_hash.parse().expect("Valid hash"))
                .await
                .expect("Failed to get receipt")
    ...
    async fn test_evm_concurrent_transactions() {
        ...
        assert!(
            success_count >= 3,
            "At least 60% of concurrent transactions should succeed, got {success_count}/5"
        );
    }
}
