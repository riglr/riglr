...
// Test constants should never fail to parse
...
fn eth_to_wei_decimal(eth_amount: Decimal) -> U256 {
    ...
    let wei_decimal = eth_amount
        * Decimal::from_str("1000000000000000000")
            .expect("ETH-to-wei conversion constant should be valid");
...
fn wei_to_eth_decimal(wei_amount: U256) -> Decimal {
    ...
    let result = wei_decimal
        / Decimal::from_str("1000000000000000000")
            .expect("Wei-to-ETH conversion constant should be valid");
    result
}
...
fn test_precision_comparison_small_values() {
    ...
    let small_eth = 0.000_000_000_000_000_001; // 1 wei
    ...
    // Decimal approach
    let small_eth_decimal =
        Decimal::from_str("0.000000000000000001").expect("Small ETH decimal value should be valid");
    ...
    println!("  f64 roundtrip: {back_f64} (error: {error_f64})");
    println!("  Decimal roundtrip: {back_decimal} (error: {error_decimal})");
...
fn test_precision_comparison_18_decimals() {
    ...
    let precise_f64 = precise_eth_str
        .parse::<f64>()
        .expect("Precise ETH string should parse to f64");
    let wei_f64 = eth_to_wei_f64(precise_f64);
    let back_f64 = wei_to_eth_f64(wei_f64);

    // Decimal approach (preserves all 18 decimal places)
    let precise_decimal =
        Decimal::from_str(precise_eth_str).expect("Precise ETH string should parse to Decimal");
    let wei_decimal = eth_to_wei_decimal(precise_decimal);
    let back_decimal = wei_to_eth_decimal(wei_decimal);

    println!("\n18 decimal precision test:");
    println!("  Original string: {precise_eth_str}");
    println!("  f64 value: {precise_f64:.18}");
    println!("  f64 roundtrip: {back_f64:.18}");
    ...
    assert_eq!(
        wei_decimal,
        U256::from_str("123456789012345678").expect("Wei value should be valid U256")
    );
}
...
fn test_precision_comparison_large_values() {
    ...
    // Decimal approach
    let large_eth_decimal =
        Decimal::from_str("1000000000").expect("Large ETH decimal value should be valid");
    ...
    println!("  f64 roundtrip: {back_f64} (relative error: {error_f64:.2e})");
    println!("  Decimal roundtrip: {back_decimal} (relative error: {error_decimal})");
...
fn test_gas_price_precision() {
    ...
    let wei_from_gwei_decimal = Decimal::from_str("1.5").expect("1.5 should be valid decimal")
        * Decimal::from_str("1000000000").expect("1 billion should be valid decimal");

    println!("\nGas price precision (1.5 gwei):");
    println!("  f64: {wei_from_gwei_f64} wei");
    println!("  Decimal: {wei_from_gwei_decimal} wei");
    ...
    assert_eq!(
        ...
        Decimal::from_str("1500000000").expect("1.5 billion should be valid decimal")
    );
}
...
fn test_defi_precision_requirements() {
    ...
    let amount1 = 1.000_000_000_000_000_001; // Slightly more than 1 ETH
    let amount2 = 1.000_000_000_000_000_002; // Slightly more than amount1
    ...
    let amount1_decimal =
        Decimal::from_str("1.000000000000000001").expect("Amount1 decimal should be valid");
    let amount2_decimal =
        Decimal::from_str("1.000000000000000002").expect("Amount2 decimal should be valid");
    ...
    println!("  Amount1: {amount1} ETH");
    println!("  Amount2: {amount2} ETH");
    println!("  f64 can distinguish: {can_distinguish_f64} ({wei1_f64} vs {wei2_f64})");
    println!(
        "  Decimal can distinguish: {can_distinguish_decimal} ({wei1_decimal} vs {wei2_decimal})"
    );
