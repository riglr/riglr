...
// Benchmark code uses expect() on constant values guaranteed to parse
...
fn eth_to_wei_decimal(eth_amount: Decimal) -> U256 {
    ...
    let wei_decimal = eth_amount
        * Decimal::from_str("1000000000000000000")
            .expect("Wei conversion constant should be valid");
...
fn benchmark_eth_to_wei(c: &mut Criterion) {
    ...
    let test_amounts = vec![1.0, 0.123_456_789, 1_000_000.0, 0.000_000_001];
    ...
    for amount in test_amounts {
        ...
        let decimal_amount =
            Decimal::from_f64(amount).expect("Valid f64 should convert to Decimal");
...
fn benchmark_wei_to_eth(c: &mut Criterion) {
    ...
    let test_amounts = vec![
        U256::from_str("1000000000000000000").expect("Valid wei amount constant"), // 1 ETH
        U256::from_str("123456789012345678").expect("Valid wei amount constant"), // 0.123456789012345678 ETH
        U256::from_str("1000000000000000000000000").expect("Valid wei amount constant"), // 1000000 ETH
        U256::from_str("1000000000").expect("Valid wei amount constant"), // 0.000000001 ETH
    ];
...
fn benchmark_precision(c: &mut Criterion) {
    ...
    group.bench_function("decimal_precision_preserved", |b| {
        b.iter(|| {
            let original =
                Decimal::from_str("0.123456789012345678").expect("Valid decimal constant");
