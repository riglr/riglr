...
pub trait AddressValidator: Send + Sync {
    ...
    ///
    /// # Errors
    /// Returns `ConfigError` if the address validation fails
...
pub struct NetworkConfig {
    ...
    /// EVM RPC URLs using `RPC_URL`_{`CHAIN_ID`} convention
...
pub struct ChainConfig {
    ...
    /// RPC URL (overrides global `RPC_URL`_{`CHAIN_ID`} if set)
    #[serde(default)]
...
static NETWORK_NAME_MAP: LazyLock<HashMap<&'static str, u64>> = LazyLock::new(|| {
...
impl NetworkConfig {
    /// Extract RPC URLs from environment using `RPC_URL`_{`CHAIN_ID`} or `RPC_URL`_{`NETWORK_NAME`} convention
    ...
    /// - The chains.toml file cannot be read
    /// - The TOML content is malformed or invalid
    pub fn load_chain_contracts(&mut self) -> ConfigResult<()> {
        let chains_path =
            env::var(RIGLR_CHAINS_CONFIG).unwrap_or_else(|_| "chains.toml".to_string());

        // Only try to load if file exists
        if !Path::new(&chains_path).exists() {
            tracing::debug!("chains.toml not found at {}, using defaults", chains_path);
            return Ok(());
        }

        let content = fs::read_to_string(&chains_path).map_err(|e| {
            ConfigError::io(format!(
                "Failed to read chains config from {chains_path}: {e}"
            ))
        })?;
        ...
        for (_name, toml_chain) in chains_file.chains {
            ...
            Self::apply_core_contract_overrides(&mut chain.contracts, chain.id);
            Self::apply_defi_protocol_overrides(&mut chain.contracts, chain.id);
    ...
    /// Apply core contract environment variable overrides
    fn apply_core_contract_overrides(contracts: &mut ChainContract, chain_id: u64) {
        if let Ok(router) = env::var(format!("ROUTER_{chain_id}")) {
            contracts.router = Some(router);
        }
        if let Ok(quoter) = env::var(format!("QUOTER_{chain_id}")) {
            contracts.quoter = Some(quoter);
        }
        if let Ok(factory) = env::var(format!("FACTORY_{chain_id}")) {
            contracts.factory = Some(factory);
        }
        if let Ok(weth) = env::var(format!("WETH_{chain_id}")) {
            contracts.weth = Some(weth);
        }
        if let Ok(usdc) = env::var(format!("USDC_{chain_id}")) {
            contracts.usdc = Some(usdc);
        }
        if let Ok(usdt) = env::var(format!("USDT_{chain_id}")) {
            contracts.usdt = Some(usdt);
        }
    }

    /// Apply `DeFi` protocol contract environment variable overrides
    fn apply_defi_protocol_overrides(contracts: &mut ChainContract, chain_id: u64) {
        if let Ok(sushiswap_router) = env::var(format!("SUSHISWAP_ROUTER_{chain_id}")) {
            contracts.sushiswap_router = Some(sushiswap_router);
        }
        if let Ok(sushiswap_factory) = env::var(format!("SUSHISWAP_FACTORY_{chain_id}")) {
            contracts.sushiswap_factory = Some(sushiswap_factory);
        }
        if let Ok(aave_v3_pool) = env::var(format!("AAVE_V3_POOL_{chain_id}")) {
            contracts.aave_v3_pool = Some(aave_v3_pool);
        }
        if let Ok(aave_v3_pool_data_provider) =
            env::var(format!("AAVE_V3_POOL_DATA_PROVIDER_{chain_id}"))
        {
            contracts.aave_v3_pool_data_provider = Some(aave_v3_pool_data_provider);
        }
        if let Ok(aave_v3_oracle) = env::var(format!("AAVE_V3_ORACLE_{chain_id}")) {
            contracts.aave_v3_oracle = Some(aave_v3_oracle);
        }
        if let Ok(compound_v3_usdc) = env::var(format!("COMPOUND_V3_USDC_{chain_id}")) {
            contracts.compound_v3_usdc = Some(compound_v3_usdc);
        }
        if let Ok(curve_registry) = env::var(format!("CURVE_REGISTRY_{chain_id}")) {
            contracts.curve_registry = Some(curve_registry);
        }
        if let Ok(oneinch_aggregation_router) =
            env::var(format!("ONEINCH_AGGREGATION_ROUTER_{chain_id}"))
        {
            contracts.oneinch_aggregation_router = Some(oneinch_aggregation_router);
        }
        if let Ok(balancer_vault) = env::var(format!("BALANCER_VAULT_{chain_id}")) {
            contracts.balancer_vault = Some(balancer_vault);
        }
        if let Ok(quickswap_router) = env::var(format!("QUICKSWAP_ROUTER_{chain_id}")) {
            contracts.quickswap_router = Some(quickswap_router);
        }
        if let Ok(gmx_router) = env::var(format!("GMX_ROUTER_{chain_id}")) {
            contracts.gmx_router = Some(gmx_router);
        }
        if let Ok(maker_dai) = env::var(format!("MAKER_DAI_{chain_id}")) {
            contracts.maker_dai = Some(maker_dai);
        }
    }
    ...
    /// # Errors
    /// Returns `ConfigError` if:
    /// - Solana RPC URL is invalid
    /// - Any EVM RPC URLs are invalid
    /// - Chain ID consistency check fails
    /// - Contract address validation fails (if validator provided)
        ...
        for (chain_id, url) in &self.rpc_urls {
            if !Self::is_valid_rpc_url(url) {
                return Err(ConfigError::validation(format!(
                    "Invalid RPC URL for chain {chain_id}: {url}"
                )));
            }
        }
        Ok(())
    ...
    fn validate_chain_id_consistency(chain_id: u64, chain: &ChainConfig) -> ConfigResult<()> {
    ...
    fn validate_core_contracts(
        ...
        for (address_opt, contract_name) in core_contracts {
            if let Some(addr) = address_opt.as_ref() {
...
"#
    ...
    fn test_network_config_default() {
        ...
        assert_eq!(config.timeouts.rpc, 30);
        assert_eq!(config.timeouts.websocket, 60);
    ...
    fn test_network_timeouts_default() {
        ...
        assert_eq!(timeouts.rpc, 30);
        assert_eq!(timeouts.websocket, 60);
        assert_eq!(timeouts.http, 30);
    }
    ...
    fn test_get_supported_chains_from_chains_only() {
        ...
        config.chains.insert(1, ethereum_config);
        config.chains.insert(56, bsc_config);

        let supported_chain_ids = config.get_supported_chains();
        assert_eq!(supported_chain_ids.len(), 2);
        assert!(supported_chain_ids.contains(&1));
        assert!(supported_chain_ids.contains(&56));
    }
    ...
    fn test_get_supported_chains_mixed_sources_with_duplicates() {
        ...
        let supported_chain_ids = config.get_supported_chains();
        assert_eq!(supported_chain_ids.len(), 3); // 1, 56, 137 (deduplicated)
        assert!(supported_chain_ids.contains(&1));
        assert!(supported_chain_ids.contains(&56));
        assert!(supported_chain_ids.contains(&137));
    }
    ...
    fn test_validate_invalid_solana_rpc_url_no_protocol() {
        let config = NetworkConfig {
            solana_rpc_url: "api.mainnet-beta.solana.com".to_string(),
            ..NetworkConfig::default()
        };
    ...
    fn test_validate_invalid_solana_rpc_url_ftp_protocol() {
        let config = NetworkConfig {
            solana_rpc_url: "ftp://api.mainnet-beta.solana.com".to_string(),
            ..NetworkConfig::default()
        };
