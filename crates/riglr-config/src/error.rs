...
mod tests {
    ...
    fn test_validation_constructor_with_string() {
        ...
        assert!(matches!(error, ConfigError::ValidationError(ref msg) if msg == "Invalid value"));
    }
    ...
    fn test_parse_constructor_with_string() {
        ...
        assert!(matches!(error, ConfigError::ParseError(ref msg) if msg == "Parse failed"));
    }
    ...
    fn test_io_constructor_with_string() {
        ...
        assert!(matches!(error, ConfigError::IoError(ref msg) if msg == "IO failed"));
    }
    ...
    fn test_generic_constructor_with_string() {
        ...
        assert!(matches!(error, ConfigError::Generic(ref msg) if msg == "Generic error"));
    }
    ...
    fn test_from_std_io_error() {
        ...
        assert!(
            matches!(config_error, ConfigError::IoError(ref msg) if msg.contains("File not found"))
        );
    }
    ...
    fn test_error_equality_and_matching() {
        ...
        assert!(
            matches!((&error1, &error2), (&ConfigError::ChainNotSupported(id1), &ConfigError::ChainNotSupported(id2)) if id1 == id2)
        );
    }

    #[test]
    fn test_error_with_empty_strings() {
        let error = ConfigError::MissingEnvVar(String::new());
    ...
    fn test_error_with_special_characters() {
        ...
        assert!(
            matches!(error, ConfigError::ValidationError(ref msg) if msg == "Invalid: üñíçødé")
        );
    }
