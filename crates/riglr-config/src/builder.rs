...
impl ConfigBuilder {
    ...
    fn merge_network_config(&mut self, other: NetworkConfig) {
        ...
        if other.timeouts.rpc != default_network.timeouts.rpc
            || other.timeouts.websocket != default_network.timeouts.websocket
            || other.timeouts.http != default_network.timeouts.http
        {
    ...
    #[must_use]
    pub const fn app(mut self, config: AppConfig) -> Self {
        self.app = config;
        self
    }
    ...
    /// ```
    #[must_use]
    pub const fn use_testnet(mut self, use_testnet: bool) -> Self {
    ...
    /// ```
    #[must_use]
    pub const fn default_chain_id(mut self, id: u64) -> Self {
    ...
    /// ```
    #[must_use]
    pub const fn development(mut self) -> Self {
    ...
    /// ```
    #[must_use]
    pub const fn staging(mut self) -> Self {
        use crate::{Environment, LogLevel};
    ...
    /// ```
    #[must_use]
    pub const fn production(mut self) -> Self {
        use crate::{Environment, LogLevel};
    ...
    /// ```
    #[must_use]
    pub const fn enable_trading(mut self, enabled: bool) -> Self {
    ...
    /// ```
    #[must_use]
    pub const fn enable_graph_memory(mut self, enabled: bool) -> Self {
    ...
    /// - The TOML content cannot be deserialized into a valid Config structure
    ///
    /// # Example
    /// ```rust,no_run
    /// use riglr_config::ConfigBuilder;
    ...
    ///     .unwrap();
    /// ```
    pub fn from_file<P: AsRef<Path>>(mut self, path: P) -> ConfigResult<Self> {
    ...
    /// - Required environment variable format is invalid
    /// - Configuration values from environment are invalid
    ///
    /// # Example
    /// ```rust
    /// use riglr_config::ConfigBuilder;
    ...
    /// - Configuration validation rules fail
    /// - Cross-field validation constraints are not met
    ///
    /// # Example
    /// ```rust
    /// use riglr_config::ConfigBuilder;
...
mod tests {
    ...
    fn test_mixed_bulk_and_individual_setters() {
        ...
        let app_config = AppConfig {
            environment: Environment::Development, // Use development to avoid validation issues
            ..AppConfig::default()
        };
