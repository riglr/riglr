...
pub mod test_urls {
    ...
    pub fn postgres() -> String {
        TestUrlBuilder::from_template(templates::POSTGRES_URL)
            .with_username(
                env::var(env_vars::TEST_DB_USER)
                    .unwrap_or_else(|_| "user".to_string())
                    .as_str(),
            )
            .with_password(
                env::var(env_vars::TEST_DB_PASS)
    ...
    pub fn websocket() -> String {
        TestUrlBuilder::from_template(templates::WSS_URL)
            .with_username(
                env::var(env_vars::TEST_WS_USER)
                    .unwrap_or_else(|_| "user".to_string())
                    .as_str(),
            )
            .with_password(
                env::var(env_vars::TEST_WS_PASS)
                    .unwrap_or_else(|_| "pass".to_string())
                    .as_str(),
            )
...
mod tests {
    ...
    fn test_postgres_url() {
        #[expect(unsafe_code)]
        unsafe {
            env::set_var(env_vars::TEST_DB_USER, "u");
            env::set_var(env_vars::TEST_DB_PASS, "p");
        }
        let url = test_urls::postgres();
        assert_eq!(url, "postgresql://u:p@localhost:5432/testdb");
        #[expect(unsafe_code)]
        unsafe {
            env::remove_var(env_vars::TEST_DB_USER);
            env::remove_var(env_vars::TEST_DB_PASS);
        }
    }
    ...
    fn test_websocket_url() {
        #[expect(unsafe_code)]
        unsafe {
            env::set_var(env_vars::TEST_WS_USER, "u");
            env::set_var(env_vars::TEST_WS_PASS, "p");
        }
        let url = test_urls::websocket();
        assert_eq!(url, "wss://u:p@example.com:443/path");
        #[expect(unsafe_code)]
        unsafe {
            env::remove_var(env_vars::TEST_WS_USER);
            env::remove_var(env_vars::TEST_WS_PASS);
        }
    }
}
