...
impl FeaturesConfig {
    ...
    /// Enable a feature
    pub const fn enable(&mut self, feature: Feature) {
        match feature {
    ...
    /// # Errors
    ///
    /// Currently returns `Ok(())` but may return configuration validation errors in the future.
...
impl fmt::Display for Feature {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            Self::SocialMonitoring => write!(f, "social_monitoring"),
            Self::GraphMemory => write!(f, "graph_memory"),
