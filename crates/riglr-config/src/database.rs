...
static REDIS_URL_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^rediss?://").unwrap());
#[expect(clippy::unwrap_used)]
static NEO4J_URL_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^(neo4j(\+s)?|bolt(\+s)?)://").unwrap());
#[expect(clippy::unwrap_used)]
static HTTP_URL_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^https?://").unwrap());
#[expect(clippy::unwrap_used)]
static POSTGRES_URL_REGEX: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^postgres(ql)?://").unwrap());

/// Private helper function for Redis URL validation
fn validate_redis_url_inner(url: &str) -> Result<(), validator::ValidationError> {
...
mod tests {
    ...
    fn test_validate_neo4j_url_when_malformed_url_should_return_err() {
        ...
        let err = result.expect_err("Expected validation to fail");
        assert!(err.to_string().contains("Invalid Neo4j URL"));
    }
