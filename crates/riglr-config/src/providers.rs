...
impl ProvidersConfig {
    ...
    #[must_use]
    pub const fn has_ai_provider(&self, provider: AiProvider) -> bool {
        match provider {
    ...
    #[must_use]
    pub const fn has_blockchain_provider(&self, provider: BlockchainProvider) -> bool {
        match provider {
    ...
    #[must_use]
    pub const fn has_data_provider(&self, provider: DataProvider) -> bool {
        match provider {
    ...
    /// - Any API key is present but empty
    /// - Twitter bearer token is set but doesn't start with "Bearer "
...
mod tests {
    ...
    fn test_validate_when_twitter_token_is_empty_should_return_ok() {
        let config = ProvidersConfig {
            twitter_bearer_token: Some(String::new()),
            ..Default::default()
        };
        assert!(config.validate_config().is_ok());
    }
