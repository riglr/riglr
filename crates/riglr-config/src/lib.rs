...
//! - **Convention over configuration**: `RPC_URL`_{`CHAIN_ID`} pattern for dynamic chain support
...
impl Config {
    ...
    /// 3. Apply convention-based patterns (`RPC_URL`_{`CHAIN_ID`})
    ...
    /// - Chain contracts file cannot be loaded (warning logged, not fatal)
    ...
    /// # Errors
    ///
    /// Returns a `ConfigError` if any validation fails, including:
    /// - Invalid application configuration
    /// - Invalid database configuration
    /// - Invalid network configuration
    /// - Invalid providers configuration
    /// - Invalid features configuration
    /// - Cross-dependency validation failures
...
mod tests {
    ...
    fn create_test_config() -> Config {
        let features = FeaturesConfig {
            enable_bridging: false,     // Requires lifi_api_key
            enable_graph_memory: false, // Requires neo4j_url
    ...
    fn test_config_builder_should_validate_during_build() {
        ...
        let database_config = DatabaseConfig {
            neo4j_url: None, // This should cause validation to fail
            ..DatabaseConfig::default()
        };
