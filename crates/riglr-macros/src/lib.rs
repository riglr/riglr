// Allow pattern type mismatch warnings for complex syn pattern matching
#![allow(
    clippy::pattern_type_mismatch,
    reason = "Complex syn pattern matching requires type mismatches"
)]
#![allow(
    clippy::similar_names,
    reason = "Test functions naturally have similar variable names"
)]
...
```rust,ignore
// This example is ignored because it demonstrates generated code structure, not runnable code
// Your function:
#[tool]
...
```rust,ignore
// This example is ignored because it shows the generated implementation code, not user-written code
...
```rust,ignore
// This example is ignored because it uses `...` to indicate omitted implementation
...
1. **Return Type**: Must be `Result<T, E>` where `E: Into<riglr_core::ToolError>`
   ```rust,ignore
   // These examples are ignored because they demonstrate valid/invalid patterns with `...` indicating omitted implementation
...
```rust,ignore
// This example is ignored because it's a simplified demonstration - real usage requires ApplicationContext parameter
...
fn calculate_compound_interest(
    ...
    if principal <= 0.0 {
        Err(ToolError::invalid_input_string("Principal must be positive"))
    }
    if annual_rate < 0.0 {
        Err(ToolError::invalid_input_string("Interest rate cannot be negative"))
    }
    if years < 0.0 {
        Err(ToolError::invalid_input_string("Time period cannot be negative"))
    }
    if compounds_per_year == 0 {
        Err(ToolError::invalid_input_string("Compounds per year must be at least 1"))
    }
...
/// Tool attribute structure for parsing macro attributes
...
pub fn tool(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let tool_attrs =
        syn::parse::<ToolAttr>(attr).map_or(ToolAttr { description: None }, |attrs| attrs);
    ...
    if let Ok(function) = function_parse_result {
        return handle_function(&function, tool_attrs).into();
    }
    let struct_parse_result = syn::parse::<ItemStruct>(input);
    struct_parse_result.map_or_else(|_| syn::Error::new_spanned(
        ...
        .to_compile_error()
        .into(), |structure| handle_struct(&structure, tool_attrs).into())
}
    ...
    match ty {
        ...
        syn::Type::Path(type_path) => {
            type_path.path.segments.last().is_some_and(|segment| {
                let segment_name = segment.ident.to_string();
                // All type patterns return true (serializable) since custom types
                // are assumed serializable and all standard types are supported
                matches!(
                    segment_name.as_str(),
                    // Primitive types
        ...
        // Other types - be conservative and reject (catch-all for all other syn::Type variants)
        &syn::Type::BareFn(_)
        | &syn::Type::Group(_)
...
/// Contains all data needed to generate tool code from a function
struct FunctionProcessingData<'a> {
    fn_name: &'a syn::Ident,
    fn_vis: &'a syn::Visibility,
    selected_description: String,
    param_fields: Vec<proc_macro2::TokenStream>,
    args_struct_name: syn::Ident,
    tool_struct_name: syn::Ident,
    tool_fn_name: syn::Ident,
...
type ProcessedParameters<'a> = (
    Vec<(&'a syn::Pat, &'a syn::Type, &'a [Attribute])>,
    Vec<(&'a syn::Pat, &'a syn::Type, &'a [Attribute])>,
);

fn process_function_parameters(
    function: &ItemFn,
) -> Result<ProcessedParameters<'_>, proc_macro2::TokenStream> {
    ...
    for input in &function.sig.inputs {
        ...
        }) = input
        {
            if is_context_param(ty) {
                context_params.push((pat.as_ref(), ty.as_ref(), attrs.as_slice()));
            } else {
                user_params.push((pat.as_ref(), ty.as_ref(), attrs.as_slice()));
            }
        }
    }
    ...
    Ok((user_params, context_params))
}

/// Validate function signature requirements
fn validate_function_signature(function: &ItemFn) -> Result<(), proc_macro2::TokenStream> {
    // Validate function signature requirements
    if function.sig.asyncness.is_none() {
        return Err(
            syn::Error::new_spanned(&function.sig, "`#[tool]` functions must be async")
                .to_compile_error(),
        );
    }
        ...
        if let syn::Pat::Ident(ident) = pat {
            ...
            if !is_serializable_type(ty) {
                return Err(syn::Error::new_spanned(
                    ty,
                    format!(
                        "Parameter '{param_name}' must be a serializable type. Consider using String, numbers, bool, Vec<T>, Option<T>, or custom types that implement Serialize/Deserialize"
                    )
                ).to_compile_error());
            }
        } else {
            // Handle other pattern types if necessary
        }
    }

    for &(pat, ty, attrs) in user_params {
        if let syn::Pat::Ident(ident) = pat {
            let param_name = &ident.ident;
            let param_type = ty;
...
/// Generate call arguments for the function invocation
fn generate_call_args(function: &ItemFn) -> proc_macro2::TokenStream {
    ...
    for input in &function.sig.inputs {
        ...
        }) = input
        {
            if is_context_param(ty) {
                ...
                // This case should not happen in practice, but we need an else branch
                // to satisfy the clippy lint
            }
        }
    }
    call_args
...
fn generate_tool_implementation(
    function: &ItemFn,
    data: &FunctionProcessingData<'_>,
) -> proc_macro2::TokenStream {
    let fn_vis = data.fn_vis;
    let module_name = &data.module_name;
    let args_struct_name = &data.args_struct_name;
    let tool_struct_name = &data.tool_struct_name;
    let tool_fn_name = &data.tool_fn_name;
    let fn_name = data.fn_name;
    let selected_description = &data.selected_description;
    let param_fields = &data.param_fields;
    let call_args = &data.call_args;
    let await_token = &data.await_token;
...
/// Handles the code generation for function-based tools
///
/// This function processes a function annotated with #[tool] and generates:
/// - Parameter struct for JSON schema generation
/// - Tool implementation with proper error handling
/// - Integration with the rig framework
fn handle_function(function: &ItemFn, tool_attrs: ToolAttr) -> proc_macro2::TokenStream {
    ...
    let selected_description = tool_attrs.description.map_or(description, |desc| desc);

    // Process and validate parameters
    let (user_params, _context_params) = match process_function_parameters(function) {
        Ok(params) => params,
        Err(error) => return error,
    };

    // Validate function signature
    if let Err(error) = validate_function_signature(function) {
        return error;
    }

    // Build parameter fields
    let param_fields = match build_parameter_fields(&user_params) {
        Ok(fields) => fields,
        Err(error) => return error,
    };
    ...
    let tool_fn_name = syn::Ident::new(&format!("{fn_name}_tool"), fn_name.span());

    // Check if function is async
    let await_token = if function.sig.asyncness.is_some() {
        quote! { .await }
    } else {
        quote! {}
    };
    ...
    let call_args = generate_call_args(function);
    ...
    let data = FunctionProcessingData {
        fn_name,
        fn_vis,
    ...
    generate_tool_implementation(function, &data)
}

/// Handles the code generation for struct-based tools
///
/// This function processes a struct annotated with #[tool] and generates:
/// - Tool trait implementation expecting an async execute method
/// - JSON schema generation for the struct fields
...
///
/// This function processes `#[doc = "..."]` attributes and combines them into a single string,
/// removing leading spaces and joining multiple doc comments with newlines.
        ...
        if attr.path().is_ident("doc") {
            if let syn::Meta::NameValue(meta) = &attr.meta {
                ...
                }) = &meta.value
                {
                    ...
                    docs.push(trimmed_line.to_owned());
                }
            }
        }
    }
...
pub fn derive_into_tool_error(input: TokenStream) -> TokenStream {
    ...
    let variants = match &parsed_input.data {
    ...
    let match_arms = variants.iter().map(|variant| {
        ...
        let conversion = classification.map_or_else(
            || {
                ...
                let is_retriable = retriable_patterns
                    .iter()
                    .any(|error_pattern| variant_name_str.contains(error_pattern));
            ...
            |class| match class.to_string().as_str() {
                "retriable" => {
                    quote! {
                ...
                // Merge permanent and wildcard cases since they have identical bodies
                _ => {
                    quote! {
...
mod tests {
    ...
    fn test_extract_doc_comments_mixed_with_other_attrs() {
        ...
        let other_attr = syn::parse_quote! { #[allow(dead_code)] };
    ...
    #[test]
    fn test_tool_error_match_arms_structure() {
        // Test that the inlined match arms structure compiles correctly
    ...
    fn test_tool_attr_parse_empty() {
        ...
        assert!(result.is_ok());
        let attr = result.expect("Failed to parse empty ToolAttr");
    ...
    fn test_tool_attr_parse_description() {
        ...
        let result: Result<ToolAttr, _> = syn::parse_str(input);
        assert!(result.is_ok());
        let attr = result.expect("Failed to parse ToolAttr with description");
    ...
    fn test_tool_attr_parse_invalid_key() {
        ...
        let err = result.expect_err("Expected parsing to fail for invalid key");
    ...
    fn test_retriable_error_patterns() {
        ...
        for pattern in &retriable_patterns {
            let test_variant = format!("Test{pattern}Error");
    ...
    fn test_permanent_error_patterns() {
        ...
        for variant in &permanent_variants {
            ...
            assert!(!is_retriable, "Variant {variant} should not be retriable");
        }
    }
    ...
    fn test_tool_attr_parse_description_with_quotes() {
        ...
        let result: Result<ToolAttr, _> = syn::parse_str(input);
        assert!(result.is_ok());
        let attr = result.expect("Failed to parse ToolAttr with escaped quotes");
        assert_eq!(
            attr.description,
    ...
    fn test_tool_attr_parse_description_empty_string() {
        ...
        let result: Result<ToolAttr, _> = syn::parse_str(input);
        assert!(result.is_ok());
        let attr = result.expect("Failed to parse ToolAttr with empty description");
        assert_eq!(attr.description, Some(String::new()));
    }
    ...
    fn test_tool_attr_parse_malformed_syntax() {
        ...
        for input in test_cases {
            let result: Result<ToolAttr, _> = syn::parse_str(input);
            assert!(result.is_err(), "Input '{input}' should fail to parse");
        }
    }
    ...
    #[test]
    fn test_tool_error_match_arms_output_consistency() {
        // Test that inlined match arms produce consistent output
        let match_arms1 = quote! {
            _ => Ok(riglr_core::JobResult::Failure {
                error: tool_error,
            })
        };
        let match_arms2 = quote! {
            _ => Ok(riglr_core::JobResult::Failure {
                error: tool_error,
            })
        };
    ...
    fn test_error_pattern_case_sensitivity() {
        ...
        for (variant_name, expected_match) in case_sensitive_tests {
            ...
            assert_eq!(
                ...
                "Case sensitivity test failed for '{variant_name}'"
            );
        }
    }
    ...
    fn test_parameter_parsing_error_handling() {
        ...
        let error = parse_result.expect_err("Expected JSON parsing to fail");
    ...
    fn test_tool_error_match_arms_invalid_input_handling() {
        // Test that the inlined match arms handle all errors with simplified structure
        let match_arms = quote! {
            _ => Ok(riglr_core::JobResult::Failure {
                error: tool_error,
            })
