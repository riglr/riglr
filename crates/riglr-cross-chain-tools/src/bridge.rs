...
use crate::lifi::{
    chain_id_to_name, chain_name_to_id, BridgeStatus, ChainType, CrossChainRoute, LiFiClient,
    LiFiError, RouteRequest, Token,
};
use alloy::{
    primitives::{Bytes, U256},
    rpc::types::TransactionRequest,
};
use core::{cmp::Ordering, error::Error as CoreError, str::FromStr};
#[cfg(test)]
...
use riglr_evm_tools::parse_evm_address;
...
/// Parse U256 from string accepting hex or decimal format
fn parse_u256(s: &str) -> Result<U256, String> {
    let s = s.trim();
    if s.starts_with("0x") || s.starts_with("0X") {
        return U256::from_str_radix(&s[2..], 16).map_err(|e| e.to_string());
    }
    U256::from_str(s).map_err(|e| e.to_string())
}

/// Create a `LiFi` client instance
fn create_lifi_client() -> LiFiClient {
    ...
    if let Ok(api_key) = env::var(LIFI_API_KEY) {
        return client.with_api_key(api_key);
    }
    client
}
...
pub async fn get_cross_chain_routes(
    ...
    let from_address = if from_chain.to_lowercase().contains("solana") {
        signer.as_solana().map(SolanaSigner::pubkey)
    } else {
        signer.as_evm().map(EvmSigner::address)
    };

    // Create LiFi client
    let lifi_client = create_lifi_client();
    ...
    let routes = lifi_client
        ...
        .map_err(|e| match e {
            LiFiError::RouteNotFound { .. } => ToolError::permanent_string(format!(
                "No routes found between {from_chain} and {to_chain}"
            )),
    ...
    if routes.is_empty() {
        return Err(ToolError::permanent_string(format!(
            "No routes available from {from_chain} to {to_chain} for token {from_token} -> {to_token}"
        )));
    }
...
///
/// ```rust,ignore
/// use riglr_cross_chain_tools::bridge::{get_cross_chain_routes, execute_cross_chain_transfer};
///
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// // First get routes
...
///
/// // Execute the bridge
/// let result = execute_cross_chain_transfer(
///     best_route.id.clone(),
///     "ethereum".to_string(),
///     "polygon".to_string(),
...
pub async fn execute_cross_chain_transfer(
    ...
    let route = routes
        ...
        .ok_or_else(|| ToolError::permanent_string(format!("Route ID {route_id} not found")))?;
    ...
    Ok(ExecutionResult {
        ...
        message: format!(
            "Bridge transaction submitted successfully. Track progress with bridge ID: {bridge_id}"
        ),
    })
}
...
/// and source transaction hash returned from `execute_cross_chain_transfer`. Essential for
/// tracking multi-step bridge operations that can take several minutes to complete.
///
/// # Arguments
///
/// * `bridge_id` - Unique bridge operation identifier from `execute_cross_chain_transfer`
/// * `source_tx_hash` - Transaction hash from the source chain
///
/// # Returns
///
/// Returns `StatusResult` containing:
...
pub async fn get_bridge_status(
    ...
    let status_response = lifi_client
        ...
        .map_err(|e| match e {
            LiFiError::ApiError { code: 404, .. } => {
                ToolError::permanent_string(format!("Bridge ID {bridge_id} not found"))
            }
            LiFiError::ApiError { code, message } => {
                if code >= 500 {
    ...
    let (is_complete, is_failed, message) = match status_response.status {
        BridgeStatus::Done => (true, false, "Bridge completed successfully".to_string()),
        BridgeStatus::Failed => (true, true, "Bridge transaction failed".to_string()),
        BridgeStatus::Pending => (false, false, "Bridge transaction is pending".to_string()),
        BridgeStatus::NotFound => (false, false, "Bridge transaction not found".to_string()),
    };
...
/// # Returns
///
/// Returns `FeeEstimate` containing:
/// - `from_chain`, `to_chain`: Source and destination networks
...
/// # Panics
///
/// Panics if the routes list becomes empty after the empty check. This should not happen
/// in normal operation due to the explicit empty check, but could theoretically occur
/// if there's a race condition in the underlying route discovery logic.
///
/// # Examples
///
/// ```rust,ignore
...
pub async fn estimate_bridge_fees(
    ...
    if routes_result.routes.is_empty() {
        return Err(ToolError::permanent_string(format!(
            "No routes available for fee estimation between {from_chain} and {to_chain}"
        )));
    }

    // Use the best (first) route for fee estimation
    let best_route = routes_result
        .routes
        .first()
        .ok_or_else(|| ToolError::permanent_string("Route should exist after empty check"))?;
...
async fn execute_solana_bridge_transaction(
...
) -> Result<String, Box<dyn CoreError + Send + Sync>> {
    tracing::info!(
    ...
    let program_id =
        Pubkey::from_str(&tx_data.to).map_err(|e| format!("Invalid program ID: {e}"))?;
    ...
    // Convert to JSON format for legacy interface
    ...
    let signature = sol_signer
        ...
        .await
        .map_err(|e| format!("Failed to sign and send Solana transaction: {e}"))?;
...
async fn execute_evm_bridge_transaction(
    ...
    // Parse U256 values for EVM transaction
    let value = parse_u256(&tx_data.value).map_err(|e| format!("Invalid value: {e}"))?;
    let gas_limit = parse_u256(&tx_data.gas_limit).unwrap_or_else(|_| U256::from(200_000u64));
    let _gas_price =
    ...
    let from_opt = signer
        .as_evm()
        .and_then(|evm_signer| parse_evm_address(&evm_signer.address()).ok());
    let mut evm_tx = TransactionRequest::default()
        .to(to_address)
        .input(Bytes::from(data).into())
        .value(value)
        .gas_limit(gas_limit.try_into().unwrap_or(200_000u64));
    ...
    let tx_hash = evm_signer
        .sign_and_send_transaction(tx_json)
        .await
        .map_err(|e| format!("Failed to sign and send EVM transaction: {e}"))?;
...
mod tests {
    ...
    use riglr_core::signer::{Chain, EvmClient, EvmSigner, SignerBase, SolanaClient, SolanaSigner};
    use std::panic;
    ...
    impl SolanaSigner for MockCrossChainSigner {
        ...
        ) -> Result<String, Box<dyn riglr_core::SignerError>> {
        ...
        fn client(&self) -> &dyn SolanaClient {
            unimplemented!("Mock Solana client not implemented")
        }
    }
    ...
    impl EvmSigner for MockCrossChainSigner {
        ...
        fn client(&self) -> &dyn EvmClient {
            unimplemented!("Mock EVM client not implemented")
        }
    }
    ...
    fn test_convert_route_when_valid_route_should_convert_successfully() {
        ...
        assert!(result.is_ok());
        let route_info = result.expect("Route conversion should succeed with valid data");
    ...
    fn test_convert_route_when_invalid_from_chain_id_should_return_error() {
        ...
        let error = result.expect_err("Route conversion should fail with invalid from_chain_id");
    ...
    fn test_convert_route_when_invalid_to_chain_id_should_return_error() {
        ...
        assert!(result.is_err());
        // The chain ID conversion should fail for invalid IDs
        let error = result.expect_err("Route conversion should fail with invalid to_chain_id");
        assert!(error.to_string().contains("Permanent error"));
    }
    ...
    fn test_convert_route_when_empty_fees_should_set_none() {
        ...
        assert!(result.is_ok());
        let route_info = result.expect("Route conversion should succeed with empty fees");
        assert_eq!(route_info.fees_usd, None);
    }
    ...
    fn test_convert_route_when_fees_with_no_usd_amounts_should_set_none() {
        ...
        assert!(result.is_ok());
        let route_info =
            result.expect("Route conversion should succeed with fees without USD amounts");
        assert_eq!(route_info.fees_usd, None);
    }
    ...
    fn test_convert_route_when_zero_total_fees_should_set_none() {
        ...
        assert!(result.is_ok());
        let route_info = result.expect("Route conversion should succeed with zero total fees");
        assert_eq!(route_info.fees_usd, None);
    }
    ...
    async fn test_create_lifi_client_when_no_api_key_should_return_default_client() {
        ...
        let _result = create_lifi_client();

        // Function always succeeds, no need to check result
    }
    ...
    async fn test_create_lifi_client_when_api_key_present_should_use_key() {
        ...
        unsafe {
            env::set_var(LIFI_API_KEY, "test-api-key");
        };
    ...
    #[tokio::test]
    async fn test_execute_cross_chain_transfer_when_no_signer_context_should_fail() {
        let config = Config::from_env();
        let context = ApplicationContext::from_config(&config);
        let result = execute_cross_chain_transfer(
            &context,
        ...
        let error = result.expect_err("Cross-chain bridge should fail without signer context");
        assert!(error.to_string().contains("Permanent error"));
    }
    ...
    mod mock_signer_tests {
        ...
        async fn test_mock_signer_solana_transaction_should_return_mock_signature() {
            ...
            let tx_json = serde_json::json!({"data": "mock_tx_data"});
            let result = SolanaSigner::sign_and_send_transaction(&signer, tx_json).await;

            assert!(result.is_ok());
            assert_eq!(
                result.expect("Mock Solana signer should return signature"),
                "mock_solana_signature"
            );
        }
        ...
        async fn test_mock_signer_evm_transaction_should_return_mock_signature() {
            ...
            assert!(result.is_ok());
            assert_eq!(
                result.expect("Mock EVM signer should return signature"),
                "0xmock_evm_signature"
            );
        }
        ...
        fn test_mock_signer_solana_client_should_return_client() {
            ...
            // Since the mock implementation uses unimplemented!(), we expect a panic
            let result = panic::catch_unwind(|| SolanaSigner::client(&signer));
            assert!(
                result.is_err(),
                "Mock client should panic with unimplemented"
            );
        }
        ...
        fn test_mock_signer_evm_client_should_return_error() {
            ...
            // Since the mock implementation uses unimplemented!(), we expect a panic
            let result = panic::catch_unwind(|| EvmSigner::client(&signer));
            assert!(
                result.is_err(),
                "Mock EVM client should panic with unimplemented"
            );
        }
    }
        ...
        fn test_bridge_fee_estimate_creation() {
            ...
            assert_eq!(
                estimate
                    .fees
                    .first()
                    .expect("Should have at least one fee")
                    .name,
                "Bridge Fee"
            );
    ...
    mod bridge_execution_tests {
        ...
        pub fn create_mock_solana_route() -> CrossChainRoute {
            ...
            route.transaction_request = Some(TransactionRequest {
                ...
                chain_id: 1_151_111_081_099_710, // Solana chain ID
        ...
        async fn test_execute_solana_bridge_transaction_when_no_transaction_data_should_fail() {
            ...
            assert!(result.is_err());
            let error =
                result.expect_err("Solana bridge execution should fail without transaction data");
        ...
        async fn test_execute_solana_bridge_transaction_when_invalid_program_id_should_fail() {
            ...
            assert!(result.is_err());
            let error =
                result.expect_err("Solana bridge execution should fail with invalid program ID");
        ...
        async fn test_execute_solana_bridge_transaction_when_invalid_instruction_data_should_fail()
        {
            ...
            assert!(result.is_err());
            let error = result
                .expect_err("Solana bridge execution should fail with invalid instruction data");
            assert!(error
                .to_string()
        ...
        async fn test_execute_evm_bridge_transaction_when_no_transaction_data_should_fail() {
            ...
            assert!(result.is_err());
            let error =
                result.expect_err("EVM bridge execution should fail without transaction data");
            assert!(error.to_string().contains("No transaction data in route"));
        }
        ...
        async fn test_execute_evm_bridge_transaction_when_invalid_to_address_should_fail() {
            ...
            assert!(result.is_err());
            let error =
                result.expect_err("EVM bridge execution should fail with invalid to address");
        ...
        async fn test_execute_evm_bridge_transaction_when_invalid_transaction_data_should_fail() {
            ...
            assert!(result.is_err());
            let error =
                result.expect_err("EVM bridge execution should fail with invalid transaction data");
        ...
        async fn test_execute_evm_bridge_transaction_when_invalid_value_should_fail() {
            ...
            assert!(result.is_err());
            let error = result.expect_err("EVM bridge execution should fail with invalid value");
        ...
        async fn test_execute_evm_bridge_transaction_with_hex_values_should_succeed() {
            ...
            assert!(result.is_ok());
            assert_eq!(
                result.expect("EVM bridge transaction with hex values should succeed"),
                "0xmock_evm_signature"
            );
        }
        ...
        async fn test_execute_evm_bridge_transaction_with_decimal_values_should_succeed() {
            ...
            assert!(result.is_ok());
            assert_eq!(
                result.expect("EVM bridge transaction with decimal values should succeed"),
                "0xmock_evm_signature"
            );
        }
        ...
        async fn test_execute_evm_bridge_transaction_with_default_gas_values_should_succeed() {
            ...
            assert!(result.is_ok());
            assert_eq!(
                result.expect("EVM bridge transaction with default gas values should succeed"),
                "0xmock_evm_signature"
            );
        }
    }
    ...
    mod route_sorting_tests {
        ...
        fn test_route_sorting_recommended_first() {
            ...
            assert_eq!(
                routes.first().expect("Should have first route").id,
                "route2"
            ); // RECOMMENDED should be first
            assert_eq!(
                routes.get(1).expect("Should have second route").id,
                "route1"
            );
        }
        ...
        fn test_route_sorting_by_cost_when_no_recommended() {
            ...
            assert_eq!(routes.first().expect("Should have first route").id, "cheap"); // Lower cost should be first
            assert_eq!(
                routes.get(1).expect("Should have second route").id,
                "expensive"
            );
        ...
        fn test_route_sorting_with_none_costs() {
            ...
            assert_eq!(
                routes.first().expect("Should have first route").id,
                "route_no_costs"
            ); // None costs (0.0) should be first
            assert_eq!(
                routes.get(1).expect("Should have second route").id,
                "route_with_costs"
            );
    ...
    mod additional_coverage_tests {
        ...
        async fn test_solana_bridge_execution_without_pubkey_should_fail() {
            ...
            assert!(result.is_err());
            let error =
                result.expect_err("Solana bridge execution should fail without Solana pubkey");
