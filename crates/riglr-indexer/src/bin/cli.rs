...
use riglr_indexer::storage::{create_store, EventAggregator, EventFilter, EventQuery};
use riglr_indexer::utils::format_bytes;
use std::{env, process};
...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    // and we're only setting environment variables for logging configuration
    #[expect(unsafe_code)]
    unsafe {
    ...
    if let Some(config_file) = cli.config {
        // SAFETY: This is safe because we're in main before any threads are spawned
        // and we're only setting environment variables for configuration
        #[expect(unsafe_code)]
        unsafe {
            env::set_var(ENV_RIGLR_INDEXER_CONFIG, config_file);
        }
    }
...
async fn handle_database_command(
    ...
    match cmd {
        DatabaseCommands::Cleanup { days, dry_run } => {
            return handle_database_cleanup(store.as_ref(), days, dry_run).await
        }
        DatabaseCommands::Init => return handle_database_init(store.as_ref()).await,
        DatabaseCommands::Migrate => {
            handle_database_migrate();
            Ok(())
        }
        DatabaseCommands::Stats => return handle_database_stats(store.as_ref()).await,
    }
}

/// Helper function to initialize database schema
async fn handle_database_init(
    store: &dyn riglr_indexer::DataStore,
) -> Result<(), Box<dyn Error + Send + Sync>> {
...
fn handle_database_migrate() {
    info!("Running database migrations...");
    // In a real implementation, you'd run migrations here
    println!("Database migrations completed successfully");
}

/// Helper function to display database statistics
...
/// Helper function to clean up old database events
async fn handle_database_cleanup(
    store: &dyn riglr_indexer::DataStore,
    days: u32,
    dry_run: bool,
) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    if dry_run {
        ...
        println!("Would delete {count} events (dry run)");
    } else {
        let deleted = store.delete_events(&filter).await?;
        println!("Deleted {deleted} events");
    }
    Ok(())
}

async fn handle_query_command(cmd: QueryCommands) -> Result<(), Box<dyn Error + Send + Sync>> {
    let config = IndexerConfig::from_env()?;
    let store = create_store(&config.storage).await?;
    ...
    match cmd {
        ...
        QueryCommands::Get { id, format } => handle_get_query(store.as_ref(), id, format).await,
        QueryCommands::Stats => handle_stats_query(store.as_ref()).await,
    }
}
async fn handle_health_command(endpoint: String) -> Result<(), Box<dyn Error + Send + Sync>> {
    let client = reqwest::Client::new();
    let health_url = format!("{endpoint}/api/v1/health");
...
fn handle_config_command(cmd: &ConfigCommands) -> Result<(), Box<dyn Error + Send + Sync>> {
    match *cmd {
...
async fn handle_metrics_command(cmd: MetricsCommands) -> Result<(), Box<dyn Error + Send + Sync>> {
    let config = IndexerConfig::from_env()?;
...
async fn handle_events_query(
    ...
    match format.as_str() {
        ...
        _ => return Err("Invalid format. Use 'json' or 'table'".into()),
    }
...
async fn handle_get_query(
    ...
    match event {
        Some(event) => match format.as_str() {
            ...
            _ => return Err("Invalid format. Use 'json'".into()),
        },
        None => {
...
async fn handle_stats_query(
    ...
    let total_events = store.count_events(&EventFilter::default()).await?;

    let events_by_type = EventAggregator::aggregate_by_type(store, &EventFilter::default()).await?;

    let events_by_source =
        EventAggregator::aggregate_by_source(store, &EventFilter::default()).await?;
...
mod tests {
    ...
    fn test_cli_parse_database_init_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for database init should succeed");
    ...
    fn test_cli_parse_database_migrate_should_succeed() {
        ...
        let cli =
            Cli::try_parse_from(args).expect("CLI parsing for database migrate should succeed");
    ...
    fn test_cli_parse_database_stats_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for database stats should succeed");
    ...
    fn test_cli_parse_database_cleanup_with_defaults_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for database cleanup with defaults should succeed");
    ...
    fn test_cli_parse_database_cleanup_with_custom_values_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for database cleanup with custom values should succeed");
    ...
    fn test_cli_parse_query_events_with_defaults_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for query events with defaults should succeed");
    ...
    fn test_cli_parse_query_events_with_all_options_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for query events with all options should succeed");
        ...
        match cli.command {
            Commands::Query(QueryCommands::Events {
                ...
                assert_eq!(
                    event_types.expect("event_types should be Some"),
                    "swap,transfer"
                );
                assert_eq!(sources.expect("sources should be Some"), "ethereum,polygon");
                assert_eq!(
                    start_time.expect("start_time should be Some"),
                    "2023-01-01T00:00:00Z"
                );
                assert_eq!(
                    end_time.expect("end_time should be Some"),
                    "2023-12-31T23:59:59Z"
                );
                assert_eq!(limit, 500);
    ...
    fn test_cli_parse_query_get_with_defaults_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for query get with defaults should succeed");
    ...
    fn test_cli_parse_query_get_with_custom_format_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for query get with custom format should succeed");
    ...
    fn test_cli_parse_query_stats_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for query stats should succeed");
    ...
    fn test_cli_parse_health_with_default_endpoint_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for health with default endpoint should succeed");
    ...
    fn test_cli_parse_health_with_custom_endpoint_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for health with custom endpoint should succeed");
    ...
    fn test_cli_parse_config_validate_should_succeed() {
        ...
        let cli =
            Cli::try_parse_from(args).expect("CLI parsing for config validate should succeed");
    ...
    fn test_cli_parse_config_show_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for config show should succeed");
    ...
    fn test_cli_parse_config_example_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for config example should succeed");
    ...
    fn test_cli_parse_metrics_show_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for metrics show should succeed");
    ...
    fn test_cli_parse_metrics_export_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing for metrics export should succeed");
    ...
    fn test_cli_parse_with_verbose_flag_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing with verbose flag should succeed");
    ...
    fn test_cli_parse_with_short_verbose_flag_should_succeed() {
        ...
        let cli =
            Cli::try_parse_from(args).expect("CLI parsing with short verbose flag should succeed");
    ...
    fn test_cli_parse_with_config_flag_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing with config flag should succeed");

        assert_eq!(
            cli.config.expect("config should be Some"),
            "/path/to/config.toml"
        );
        match cli.command {
            Commands::Health { .. } => {}
    ...
    fn test_cli_parse_with_short_config_flag_should_succeed() {
        ...
        let cli =
            Cli::try_parse_from(args).expect("CLI parsing with short config flag should succeed");
    ...
    fn test_cli_parse_with_both_flags_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args).expect("CLI parsing with both flags should succeed");
    ...
    fn test_cli_parse_database_cleanup_with_zero_days_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for database cleanup with zero days should succeed");
    ...
    fn test_cli_parse_database_cleanup_with_large_days_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for database cleanup with large days should succeed");

        match cli.command {
            Commands::Database(DatabaseCommands::Cleanup { days, .. }) => {
                assert_eq!(days, 999_999);
            }
            _ => panic!("Expected Database Cleanup command"),
        }
    }
    ...
    fn test_cli_parse_query_events_with_zero_limit_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for query events with zero limit should succeed");
    ...
    fn test_cli_parse_query_events_with_large_limit_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for query events with large limit should succeed");
    ...
    fn test_cli_parse_empty_strings_should_succeed() {
        let args = vec!["riglr-indexer-cli", "query", "get", ""];
        let cli = Cli::try_parse_from(args).expect("CLI parsing for empty strings should succeed");
    ...
    fn test_cli_parse_special_characters_in_id_should_succeed() {
        ...
        let cli = Cli::try_parse_from(args)
            .expect("CLI parsing for special characters in ID should succeed");
    ...
    fn test_cli_parse_unicode_characters_should_succeed() {
        ...
        let cli =
            Cli::try_parse_from(args).expect("CLI parsing for unicode characters should succeed");
