...
use crate::metrics::{
    ErrorMetrics, IndexerMetrics, LatencyMetrics, MetricsRegistry, PerformanceMetrics,
...
impl MetricsCollector {
    ...
    /// # Errors
    ///
    /// Returns error if metrics registry initialization fails
    ...
    pub fn record_counter(&self, name: &str, value: u64) {
        ...
        if let Err(e) = self.registry.set_gauge(name, value_f64) {
    ...
    /// # Errors
    ///
    /// Returns error if metrics serialization fails
    ...
    pub async fn update_indexer_metrics(&self, metrics: IndexerMetrics) {
        // Store values for debug logging before moving
        let events_total = metrics.events_processed_total;
        let events_rate = metrics.events_processing_rate;
        ...
        // Record metrics with appropriate precision loss handling
        #[expect(clippy::cast_precision_loss)]
        {
            self.record_gauge(
    ...
    /// # Errors
    ///
    /// Returns error if metrics flushing operation fails
    ...
    pub async fn get_performance_metrics(&self) -> PerformanceMetrics {
        ...
        PerformanceMetrics {
            ...
            errors: ErrorMetrics {
                total_errors: {
                    let errors_f64 = current.events_processed_total as f64 * current.error_rate;

                    errors_f64 as u64
                },
                error_rate: current.error_rate,
                errors_by_category: HashMap::new(), // Would need tracking
    ...
    #[must_use]
    pub fn start_collection_task(&self) -> JoinHandle<()> {
        let collector = self.clone();

        tokio::spawn(async move {
            let mut interval = interval(collector.config.collection_interval);
...
mod tests {
    ...
    async fn test_new_when_valid_config_should_create_collector() {
        ...
        assert!(result.is_ok());
        let collector = result.expect("Should create metrics collector with valid config");
    ...
    async fn test_increment_counter_when_valid_name_should_not_panic() {
        ...
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for counter test");
    ...
    async fn test_increment_counter_when_empty_name_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for empty name test");
    ...
    async fn test_record_counter_when_valid_values_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for counter values test");
    ...
    async fn test_record_counter_when_empty_name_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for empty counter name test");
    ...
    async fn test_record_gauge_when_valid_values_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for gauge values test");
    ...
    async fn test_record_gauge_when_nan_should_not_panic() {
        let config = create_test_config();
        let collector =
            MetricsCollector::new(config).expect("Should create metrics collector for NaN test");
    ...
    async fn test_record_gauge_when_empty_name_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for empty gauge name test");

        collector.record_gauge("", f64::consts::PI);
    }
    ...
    async fn test_record_histogram_when_valid_values_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for histogram values test");
    ...
    async fn test_record_histogram_when_empty_name_should_not_panic() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for empty histogram name test");
    ...
    async fn test_update_indexer_metrics_when_valid_metrics_should_update_successfully() {
        let config = create_test_config();
        let collector =
            MetricsCollector::new(config).expect("Should create metrics collector for update test");
    ...
    async fn test_update_indexer_metrics_when_default_metrics_should_update_successfully() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for default metrics test");
    ...
    async fn test_update_indexer_metrics_when_extreme_values_should_handle_gracefully() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for extreme values test");
    ...
    async fn test_get_indexer_metrics_when_no_metrics_set_should_return_default() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for default get test");
    ...
    async fn test_get_performance_metrics_when_default_metrics_should_return_calculated_values() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for performance metrics test");
    ...
    async fn test_get_performance_metrics_when_valid_metrics_should_calculate_correctly() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for performance calculation test");
        let test_metrics = create_test_indexer_metrics();
        ...
        let events_as_f64 = test_metrics.events_processed_total as f64;
        let expected_total_errors = (events_as_f64 * test_metrics.error_rate) as u64;
    ...
    async fn test_start_collection_task_when_called_should_return_join_handle() {
        ...
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for collection task test");
    ...
    async fn test_start_collection_task_when_long_interval_should_handle_cancellation() {
        ...
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for long interval test");
    ...
    async fn test_export_prometheus_when_called_should_return_string() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for prometheus export test");
    ...
    async fn test_export_json_when_called_should_return_valid_json() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for JSON export test");

        let json_result = collector.export_json();

        assert!(json_result.is_ok());
        let json_value = json_result.expect("Should export metrics as valid JSON");
    ...
    async fn test_flush_when_called_should_complete_successfully() {
        let config = create_test_config();
        let collector =
            MetricsCollector::new(config).expect("Should create metrics collector for flush test");
    ...
    async fn test_reset_when_called_should_clear_and_reinitialize() {
        let config = create_test_config();
        let collector =
            MetricsCollector::new(config).expect("Should create metrics collector for reset test");
    ...
    async fn test_clone_when_called_should_create_identical_collector() {
        let config = create_test_config();
        let original = MetricsCollector::new(config)
            .expect("Should create original metrics collector for clone test");
        let test_metrics = create_test_indexer_metrics();
    ...
    async fn test_init_standard_metrics_when_called_should_set_all_gauges() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for standard metrics test");
    ...
    async fn test_update_indexer_metrics_when_called_should_update_uptime() {
        let config = create_test_config();
        let collector =
            MetricsCollector::new(config).expect("Should create metrics collector for uptime test");
    ...
    async fn test_multiple_concurrent_updates_should_handle_correctly() {
        ...
        let collector = Arc::new(
            MetricsCollector::new(config)
                .expect("Should create metrics collector for concurrent test"),
        );
        ...
        for handle in handles {
            handle
                .await
                .expect("Should complete concurrent update task");
        }
    ...
    async fn test_performance_metrics_edge_cases() {
        let config = create_test_config();
        let collector = MetricsCollector::new(config)
            .expect("Should create metrics collector for edge cases test");
