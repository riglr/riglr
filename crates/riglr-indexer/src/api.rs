...
pub use rest::Handler as RestHandler;
pub use websocket::WebSocketStreamer;

/// HTTP and WebSocket server for the indexer service
#[derive(Debug)]
pub struct Server {
    /// Configuration
    config: ApiConfig,
...
impl Server {
    ...
    pub fn create_router(&self) -> Router<Arc<ServiceContext>> {
        ...
        if let Some(ref ws_streamer) = self.websocket_streamer {
    ...
    /// # Errors
    ///
    /// Returns error if REST handler or WebSocket streamer initialization fails
    ...
    /// # Errors
    ///
    /// Returns error if binding to the configured address fails or server startup fails
    pub async fn start(&self) -> IndexerResult<()> {
        ...
        let listener = bind_result.map_err(|_e| {
            IndexerError::Network(NetworkError::HttpFailed {
                status: 0,
                url: addr.clone(),
            })
        })?;
...
mod tests {
    ...
    fn create_test_context() -> Arc<ServiceContext> {
        ...
        let metrics = Arc::new(
            MetricsCollector::new(config.metrics.clone()).unwrap(), // Test helper - safe to unwrap
        );
    ...
    fn test_api_server_new_when_valid_context_should_create_server() {
        ...
        let result = Server::new(context);

        assert!(result.is_ok());
        let server = result.unwrap(); // Test assertion - safe to unwrap
    ...
    // Tests for Server::start() - these test the binding logic, not the actual server
    #[tokio::test]
    async fn test_api_server_start_when_invalid_bind_address_should_return_network_error() {
        ...
        let error = result.unwrap_err(); // Test - safe to unwrap error
        match error {
            ...
            other => {
                panic!("Expected NetworkError::HttpFailed, got: {other:?}");
            }
        }
    }
    ...
    async fn test_api_server_start_when_port_already_in_use_should_return_network_error() {
        ...
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap(); // Test helper - safe to unwrap
        let addr = listener.local_addr().unwrap(); // Test helper - safe to unwrap
        let port = addr.port();
    ...
    async fn test_api_server_start_when_valid_config_should_bind_successfully() {
        ...
        if server.config.http.port == 0 {
            ...
            assert!(!ptr::eq(&raw const server, ptr::null()));
        }
    }
    ...
    fn test_api_server_new_when_empty_bind_address_should_work() {
        let mut config = create_test_config();
        config.api.http.bind = String::new();
    ...
    fn test_api_server_new_when_extreme_config_values_should_work() {
        ...
        config.api.websocket.buffer_size = 1_000_000;
    ...
    fn test_api_server_create_router_when_called_multiple_times_should_work() {
        ...
        assert!(!ptr::eq(&raw const router1, ptr::null()));
        assert!(!ptr::eq(&raw const router2, ptr::null()));
    }
