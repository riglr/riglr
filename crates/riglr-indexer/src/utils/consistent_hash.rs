...
impl ConsistentHash {
    ...
    pub fn add_node(&mut self, node_id: &str, weight: u32, metadata: HashMap<String, String>) {
        ...
        self.nodes.insert(node_id.to_string(), node_info);

        // Add virtual nodes based on weight
        // Use saturating operations to prevent overflow and ensure positive results
        let weight_ratio = f64::from(weight) / 100.0;
        #[expect(
            clippy::cast_precision_loss,
            clippy::cast_possible_truncation,
            clippy::cast_sign_loss
        )]
        let virtual_node_count = ((self.virtual_nodes as f64 * weight_ratio) as usize).max(1);
...
impl ConsistentHashBuilder {
    ...
    #[must_use]
    pub const fn virtual_nodes(mut self, count: usize) -> Self {
...
mod tests {
    ...
    fn test_consistent_hash_basic() {
        ...
        // Add some nodes
        ring.add_node("node1", 100, HashMap::default());
        ring.add_node("node2", 100, HashMap::default());
        ring.add_node("node3", 100, HashMap::default());
    ...
    fn test_consistent_hash_node_removal() {
        ...
        let key = "test_key";
        let original_node = ring
            .get_node(key)
            .expect("Should find node for test key")
            .id
            .clone();
        ...
        let new_node = ring
            .get_node(key)
            .expect("Should find node after removal")
            .id
            .clone();
    ...
    fn test_consistent_hash_replication() {
        ...
        assert_ne!(
            nodes.first().expect("Should have first node").id,
            nodes.get(1).expect("Should have second node").id
    ...
    fn test_load_distribution() {
        ...
        let expected = 500_usize;
        let tolerance = 150_usize;
        let node1_diff = (*node1_load).abs_diff(expected);
        let node2_diff = (*node2_load).abs_diff(expected);
        assert!(node1_diff < tolerance);
        assert!(node2_diff < tolerance);
    }
    ...
    fn test_add_node_with_different_weights() {
        ...
        ring.add_node("high_weight", 200, metadata.clone());
        ring.add_node("low_weight", 50, HashMap::new());
        ring.add_node("zero_weight", 0, HashMap::new());
        ...
        let node_info = ring
            .nodes
            .get("high_weight")
            .expect("high_weight node should exist");
        assert_eq!(node_info.weight, 200);
        assert_eq!(
            node_info
                .metadata
                .get("region")
                .expect("region metadata should exist"),
            "us-east"
        );
    }
    ...
    fn test_remove_node_nonexistent() {
        let mut ring = ConsistentHash::new(100);
        ring.add_node("node1", 100, HashMap::new());
    ...
    fn test_remove_node_success() {
        let mut ring = ConsistentHash::new(100);
        ring.add_node("node1", 100, HashMap::new());
        ring.add_node("node2", 100, HashMap::new());
    ...
    fn test_get_node_single_node() {
        let mut ring = ConsistentHash::new(100);
        ring.add_node("only_node", 100, HashMap::new());
    ...
    fn test_get_nodes_single_node() {
        ...
        assert_eq!(
            nodes.first().expect("Should have only node").id,
            "only_node"
        );
    }
    ...
    fn test_get_nodes_multiple_nodes() {
        ...
        ring.add_node("node1", 100, HashMap::new());
        ring.add_node("node2", 100, HashMap::new());
        ring.add_node("node3", 100, HashMap::new());
    ...
    fn test_get_all_nodes() {
        ...
        let node_ids: HashSet<_> = all_nodes.iter().map(|n| &n.id).collect();
    ...
    fn test_get_load_distribution_no_keys() {
        ...
        assert_eq!(
            *distribution
                .get("node1")
                .expect("node1 should be in distribution"),
            0
        );
        assert_eq!(
            *distribution
                .get("node2")
                .expect("node2 should be in distribution"),
            0
        );
    }
    ...
    fn test_hash_key_consistency() {
        ...
        let hash1 = ConsistentHash::hash_key(key);
        let hash2 = ConsistentHash::hash_key(key);
    ...
    fn test_hash_key_different_keys() {
        let hash1 = ConsistentHash::hash_key("key1");
        let hash2 = ConsistentHash::hash_key("key2");
    ...
    fn test_node_info_creation() {
        ...
        assert_eq!(
            node_info
                .metadata
                .get("datacenter")
                .expect("datacenter metadata should exist"),
            "dc1"
        );
        assert_eq!(
            node_info
                .metadata
                .get("rack")
                .expect("rack metadata should exist"),
            "rack42"
        );
    }
    ...
    fn test_consistent_hash_builder_add_node() {
        ...
        assert_eq!(
            builder.nodes.first().expect("Should have first node").0,
            "node1"
        );
        assert_eq!(
            builder.nodes.first().expect("Should have first node").1,
            120
        );
        assert_eq!(
            builder
                .nodes
                .first()
                .expect("Should have first node")
                .2
                .get("type")
                .expect("type metadata should exist"),
            "ssd"
        );
    }
    ...
    fn test_consistent_hash_builder_add_simple_node() {
        ...
        assert_eq!(builder.nodes.len(), 1);
        assert_eq!(
            builder.nodes.first().expect("Should have simple node").0,
            "simple_node"
        );
        assert_eq!(
            builder.nodes.first().expect("Should have simple node").1,
            100
        ); // Default weight
        assert!(builder
            .nodes
            .first()
            .expect("Should have simple node")
            .2
            .is_empty()); // Default empty metadata
    }
    ...
    fn test_consistent_hash_builder_build() {
        ...
        let node2 = ring.nodes.get("node2").expect("node2 should exist in ring");
    ...
    fn test_edge_case_empty_string_key() {
        ...
        assert!(node.is_some());
        assert_eq!(
            node.expect("Should find node for empty string key").id,
            "node1"
        );
    }
    ...
    fn test_edge_case_very_long_key() {
        ...
        assert!(node.is_some());
        assert_eq!(
            node.expect("Should find node for very long key").id,
            "node1"
        );
    }
    ...
    fn test_edge_case_unicode_key() {
        ...
        assert!(node.is_some());
        assert_eq!(node.expect("Should find node for unicode key").id, "node1");
    }
}
