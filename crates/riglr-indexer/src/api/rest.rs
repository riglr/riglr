...
use crate::{
    core::ServiceContext,
    error::IndexerResult,
    storage::{EventAggregator, EventFilter, EventQuery, StorageStats, StoredEvent},
};
...
impl Handler {
    ...
    /// with the error handling patterns and potential future validation.
    pub const fn new(context: Arc<ServiceContext>) -> IndexerResult<Self> {
        Ok(Self { context })
    }
...
fn build_event_filter(params: &EventsQueryParams) -> EventFilter {
    ...
    if let Some(ref tx_hash) = params.tx_hash {
        filter.transaction_hash = Some(tx_hash.clone());
    }

    filter
}

/// Build sort configuration from query parameters
fn build_sort_config(params: &EventsQueryParams) -> Option<(String, bool)> {
...
/// # Errors
///
/// Returns a JSON response with error information if:
/// - The storage layer fails to query events
...
pub async fn get_events(
    ...
    let filter = build_event_filter(&params);
    let sort = build_sort_config(&params);
...
///
/// Returns a JSON response with error information if:
/// - The storage layer fails to retrieve the event
...
async fn get_total_event_count(context: &ServiceContext) -> Result<u64, String> {
    context
        .store
...
async fn get_events_by_type(context: &ServiceContext) -> HashMap<String, u64> {
    match EventAggregator::aggregate_by_type(context.store.as_ref(), &EventFilter::default()).await
    {
        Ok(stats) => stats,
        Err(e) => {
...
/// Get events aggregated by source
async fn get_events_by_source(context: &ServiceContext) -> HashMap<String, u64> {
    match EventAggregator::aggregate_by_source(context.store.as_ref(), &EventFilter::default())
        .await
    {
        Ok(stats) => stats,
        Err(e) => {
...
async fn get_storage_statistics(context: &ServiceContext) -> Result<StorageStats, String> {
    context.store.get_stats().await.map_err(|e| {
...
///
/// Returns a JSON response with error information if:
/// - The storage layer fails to count events or retrieve statistics
...
pub async fn get_event_stats(
    ...
    let total_events = match get_total_event_count(&context).await {
        Ok(count) => count,
        Err(e) => return Ok(Json(ApiResponse::<EventStatsResponse>::error(e))),
    };

    let events_by_type = get_events_by_type(&context).await;
    let events_by_source = get_events_by_source(&context).await;

    let storage_stats = match get_storage_statistics(&context).await {
        Ok(stats) => stats,
        Err(e) => return Ok(Json(ApiResponse::<EventStatsResponse>::error(e))),
    };
...
/// - Component health status cannot be retrieved
...
///
/// Returns a JSON response with error information if:
/// - The metrics export operation fails
/// - Metrics collection is unavailable
...
mod tests {
    ...
    use crate::error::{IndexerError, StorageError};
    ...
    fn create_test_context() -> Arc<ServiceContext> {
        ...
        let metrics = Arc::new(MetricsCollector::new(config.metrics.clone()).unwrap()); // Test helper
    ...
    fn test_rest_handler_new_when_valid_context_should_return_ok() {
        ...
        let result = Handler::new(context);
    ...
    fn test_rest_handler_create_router_when_called_should_return_router() {
        let context = create_test_context();
        let handler = Handler::new(context).unwrap(); // Test helper
    ...
    async fn test_get_events_when_no_params_should_return_default_events() {
        ...
        let result = get_events(State(context), Query(params)).await.unwrap(); // Test assertion
        let response = result.0;

        assert!(response.success);
        assert!(response.data.is_some());
        assert_eq!(response.data.unwrap().len(), 2); // Test assertion
        assert!(response.error.is_none());
    }
    ...
    async fn test_get_event_when_event_exists_should_return_event() {
        ...
        let result = get_event(State(context), Path("test-id".to_string()))
            .await
            .unwrap(); // Test assertion
        let response = result.0;

        assert!(response.success);
        assert!(response.data.is_some());
        assert_eq!(response.data.unwrap().unwrap().id, "test-id"); // Test assertion
        assert!(response.error.is_none());
    }
    ...
    async fn test_get_event_when_event_not_found_should_return_none() {
        ...
        assert!(response.success);
        assert!(response.data.is_some());
        assert!(response.data.unwrap().is_none()); // Test assertion
        assert!(response.error.is_none());
    }
    ...
    async fn test_get_event_stats_when_all_succeed_should_return_stats() {
        ...
        let result = get_event_stats(State(context)).await.unwrap(); // Test assertion
        let response = result.0;

        assert!(response.success);
        assert!(response.data.is_some());

        let stats = response.data.unwrap(); // Test assertion
    ...
    async fn test_get_health_when_healthy_should_return_healthy_status() {
        ...
        let result = get_health(State(context.clone())).await.unwrap(); // Test assertion
        let response = result.0;

        assert!(response.success);
        assert!(response.data.is_some());

        let health = response.data.unwrap(); // Test assertion
    ...
    async fn test_get_health_when_health_check_fails_should_return_error() {
        ...
        let result = get_health(State(context)).await.unwrap(); // Test assertion
        let response = result.0;
    ...
    async fn test_get_metrics_when_export_succeeds_should_return_metrics() {
        ...
        let result = get_metrics(State(context)).await.unwrap(); // Test assertion
        let response = result.0;
    ...
    async fn test_get_metrics_when_export_fails_should_return_error() {
        ...
        let _result = get_metrics(State(context)).await.unwrap(); // Test assertion

        // With the real MetricsCollector, this should succeed
        // The test verifies the function handles both success and error cases
    }
