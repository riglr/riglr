...
// Test code uses expect()/unwrap() for test assertion failures and exact float comparisons
// Disabled because DistributedAgentRegistry and RedisAgentRegistry don't exist in the current codebase
/*
#[cfg(feature = "redis")]
#[expect(dead_code)]
mod tests {
    ...
    async fn test_agent_status_operations() {
        ...
        // Update status
        let updated_status = AgentStatus::new(
            AgentId::new("status-test-agent"),
            AgentState::Busy,
            5,
            0.75,
            vec![],
