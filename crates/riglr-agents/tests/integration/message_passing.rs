...
async fn test_inter_agent_messaging() {
    ...
    let sender_id = AgentId::new("agent-a");
    let receiver_id = AgentId::new("agent-b");

    // Subscribe agents to communication
    let mut receiver_a = comm_system
        .subscribe(&sender_id)
        .await
        .expect("Failed to subscribe agent_a in test_inter_agent_messaging");
    let mut receiver_b = comm_system
        .subscribe(&receiver_id)
        .await
        .expect("Failed to subscribe agent_b in test_inter_agent_messaging");
    ...
    comm_system
        ...
        .await
        .expect("Failed to send message in test_inter_agent_messaging");

    // Verify B received the message
    let received = receiver_b.receive().await;
    assert!(received.is_some());
    let received_msg = received.expect("Expected to receive message in test_inter_agent_messaging");
    assert_eq!(received_msg.id, message.id);
    assert_eq!(received_msg.message_type, "task_update");
    assert_eq!(received_msg.from, sender_id);
...
async fn test_broadcast_messaging() {
    ...
    for agent_id in &agent_ids {
        let receiver = comm_system
            .subscribe(agent_id)
            .await
            .expect("Failed to subscribe agent in test_broadcast_messaging");
    ...
    comm_system
        ...
        .await
        .expect("Failed to broadcast message in test_broadcast_messaging");
    ...
    for mut receiver in receivers {
        ...
        assert!(received.is_some());
        let received_msg =
            received.expect("Expected to receive broadcast message in test_broadcast_messaging");
...
async fn test_agent_coordination_through_messaging() {
    ...
    let first_worker_id = AgentId::new("worker-a");
    let second_worker_id = AgentId::new("worker-b");

    let mut coordinator_receiver = comm_system
        .subscribe(&coordinator_id)
        .await
        .expect("Failed to subscribe coordinator in test_agent_coordination_through_messaging");
    let mut first_worker_receiver = comm_system
        .subscribe(&first_worker_id)
        .await
        .expect("Failed to subscribe worker_a in test_agent_coordination_through_messaging");
    let mut second_worker_receiver = comm_system
        .subscribe(&second_worker_id)
        .await
        .expect("Failed to subscribe worker_b in test_agent_coordination_through_messaging");
    ...
    comm_system
        .send_message(work_assignment_a)
        .await
        .expect("Failed to send work_assignment_a in test_agent_coordination_through_messaging");
    comm_system
        .send_message(work_assignment_b)
        .await
        .expect("Failed to send work_assignment_b in test_agent_coordination_through_messaging");

    // Workers receive assignments
    let assignment_a = first_worker_receiver.receive().await.expect(
        "Expected worker_a to receive assignment in test_agent_coordination_through_messaging",
    );
    let assignment_b = second_worker_receiver.receive().await.expect(
        "Expected worker_b to receive assignment in test_agent_coordination_through_messaging",
    );
    ...
    comm_system
        .send_message(completion_a)
        .await
        .expect("Failed to send completion_a in test_agent_coordination_through_messaging");
    comm_system
        .send_message(completion_b)
        .await
        .expect("Failed to send completion_b in test_agent_coordination_through_messaging");

    // Coordinator receives completion notifications
    let result_a = coordinator_receiver.receive().await.expect(
        "Expected coordinator to receive result_a in test_agent_coordination_through_messaging",
    );
    let result_b = coordinator_receiver.receive().await.expect(
        "Expected coordinator to receive result_b in test_agent_coordination_through_messaging",
    );
...
async fn test_message_priority_handling() {
    ...
    let mut receiver = comm_system
        .subscribe(&agent_id)
        .await
        .expect("Failed to subscribe agent in test_message_priority_handling");
    ...
    comm_system
        ...
        .await
        .expect("Failed to send low_priority_msg in test_message_priority_handling");
    comm_system
        .send_message(critical_msg.clone())
        .await
        .expect("Failed to send critical_msg in test_message_priority_handling");
    comm_system
        .send_message(normal_msg.clone())
        .await
        .expect("Failed to send normal_msg in test_message_priority_handling");

    // Receive messages (order depends on channel implementation)
    let msg1 = receiver
        .receive()
        .await
        .expect("Expected to receive msg1 in test_message_priority_handling");
    let msg2 = receiver
        .receive()
        .await
        .expect("Expected to receive msg2 in test_message_priority_handling");
    let msg3 = receiver
        .receive()
        .await
        .expect("Expected to receive msg3 in test_message_priority_handling");

    // Verify all messages were received
    let received_ids = [msg1.id, msg2.id, msg3.id];
...
async fn test_message_expiration() {
    ...
    let _receiver = comm_system
        .subscribe(&agent_id)
        .await
        .expect("Failed to subscribe agent in test_message_expiration");
...
async fn test_communication_stats_tracking() {
    ...
    let first_agent_id = AgentId::new("agent-a");
    let second_agent_id = AgentId::new("agent-b");

    let mut _receiver_a = comm_system
        .subscribe(&first_agent_id)
        .await
        .expect("Failed to subscribe agent_a in test_communication_stats_tracking");
    let mut _receiver_b = comm_system
        .subscribe(&second_agent_id)
        .await
        .expect("Failed to subscribe agent_b in test_communication_stats_tracking");
    ...
    comm_system
        ...
        .await
        .expect("Failed to send message1 in test_communication_stats_tracking");
    comm_system
        .send_message(message2)
        .await
        .expect("Failed to send message2 in test_communication_stats_tracking");
...
async fn test_large_scale_message_passing() {
    ...
    for i in 0..num_agents {
        ...
        let receiver = comm_system
            .subscribe(&agent_id)
            .await
            .expect("Failed to subscribe agent in test_large_scale_message_passing");
        ...
        for receiver_idx in 0..num_agents {
            if sender_idx != receiver_idx {
                ...
                comm_system
                    ...
                    .await
                    .expect("Failed to send message in test_large_scale_message_passing");
                total_messages += 1;
            }
        }
    }
...
async fn test_message_delivery_reliability() {
    ...
    let mut receiver = comm_system
        .subscribe(&receiver_id)
        .await
        .expect("Failed to subscribe receiver in test_message_delivery_reliability");
    ...
    for i in 0..num_messages {
        ...
        comm_system
            .send_message(message)
            .await
            .expect("Failed to send message in test_message_delivery_reliability");
    }
...
async fn test_concurrent_message_passing() {
    ...
    for i in 0..num_senders {
        ...
        let receiver = comm_system
            .subscribe(&receiver_id)
            .await
            .expect("Failed to subscribe receiver in test_concurrent_message_passing");
        receivers.push(receiver);
    }
                ...
                for receiver_idx in 0..num_senders {
                    if sender_idx != receiver_idx {
                        ...
                        comm_system_clone
                            .send_message(message)
                            .await
                            .expect("Failed to send message in test_concurrent_message_passing");
                    }
                }
            }
        });
    ...
    for handle in sender_handles {
        handle
            .await
            .expect("Failed to await sender handle in test_concurrent_message_passing");
    }
...
async fn test_agent_message_handling_workflow() {
    ...
    struct MessageHandlingAgent {
        ...
        received_messages: Arc<Mutex<Vec<AgentMessage>>>,
    }
    ...
    registry
        .register_agent(message_agent)
        .await
        .expect("Failed to register agent in test_agent_message_handling_workflow");

    let mut receiver = comm_system
        .subscribe(agent_clone.id())
        .await
        .expect("Failed to subscribe agent in test_agent_message_handling_workflow");
    ...
    comm_system
        ...
        .await
        .expect("Failed to send test_message in test_agent_message_handling_workflow");

    // Agent receives and processes the message
    let received_msg = receiver
        .receive()
        .await
        .expect("Expected to receive message in test_agent_message_handling_workflow");
    agent_clone
        .handle_message(received_msg)
        .await
        .expect("Failed to handle message in test_agent_message_handling_workflow");
    ...
    assert_eq!(processed_messages.first().unwrap().id, test_message.id);
    assert_eq!(
        processed_messages.first().unwrap().message_type,
        "test_workflow"
    );
}
