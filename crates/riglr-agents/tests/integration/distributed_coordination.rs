...
//
// DISABLED: This test file references types (RedisAgentRegistry, DistributedAgentRegistry)
// that don't exist in the current codebase. These tests need to be updated or removed.

/*
#[cfg(feature = "e2e-tests")]
use riglr_agents::{
    dispatcher::{Dispatcher, DispatchConfig, RoutingStrategy},
    registry::{Registry, Distributed},
    types::{
...
async fn test_distributed_agent_discovery() -> Result<()> {
    ...
    let status1 = AgentStatus::new(
        agent1.id().clone(),
        AgentState::Active,
        0,
        0.0,
        vec![Capability::new("trading", "1.0")],
    );
...
async fn test_distributed_task_dispatch() -> Result<()> {
    ...
    // Update its status
    let local_status = AgentStatus::new(
        local_agent.id().clone(),
        AgentState::Active,
        0,
        0.1,
        vec![Capability::new("research", "1.0")],
    );
    registry1.update_agent_status(local_status).await?;

    // Simulate a remote agent by just adding its status (no local instance)
    let remote_status = AgentStatus::new(
        AgentId::new("remote-agent"),
        AgentState::Active,
        1,
        0.3,
        vec![Capability::new("research", "1.0")],
