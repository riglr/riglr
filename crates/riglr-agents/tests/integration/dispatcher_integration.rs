...
async fn test_dispatcher_registry_router_integration() {
    ...
    registry
        ...
        .await
        .expect("Failed to register risk agent");
...
async fn test_capability_routing(registry: &Arc<LocalAgentRegistry>) {
    ...
    let data = extract_task_result_data(&result).expect("Failed to extract task result data");
...
async fn test_round_robin_routing(registry: &Arc<LocalAgentRegistry>) {
    ...
    for task in tasks {
        ...
        let agent_id = data["agent_id"]
            .as_str()
            .expect("Failed to get agent_id from result data");
    ...
    assert_eq!(agent_usage.get("trader-1").copied().unwrap_or(0), 6);
}
...
async fn test_dispatcher_with_different_task_types() {
    ...
    let trading_result = dispatcher
        ...
        .await
        .expect("Failed to dispatch trading task");
    let research_result = dispatcher
        .dispatch_task(research_task)
        .await
        .expect("Failed to dispatch research task");
    let risk_result = dispatcher
        .dispatch_task(risk_task)
        .await
        .expect("Failed to dispatch risk task");
    ...
    let trading_data =
        extract_task_result_data(&trading_result).expect("Failed to extract trading result data");
    let research_data =
        extract_task_result_data(&research_result).expect("Failed to extract research result data");
    let risk_data =
        extract_task_result_data(&risk_result).expect("Failed to extract risk result data");
...
async fn test_dispatcher_concurrent_execution() {
    ...
    for result in results {
        assert!(result.is_ok());
        assert!(result.expect("Failed to get task result").is_success());
    }
}
...
async fn test_dispatcher_stats_and_health() {
    ...
    let health = dispatcher
        .health_check()
        .await
        .expect("Failed to check dispatcher health");
    assert!(health);
    ...
    let load = stats.average_agent_load;
    assert!(
        load.abs() < f64::EPSILON,
        "Expected average_agent_load to be 0.0, got {load}"
    );
}
...
async fn test_dispatcher_routing_strategy_changes() {
    ...
    let capability_dispatcher = Dispatcher::with_config(registry.clone(), &capability_config);

    let task1 = create_trading_task();
    let result1 = capability_dispatcher
        .dispatch_task(task1)
        .await
        .expect("Failed to dispatch capability task");
    ...
    let round_robin_dispatcher = Dispatcher::with_config(registry.clone(), &round_robin_config);

    let task2 = create_trading_task();
    let result2 = round_robin_dispatcher
        .dispatch_task(task2)
        .await
        .expect("Failed to dispatch round robin task");
    ...
    let random_dispatcher = Dispatcher::with_config(registry.clone(), &random_config);

    let task3 = create_trading_task();
    let result3 = random_dispatcher
        .dispatch_task(task3)
        .await
        .expect("Failed to dispatch random task");
...
async fn test_dispatcher_custom_task_type() {
    ...
    registry
        ...
        .await
        .expect("Failed to register custom agent");
    ...
    let result = dispatcher
        .dispatch_task(custom_task)
        .await
        .expect("Failed to dispatch custom task");
