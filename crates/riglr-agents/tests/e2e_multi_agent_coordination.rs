...
async fn test_2_3_parallel_agent_execution() -> Result<()> {
    ...
    for handle in handles {
        ...
        match handle_result {
            Ok((name, task_result, duration)) => match task_result {
                Ok(response) => {
                    println!("{name} completed in {duration:?}: {response:?}");
    ...
    for (name, response) in &results {
        assert!(response.is_success(), "{name} should succeed");

        let data = response.data().map(ToString::to_string).unwrap_or_default();
        match name.as_str() {
            "technical_analyst" => {
                assert!(
