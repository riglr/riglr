...
impl MockCompletionModel {
    ...
    /// # Errors
    ///
    /// Returns an error if the mock is configured to simulate failures.
...
pub struct MockOpenAIProvider {
    ...
    #[expect(dead_code)] // Kept for API compatibility
    api_key: String,
}
...
impl MockRigAgent {
    ...
    /// # Errors
    ///
    /// Returns an error if the underlying completion model fails.
    pub async fn prompt(&self, prompt: &str) -> Result<String, MockLLMError> {
        // Combine preamble with prompt if available
        let full_prompt = self.preamble.as_ref().map_or_else(
            || prompt.to_string(),
...
pub mod test_utils {
    ...
    ///
    /// # Errors
    /// Returns an error if the response string is not valid JSON.
...
mod tests {
    ...
    async fn test_mock_completion_model_basic() {
        ...
        let response = model
            .complete("transfer some SOL")
            .await
            .expect("Mock completion should succeed");
        let action = extract_action(&response).expect("Response should contain valid action field");
    ...
    async fn test_mock_completion_model_failure() {
        ...
        assert!(matches!(
            result.expect_err("Mock completion should fail when configured with failure"),
    ...
    async fn test_mock_rig_agent_with_preamble() {
        ...
        let response = agent
            ...
            .await
            .expect("Mock agent prompt should succeed");
        let action = extract_action(&response).expect("Response should contain valid action field");
        assert_eq!(action, "transfer_sol");
    }
    ...
    fn test_response_extraction() {
        ...
        assert_eq!(
            extract_action(&response).expect("Valid JSON should parse action field"),
            "test_action"
        );
        assert_eq!(
            extract_reasoning(&response).expect("Valid JSON should parse reasoning field"),
            "test reasoning"
        );
    }
    ...
    fn test_response_extraction_missing_fields() {
        ...
        assert_eq!(
            extract_action(&response).expect("Valid JSON should parse action field with fallback"),
            "unknown"
        );
        assert_eq!(
            extract_reasoning(&response)
                .expect("Valid JSON should parse reasoning field with fallback"),
