...
impl MockTradingAgent {
    ...
    #[must_use]
    pub const fn with_state(mut self, state: AgentState) -> Self {
        self.state = state;
        self
    }
...
impl Agent for MockTradingAgent {
    ...
    fn is_available(&self) -> bool {
        match self.state {
            ...
            AgentState::Full | AgentState::Offline | AgentState::Maintenance | _ => false,
        }
    }
}
...
pub struct MockResearchAgent {
    id: AgentId,
    capabilities: Vec<CapabilityType>,
    shared_state: Option<Arc<RwLock<SharedTradingState>>>,
}
...
impl MockResearchAgent {
    ...
    #[must_use]
    pub fn with_shared_state(mut self, state: Arc<RwLock<SharedTradingState>>) -> Self {
...
impl SharedTradingState {
    ...
    #[must_use]
    pub const fn is_workflow_complete(&self) -> bool {
...
#[must_use]
pub fn create_workflow_agents() -> (Vec<Arc<dyn Agent>>, Arc<RwLock<SharedTradingState>>) {
    let shared_state = Arc::new(RwLock::new(SharedTradingState::default()));
