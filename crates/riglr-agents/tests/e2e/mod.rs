...
mod tests {
    ...
    async fn test_e2e_placeholder() {
        ...
        assert_eq!(
            task.parameters.get("blockchain").unwrap(),
            &serde_json::Value::String("solana".to_string())
        );
    ...
    async fn test_blockchain_harness_availability() {
        ...
        let harness = harness.expect("Failed to create blockchain test harness for e2e test");
    ...
    async fn test_rig_mocks_availability() {
        ...
        let response = model
            .complete("transfer some tokens")
            .await
            .expect("Failed to get completion response from mock model");
        ...
        assert!(
            ...
            "Expected transfer_sol action in response: {response}"
        );
