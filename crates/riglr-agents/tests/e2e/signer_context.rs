...
use tokio::{sync::RwLock, task::block_in_place, time::sleep};
...
impl SecurityMonitor {
    ...
    pub fn log_access(&self, entry: AccessLogEntry) {
        ...
        self.access_log
            .lock()
            .expect("Failed to acquire access log lock")
            .push(entry);
    }
...
impl Debug for SecurityAwareAgent {
...
impl SecurityAwareAgent {
    ...
    async fn perform_authorized_operation(&self, operation: &str) -> Result<String> {
        ...
        let result = SignerContext::with_signer(signer, async move {
            ...
            if current_signer.supports_solana() {
                ...
                Err(Box::new(SignerErrorStandard::UnsupportedOperation(
    ...
    fn attempt_unauthorized_access(&self) -> bool {
...
impl Agent for SecurityAwareAgent {
    async fn execute_task(&self, task: Task) -> Result<TaskResult> {
        ...
        match task_type {
            ...
            "unauthorized_test" => {
                let unauthorized_succeeded = self.attempt_unauthorized_access();
            ...
            "concurrent_test" => {
                ...
                match self.test_concurrent_access(operation_id).await {
                    Ok(status) => Ok(TaskResult::success(
                        ...
                        Some(format!("Concurrent operation {operation_id} completed")),
                        start_time.elapsed(),
                    )),
                    Err(e) => Ok(TaskResult::failure(
...
async fn test_signer_context_isolation() {
    ...
    info!(
        ...
        u8::from(security_monitor.has_violations())
    );
...
async fn test_concurrent_signer_access() {
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        if task_result.is_success() {
            ...
            if let TaskResult::Success { ref data, .. } = task_result {
...
async fn test_unauthorized_access_prevention() {
    ...
    for &(scenario_type, description) in &unauthorized_scenarios {
        ...
        assert!(
            result.is_success(),
            "Unauthorized access test should succeed when access is blocked: {scenario_type}"
        );

        let output = match result {
            TaskResult::Success { ref data, .. } => {
                data.as_object().expect("Task output should be JSON object")
            }
            _ => {
                unreachable!("Expected successful task result")
            }
        };

        assert_eq!(
            output["status"], "unauthorized_access_blocked",
            "Status should indicate blocked access for: {scenario_type}"
        );
