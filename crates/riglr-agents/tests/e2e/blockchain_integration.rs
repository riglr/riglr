...
use riglr_core::{
    ...
    signer::{
        error::{Error as SignerError, Standard as StandardSignerError},
        SignerContext, UnifiedSigner,
    },
    Config,
};
use riglr_solana_tools::{
    balance::get_sol, signer::Local as LocalSolanaSigner, transaction::transfer_sol,
};
...
// Helper function to create ApplicationContext with ExternalClients for tests
fn create_test_app_context() -> ApplicationContext {
    let config = Config::from_env();

    // Create and inject API clients - commented out as ExternalClients is internal
    // let api_clients = ExternalClients::new(&config.providers);
    // app_context.set_extension(Arc::new(api_clients));

    ApplicationContext::from_config(&Arc::new(config))
}
...
impl BlockchainTradingAgent {
    ...
    fn extract_transfer_params(task: &Task) -> Result<(Pubkey, u64)> {
        let params = task
            .parameters
            .as_object()
            .ok_or_else(|| AgentError::task_execution("Task parameters must be a JSON object"))?;

        let to_address_str = params
            .get("to_address")
            .and_then(|v| v.as_str())
            .ok_or_else(|| AgentError::task_execution("Missing required parameter: to_address"))?;

        let to_pubkey = Pubkey::from_str(to_address_str)
            .map_err(|e| AgentError::task_execution(format!("Invalid to_address format: {e}")))?;

        let amount_sol = params
            .get("amount")
            .and_then(serde_json::Value::as_f64)
            .ok_or_else(|| AgentError::task_execution("Missing or invalid parameter: amount"))?;

        if amount_sol <= 0.0 {
            return Err(AgentError::task_execution("Amount must be positive"));
        }

        // Allow precision and sign loss for financial calculations - amounts are validated to be positive
        #[expect(
...
impl Agent for BlockchainTradingAgent {
    async fn execute_task(&self, task: Task) -> Result<TaskResult> {
        ...
        // Allow precision loss for display purposes in financial calculations
        #[expect(clippy::cast_precision_loss)]
        let display_amount = amount_lamports as f64 / LAMPORTS_PER_SOL as f64;
...
async fn test_end_to_end_solana_transfer() {
    ...
    let sender_balance_result = get_sol(sender_keypair.pubkey().to_string(), &app_context).await;
    let initial_sender_balance =
        sender_balance_result.expect("Failed to get initial sender balance");

    let receiver_balance_result =
        get_sol(receiver_keypair.pubkey().to_string(), &app_context).await;
    ...
    let dispatcher = Dispatcher::with_config(Arc::new(registry), &config);
    ...
    let task_result = SignerContext::with_signer(unified_signer, async {
        dispatcher.dispatch_task(transfer_task).await.map_err(|e| {
            Box::new(StandardSignerError::Configuration(format!(
                "Task dispatch failed: {e}"
    ...
    assert!(
        ...
        "Task should have succeeded: {task_result:?}"
    );
    ...
    assert!(
        ...
        "Transaction should be successful: {signature_status:?}"
    );
    ...
    let final_sender_balance_result =
        get_sol(sender_keypair.pubkey().to_string(), &app_context).await;
    ...
    // Allow cast_possible_wrap for balance calculations - u64 lamport values fit within i64 range for SOL amounts
    ...
    // Allow cast_possible_wrap for comparison - SOL amounts fit within i64 range
    #[expect(clippy::cast_possible_wrap)]
    let expected_transfer_i64 = expected_transfer_lamports as i64;
    assert!(
        sender_balance_change >= expected_transfer_i64,
        "Sender balance should decrease by at least transfer amount"
    );
    #[expect(clippy::cast_possible_wrap, clippy::arithmetic_side_effects)]
    let max_expected_change = (expected_transfer_lamports + 10000) as i64;
    assert!(
        sender_balance_change <= max_expected_change,
    ...
    assert_eq!(
        receiver_balance_change, expected_transfer_i64,
...
async fn test_insufficient_balance_error_handling() {
    ...
    assert!(
        contains_error,
        "Error message should indicate insufficient balance or failure: {error_message}"
    );
...
async fn test_concurrent_blockchain_operations() {
    ...
    for i in 0..num_agents {
        let sender_keypair = harness
            ...
            .expect("Failed to get sender keypair");
        let receiver_keypair = harness
            .get_funded_keypair((i.saturating_add(num_agents)) % 6)
            .expect("Failed to get receiver keypair");
    ...
    for (i, receiver) in receivers.iter().enumerate().take(num_agents) {
        let task = Task::new(
            TaskType::Trading,
            serde_json::json!({
                "operation": "sol_transfer",
    ...
    let unique_signatures: HashSet<_> = signatures.iter().collect();
    assert_eq!(
...
mod tests {
    ...
    fn test_extract_transfer_params_valid() {
        ...
        // Allow precision and sign loss for test assertion financial calculations
        #[expect(
            clippy::cast_possible_truncation,
            clippy::cast_sign_loss,
            clippy::cast_precision_loss
        )]
        let expected_lamports = (1.5 * LAMPORTS_PER_SOL as f64) as u64;
    ...
    fn test_extract_transfer_params_negative_amount() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should fail with negative amount")
            .to_string()
    ...
    fn test_extract_transfer_params_missing_fields() {
        ...
        assert!(result.is_err());
        assert!(result
            .expect_err("Should fail with missing parameter")
            .to_string()
