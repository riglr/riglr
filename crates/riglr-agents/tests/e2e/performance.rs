...
use futures::future;
use riglr_agents::{CapabilityType, DispatchConfig, *};
...
impl PerformanceMetrics {
    ...
    pub async fn get_performance_summary(&self) -> PerformanceSummary {
        ...
        let mut sorted_latencies: Vec<u64> = self.latencies.read().await.iter().copied().collect();
        ...
        let (avg_latency_ns, p50_latency_ns, p95_latency_ns, p99_latency_ns) =
            if sorted_latencies.is_empty() {
                ...
                (
                    avg,
                    sorted_latencies.get(p50_idx).copied().unwrap_or(0),
                    sorted_latencies
                        .get(p95_idx.min(sorted_latencies.len().saturating_sub(1)))
                        .copied()
                        .unwrap_or(0),
                    sorted_latencies
                        .get(p99_idx.min(sorted_latencies.len().saturating_sub(1)))
                        .copied()
                        .unwrap_or(0),
                )
            };
...
impl LoadTestAgent {
    ...
    async fn process_performance_task(&self, task: &Task) -> Result<TaskResult> {
        ...
        match task_type {
            ...
            "cpu_intensive" => {
                ...
                Ok(self.simulate_cpu_intensive_operation(task, start_time))
            }
    ...
    async fn simulate_blockchain_operation(
        ...
        let job_result = SignerContext::with_signer(signer_context, async {
            ...
            if current_signer.as_solana().is_some() {
                Ok(serde_json::json!({
                    "simulated": true,
                ...
                use riglr_core::signer::error::{Error, Standard};
    ...
    fn simulate_cpu_intensive_operation(&self, _task: &Task, start_time: Instant) -> TaskResult {
...
impl Agent for LoadTestAgent {
    ...
    fn load(&self) -> f64 {
        ...
        #[expect(clippy::cast_precision_loss)]
        let load_value = 1.0 - (available as f64 / f64::from(max_concurrent));
        load_value
    }
}
...
async fn test_high_throughput_task_processing() {
    ...
    for batch_start in (0..num_tasks).step_by(batch_size) {
        ...
        let batch_results = future::join_all(task_handles).await;
...
async fn test_agent_system_under_load() {
    ...
    for i in 0..num_agents {
        ...
        let processing_delay =
            Duration::from_millis(50_u64.saturating_add((i as u64).saturating_mul(10)));
    ...
    let mut expected_total_tasks: usize = 0;

    for (task_type, count) in task_types {
        info!("Creating {} {} tasks", count, task_type);
        expected_total_tasks = expected_total_tasks.saturating_add(count);
        ...
        for task_idx in 0..count {
            ...
            let handle = tokio::spawn(async move {
                ...
                let delay_ms = (task_idx % 10).saturating_mul(5);
                let delay_duration =
                    Duration::from_millis(u64::from(delay_ms.try_into().unwrap_or(0_u32)));
    ...
    let stress_results = timeout(
        stress_timeout,
        future::join_all(all_task_handles.into_iter().map(|(_, handle)| handle)),
    )
    .await;

    let Ok(task_results) = stress_results else {
        error!("Stress test timed out after {:?}", stress_timeout);
        {
            unreachable!("System failed stress test - tasks did not complete within timeout");
        }
    };
    ...
    let mut task_type_counts = HashMap::with_capacity(4);
...
async fn test_memory_and_resource_management() {
    ...
    let num_rounds: usize = 5;
    let tasks_per_round: usize = 50;

    for round in 0..num_rounds {
        info!(
            "Starting memory test round {}/{}",
        ...
        let round_results = future::join_all(round_handles).await;
        ...
        assert!(
            round_successful >= tasks_per_round.saturating_mul(90).saturating_div(100),
    ...
    let expected_total_tasks = num_rounds.saturating_mul(tasks_per_round);
    assert!(
        summary.total_tasks >= expected_total_tasks.saturating_mul(90).saturating_div(100),
...
async fn test_system_latency_benchmarks() {
    ...
    latencies.sort();
    let total_duration = latencies.iter().sum::<Duration>();
    let avg_latency = Duration::from_nanos({
        #[expect(clippy::arithmetic_side_effects)]
        {
            u64::try_from(total_duration.as_nanos() / u128::try_from(latencies.len()).unwrap_or(1))
                .unwrap_or(0)
        }
    });
    let median_latency = latencies
        .get(latencies.len().saturating_div(2))
        .copied()
        .unwrap_or_default();
    let p95_latency = latencies
        .get(latencies.len().saturating_mul(95).saturating_div(100))
        .copied()
        .unwrap_or_default();
    let p99_latency = latencies
        .get(latencies.len().saturating_mul(99).saturating_div(100))
        .copied()
        .unwrap_or_default();
