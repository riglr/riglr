...
#[path = "dispatcher/proxy.rs"]
pub mod proxy;
#[path = "dispatcher/queue.rs"]
pub mod queue;
#[path = "dispatcher/queue_trait.rs"]
pub mod queue_trait;
#[path = "dispatcher/router.rs"]
pub mod router;
...
use crate::{
    registry::Registry, util::task_type_to_capability, Agent, AgentError, Result, Task, TaskResult,
};
...
#[derive(Debug)]
pub struct Dispatcher<R: Registry> {
...
impl<R: Registry> Dispatcher<R> {
    ...
    ///
    /// Returns an error if:
    /// - No suitable agents are found for the task
    /// - All retry attempts fail with non-retriable errors
    /// - The task times out or is cancelled
    /// - Agent execution fails with a permanent error
    ...
    pub async fn dispatch_task(&self, mut task: Task) -> Result<TaskResult> {
        ...
        error!("Task {} exhausted all retry attempts", task.id);
        Err(AgentError::generic("Maximum retry attempts exceeded"))
    }
    ...
    pub async fn dispatch_tasks(&self, tasks: Vec<Task>) -> Vec<Result<TaskResult>> {
        ...
        let futures = tasks
            .into_iter()
            .map(|task| Box::pin(async move { self.dispatch_task(task).await }));
        ...
        match timeout(task_timeout, agent.execute_task(task.clone())).await {
            Ok(Ok(result)) => {
                info!(
                    ...
                    agent.id(),
                    match &result {
                        &TaskResult::Success { duration, .. } => Some(duration),
                        _ => None,
                    }
                );
                Ok(result)
    ...
    async fn execute_remote_task(
        ...
        if let Some(ref queue) = self.distributed_queue {
            // Determine task timeout
            let task_timeout: Duration = task.timeout.unwrap_or(self.config.default_task_timeout);
    ...
    async fn find_suitable_agents(&self, task: &Task) -> Result<Vec<Proxy>> {
        debug!("Finding suitable agents for task type {:?}", task.task_type);

        let capability = task_type_to_capability(&task.task_type);
        ...
        for status in agent_statuses {
            ...
            if let Ok(Some(local_agent)) = local_agent_result {
                ...
                suitable_proxies.push(Proxy::Local(local_agent));
            } else {
                // This is a remote agent
                suitable_proxies.push(Proxy::Remote(status));
            }
        }
        ...
        if self.config.enable_load_balancing {
            ...
            suitable_proxies
                .sort_by(|a, b| a.load().partial_cmp(&b.load()).unwrap_or(Ordering::Equal));
        }
    ...
    ///
    /// Returns an error if:
    /// - Registry health check fails
    /// - Registry communication fails
    /// - Unable to retrieve agent count
    #[expect(clippy::cognitive_complexity)]
    pub async fn health_check(&self) -> Result<bool> {
        // Check registry health
        if let Err(e) = self.registry.health_check().await {
            error!("Registry health check failed: {}", e);
            return Err(e);
        }

        // Check if we have any agents registered
        match self.registry.agent_count().await {
            Ok(agent_count) => {
                if agent_count == 0 {
                    warn!("No agents registered in dispatcher");
                    Ok(false) // Unhealthy - no agents
                } else {
                    debug!("Dispatcher health check passed with {} agents", agent_count);
                    Ok(true) // Healthy - has agents
                }
            }
            Err(e) => {
                error!("Failed to get agent count: {}", e);
                Err(e)
            }
        }
    }
    ...
    ///
    /// Returns an error if:
    /// - Registry fails to provide agent count
    /// - Registry fails to list agent statuses
    ...
    pub async fn stats(&self) -> Result<Stats> {
        ...
        let average_load = if statuses.is_empty() {
            ...
            let len_f64 = statuses.len() as f64;
            statuses.iter().map(|s| s.load).sum::<f64>() / len_f64
        };
    ...
    async fn try_dispatch_task(&self, task: &Task) -> Result<TaskResult> {
        ...
        let selected_agent = self.router.select_agent(&agents, task)?;
    ...
    #[must_use]
    pub fn with_redis_connection(mut self, connection: MultiplexedConnection) -> Self {
        let redis_queue = queue::Redis::new(connection);
...
/// Type alias for dispatcher statistics.
pub type DispatchMetrics = Stats;
...
mod tests {
    ...
    async fn test_dispatcher_basic_dispatch() {
        ...
        registry
            ...
            .await
            .expect("Failed to register mock agent in test");
        ...
        let result = dispatcher
            .dispatch_task(task)
            .await
            .expect("Failed to dispatch task in test");
    ...
    async fn test_dispatcher_no_suitable_agent() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register mock agent in no suitable agent test");
        ...
        assert!(matches!(
            result.expect_err("Expected task dispatch to fail with no suitable agent"),
    ...
    async fn test_dispatcher_retry_logic() {
        ...
        let dispatcher = Dispatcher::with_config(registry.clone(), &config);
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register failing agent in retry test");
        ...
        let result = dispatcher
            .dispatch_task(task)
            .await
            .expect("Failed to dispatch task to failing agent");
        assert!(!result.is_success());
    ...
    async fn test_dispatcher_timeout() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register slow agent in timeout test");
        ...
        assert!(result.is_err());
        assert!(matches!(
            result.expect_err("Expected task to timeout in timeout test"),
    ...
    async fn test_dispatcher_concurrent_tasks() {
        ...
        for i in 0..3 {
            ...
            registry
                .register_agent(agent)
                .await
                .expect("Failed to register concurrent test agent");
        }
    ...
    async fn test_dispatcher_stats() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register agent for stats test");

        let stats = dispatcher
            .stats()
            .await
            .expect("Failed to get dispatcher stats");
    ...
    async fn test_dispatcher_health_check() {
        ...
        assert!(!dispatcher
            .health_check()
            .await
            .expect("Failed to check dispatcher health with no agents"));
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register agent for health check test");

        // Should be healthy now
        assert!(dispatcher
            .health_check()
            .await
            .expect("Failed to check dispatcher health with registered agent"));
    }
    ...
    fn test_dispatcher_config_getter() {
        ...
        let dispatcher = Dispatcher::with_config(registry, &custom_config);
    ...
    fn test_task_type_to_capability_all_variants() {
        assert_eq!(
            task_type_to_capability(&TaskType::Trading),
            crate::CapabilityType::Trading
        );
        assert_eq!(
            task_type_to_capability(&TaskType::Research),
            crate::CapabilityType::Research
        );
        assert_eq!(
            task_type_to_capability(&TaskType::RiskAnalysis),
            crate::CapabilityType::RiskAnalysis
        );
        assert_eq!(
            task_type_to_capability(&TaskType::Portfolio),
            crate::CapabilityType::Portfolio
        );
        assert_eq!(
            task_type_to_capability(&TaskType::Monitoring),
            crate::CapabilityType::Monitoring
        );
        assert_eq!(
            task_type_to_capability(&TaskType::Custom("custom_capability".to_string())),
    ...
    async fn test_find_suitable_agents_unavailable_agents() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register unavailable agent in test");
        ...
        assert!(result.is_err());
        assert!(matches!(
            result.expect_err("Expected task dispatch to fail with unavailable agent"),
            AgentError::NoSuitableAgent { .. }
        ));
    }
    ...
    async fn test_find_suitable_agents_cannot_handle() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register cannot-handle agent in test");
        ...
        assert!(result.is_err());
        assert!(matches!(
            result.expect_err("Expected task dispatch to fail with cannot-handle agent"),
            AgentError::NoSuitableAgent { .. }
        ));
    }
    ...
    async fn test_dispatcher_non_retriable_error() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register non-retriable agent in test");
        ...
        assert!(result.is_err());
        assert!(matches!(
            result.expect_err("Expected task dispatch to fail with non-retriable error"),
    ...
    async fn test_dispatcher_custom_task_timeout() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register slow agent for timeout test");
        ...
        assert!(result.is_err());
        assert!(matches!(
            result.expect_err("Expected custom timeout task to fail"),
            AgentError::TaskTimeout { .. }
        ));
    }
    ...
    async fn test_dispatcher_load_balancing_disabled() {
        ...
        for i in 0..3 {
            ...
            registry
                .register_agent(agent)
                .await
                .expect("Failed to register agent for load balancing test");
        }
    ...
    async fn test_dispatcher_stats_with_empty_registry() {
        ...
        let stats = dispatcher
            .stats()
            .await
            .expect("Failed to get stats from empty registry");
        assert_eq!(stats.registered_agents, 0);
        assert_eq!(stats.total_active_tasks, 0);
        assert!((stats.average_agent_load - 0.0).abs() < f64::EPSILON);
        assert_eq!(stats.routing_strategy, RoutingStrategy::Capability);
    }
    ...
    fn test_dispatcher_stats_struct() {
        ...
        assert!((cloned_stats.average_agent_load - 2.5).abs() < f64::EPSILON);
    ...
    async fn test_dispatch_task_max_retries_exhausted() {
        ...
        let result = dispatcher
            .dispatch_task(task)
            .await
            .expect("Failed to get result from always-failing agent");
        assert!(!result.is_success());
        assert!(result.is_retriable());
    }
    ...
    async fn test_find_suitable_agents_with_load_balancing() {
        ...
        registry
            .register_agent(high_load_agent)
            .await
            .expect("Failed to register high load agent");
        registry
            .register_agent(low_load_agent)
            .await
            .expect("Failed to register low load agent");
    ...
    async fn test_dispatch_task_success_with_duration() {
        ...
        registry
            .register_agent(agent)
            .await
            .expect("Failed to register duration test agent");
        ...
        let result = dispatcher
            .dispatch_task(task)
            .await
            .expect("Failed to dispatch task for duration test");
        assert!(result.is_success());
