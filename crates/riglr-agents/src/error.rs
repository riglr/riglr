...
impl Error {
    ...
    pub const fn is_retriable(&self) -> bool {
        match *self {
            ...
            Self::ToolExecutionFailed(ref tool_error) => tool_error.is_retriable(),
    ...
    pub const fn retry_delay(&self) -> Option<Duration> {
        match *self {
            Self::ToolExecutionFailed(ref tool_error) => tool_error.retry_after(),
            Self::Communication { .. } => Some(Duration::from_secs(1)),
            Self::MessageDeliveryFailed { .. } => Some(Duration::from_millis(500)),
            Self::Registry { .. } => Some(Duration::from_millis(100)),
            Self::Storage { .. } => Some(Duration::from_millis(200)),
            Self::AgentNotFound { .. }
            | Self::AgentUnavailable { .. }
            | Self::Capacity { .. }
            | Self::Configuration { .. }
    ...
    #[inline]
    pub fn task_timeout(task_id: impl Into<String>, duration: Duration) -> Self {
        Self::TaskTimeout {
            task_id: task_id.into(),
            duration,
        }
...
impl From<Error> for ToolError {
    #[inline]
    fn from(agent_error: Error) -> Self {
        match agent_error {
            // If it's already a ToolError wrapped in AgentError, unwrap it to preserve context
            Error::ToolExecutionFailed(tool_error) => tool_error,
            agent_error if agent_error.is_retriable() => {
                if let Some(delay) = agent_error.retry_delay() {
                    Self::rate_limited_with_source(agent_error, "Agent error", Some(delay))
                } else {
                    Self::retriable_with_source(agent_error, "Agent error")
                }
            }
            agent_error @ (Error::AgentNotFound { .. }
            ...
            | Error::Dispatcher { .. }
            | Error::Generic { .. }
            | Error::InvalidRoutingRule { .. }
            | Error::MessageDeliveryFailed { .. }
            | Error::NoSuitableAgent { .. }
            | Error::Registry { .. }
            | Error::Serialization { .. }
            | Error::Storage { .. }
            | Error::TaskCancelled { .. }
            | Error::TaskExecution { .. }
            | Error::TaskTimeout { .. }) => Self::permanent_with_source(agent_error, "Agent error"),
        }
    }
}
...
impl From<Elapsed> for Error {
    ...
    fn from(err: Elapsed) -> Self {
        Self::generic_with_source("Operation timed out", err)
    }
}
...
impl From<WorkerError> for Error {
    ...
    fn from(err: WorkerError) -> Self {
        Self::task_execution_with_source("Worker error", err)
    }
}
...
impl From<PromptError> for Error {
    ...
    fn from(err: PromptError) -> Self {
        Self::task_execution_with_source("LLM prompt error", err)
    }
}
...
mod tests {
    ...
    fn test_error_retriability() {
        ...
        assert!(AgentError::task_timeout("task", Duration::from_secs(30)).is_retriable());
    ...
    fn test_agent_not_found_constructor() {
        ...
        match error {
            AgentError::AgentNotFound { ref agent_id } => {
    ...
    fn test_no_suitable_agent_constructor() {
        ...
        match error {
            AgentError::NoSuitableAgent { ref task_type } => {
    ...
    fn test_invalid_routing_rule_constructor() {
        ...
        match error {
            AgentError::InvalidRoutingRule { ref rule } => {
    ...
    async fn test_tokio_elapsed_error_conversion() {
        ...
        let result = timeout(Duration::from_millis(1), sleep(Duration::from_millis(10))).await;
    ...
    fn test_is_retriable_all_variants() {
        ...
        assert!(AgentError::agent_unavailable("agent", "busy").is_retriable());
        assert!(AgentError::task_timeout("task", Duration::from_secs(1)).is_retriable());
    ...
    fn test_retry_delay_all_variants() {
        ...
        let comm_error = AgentError::communication("timeout");
        assert_eq!(comm_error.retry_delay(), Some(Duration::from_secs(1)));
        ...
        let timeout = AgentError::task_timeout("task", Duration::from_secs(1));
    ...
    fn test_tool_error_retry_delay_delegation() {
        ...
        assert_eq!(agent_error.retry_delay(), Some(Duration::from_secs(5)));
    ...
    fn test_from_agent_error_to_tool_error_all_cases() {
        ...
        assert_eq!(converted.retry_after(), Some(Duration::from_secs(1)));
    ...
    fn test_error_display_messages() {
        ...
        let timeout = AgentError::task_timeout("task-1", Duration::from_secs(30));
        ...
        assert_eq!(format!("{invalid_rule}"), "Invalid routing rule: bad-rule");
        ...
        assert_eq!(format!("{dispatcher}"), "Dispatcher error: Route not found");
        ...
        assert_eq!(format!("{generic}"), "Agent system error: Something failed");
    }
