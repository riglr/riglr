...
pub mod agents {
    ...
    // Re-export the ToolAgent for convenience
    pub use tool_calling::ToolAgent;
}
pub mod builder;
...
pub use agents::tool_calling::{DebuggableCompletionModel, ToolAgent};
pub use builder::{AgentSystem, SystemConfig, SystemHealth, SystemStats};
pub use communication::{Channel as ChannelCommunication, Communication};
pub use dispatcher::{DispatchConfig, Dispatcher, RoutingStrategy};
pub use error::{Error as AgentError, Result};
pub use integration::Context;
pub use registry::{local::Local as LocalAgentRegistry, Registry as AgentRegistry};
...
mod tests {
    ...
    async fn test_agent_basic_functionality() {
        ...
        let result = agent
            .execute_task(task)
            .await
            .expect("Agent task execution should succeed in test");
    ...
    async fn test_agent_error_handling() {
        ...
        let result = agent
            .execute_task(task)
            .await
            .expect("Agent task execution should complete even when failing in test");
    ...
    fn test_agent_status_with_single_capability() {
        ...
        assert_eq!(
            status
                .capabilities
                .first()
                .expect("Should have monitoring capability")
                .name,
            "monitoring"
        );
        assert_eq!(
