    ...
    async fn perform_technical_analysis(&self, symbol: &str) -> serde_json::Value {
        ...
        let exposure_ratio = {
            let portfolio = self
                .portfolio
                .lock()
                .expect("Failed to acquire portfolio lock for exposure calculation");
    ...
    async fn execute_task(&self, task: Task) -> riglr_agents::Result<TaskResult> {
        ...
        let (current_exposure, recommended_position_size, action) = {
            let portfolio = self
                .portfolio
                .lock()
                .expect("Failed to acquire portfolio lock for recommendation calculation");
            let current_position = portfolio.positions.get(symbol).copied().unwrap_or(0.0);
    ...
    async fn calculate_position_risk(&self, _symbol: &str, amount: f64, price: f64) -> f64 {
        ...
        let position_risk = {
            let portfolio = self
                .portfolio
                .lock()
                .expect("Failed to acquire portfolio lock for risk calculation");
            let risk = position_value / portfolio.total_value;
            let volatility_multiplier = 1.2; // Simulate volatility factor
            drop(portfolio); // Release lock early
            risk * volatility_multiplier
        };
    ...
    async fn validate_trade(
        ...
        let new_total_risk = {
            let portfolio = self
                .portfolio
                .lock()
                .expect("Failed to acquire portfolio lock for risk validation");
            portfolio.risk_exposure + position_risk
        };

        let approved =
            position_risk <= self.max_position_percent && new_total_risk <= self.max_portfolio_risk;
...
impl TradeExecutionAgent {
    ...
    async fn execute_blockchain_trade(
        &self,
        trade_params: &serde_json::Value,
    ) -> riglr_agents::Result<serde_json::Value> {
        ...
        println!("🔗 Executing {action} {amount} {symbol} on blockchain...");
        ...
        // Update portfolio state
        {
            let mut portfolio = self
                .portfolio
                .lock()
                .expect("Failed to acquire portfolio lock for trade execution");
            if action == "buy" {
...
impl PortfolioMonitorAgent {
    ...
    fn generate_portfolio_report(&self) -> serde_json::Value {
        let portfolio = self
            .portfolio
            .lock()
            .expect("Failed to acquire portfolio lock for report generation");

        let total_positions: f64 = portfolio.positions.values().sum();
        let position_diversity = portfolio.positions.len();
        let utilization_rate =
            (portfolio.total_value - portfolio.available_capital) / portfolio.total_value;
...
impl Agent for PortfolioMonitorAgent {
    async fn execute_task(&self, _task: Task) -> riglr_agents::Result<TaskResult> {
        ...
        let report = self.generate_portfolio_report();
...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let portfolio = Arc::new(Mutex::new(PortfolioState::new(100_000.0)));
    ...
    let portfolio_monitor = Arc::new(PortfolioMonitorAgent::new(
        "monitor-omega",
        communication.clone(),
        portfolio.clone(),
    ));
    ...
    println!(
        "✅ Registered {} specialized agents in trading swarm",
    ...
    // Execute coordinated trading workflow
    {
        ...
        println!(
            "✅ Research completed with confidence: {}",
            research_result
                .data()
                .expect("Failed to get research data for confidence display")
                .get("recommendation")
        ...
        let risk_task = Task::new(
            ...
            json!({
                "analysis": research_result.data().expect("Failed to get research data for risk analysis"),
                "trade_params": {
                    "symbol": "BTC",
                    "action": "buy",
                    "amount": 2.5
        ...
        let risk_approved = risk_result
            .data()
            .expect("Failed to get risk assessment data")
            .get("approved")
            .and_then(serde_json::Value::as_bool)
            .unwrap_or(false);
        ...
        println!(
            ...
            risk_result
                .data()
                .expect("Failed to get risk data for display")
                .get("risk_level")
                .and_then(|l| l.as_str())
                .unwrap_or("unknown")
        );
        ...
        if risk_approved {
            ...
            let execution_task = Task::new(
                TaskType::Trading,
                json!({
                    "risk_approval": risk_result.data().expect("Failed to get risk approval data"),
                    "trade_params": {
                        "symbol": "BTC",
                        "action": "buy",
                        "amount": 2.5
                    },
                    "research_data": research_result.data().expect("Failed to get research data for execution")
                }),
            )
            ...
            println!(
                ...
                execution_result
                    .data()
                    .expect("Failed to get execution result data")
                    .get("transaction_hash")
                    .and_then(|h| h.as_str())
                    .unwrap_or("unknown")
            );
        ...
        println!(
            "✅ Portfolio updated - Total Value: ${:.2}",
            monitor_result
                .data()
                .expect("Failed to get portfolio monitor data")
                .get("total_value")
                .and_then(serde_json::Value::as_f64)
                .unwrap_or(0.0)
        );
    ...
    // Show final portfolio state
    {
        let portfolio = portfolio
            .lock()
            .expect("Failed to acquire portfolio lock for final status");
