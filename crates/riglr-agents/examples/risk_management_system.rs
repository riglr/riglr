...
use riglr_agents::{
    Agent, AgentId, AgentMessage, AgentRegistry, CapabilityType, ChannelCommunication,
    Communication, DispatchConfig, Dispatcher, LocalAgentRegistry, Priority, RoutingStrategy, Task,
    TaskResult, TaskType,
};
...
impl PositionRiskAgent {
    fn analyze_position_risk(&self, position_data: &serde_json::Value) -> serde_json::Value {
        ...
        let position_size = position_data
            .get("size")
            .and_then(serde_json::Value::as_f64)
            .unwrap_or(0.0);

        let price = position_data
            .get("current_price")
            .and_then(serde_json::Value::as_f64)
            .unwrap_or(1.0);

        let volatility = position_data
            .get("volatility")
            .and_then(serde_json::Value::as_f64)
            .unwrap_or(0.25);
        ...
        let position_percent = {
            let risk_state = self
                .risk_state
                .lock()
                .expect("Failed to acquire risk state lock");
            position_value / (risk_state.total_exposure * 100_000.0) // Assuming 100k portfolio
        };
...
impl Agent for PositionRiskAgent {
    async fn execute_task(&self, task: Task) -> riglr_agents::Result<TaskResult> {
        ...
        let risk_analysis = self.analyze_position_risk(position_data);

        // Update risk state with new position data
        {
            let mut risk_state = self
                .risk_state
                .lock()
                .expect("Failed to acquire risk state lock");
            let symbol = position_data
                ...
                .and_then(|s| s.as_str())
                .unwrap_or("UNKNOWN");
            let risk_score = risk_analysis
                .get("risk_score")
                .and_then(serde_json::Value::as_f64)
                .unwrap_or(0.0);
            let position_percent = risk_analysis
            ...
            risk_state
                .position_risks
                .insert(symbol.to_string(), risk_score);
            ...
            if let Some(sector) = position_data.get("sector").and_then(|s| s.as_str()) {
                let current_sector_exposure = risk_state
                    .sector_exposures
                    .get(sector)
                    .copied()
                    .unwrap_or(0.0);
                risk_state.sector_exposures.insert(
                    sector.to_string(),
                    current_sector_exposure + position_percent,
                );
            }
            ...
            if symbol.contains("BTC") || symbol.contains("ETH") {
                let existing_symbols: Vec<String> =
                    risk_state.position_risks.keys().cloned().collect();
                for existing_symbol in existing_symbols {
                    if existing_symbol != symbol
                        && (existing_symbol.contains("BTC") || existing_symbol.contains("ETH"))
                    {
                        risk_state
                            .correlation_matrix
...
impl PortfolioRiskMonitor {
    ...
    fn calculate_portfolio_risk(&self) -> serde_json::Value {
        let mut risk_state = self
            .risk_state
            .lock()
            .expect("Failed to acquire risk state lock");
        ...
        limit_breaches.extend(sector_violations.iter().map(String::as_str));
    ...
    fn calculate_correlation_exposure(risk_state: &RiskState) -> f64 {
...
impl Agent for PortfolioRiskMonitor {
    async fn execute_task(&self, _task: Task) -> riglr_agents::Result<TaskResult> {
        println!(
            "📊 Portfolio Risk Monitor {} calculating portfolio risk",
            self.id
        );

        let risk_summary = self.calculate_portfolio_risk();

        // If alert level changed or we have breaches, send high-priority message
        let alert_changed = risk_summary
            .get("alert_changed")
            .and_then(serde_json::Value::as_bool)
            .unwrap_or(false);

        let has_breaches = risk_summary
            .get("limit_breaches")
        ...
        self.communication
            .broadcast_message(status_message)
            .await
            .map_err(|e| riglr_agents::AgentError::communication(e.to_string()))?;
...
impl ComplianceAgent {
    fn new(id: &str, communication: Arc<ChannelCommunication>) -> Self {
        ...
        regulatory_rules.insert(
            "prohibited_sectors".to_string(),
            json!(["gambling", "adult_content"]),
        );
...
impl Agent for ComplianceAgent {
    async fn execute_task(&self, task: Task) -> riglr_agents::Result<TaskResult> {
        println!(
            "⚖️ Compliance Agent {} checking regulatory compliance",
            self.id
        );
        ...
        let has_violations = compliance_result
            .get("violations")
...
impl EmergencyResponseAgent {
    ...
    async fn execute_emergency_response(
        ...
        let alert_level = alert_data
            .get("alert_level")
            .and_then(|l| l.as_str())
            .unwrap_or("Green");

        let default_breaches = vec![];
        let limit_breaches = alert_data
            .get("limit_breaches")
            .and_then(|b| b.as_array())
        ...
        json!({
            ...
            "next_review": chrono::Utc::now().timestamp().saturating_add(3600), // 1 hour
...
impl Agent for EmergencyResponseAgent {
    async fn execute_task(&self, task: Task) -> riglr_agents::Result<TaskResult> {
        println!(
            "🚨 Emergency Response Agent {} handling risk alert",
            self.id
        );
    ...
    async fn handle_message(&self, message: AgentMessage) -> riglr_agents::Result<()> {
        match message.message_type.as_str() {
            "risk_alert" => {
                println!(
                    "🚨 Emergency Response Agent {} received risk alert",
                    self.id
                );
            ...
            "compliance_violation" => {
                println!(
                    "🚨 Emergency Response Agent {} received compliance violation alert",
                    self.id
                );
...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let emergency_agent = Arc::new(EmergencyResponseAgent::new(
    ...
    let dispatcher = Arc::new(Dispatcher::with_config(registry.clone(), &dispatch_config));
    ...
    // Execute comprehensive risk management workflow
    {
        ...
        println!(
            "✅ Position risk analysis: {} (Risk Score: {:.2})",
            if position_result
                .data()
                .expect("Position result data should exist")
                .get("approved")
                .and_then(serde_json::Value::as_bool)
                .unwrap_or(false)
            {
        ...
        let alert_level = portfolio_result
            .data()
            .expect("Portfolio result data should exist")
            .get("alert_level")
            .and_then(|l| l.as_str())
            .unwrap_or("Green");
        ...
        let compliant = compliance_result
            .data()
            .expect("Compliance result data should exist")
            .get("compliant")
            .and_then(serde_json::Value::as_bool)
            .unwrap_or(false);
        println!(
        ...
        let actions_taken = emergency_result
            .data()
            .expect("Emergency result data should exist")
            .get("actions_taken")
            .and_then(|a| a.as_array())
            .map_or(0, Vec::len);
        println!("✅ Emergency response executed: {actions_taken} corrective actions taken");
        ...
        let final_alert = final_result
            .data()
            .expect("Final result data should exist")
            .get("alert_level")
            .and_then(|l| l.as_str())
            .unwrap_or("Unknown");
    ...
    // Display final risk state
    {
        ...
        println!("\n🛡️ Final Risk State:");
        println!(
            "  Portfolio VaR: {:.2}% (Limit: {:.2}%)",
            risk_state.current_var * 100.0,
            risk_limits.max_portfolio_var * 100.0
        );
        println!(
            "  Daily P&L: {:.2}% (Limit: {:.2}%)",
            risk_state.daily_pnl * 100.0,
