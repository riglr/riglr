...
pub use client::Client as Neo4jClient;
pub use document::RawText as RawTextDocument;
pub use error::{Error as GraphMemoryError, Result};
pub use extractor::Extractor as EntityExtractor;
pub use graph::Memory as GraphMemory;
...
mod tests {
    ...
    fn test_version_when_valid_semver_should_pass() {
        ...
        let major_version = parts.first().unwrap_or(&"");
        assert!(
            major_version.parse::<u32>().is_ok(),
            "Major version should be numeric"
        );

        // Verify minor version is numeric (may contain additional text for pre-release)
        let minor_part = parts.get(1).unwrap_or(&"");
        let minor_numeric = minor_part.split('-').next().unwrap_or(minor_part);
        assert!(
    ...
    fn test_module_re_exports_accessibility() {
        ...
        let _ = PhantomData::<Neo4jClient>;
        let _ = PhantomData::<RawTextDocument>;
        let _ = PhantomData::<GraphMemoryError>;
        let _ = PhantomData::<Result<()>>;
        let _ = PhantomData::<EntityExtractor>;
        let _ = PhantomData::<GraphMemory>;
        let _ = PhantomData::<GraphRetriever>;
        let _ = PhantomData::<GraphVectorStore>;
        let _ = PhantomData::<RigDocument>;
    ...
    fn test_version_edge_cases() {
        ...
        #[expect(clippy::const_is_empty)] // Defensive check for build environment
        {
