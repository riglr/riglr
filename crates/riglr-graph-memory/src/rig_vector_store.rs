...
impl GraphVectorStore {
    ...
    #[must_use]
    pub const fn new(client: Arc<Neo4jClient>, index_name: String) -> Self {
        Self {
            client,
...
impl GraphVectorStore {
    ...
    /// # Errors
    ///
    /// Returns error if document embedding generation fails or graph storage fails
    pub async fn add_documents(&self, documents: Vec<RigDocument>) -> Result<Vec<String>> {
        ...
        let document_ids = self.process_document_batch(documents).await?;
    ...
    async fn process_document_batch(&self, documents: Vec<RigDocument>) -> Result<Vec<String>> {
        let mut document_ids = Vec::new();

        for doc in documents {
            let store_result = self.store_single_rig_document(doc).await;
            match store_result {
                Ok(doc_id) => {
    ...
    /// Create parameters for RIG document storage
    fn create_rig_document_params(doc: &RigDocument) -> HashMap<String, Value> {
        HashMap::from([
    ...
    /// Store a single RIG document
    async fn store_single_rig_document(&self, doc: RigDocument) -> Result<String> {
        let params = Self::create_rig_document_params(&doc);
        let doc_id = doc.id.clone();

        self.execute_rig_document_query(doc_id, params).await
    }

    /// Execute document creation query for RIG documents
    async fn execute_rig_document_query(
        &self,
        doc_id: String,
        ...
        return self
            ...
            .map(|_| {
                debug!("Successfully added document {}", doc_id);
                doc_id
            });
    }
    ...
    /// # Errors
    ///
    /// Returns error if deletion query fails
    pub async fn delete_documents(&self, ids: Vec<String>) -> Result<Vec<String>> {
        ...
        self.client
            .execute_query(query, Some(params))
            .await
            .map_err(|e| Error::Database(format!("Failed to delete documents: {e}")))?;
    ...
    /// # Errors
    ///
    /// Returns error if document retrieval query fails or result parsing fails
    pub async fn get_documents(&self, ids: Vec<String>) -> Result<Vec<RigDocument>> {
        ...
        let result = self
            ...
            .execute_query(query, Some(params))
            .await
            .map_err(|e| Error::Database(format!("Failed to get documents: {e}")))?;
    ...
    /// # Errors
    /// Returns an error if:
    /// - Vector index query fails
    /// - Database connection is lost
    /// - Query result deserialization fails
    /// - Vector similarity computation fails
    pub async fn search(
        &self,
        ...
        let result = self
            ...
            .await
            .map_err(|e| Error::Database(format!("Failed to perform vector search: {e}")))?;
...
pub trait VectorStore: Send + Sync {
    ...
    ) -> CoreResult<Vec<String>, Self::Error>;
    ...
    async fn search(
        ...
        query_embedding: Vec<f32>,
        limit: usize,
    ) -> CoreResult<Vec<(Self::Document, f32)>, Self::Error>;

    /// Get documents by their IDs
    async fn get_documents(&self, ids: Vec<String>)
        -> CoreResult<Vec<Self::Document>, Self::Error>;

    /// Delete documents by their IDs
    async fn delete_documents(&self, ids: Vec<String>) -> CoreResult<Vec<String>, Self::Error>;
}
...
impl From<RawText> for RigDocument {
    fn from(raw_doc: RawText) -> Self {
...
impl From<RigDocument> for RawText {
    fn from(rig_doc: RigDocument) -> Self {
        ...
        // Convert metadata back to Metadata format
        let metadata = if rig_doc.metadata.is_empty() {
            None
        } else {
            use crate::document::Metadata;
            let mut doc_meta = Metadata::default();
        ...
        Self {
            ...
            id: rig_doc.id,
            metadata,
            source: Source::UserInput, // Default
        }
    }
}
...
mod tests {
    ...
    fn create_test_raw_document() -> RawText {
        let metadata = Metadata {
            ...
            custom_fields: HashMap::from([("custom".to_string(), json!("value"))]),
            ..Default::default()
        };
    ...
    fn test_neo4j_query_response_extract_documents_valid_data() {
        ...
        assert_eq!(
            documents.first().expect("Should have first document").id,
            "doc-1"
        );
        assert_eq!(
            documents
    ...
    fn test_neo4j_query_response_extract_documents_with_scores_valid() {
        ...
        {
            let score = results.first().expect("Should have first result").1;
            assert!(
                (score - 0.85).abs() < f32::EPSILON,
    ...
    fn test_neo4j_query_response_extract_documents_with_scores_missing_score() {
        ...
        assert_eq!(results.len(), 1);
        {
            ...
            assert!(
                ...
                "Expected score ~0.0, got {score}"
            ); // Default score when missing
        }
    }
    ...
    fn test_parse_document_row_valid_id_only() {
        ...
        let doc = result.expect("Failed to parse document from row");
    ...
    fn test_parse_document_row_missing_content() {
        ...
        assert!(result.is_some());
        let doc = result.expect("Failed to parse document from row with missing content");
    ...
    fn test_parse_document_row_invalid_embedding() {
        ...
        let result =
            Neo4jQueryResponse::parse_document_row(&row, Some(0), Some(1), Some(2), Some(3), None);
        assert!(result.is_some());
        let doc = result.expect("Failed to parse document from row with invalid embedding");
    ...
    fn test_parse_document_row_invalid_metadata() {
        ...
        let result =
            Neo4jQueryResponse::parse_document_row(&row, Some(0), Some(1), Some(2), Some(3), None);
        assert!(result.is_some());
        let doc = result.expect("Failed to parse document from row with invalid metadata");
    ...
    fn test_graph_vector_store_new() {
        let client = Arc::new(Neo4jClient::default());
    ...
    fn test_graph_vector_store_empty_index_name() {
        let client = Arc::new(Neo4jClient::default());
        let store = GraphVectorStore::new(client, String::new());
    ...
    fn test_from_rig_document_to_raw_text_document_with_metadata() {
        ...
        let raw_doc: RawText = rig_doc.into();
        ...
        let metadata = raw_doc.metadata.expect("Expected metadata to be present");
    ...
    fn test_from_rig_document_to_raw_text_document_custom_fields() {
        ...
        let raw_doc: RawText = rig_doc.into();
        let doc_metadata = raw_doc
            .metadata
            .expect("Expected metadata to be present for custom fields test");
    ...
    fn test_from_rig_document_to_raw_text_document_invalid_tags() {
        ...
        let doc_metadata = raw_doc
            .metadata
            .expect("Expected metadata to be present for invalid tags test");
    ...
    fn test_extract_documents_multiple_results() {
        ...
        assert_eq!(
            documents.get(1).expect("Should have second document").id,
            "doc-2"
        );
    }
    ...
    fn test_rig_document_serialization() {
        ...
        let deserialized =
            serde_json::from_str(&serialized.expect("Failed to serialize RigDocument"));
        assert!(deserialized.is_ok());
        let deserialized_doc: RigDocument =
            deserialized.expect("Failed to deserialize RigDocument");
    ...
    fn test_embedding_conversion_mixed_types() {
        ...
        let result =
            Neo4jQueryResponse::parse_document_row(&row, Some(0), Some(1), Some(2), Some(3), None);
        assert!(result.is_some());
        let doc = result.expect("Failed to parse document with mixed embedding types");
