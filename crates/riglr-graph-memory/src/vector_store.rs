...
impl GraphRetriever {
    ...
    async fn ensure_vector_index(&self) -> Result<()> {
        ...
        self.client
            ...
            .await
            .map_err(|e| Error::Database(format!("Failed to create vector index: {e}")))?;
    ...
    /// - Index configuration is invalid
    pub async fn new(
        client: Arc<Neo4jClient>,
    ...
    pub async fn search_with_graph_context(
        ...
        let (mut documents, entity_set) = self
            .perform_vector_search(query_embedding, limit, &mut metrics)
            .await?;
        self.enhance_with_graph_context(&mut documents, &entity_set, &mut metrics)
            .await?;
        Self::finalize_search_results(&mut documents, limit);

        #[expect(clippy::cast_possible_truncation)] // Time measurements unlikely to overflow u64
        {
    ...
    async fn perform_vector_search(
    ...
    ) -> Result<(Vec<GraphDocument>, HashSet<String>)> {
        ...
        let vector_search_query = format!(
            "CALL db.index.vector.queryNodes('{}', {}, $embedding)
             ...
             LIMIT $limit",
            self.index_name,
            limit.saturating_mul(2) // Get more candidates for graph expansion
        );
        ...
        let (documents, entity_set) = self.parse_vector_search_results(&vector_results, metrics);
        Ok((documents, entity_set))
    }

    /// Parse vector search results from Neo4j response
    fn parse_vector_search_results(
        &self,
        vector_results: &Value,
        metrics: &mut SearchMetrics,
    ) -> (Vec<GraphDocument>, HashSet<String>) {
        let mut documents = Vec::new();
        let mut entity_set = HashSet::new();

        if let Some(results) = vector_results.get("results").and_then(|v| v.as_array()) {
            for result in results {
                if let Some(data) = result["data"].as_array() {
                    for row in data {
                        if let Some(row_data) = row["row"].as_array() {
                            if let Some(doc) = self.create_graph_document_from_row(row_data) {
                                entity_set.extend(doc.entities.iter().cloned());
                                documents.push(doc);
                                {
                                    metrics.nodes_examined =
                                        metrics.nodes_examined.saturating_add(1);
                                }
                            }
                        }
                    }
        ...
        (documents, entity_set)
    }

    /// Create `GraphDocument` from database row data
    fn create_graph_document_from_row(&self, row_data: &[Value]) -> Option<GraphDocument> {
        if let (Some(id), Some(content), Some(score)) = (
            row_data.first().and_then(|v| v.as_str()),
            row_data.get(1).and_then(|v| v.as_str()),
            row_data.get(5).and_then(serde_json::Value::as_f64), // Score is now at index 5 due to added embedding field
        ) {
            ...
            if similarity_score >= self.similarity_threshold {
                let metadata = row_data
                    .get(2)
                    .map(Self::extract_metadata)
                    .unwrap_or_default();
                let entities = row_data
                    .get(3)
                    .map(Self::extract_entities)
                    .unwrap_or_default();
                let embedding = row_data
                    .get(4)
                    .map(|v| Self::extract_embedding(v, id))
                    .unwrap_or_default();

                return Some(GraphDocument {
                    id: id.to_string(),
    ...
    /// Extract metadata from database value
    fn extract_metadata(value: &Value) -> HashMap<String, Value> {
        value
            .as_object()
    ...
    async fn enhance_with_graph_context(
        &self,
    ...
    /// Extract embedding from database value
    fn extract_embedding(value: &Value, doc_id: &str) -> Vec<f32> {
        value.as_array().map_or_else(
            || {
                warn!(
                    "Failed to extract embedding for document {}, using empty vector",
                    doc_id
                );
                Vec::new()
            },
    ...
    /// Extract entities from database value
    fn extract_entities(value: &Value) -> Vec<String> {
        value
            .as_array()
            .map(|arr| {
                arr.iter()
    ...
    /// Finalize search results by sorting and limiting
    fn finalize_search_results(documents: &mut Vec<GraphDocument>, limit: usize) {
    ...
    async fn find_related_entities(
        &self,
        initial_entities: &HashSet<String>,
    ) -> Result<Vec<String>> {
        let entities_list: Vec<&str> = initial_entities.iter().map(String::as_str).collect();
        ...
        let graph_query = format!(
            "UNWIND $entities as entity
             ...
             LIMIT {}",
            self.max_graph_hops.saturating_mul(50) // Reasonable limit for related entities
        );
        ...
        if let Some(results) = result.get("results").and_then(|v| v.as_array()) {
            for result in results {
                if let Some(data) = result["data"].as_array() {
                    for row in data {
                        if let Some(row_data) = row["row"].as_array() {
                            if let Some(entity) = row_data.first().and_then(|v| v.as_str()) {
...
impl GraphRetriever {
    ...
    pub async fn add_documents(&self, documents: Vec<RawText>) -> Result<Vec<String>> {
        ...
        for doc in documents {
            let store_result = self.store_single_document(doc).await;
            match store_result {
                Ok(doc_id) => {
    ...
    /// Create query parameters for document creation
    fn create_document_query_params(doc: &RawText) -> HashMap<String, Value> {
        let mut params = HashMap::new();
    ...
    /// Store a single document in the graph
    async fn store_single_document(&self, doc: RawText) -> Result<String> {
        ...
        let query_params = Self::create_document_query_params(&doc);
        let doc_id = doc.id.clone();

        return self
            .execute_document_creation_query(doc_id, query_params)
            .await;
    }

    /// Execute document creation query
    async fn execute_document_creation_query(
        &self,
        doc_id: String,
        ...
        return self
            ...
            .map_err(|e| {
                ...
                Error::Database(format!("Failed to add document: {e}"))
            })
            .map(|_| doc_id);
    }
    ...
    /// # Errors
    ///
    /// Returns error if similarity search query fails or result parsing fails
...
impl From<GraphDocument> for RawText {
...
mod tests {
    use super::*;
    use crate::document::{Metadata, Source};
    use chrono::Utc;
    use serde_json::json;
    ...
    // Helper function to create test RawText
    fn create_test_raw_document() -> RawText {
        RawText {
    ...
    fn test_graph_retriever_config_new_default() {
        ...
        assert!(
            (config.similarity_threshold - 0.7).abs() < f32::EPSILON,
            "Expected similarity_threshold ~0.7, got {}",
    ...
    fn test_graph_retriever_config_high_precision() {
        ...
        assert!(
            (config.similarity_threshold - 0.8).abs() < f32::EPSILON,
            "Expected similarity_threshold ~0.8, got {}",
            config.similarity_threshold
        );
    ...
    fn test_graph_retriever_config_broad_context() {
        ...
        assert!(
            (config.similarity_threshold - 0.6).abs() < f32::EPSILON,
            "Expected similarity_threshold ~0.6, got {}",
            config.similarity_threshold
        );
    ...
    fn test_graph_retriever_config_clone() {
        ...
        assert!(
            (config.similarity_threshold - cloned.similarity_threshold).abs() < f32::EPSILON,
            "Cloned similarity_threshold should match original"
        );
    ...
    fn test_graph_document_serialization() {
        let doc = create_test_graph_document();
        let serialized =
            serde_json::to_string(&doc).expect("Document should serialize successfully");
    ...
    fn test_graph_document_deserialization() {
        ...
        let doc: GraphDocument =
            serde_json::from_str(json_str).expect("JSON should deserialize successfully");
        assert_eq!(doc.id, "test_id");
        assert_eq!(doc.content, "test content");
        assert_eq!(doc.embedding, vec![0.1, 0.2, 0.3]);
    ...
    fn test_from_graph_document_to_raw_text_document() {
        ...
        let raw_doc: RawText = graph_doc.into();
        ...
        assert!(matches!(raw_doc.source, Source::UserInput));
    }
    ...
    fn test_from_graph_document_large_embedding() {
        ...
        let first_embedding = *raw_doc
            .embedding
            .as_ref()
            .expect("Should have embedding")
            .first()
            .expect("Embedding should have at least one element");
        assert!(
            (first_embedding - 0.1).abs() < f32::EPSILON,
            "Expected first embedding ~0.1, got {first_embedding}"
        );
        let last_embedding = *raw_doc
            .embedding
            .as_ref()
            .expect("Should have embedding")
            .get(1535)
            .expect("Embedding should have element at index 1535");
        assert!(
            (last_embedding - 0.1).abs() < f32::EPSILON,
            "Expected last embedding ~0.1, got {last_embedding}"
        );
    }
    ...
    fn test_document_sorting_by_similarity_score() {
        ...
        assert_eq!(docs.first().expect("Should have first doc").id, "doc2"); // 0.9
        assert_eq!(docs.get(1).expect("Should have second doc").id, "doc3"); // 0.7
        assert_eq!(docs.get(2).expect("Should have third doc").id, "doc1"); // 0.5
    }
    ...
    fn test_query_parameter_construction() {
        ...
        assert_eq!(
            params
                .get("embedding")
                .expect("Should have embedding param"),
            &json!([0.1, 0.2, 0.3])
        );
        assert_eq!(
            params.get("limit").expect("Should have limit param"),
            &json!(10)
        );
    }
    ...
    // Helper function to create test vector search response JSON
    fn create_test_vector_search_response() -> Value {
        json!({
            "results": [{
                "data": [{
                    "row": [
    ...
    // Helper function to parse a single vector search row
    fn parse_vector_search_row(
        row_data: &[Value],
        similarity_threshold: f32,
    ) -> Option<(GraphDocument, Vec<String>)> {
        if let (Some(id), Some(content), Some(score)) = (
            row_data.first().and_then(|v| v.as_str()),
            row_data.get(1).and_then(|v| v.as_str()),
            row_data.get(5).and_then(serde_json::Value::as_f64),
        ) {
            #[expect(clippy::cast_possible_truncation)]
            let similarity_score = score as f32;

            if similarity_score >= similarity_threshold {
                let metadata: HashMap<String, Value> = row_data
                    .get(2)
                ...
                let entities: Vec<String> = row_data
                    .get(3)
                ...
                let actual_embedding: Vec<f32> = row_data
                    .get(4)
                    .and_then(|v| v.as_array())
    ...
    fn parse_test_vector_search_results(
        vector_results: &Value,
        similarity_threshold: f32,
    ) -> (Vec<GraphDocument>, HashSet<String>) {
        let mut documents = Vec::new();
        ...
        if let Some(results) = vector_results.get("results").and_then(|v| v.as_array()) {
            for result in results {
                if let Some(data) = result["data"].as_array() {
                    for row in data {
                        if let Some(row_data) = row["row"].as_array() {
                            if let Some((document, entities)) =
                                parse_vector_search_row(row_data, similarity_threshold)
                            {
                                entity_set.extend(entities.iter().cloned());
    ...
    fn test_vector_search_response_parsing_success() {
        let vector_results = create_test_vector_search_response();
        let similarity_threshold = 0.7;

        let (documents, entity_set) =
            parse_test_vector_search_results(&vector_results, similarity_threshold);

        assert_eq!(documents.len(), 1);
        let first_doc = documents.first().expect("Should have first document");
        assert_eq!(first_doc.id, "doc_id_1");
        assert_eq!(first_doc.content, "test content");
        assert_eq!(first_doc.similarity_score, Some(0.85));
        assert_eq!(first_doc.embedding, vec![0.1, 0.2, 0.3]);
                    ...
                    for row in data {
                        if let Some(row_data) = row["row"].as_array() {
                            if let (Some(_id), Some(_content), Some(_score)) = (
                                row_data.first().and_then(|v| v.as_str()),
                                row_data.get(1).and_then(|v| v.as_str()), // This will be None due to null
                                row_data.get(5).and_then(serde_json::Value::as_f64),
                            ) {
                                documents.push(create_test_graph_document());
                            }
    ...
    fn test_vector_search_response_parsing_invalid_embedding() {
        ...
        assert!(documents
            .first()
            .expect("Should have first document")
            .embedding
            .is_empty()); // Should be empty due to invalid embedding
    }
    ...
    fn test_related_entities_response_parsing_success() {
        ...
        assert_eq!(
            related.first().expect("Should have first related entity"),
            "related_entity_1"
        );
        assert_eq!(
            related.get(1).expect("Should have second related entity"),
            "related_entity_2"
        );
        assert_eq!(
            related.get(2).expect("Should have third related entity"),
            "related_entity_3"
        );
    }
    ...
    fn test_related_entities_response_parsing_null_values() {
        ...
        assert_eq!(
            related.first().expect("Should have first valid entity"),
            "valid_entity"
        );
        assert_eq!(
            related.get(1).expect("Should have second valid entity"),
    ...
    fn test_entity_list_conversion() {
        let mut initial_entities = HashSet::new();
    ...
    fn test_entity_list_conversion_empty() {
        let initial_entities: HashSet<String> = HashSet::new();
        assert_eq!(initial_entities.len(), 0);
    }
    ...
    fn test_entity_list_conversion_single_entity() {
        ...
        assert_eq!(
            *entities_list.first().expect("Should have single entity"),
            "single_entity"
        );
    }
    ...
    fn test_vector_index_query_building() {
        ...
        let create_index_query = format!(
            "CREATE VECTOR INDEX IF NOT EXISTS {index_name} FOR (d:Document) ON (d.embedding)
    ...
    fn test_document_id_collection() {
        ...
        assert_eq!(ids.len(), 3);
        assert_eq!(ids.first().expect("Should have first doc"), "doc1");
        assert_eq!(ids.get(1).expect("Should have second doc"), "doc2");
        assert_eq!(ids.get(2).expect("Should have third doc"), "doc3");
    }
    ...
    fn test_document_id_collection_single() {
        ...
        assert_eq!(ids.len(), 1);
        assert_eq!(ids.first().expect("Should have single doc"), "single_doc");
    }
    ...
    fn test_add_documents_parameter_construction() {
        ...
        assert_eq!(
            params.get("id").expect("Should have id param"),
            &json!("test_doc_1")
        );
        assert_eq!(
            params.get("content").expect("Should have content param"),
            &json!("This is test content")
        );
        assert!(params
            .get("created_at")
            .expect("Should have created_at param")
            .as_str()
            .expect("created_at should be string")
            .contains('T'));
        assert_eq!(
            params.get("source").expect("Should have source param"),
            &json!("UserInput")
        );
    }
    ...
    fn test_max_graph_hops_condition_zero() {
        ...
        let entity_set = HashSet::from(["entity1".to_string()]);
    ...
    fn test_max_graph_hops_condition_empty_entities() {
        ...
        let entity_set: HashSet<String> = HashSet::new();
    ...
    fn test_relationships_assignment() {
        ...
        assert_eq!(
            documents
                .get(1)
                .expect("Should have second document")
                .relationships
                .len(),
            3
        );
        assert_eq!(
            documents
                ...
                .relationships
                .first()
                .expect("Should have first relationship"),
            "rel1"
        );
        assert_eq!(
            documents
                ...
                .relationships
                .get(2)
                .expect("Should have third relationship"),
            "rel3"
        );
    }
