...
static ETH_ADDRESS_REGEX: LazyLock<Result<Regex, regex::Error>> =
    LazyLock::new(|| Regex::new(r"0x[a-fA-F0-9]{40}\b"));

/// Solana address regex pattern (Base58 without 0, O, I, l)
static SOL_ADDRESS_REGEX: LazyLock<Result<Regex, regex::Error>> = LazyLock::new(|| {
...
static AMOUNT_REGEX: LazyLock<Result<Regex, regex::Error>> =
    LazyLock::new(|| Regex::new(r"\$?[0-9]+(?:[.,][0-9]+)*[KMBkmb]?(?:\s+[A-Z]{2,10})?"));
...
impl Extractor {
    ...
    fn classify_amount_type(amount_text: &str, context: &str) -> AmountType {
        ...
        if context_lower.contains("fee") || context_lower.contains("gas") {
            return AmountType::Fee;
        }
        if context_lower.contains("volume") || context_lower.contains("trading") {
            return AmountType::Volume;
        }
        if context_lower.contains("market cap") || context_lower.contains("mcap") {
            return AmountType::MarketCap;
        }
        if context_lower.contains("price")
    ...
    fn extract_amounts(text: &str) -> Vec<AmountMention> {
        let mut amounts = Vec::new();

        if let Ok(regex) = AMOUNT_REGEX.as_ref() {
    ...
    fn extract_chains(&self, text: &str) -> Vec<EntityMention> {
        ...
        for (canonical_name, patterns) in &self.chains {
    ...
    fn extract_protocols(&self, text: &str) -> Vec<EntityMention> {
        ...
        for (canonical_name, patterns) in &self.protocols {
            for pattern in patterns {
                let positions = Self::find_pattern_positions(text, pattern);
                for (start, end) in positions {
                    if seen.insert(canonical_name.clone()) {
    ...
    fn extract_tokens(&self, text: &str) -> Vec<EntityMention> {
        ...
        for (canonical_name, patterns) in &self.tokens {
            for pattern in patterns {
                let positions = Self::find_pattern_positions(text, pattern);
                for (start, end) in positions {
                    if seen.insert(canonical_name.clone()) {
    ...
    fn extract_wallet_addresses(text: &str) -> Vec<EntityMention> {
        let mut wallets = Vec::new();

        // Extract Ethereum addresses
        if let Ok(regex) = ETH_ADDRESS_REGEX.as_ref() {
            for mat in regex.find_iter(text) {
        ...
        if let Ok(regex) = SOL_ADDRESS_REGEX.as_ref() {
            for mat in regex.find_iter(text) {
                let address = mat.as_str().to_string();
    ...
    fn find_pattern_positions(text: &str, pattern: &str) -> Vec<(usize, usize)> {
        ...
        while let Some(pos) = text_lower[start..].find(&pattern_lower) {
            ...
            let actual_end = actual_start.saturating_add(pattern.len());
    ...
    fn is_likely_solana_address(address: &str) -> bool {
    ...
    fn parse_amount_match(text: &str) -> (String, Option<String>) {
        // Check if there's a space separating value and unit
        let parts: Vec<&str> = text.split_whitespace().collect();
        if parts.len() >= 2 {
            return (
                parts.first().map_or_else(String::new, |s| (*s).to_string()),
    ...
    fn parse_numeric_value(value_str: &str) -> Option<f64> {
        let cleaned = value_str.replace(['$', ','], "");

        cleaned.chars().last().and_then(|last_char| {
            let (num_part, multiplier) = match last_char {
                'K' | 'k' => (&cleaned[..cleaned.len().saturating_sub(1)], 1000.0),
                'M' | 'm' => (&cleaned[..cleaned.len().saturating_sub(1)], 1_000_000.0),
                'B' | 'b' => (&cleaned[..cleaned.len().saturating_sub(1)], 1_000_000_000.0),
                _ => (cleaned.as_str(), 1.0),
            };

            num_part
                .parse::<f64>()
                .map_or(None, |base_value| Some(base_value * multiplier))
        })
    }
}
...
mod tests {
    ...
    fn test_default_creates_extractor_with_patterns() {
        ...
        assert!(!extractor.protocols.is_empty());
        assert!(!extractor.tokens.is_empty());
        assert!(!extractor.chains.is_empty());
    }
    ...
    fn test_initialize_patterns_populates_all_maps() {
        ...
        assert!(extractor.protocols.contains_key("uniswap"));
        assert!(extractor.protocols.contains_key("aave"));
        assert!(extractor.protocols.contains_key("compound"));
        assert!(extractor.protocols.contains_key("jupiter"));
        assert!(extractor.protocols.contains_key("solend"));

        assert!(extractor.tokens.contains_key("ethereum"));
        assert!(extractor.tokens.contains_key("bitcoin"));
        assert!(extractor.tokens.contains_key("usdc"));
        assert!(extractor.tokens.contains_key("usdt"));
        assert!(extractor.tokens.contains_key("solana"));

        assert!(extractor.chains.contains_key("ethereum"));
        assert!(extractor.chains.contains_key("solana"));
        assert!(extractor.chains.contains_key("polygon"));
        assert!(extractor.chains.contains_key("arbitrum"));
    }
    ...
    fn test_extract_with_valid_ethereum_address() {
        ...
        let wallet = result.wallets.first().expect("Should have first wallet");
        ...
        {
            assert!(
                (wallet.confidence - 0.95).abs() < f32::EPSILON,
    ...
    fn test_extract_with_valid_solana_address() {
        ...
        {
            assert!(
                (wallet.confidence - 0.85).abs() < f32::EPSILON,
    ...
    fn test_extract_tokens_finds_known_tokens() {
        ...
        for token in &result.tokens {
            ...
            {
                assert!(
                    (token.confidence - 0.90).abs() < f32::EPSILON,
    ...
    fn test_extract_protocols_finds_known_protocols() {
        ...
        for protocol in &result.protocols {
            ...
            {
                assert!(
                    (protocol.confidence - 0.88).abs() < f32::EPSILON,
                    "Expected confidence ~0.88, got {}",
                    protocol.confidence
                );
            }
            assert_eq!(
    ...
    fn test_extract_chains_finds_known_chains() {
        ...
        for chain in &result.chains {
            ...
            {
                assert!(
                    (chain.confidence - 0.92).abs() < f32::EPSILON,
                    "Expected confidence ~0.92, got {}",
                    chain.confidence
                );
            }
    ...
    fn test_is_likely_solana_address_valid_cases() {
        ...
        assert!(Extractor::is_likely_solana_address(
            "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM"
        ));
        assert!(Extractor::is_likely_solana_address(
            "SoLEao8wTzSfqhuou8dpYXGcoyGVDEBWwgEBpAEpDWe"
        ));

        // Edge cases - minimum length
        assert!(Extractor::is_likely_solana_address(
            "ABCDEFGHJKMNPQRSTUVWXYZabcdefghjk"
        ));

        // Maximum length (44 chars)
        assert!(Extractor::is_likely_solana_address(
            "ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuv"
        ));
    }
    ...
    fn test_is_likely_solana_address_invalid_cases() {
        ...
        assert!(!Extractor::is_likely_solana_address(
            "ABCDEFGHJKMNPQRSTUVWXYZabcdefgh"
        )); // 31 chars, too short

        // Too long (45 chars)
        assert!(!Extractor::is_likely_solana_address(
            "ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxy"
        ));
        ...
        assert!(!Extractor::is_likely_solana_address(
            "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYt0WWM"
        ));
        assert!(!Extractor::is_likely_solana_address(
            "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVLOzYtAWWM"
        ));
        assert!(!Extractor::is_likely_solana_address(
            "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVLIzYtAWWM"
        ));
        assert!(!Extractor::is_likely_solana_address(
            "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVLlzYtAWWM"
        ));

        // Non-alphanumeric characters
        assert!(!Extractor::is_likely_solana_address(
            "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL-zYtAWWM"
        ));
    }
    ...
    fn test_find_pattern_positions_multiple_matches() {
        ...
        assert_eq!(
            positions.first().expect("Should have first position"),
            &(0, 3)
        );
        assert_eq!(
            positions.get(1).expect("Should have second position"),
            &(17, 20)
        );
        assert_eq!(
            positions.get(2).expect("Should have third position"),
            &(30, 33)
        );
    }
    ...
    fn test_parse_numeric_value_with_b_suffix() {
        ...
        assert_eq!(Extractor::parse_numeric_value("1b"), Some(1_000_000_000.0));
        assert_eq!(
            Extractor::parse_numeric_value("1.2B"),
    ...
    fn test_find_nearby_entity_found_in_first_list() {
        ...
        assert_eq!(
            result.expect("Should find nearby entity").canonical,
            "ethereum"
        );
    }
    ...
    fn test_find_nearby_entity_found_in_alt_lists() {
        ...
        assert_eq!(
            result
                .expect("Should find nearby entity in alt list")
                .canonical,
            "usdc"
        );
    }
    ...
    fn test_extract_wallet_addresses_no_addresses() {
        let wallets =
            Extractor::extract_wallet_addresses("Just some random text without addresses");
        assert!(wallets.is_empty());
    }
    ...
    fn test_extract_wallet_addresses_mixed_addresses() {
        ...
        let eth_wallet = wallets
            ...
            .expect("Should find Ethereum wallet address");
        assert_eq!(
        ...
        let sol_wallet = wallets
            .iter()
            .find(|w| !w.canonical.starts_with("0x"))
            .expect("Should find Solana wallet address");
        assert_eq!(
    ...
    fn test_extract_tokens_duplicate_prevention() {
        ...
        assert_eq!(
            tokens.first().expect("Should have first token").canonical,
            "ethereum"
        );
    }
    ...
    fn test_extract_protocols_duplicate_prevention() {
        ...
        assert_eq!(
            protocols
                .first()
                .expect("Should have first protocol")
                .canonical,
            "uniswap"
        );
    }
    ...
    fn test_extract_chains_duplicate_prevention() {
        ...
        assert_eq!(
            chains.first().expect("Should have first chain").canonical,
            "ethereum"
        );
    }
    ...
    fn test_static_regex_patterns() {
        ...
        if let Ok(regex) = ETH_ADDRESS_REGEX.as_ref() {
            assert!(regex.is_match("0x742d35cc6645c0532eb5d65c8092a7b8ab27d1de"));
            assert!(!regex.is_match("0x742d35cc6645c0532eb5d65c8092a7b8ab27d1def")); // Too long
            assert!(!regex.is_match("742d35cc6645c0532eb5d65c8092a7b8ab27d1de"));
            // Missing 0x
        }

        if let Ok(regex) = SOL_ADDRESS_REGEX.as_ref() {
            assert!(regex.is_match("9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM"));
            assert!(!regex.is_match("0x742d35cc6645c0532eb5d65c8092a7b8ab27d1de"));
            // Ethereum format
        }

        if let Ok(regex) = AMOUNT_REGEX.as_ref() {
            assert!(regex.is_match("123.45"));
            assert!(regex.is_match("$1,234.56"));
            assert!(regex.is_match("1K ETH"));
            assert!(regex.is_match("1.2B"));
        }

        // Test removed since TX_HASH_REGEX was removed as unused
    }
    ...
    fn test_debug_implementation() {
        ...
        assert!(debug_str.contains("Extractor"));
    }
