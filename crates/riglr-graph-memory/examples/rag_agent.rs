...
async fn main() -> anyhow::Result<()> {
    ...
    build_transaction_knowledge_graph(&memory).await?;
...
async fn build_transaction_knowledge_graph(memory: &GraphMemory) -> anyhow::Result<()> {
    ...
    for (idx, tx) in transactions.iter().enumerate() {
        ...
        doc.source = Source::OnChain {
            ...
            transaction_hash: format!("0x{:064x}", idx.saturating_add(1)),
        };
            ...
            Ok(docs) => {
                if docs.documents.is_empty() {
                    ...
                    for (idx, doc) in docs.documents.iter().take(3).enumerate() {
                        println!("   {}. {}", idx.saturating_add(1), doc.content);
                    }
                    println!();
                }
            }
            Err(e) => {
                println!("A: Error retrieving information: {e}\n");
            }
        }
    }
        ...
        Ok(history) => {
            ...
            for doc in &history.documents {
                ...
                if doc.content.contains("liquidity") {
                    liquidity = liquidity.saturating_add(1);
                }
                if doc.content.contains("staked") {
                    staking = staking.saturating_add(1);
                }
            }
...
async fn simulate_rag_agent(memory: &GraphMemory) -> anyhow::Result<()> {
    ...
    for query in queries {
        ...
        match search_result {
            ...
            Err(e) => {
                println!("Agent: I encountered an error retrieving information: {e}\n");
            }
        }
    }
...
fn generate_rag_response(query: &str, context_docs: &[GraphDocument]) -> String {
    ...
    if query.contains("total") || query.contains("activity") {
        ...
        let mut protocol_counts: HashMap<&str, u32> = HashMap::new();
        for doc in context_docs {
            for protocol in ["Uniswap", "Aave", "Lido", "Compound"] {
                if doc.content.contains(protocol) {
                    let count = protocol_counts.entry(protocol).or_insert(0);
        ...
        let summary: Vec<String> = context_docs
            ...
            .take(3)
            .enumerate()
            .map(|(i, doc)| format!("{}. {}", i.saturating_add(1), doc.content))
            .collect();
