...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // For this demonstration, we'll show the basic workflow using the GraphVectorStore
    {
        ...
        let neo4j_url = env::var(NEO4J_URL).unwrap_or_else(|_| "bolt://localhost:7687".to_string());
        let _neo4j_user = env::var(NEO4J_USER).unwrap_or_else(|_| "neo4j".to_string());
        let _neo4j_password = env::var(NEO4J_PASSWORD).unwrap_or_else(|_| "password".to_string());
...
fn create_mock_neo4j_client() -> Arc<riglr_graph_memory::Neo4jClient> {
...
fn generate_mock_embedding() -> Vec<f32> {
    ...
    // Precision loss acceptable for mock embeddings
