...
use riglr_graph_memory::document::Metadata;
use riglr_graph_memory::graph::MemoryConfig;
...
impl RagAgent {
    async fn new(name: &str) -> Result<Self> {
        ...
        let neo4j_password =
            env::var(NEO4J_PASSWORD).unwrap_or_else(|_| "test_password".to_string());
        ...
        let memory = match GraphMemory::new(config).await {
            Ok(mem) => mem,
            Err(e) => {
                println!("Warning: Could not connect to Neo4j: {e}. Skipping test.");
                return Err(e.into());
            }
        };
...
async fn test_5_1_rag_agent_workflow() -> Result<()> {
    ...
    let store_result1 = agent.execute_task(store_task1).await?;
    assert!(
        store_result1.is_success(),
        "Should successfully store knowledge"
    );

    let node_id1 = store_result1
        .data()
    ...
    let store_result2 = agent.execute_task(store_task2).await?;
    assert!(
        store_result2.is_success(),
        "Should successfully store knowledge"
    );

    let node_id2 = store_result2
        .data()
        .and_then(|d| d["stored_id"].as_str())
        .unwrap_or("");
    ...
    assert!(
        retrieve_result.is_success(),
        "Should successfully retrieve knowledge"
    );

    let results = retrieve_result.data().and_then(|d| d["results"].as_array());
    if let Some(results_array) = results {
        ...
        println!("Retrieved results: {:?}", retrieve_result.data());
    }
...
async fn test_5_2_knowledge_graph_traversal() -> Result<()> {
    ...
    let neo4j_url = env::var(NEO4J_URL).unwrap_or_else(|_| "http://localhost:7474".to_string());
    let neo4j_user = env::var(NEO4J_USER).unwrap_or_else(|_| "neo4j".to_string());
    let neo4j_password = env::var(NEO4J_PASSWORD).unwrap_or_else(|_| "test_password".to_string());
...
async fn test_5_3_memory_context_switching() -> Result<()> {
    ...
    let agent1_retrieve_result = agent1.execute_task(retrieve_task.clone()).await?;
    let agent2_retrieve_result = agent2.execute_task(retrieve_task).await?;

    // Verify context isolation
    let agent1_results = agent1_retrieve_result
        .data()
        .and_then(|d| d["results"].as_array());
    let agent2_results = agent2_retrieve_result
        .data()
        .and_then(|d| d["results"].as_array());
    ...
    // Phase 2: Create new agent and retrieve
    {
        ...
        if let Some(results_array) = results {
            let found = results_array.iter().any(|item| {
                item.get("content")
                    .and_then(|c| c.as_str())
                    .is_some_and(|s| s.contains("persistent") || s.contains(&test_id))
            });
