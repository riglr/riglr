...
use riglr_core::util::{BucketStrategy, RateLimitStrategyType, RateLimiter};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for i in 1..=7 {
        ...
        match result {
            ...
            Err(e) => println!("Request {i} blocked: {e}"),
        }
    }
    ...
    let burst_strategy = BucketStrategy::with_burst(
    ...
    for i in 1..=20 {
        ...
        match result {
            Ok(()) => println!("Burst request {i} allowed"),
            Err(e) => println!("Burst request {i} blocked: {e}"),
        }
    }
