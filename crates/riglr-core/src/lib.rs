...
//! - **[`Queue`]** - Distributed job processing with Redis backend
...
//!
//! // Register your tool
//! worker.register_tool(Arc::new(greeting_tool));
//!
...
/// This module organizes all the public exports in a structured way while maintaining API compatibility.
pub mod exports {
    // Re-export configuration types from riglr-config
    pub use riglr_config::{
        AppConfig, Config, DatabaseConfig, Environment, FeaturesConfig, NetworkConfig,
        ProvidersConfig,
    };

    pub use crate::error::{CoreError, ToolError};
    pub use crate::idempotency::*;
    pub use crate::jobs::*;
    pub use crate::queue::*;
    pub use crate::signer::SignerError;
    pub use crate::signer::{SignerContext, UnifiedSigner};
    pub use crate::tool::*;
}

// Maintain backwards compatibility by re-exporting at crate root
pub use exports::*;
...
mod tests {
    ...
    #[tokio::test]
    async fn job_creation() -> anyhow::Result<()> {
    ...
    #[tokio::test]
    async fn job_result_success() -> anyhow::Result<()> {
    ...
    #[tokio::test]
    async fn tool_worker_creation() {
        let _worker = tool::Worker::<InMemoryIdempotencyStore>::new(
            ExecutionConfig::default(),
    ...
    #[tokio::test]
    async fn tool_registration_and_execution() -> anyhow::Result<()> {
        let worker = tool::Worker::<InMemoryIdempotencyStore>::new(
            ExecutionConfig::default(),
            provider::ApplicationContext::default(),
        );
        ...
        match result.expect("Process job should succeed") {
            ...
            JobResult::Failure { .. } => {
                panic!("Expected failure job result")
            }
        }
    ...
    fn error_conversions() {
        ...
        let string_error = "Test string error".to_owned();
    ...
    async fn execution_config() {
        ...
        assert!(config.default_timeout > Duration::from_millis(0));
        assert!(config.max_concurrency > 0);
        assert!(config.initial_retry_delay > Duration::from_millis(0));
    }

    #[tokio::test]
    async fn idempotency_store() -> anyhow::Result<()> {
        ...
        store
            .set(key, Arc::new(job_result), Duration::from_secs(60))
            .await?;
    ...
    #[test]
    fn job_result_success_with_tx_hash() -> anyhow::Result<()> {
        let result = JobResult::success_with_tx(&"test result", "0x123abc")?;

        match result {
            JobResult::Success { value, tx_hash } => {
                assert_eq!(value, serde_json::json!("test result"));
                assert_eq!(tx_hash, Some("0x123abc".to_owned()));
            }
            JobResult::Failure { .. } => {
                panic!("Expected success result with tx hash")
            }
        }
        Ok(())
    }
    ...
    #[test]
    fn job_result_permanent_failure() {
        let result = JobResult::Failure {
            error: ToolError::permanent_string("invalid parameters"),
        };
        ...
        match result {
            ...
            JobResult::Success { .. } => {
                panic!("Expected permanent failure result")
            }
        }
    }

    #[test]
    fn job_retry_logic() -> anyhow::Result<()> {
    ...
    #[test]
    fn job_new_idempotent() -> anyhow::Result<()> {
    ...
    #[test]
    fn job_new_idempotent_without_key() -> anyhow::Result<()> {
    ...
    fn core_error_display() {
        let queue_error = CoreError::Queue("Failed to connect".to_owned());
        assert!(queue_error
            .to_string()
            .contains("Queue error: Failed to connect"));

        let job_error = CoreError::JobExecution("Timeout occurred".to_owned());
        assert!(job_error
            .to_string()
            .contains("Job execution error: Timeout occurred"));

        let generic_error = CoreError::Generic("Something went wrong".to_owned());
    ...
    fn tool_error_from_str() {
        ...
        assert!(!tool_error.is_retriable());
        assert!(!tool_error.is_rate_limited());
        assert!(tool_error.contains("Test str error"));
    }
    ...
    fn tool_error_from_string() {
        ...
        assert!(!tool_error.is_retriable());
        assert!(!tool_error.is_rate_limited());
        assert!(tool_error.contains("Test string error"));
    }

    #[test]
    fn execution_config_customization() {
    ...
    #[tokio::test]
    async fn tool_worker_with_non_existent_tool() -> anyhow::Result<()> {
        let worker = tool::Worker::<InMemoryIdempotencyStore>::new(
            ExecutionConfig::default(),
            provider::ApplicationContext::default(),
        );
        ...
        let error = result.expect_err("Should fail for non-existent tool");
    ...
    #[tokio::test]
    async fn job_with_empty_parameters() -> anyhow::Result<()> {
    ...
    #[tokio::test]
    async fn job_with_null_parameters() -> anyhow::Result<()> {
    ...
    #[test]
    fn job_retry_boundary_conditions() -> anyhow::Result<()> {
    ...
    #[tokio::test]
    async fn mock_tool_with_empty_message() -> anyhow::Result<()> {
        let tool = MockTool {
            name: "test_tool".to_owned(),
            should_fail: false,
        };
        ...
        match result {
            JobResult::Success { value, .. } => {
                ...
                assert!(value
                    .as_str()
                    .expect("Value should be a string")
                    .contains("Hello")); // Default message
            }
            JobResult::Failure { .. } => {
                panic!("Both tools should succeed")
            }
        }
    ...
    #[test]
    fn job_result_serialization() -> anyhow::Result<()> {
        ...
        match deserialized {
            ...
            JobResult::Failure { .. } => {
                panic!("Expected success result after deserialization")
            }
        }
    ...
    #[test]
    fn job_result_failure_serialization() -> anyhow::Result<()> {
        ...
        match deserialized {
            ...
            JobResult::Success { .. } => {
                panic!("Expected failure result after deserialization")
            }
        }
    ...
    #[tokio::test]
    async fn idempotency_store_expiration() -> anyhow::Result<()> {
        let store = InMemoryIdempotencyStore::new();
        ...
        store
            .set(key, Arc::new(job_result), Duration::from_millis(1))
            .await?;
