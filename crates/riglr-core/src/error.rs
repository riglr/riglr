...
pub trait Signer: StdError + Send + Sync {
    fn is_retriable(&self) -> bool;
    fn message(&self) -> String;
    fn kind(&self) -> SignerErrorKind;
}
...
impl Signer for StandardSigner {
    ...
    fn kind(&self) -> SignerErrorKind {
        match *self {
            Self::Configuration(_) => SignerErrorKind::Configuration,
            Self::Generic(_) => SignerErrorKind::Generic,
            Self::InvalidInput(_) => SignerErrorKind::InvalidInput,
            Self::Network(_) => SignerErrorKind::Network,
            Self::NoContext => SignerErrorKind::NoContext,
            Self::SigningFailed(_) => SignerErrorKind::SigningFailed,
            Self::UnsupportedOperation(_) => SignerErrorKind::UnsupportedOperation,
        }
    }
}
...
impl PartialEq for ToolError {
    ...
    fn eq(&self, other: &Self) -> bool {
        use ToolError::{InvalidInput, Permanent, RateLimited, Retriable, SignerContext};
        match (self, other) {
            (
                &RateLimited {
                    source_message: ref source_msg1,
                    context: ref context1,
                    retry_after: retry1,
                    ..
                },
                &RateLimited {
                    source_message: ref source_msg2,
                    context: ref context2,
                    retry_after: retry2,
                    ..
                },
            ) => source_msg1 == source_msg2 && context1 == context2 && retry1 == retry2,
            (
                &Retriable {
                    source_message: ref source_msg1,
                    context: ref context1,
            ...
            ) => source_msg1 == source_msg2 && context1 == context2,
            (SignerContext(msg1), SignerContext(msg2)) => msg1 == msg2,
            _ => false,
        }
    }
}
...
impl ToolError {
    ...
    #[must_use]
    #[inline]
    pub fn invalid_input_with_source<E: StdError + Send + Sync + 'static, C: Into<String>>(
        source: E,
        context: C,
    ) -> Self {
    ...
    #[must_use]
    #[inline]
    pub fn permanent_from_error<E: StdError + Send + Sync + 'static, C: Into<String>>(
        source: E,
        context: C,
    ) -> Self {
    ...
    #[must_use]
    #[inline]
    pub fn permanent_with_source<E: StdError + Send + Sync + 'static, C: Into<String>>(
        source: E,
        context: C,
    ) -> Self {
        let source_message = source.to_string();
    ...
    #[must_use]
    #[inline]
    pub fn rate_limited_with_source<E: StdError + Send + Sync + 'static, C: Into<String>>(
        source: E,
        context: C,
        retry_after: Option<Duration>,
    ) -> Self {
    ...
    #[must_use]
    #[inline]
    pub fn retriable_from_error<E: StdError + Send + Sync + 'static, C: Into<String>>(
        source: E,
        context: C,
    ) -> Self {
    ...
    #[must_use]
    #[inline]
    pub fn retriable_with_source<E: StdError + Send + Sync + 'static, C: Into<String>>(
        source: E,
        context: C,
    ) -> Self {
        let source_message = source.to_string();
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            Self::Retriable {
                ...
                write!(
                    formatter,
                    "Operation can be retried: {context} - {source_message}"
                )
            }
            Self::RateLimited {
                ref context,
                ref source_message,
                ..
            } => write!(formatter, "Rate limited: {context} - {source_message}"),
            Self::Permanent {
                ref context,
                ref source_message,
                ..
            } => write!(formatter, "Permanent error: {context} - {source_message}"),
            Self::InvalidInput {
                ref context,
                ref source_message,
                ..
            } => write!(formatter, "Invalid input: {context} - {source_message}"),
            Self::SignerContext(ref msg) => {
                write!(formatter, "Signer context error: {msg}")
            }
        }
    }
}
...
impl StdError for ToolError {
    ...
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match *self {
            Self::Retriable { ref source, .. }
            | Self::RateLimited { ref source, .. }
            | Self::Permanent { ref source, .. }
            | Self::InvalidInput { ref source, .. } => source
                .as_ref()
                .map(|error_source| error_source.as_ref() as &(dyn StdError + 'static)),
            Self::SignerContext(_) => None,
        }
    }
}
...
impl From<Box<dyn Signer>> for ToolError {
    #[inline]
    fn from(err: Box<dyn Signer>) -> Self {
...
mod tests {
    ...
    fn test_error_source_preservation() {
        ...
        let source = tool_error
            .source()
            .expect("Expected source to be present in test");
    ...
    fn test_core_error_serialization_variant() {
        ...
        let json_error = serde_json::from_str::<serde_json::Value>("invalid json")
            .expect_err("Expected JSON parsing to fail with invalid input");
    ...
    fn test_result_type_alias_ok() {
        ...
        match result {
            ...
            Err(error) => panic!("Expected Ok but got Err: {error}"),
        }
    }
    ...
    fn test_result_type_alias_err() {
        let expected_error = CoreError::Generic("Test error".to_string());
        let result: Result<i32> = Err(expected_error);
        assert!(result.is_err());
        match result {
            Err(error) => assert_eq!(error.to_string(), "Core error: Test error"),
    ...
    fn test_tool_error_signer_context_variant() {
        let signer_error = StandardSigner::NoContext;
    ...
    fn test_tool_error_from_signer_error() {
        let signer_error = StandardSigner::Configuration("Invalid config".to_string());
        let tool_error = ToolError::from(Box::new(signer_error) as Box<dyn Signer>);

        assert!(
            matches!(tool_error, ToolError::SignerContext(ref inner) if inner.to_string().contains("Signer configuration error: Invalid config"))
        );
    }
    ...
    fn test_worker_error_execution_timeout_various_durations() {
        // Test zero duration
        let timeout_zero = Duration::from_secs(0);
        ...
        // Test milliseconds
        let timeout_millis = Duration::from_millis(500);
        ...
        // Test minutes
        let timeout_minutes = Duration::from_secs(120);
    ...
    fn test_rate_limited_retry_after_edge_cases() {
        ...
        assert_eq!(error1.retry_after(), Some(Duration::from_secs(0)));
        ...
        assert_eq!(error2.retry_after(), Some(Duration::from_secs(u64::MAX)));
    }
    ...
    fn test_is_retriable_all_variants() {
        ...
        let signer_context = ToolError::SignerContext(StandardSigner::NoContext.to_string());
