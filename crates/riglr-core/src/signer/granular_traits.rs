...
pub trait EvmSigner: SignerBase {
    ...
    /// Signs a message with the EVM private key.
    ///
    /// # Parameters
    /// * `message` - The message to sign as a byte slice
    ///
    /// # Returns
    /// * `Ok(String)` - The signature as a hex-encoded string
    /// * `Err(SignerError)` - Error details on failure
