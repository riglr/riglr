...
// Test code uses expect() with descriptive error messages following established project patterns
// Test code uses assert!(false, msg) pattern following established project conventions
...
fn test_job_result_failure_clone_preserves_source() {
    ...
    match (job_result, cloned_result) {
        (JobResult::Failure { error: original }, JobResult::Failure { error: cloned }) => {
            ...
            assert_eq!(
                original_source
                    .expect("Original should have source")
                    .to_string(),
                cloned_source.expect("Clone should have source").to_string(),
...
fn test_all_tool_error_variants_preserve_source_on_clone() {
    ...
    assert_eq!(
        retriable
            .source()
            .expect("Retriable should have source")
            .to_string(),
        retriable_clone
            .source()
            .expect("Retriable clone should have source")
            .to_string()
    );
    assert_eq!(
        rate_limited
            .source()
            .expect("Rate limited should have source")
            .to_string(),
        rate_limited_clone
            .source()
            .expect("Rate limited clone should have source")
            .to_string()
    );
    assert_eq!(
        permanent
            .source()
            .expect("Permanent should have source")
            .to_string(),
        permanent_clone
            .source()
            .expect("Permanent clone should have source")
            .to_string()
    );
    assert_eq!(
        invalid_input
            .source()
            .expect("Invalid input should have source")
            .to_string(),
        invalid_input_clone
            .source()
            .expect("Invalid input clone should have source")
            .to_string()
    );
}
...
fn test_arc_sharing_semantics() {
    ...
    match (&tool_error, &cloned_error) {
        (
            &ToolError::Permanent {
                source: Some(ref original_arc),
                ..
            },
            &ToolError::Permanent {
                source: Some(ref cloned_arc),
                ..
            },
        ) => {
            ...
            let original_ptr = Arc::as_ptr(original_arc).cast::<()>();
            let cloned_ptr = Arc::as_ptr(cloned_arc).cast::<()>();
            assert_eq!(
...
fn test_job_result_clone_is_exhaustive() {
    ...
    let errors = vec![
        ...
        ToolError::Retriable {
            source: Some(Arc::new(test_error.clone())),
        ...
        ToolError::InvalidInput {
            source: Some(Arc::new(test_error)),
            source_message: "test".to_string(),
            context: "test".to_string(),
        },
    ...
    for error in errors {
        ...
        let cloned = job_result.clone(); // This will fail to compile if Clone doesn't handle a variant

        // Verify the clone works correctly
        match (job_result, cloned) {
