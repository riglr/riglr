...
pub trait AlertHandler: Send + Sync {
    ...
    ///
    /// # Errors
    /// Returns an error if the alert handler fails to process the alert
...
impl AlertHandler for ConsoleAlertHandler {
    fn handle_alert(&self, alert: &YieldAlert) -> Result<()> {
        let urgency_icon = Self::get_urgency_icon(alert.urgency);
        Self::log_alert_header(urgency_icon, &alert.message);
        Self::log_opportunity_details(&alert.opportunity);
        Ok(())
    }
}

impl ConsoleAlertHandler {
    const fn get_urgency_icon(urgency: AlertUrgency) -> &'static str {
        match urgency {
            AlertUrgency::Low => "📢",
    ...
    fn log_alert_header(urgency_icon: &str, message: &str) {
        info!("{} YIELD ALERT: {}", urgency_icon, message);
    }

    fn log_opportunity_details(opportunity: &YieldOpportunity) {
        Self::log_protocol_info(&opportunity.protocol);
        Self::log_pool_info(&opportunity.token_pair);
        Self::log_financial_metrics(opportunity.apy, opportunity.tvl);
        Self::log_risk_info(&opportunity.impermanent_loss_risk);
    }

    fn log_protocol_info(protocol: &Protocol) {
        info!(
            "   Protocol: {} ({:?})",
    ...
    fn log_pool_info(token_pair: &(String, String)) {
        info!("   Pool: {} / {}", token_pair.0, token_pair.1);
    }

    fn log_financial_metrics(apy: f64, tvl: f64) {
        info!("   APY: {:.2}%", apy);
        info!("   TVL: ${:.0}", tvl);
    }

    fn log_risk_info(il_risk: &ImpermanentLossRisk) {
        info!("   IL Risk: {:?}", il_risk);
    }
}
...
impl AlertSystem {
    ...
    pub fn send_alert(&mut self, alert: &YieldAlert) -> Result<()> {
...
impl YieldMonitor {
    ...
    /// Process a pool event and update yield opportunities
    ///
    /// # Errors
    ...
    pub async fn process_pool_event(&mut self, event: &dyn Event) -> Result<()> {
        ...
        if let Some(pool_data) = Self::extract_pool_data(event) {
            ...
            let opportunity = self.calculate_yield_opportunity(pool_data);
        ...
        let processing_time = start_time.elapsed().as_millis() as f64;
        {
            ...
            metrics.avg_processing_time_ms = metrics
                ...
                .mul_add(total_events - 1.0, processing_time)
                / total_events;
        }
    ...
    /// Process a price event and update impermanent loss calculations
    ///
    /// # Errors
    /// Returns an error if price processing fails
    ...
    fn extract_pool_data(event: &dyn Event) -> Option<PoolEventData> {
    ...
    fn parse_raydium_event(data: &serde_json::Value) -> Option<PoolEventData> {
    ...
    fn parse_orca_event(data: &serde_json::Value) -> Option<PoolEventData> {
    ...
    const fn parse_jupiter_event(_data: &serde_json::Value) -> Option<PoolEventData> {
        // Jupiter is primarily an aggregator, so we might track route efficiency
        None
    }

    fn parse_marinade_event(data: &serde_json::Value) -> Option<PoolEventData> {
    ...
    fn calculate_yield_opportunity(&self, pool_data: PoolEventData) -> YieldOpportunity {
        // Get protocol info
        let protocol = self
            .protocols
        ...
        let reward_apy = Self::calculate_reward_apy(&pool_data);
        let total_apy = fee_apy + reward_apy;

        // Estimate impermanent loss risk
        let il_risk = Self::estimate_impermanent_loss_risk(&pool_data.token_a, &pool_data.token_b);
    ...
    fn calculate_reward_apy(pool_data: &PoolEventData) -> f64 {
        ...
        // Simulate reward APY
        match pool_data.protocol_name.as_str() {
    ...
    fn estimate_impermanent_loss_risk(token_a: &str, token_b: &str) -> ImpermanentLossRisk {
    ...
    async fn update_impermanent_loss_estimates(&self, price_history: &[PriceData]) -> Result<()> {
        ...
        for entry in self.opportunities.iter() {
            ...
            let first_token_prices: Vec<_> = price_history
                .iter()
                .filter(|p| p.token == opportunity.token_pair.0)
                .collect();

            let second_token_prices: Vec<_> = price_history
                .iter()
                .filter(|p| p.token == opportunity.token_pair.1)
                .collect();

            if first_token_prices.len() >= 2 && second_token_prices.len() >= 2 {
                // Calculate impermanent loss based on price ratio changes
                let initial_ratio = first_token_prices.first().map_or(0.0, |p| p.price)
                    / second_token_prices.first().map_or(1.0, |p| p.price);
                let current_ratio = {
                    let first_token_last = first_token_prices.last().map_or(0.0, |p| p.price);
                    let second_token_last = second_token_prices.last().map_or(1.0, |p| p.price);
                    if second_token_last == 0.0 {
                        0.0
                    } else {
                        first_token_last / second_token_last
                    }
                };
    ...
    fn calculate_impermanent_loss(initial_ratio: f64, current_ratio: f64) -> f64 {
        // Standard impermanent loss formula for 50/50 pools
        let ratio_change = current_ratio / initial_ratio;
        let il = 2.0f64.mul_add(ratio_change.sqrt() / (1.0 + ratio_change), -1.0);
        il.abs()
    }
    ...
    pub fn get_top_opportunities(&self, limit: usize) -> Vec<YieldOpportunity> {
        ...
        sorted_opportunities.sort_by(|a, b| b.apy.partial_cmp(&a.apy).unwrap_or(Ordering::Equal));

        sorted_opportunities.into_iter().take(limit).collect()
    }
}

impl Debug for YieldMonitor {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
...
async fn main() -> Result<()> {
    ...
    let metrics_secs: u64 = env::var(ENV_METRICS_INTERVAL_SECS)
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(30);
    let opportunity_secs: u64 = env::var(ENV_OPPORTUNITY_INTERVAL_SECS)
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(10);
    let price_secs: u64 = env::var(ENV_PRICE_INTERVAL_SECS)
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(5);

    let mut metrics_interval = interval(Duration::from_secs(metrics_secs));
    let mut opportunity_interval = interval(Duration::from_secs(opportunity_secs));
    let mut price_interval = interval(Duration::from_secs(price_secs));
    ...
    loop {
        tokio::select! {
            _ = metrics_interval.tick() => {
                ...
                let top_opportunities = yield_monitor.get_top_opportunities(3);
                if !top_opportunities.is_empty() {
                    info!("🏆 Top Yield Opportunities:");
                    for (i, opp) in top_opportunities.iter().enumerate() {
                        info!("   {}. {}/{} - {:.2}% APY (TVL: ${:.0})",
                             i.saturating_add(1), opp.token_pair.0, opp.token_pair.1, opp.apy, opp.tvl);
                    }
                }
            }
            ...
            _ = opportunity_interval.tick() => {
                ...
                let protocol = protocols.get(fastrand::usize(0..protocols.len())).unwrap_or(&"Unknown");
        ...
        let max_secs: u64 = env::var(ENV_MONITOR_DURATION_SECS)
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(180);
...
fn create_simulated_pool_event(protocol: &str) -> Box<dyn Event> {
    let data = match protocol {
        "Raydium" => serde_json::json!({
            ...
            "token_a": "SOL",
            "token_b": "USDC",
            "liquidity_a": fastrand::f64().mul_add(100_000.0, 50_000.0),
            "liquidity_b": fastrand::f64().mul_add(100_000.0, 50_000.0),
            "fee_rate": 0.25,
        ...
        "Orca" => serde_json::json!({
            ...
            "token_mint_a": "SOL",
            "token_mint_b": "USDC",
            "vault_a_amount": fastrand::f64().mul_add(80_000.0, 40_000.0),
            "vault_b_amount": fastrand::f64().mul_add(80_000.0, 40_000.0),
            "fee_rate": 0.30,
        }),
        "Marinade" => serde_json::json!({
            "total_sol": fastrand::f64().mul_add(500_000.0, 100_000.0),
            "total_msol": fastrand::f64().mul_add(480_000.0, 95_000.0),
...
mod tests {
    ...
    fn test_impermanent_loss_calculation() {
        let config = YieldMonitorConfig::default();
        let _monitor = YieldMonitor::new(config);
        ...
        let il = YieldMonitor::calculate_impermanent_loss(initial_ratio, current_ratio);
    ...
    fn test_impermanent_loss_risk_estimation() {
        ...
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            let risk = YieldMonitor::estimate_impermanent_loss_risk("USDC", "USDT");
            assert!(matches!(risk, ImpermanentLossRisk::Low));

            // SOL/mSOL should be low risk
            let risk = YieldMonitor::estimate_impermanent_loss_risk("SOL", "mSOL");
            assert!(matches!(risk, ImpermanentLossRisk::Low));

            // Random tokens should be high risk
            let risk = YieldMonitor::estimate_impermanent_loss_risk("TokenA", "TokenB");
    ...
    async fn test_alert_system() {
        ...
        let result = alert_system.send_alert(&alert);
        assert!(result.is_ok());
    }
}
