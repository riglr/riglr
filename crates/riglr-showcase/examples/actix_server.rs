...
#[cfg(feature = "web-server")]
use core::{error::Error as StdError, fmt as core_fmt};
#[cfg(feature = "web-server")]
use futures_util::stream::{self, BoxStream};
#[cfg(feature = "web-server")]
use rig::providers::anthropic;
#[cfg(feature = "web-server")]
use riglr_showcase::auth::PrivySignerFactory;
#[cfg(feature = "web-server")]
...
#[cfg(feature = "web-server")]
use std::{env, io::Result as IoResult, sync::Arc};
#[cfg(feature = "web-server")]
...
struct RiglrAgent {
    #[allow(dead_code)]
    // TODO: Use this client once rig API is stable
...
#[cfg(feature = "web-server")]
impl core_fmt::Display for AgentError {
...
async fn main() -> IoResult<()> {
    ...
    let anthropic_key = env::var("ANTHROPIC_API_KEY").unwrap_or_else(|_| {
        panic!("Failed to get ANTHROPIC_API_KEY: environment variable not set")
    });
    let port = env::var("PORT")
        .unwrap_or_else(|_| panic!("Failed to get PORT: environment variable not set"))
        .parse::<u16>()
    ...
    let privy_app_id = env::var("PRIVY_APP_ID")
        .unwrap_or_else(|_| panic!("Failed to get PRIVY_APP_ID: environment variable not set"));
    let privy_app_secret = env::var("PRIVY_APP_SECRET")
        .unwrap_or_else(|_| panic!("Failed to get PRIVY_APP_SECRET: environment variable not set"));
    let mut composite = CompositeSignerFactory::default();
    composite.add_factory(
        "privy".to_string(),
        Arc::new(PrivySignerFactory::new(privy_app_id, privy_app_secret)),
    );
    let adapter = Arc::new(ActixRiglrAdapter::new(Arc::new(composite)));
...
mod tests {
    ...
    async fn test_riglr_agent_creation() {
        ...
        let agent = RiglrAgent::new("test-api-key");
