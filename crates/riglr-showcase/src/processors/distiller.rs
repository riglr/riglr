...
Please provide a concise summary of this tool output:",
            ...
            tool_output
                .error
                .as_ref()
                .map_or_else(String::default, |error| format!("Error: {error}"))
        );

        // Here you would call your preferred LLM API
        // For this example, we'll show the pattern with different providers
        match self.model.as_str() {
            m if m.starts_with("gpt-") => return self.call_openai(&user_prompt).await,
            m if m.starts_with("claude-") => return self.call_anthropic(&user_prompt).await,
            m if m.starts_with("gemini-") => return self.call_google(&user_prompt).await,
    ...
    async fn call_anthropic(&self, prompt: &str) -> Result<String> {
        ...
        let model = match self.model.as_str() {
            ...
            "claude-3-5-haiku" | "claude-3-haiku" => anthropic::CLAUDE_3_HAIKU, // Using regular haiku as 3.5 might not be available
...
impl Smart {
    ...
    ///
    /// # Panics
    /// This function will panic if processors vector is empty, but this should never
    /// happen as `Smart::new()` always initializes with 3 processors.
    ...
    fn choose_processor(&self, output: &ToolOutput) -> &DistillationProcessor {
        ...
        match output.tool_name.as_str() {
            name if name.contains("trading") || name.contains("swap") => {
                // Claude for finance (index 1), fallback to GPT-4 (index 0)
                self.processors.get(1).unwrap_or(&self.processors[0])
            }
            name if name.contains("balance") || name.contains("transaction") => {
                // Gemini for simple queries (index 2), fallback to GPT-4 (index 0)
                self.processors.get(2).unwrap_or(&self.processors[0])
            }
            _ => {
                // GPT-4 for general use (index 0)
                &self.processors[0]
            }
        }
    }

    /// Create a new `Smart` with default processors
    #[must_use]
    pub fn new() -> Self {
        Self {
            processors: vec![
...
#[async_trait]
impl OutputProcessor for Smart {
    async fn process(&self, input: ToolOutput) -> Result<ProcessedOutput> {
...
impl Mock {
    /// Create a new `Mock` with default responses
    #[must_use]
    pub fn new() -> Self {
...
mod tests {
    ...
    async fn test_distillation_processor() {
        ...
        let processed = processor
            .process(output)
            .await
            .expect("Failed to process output with DistillationProcessor");
    ...
    async fn test_mock_distiller() {
        let processor = Mock::default().with_response("test_tool", "This is a test summary");

        let output = utils::success_output("test_tool", json!({"result": "success"}));
        let processed = processor
            .process(output)
            .await
            .expect("Failed to process output with Mock");
    ...
    async fn test_smart_distiller() {
        ...
        let processor = Smart::default();
        let output = utils::success_output("trading_tool", json!({"profit": 100}));

        let processed = processor
            .process(output)
            .await
            .expect("Failed to process output with Smart");
        assert!(processed.summary.is_some());
    }

    #[test]
    fn test_processor_selection() {
        let distiller = Smart::default();
    ...
    fn test_distillation_processor_config() {
        ...
        assert_eq!(config.get("name").unwrap(), "DistillationProcessor");
        assert_eq!(config.get("type").unwrap(), "distiller");
        assert_eq!(config.get("model").unwrap(), "test-model");
        assert_eq!(config.get("max_tokens").unwrap(), 100);
        assert_eq!(config.get("temperature").unwrap(), 0.5);
    }
    ...
    async fn test_distillation_processor_process_error_output() {
        ...
        let result = processor
            .process(output)
            .await
            .expect("Failed to process error output with DistillationProcessor");
    ...
    async fn test_call_llm_unsupported_model() {
        ...
        assert!(result
            .expect_err("Should return error for unsupported model")
            .to_string()
    ...
    fn test_smart_distiller_default() {
        let distiller = Smart::default();
        assert_eq!(distiller.processors.len(), 3);
        assert_eq!(distiller.processors.first().unwrap().model, "gpt-4o-mini");
        assert_eq!(
            distiller.processors.get(1).unwrap().model,
            "claude-3-5-haiku"
        );
        assert_eq!(
            distiller.processors.get(2).unwrap().model,
            "gemini-1.5-flash"
        );
    }
    ...
    fn test_smart_distiller_name() {
        let distiller = Smart::default();
        assert_eq!(distiller.name(), "Smart");
    }
    ...
    fn test_smart_distiller_config() {
        ...
        assert_eq!(config.get("name").unwrap(), "Smart");
        assert_eq!(config.get("type").unwrap(), "smart_distiller");
        assert!(config.get("available_models").unwrap().is_array());
        assert_eq!(
            config
                .get("available_models")
                .unwrap()
                .as_array()
                .expect("available_models should be an array")
                .len(),
            3
        );
    }
    ...
    fn test_mock_distiller_new() {
        let distiller = Mock::default();
        assert!(!distiller.responses.is_empty());
    }

    #[test]
    fn test_mock_distiller_with_response() {
        let distiller = Mock::default().with_response("custom_tool", "Custom response");
        assert_eq!(
    ...
    async fn test_mock_distiller_process_success_with_unknown_tool() {
        ...
        let result = distiller
            .process(output)
            .await
            .expect("Failed to process unknown tool with Mock");
    ...
    async fn test_mock_distiller_process_success_with_default_response() {
        let distiller = Mock::default().with_response("default", "Default summary");
        let output = utils::success_output("unknown_tool", json!({"data": "test"}));

        let result = distiller
            .process(output)
            .await
            .expect("Failed to process with default response");
    ...
    async fn test_mock_distiller_process_error() {
        ...
        let result = distiller
            .process(output)
            .await
            .expect("Failed to process error output with Mock");
        assert_eq!(
            result.summary,
    ...
    async fn test_mock_distiller_process_error_no_error_response() {
        let mut distiller = Mock::default();
        ...
        let result = distiller
            .process(output)
            .await
            .expect("Failed to process with no error response");
        assert!(result.summary.is_none());
    }
    ...
    fn test_mock_distiller_config() {
        let distiller = Mock::default();
        let config = distiller.config();
        assert_eq!(config.get("name").unwrap(), "Mock");
        assert_eq!(config.get("type").unwrap(), "mock_distiller");
        assert!(config.get("responses").unwrap().is_number());
        assert!(
            config
                .get("responses")
                .unwrap()
                .as_u64()
                .expect("responses should be a number")
                > 0
        );
    }
    ...
    async fn test_call_llm_gpt_model_routing() {
        ...
        assert!(result
            .expect_err("Should return OpenAI API error")
            .to_string()
            .contains("OpenAI API"));
    }
    ...
    async fn test_call_llm_claude_model_routing() {
        ...
        assert!(result
            .expect_err("Should return Anthropic API error")
            .to_string()
    ...
    async fn test_call_llm_gemini_model_routing() {
        ...
        assert!(result
            .expect_err("Should return Gemini API error")
            .to_string()
            .contains("Gemini API"));
    }
