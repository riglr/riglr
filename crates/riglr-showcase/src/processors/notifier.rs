        ...
        for channel_name in routes {
            if let Some(channel) = self.channels.get(channel_name) {
                ...
                results.push(NotificationResult {
                    ...
                    success: false,
                    message_id: None,
                    error: Some(format!("Channel '{channel_name}' not found")),
                });
            }
        }
...
impl NotificationRouter {
    fn determine_priority(output: &ToolOutput) -> NotificationPriority {
        if !output.success {
...
impl RoutingCondition {
    ...
    pub fn matches(&self, output: &ToolOutput) -> bool {
        match *self {
            Self::Always => true,
            Self::And(ref conditions) => conditions.iter().all(|c| c.matches(output)),
            Self::ExecutionTimeOver(ref threshold) => output.execution_time_ms > *threshold,
            Self::HasMetadata(ref key) => output.metadata.contains_key(key),
            Self::Not(ref condition) => !condition.matches(output),
            Self::OnError => !output.success,
            Self::OnSuccess => output.success,
            Self::Or(ref conditions) => conditions.iter().any(|c| c.matches(output)),
            Self::ToolName(ref name) => output.tool_name == *name,
            Self::ToolNameContains(ref substr) => output.tool_name.contains(substr),
        }
    }
}
...
impl DiscordChannel {
    ...
    fn format_for_discord(&self, output: &ToolOutput) -> serde_json::Value {
        ...
        if !output.result.is_null() {
            if let serde_json::Value::Object(ref obj) = output.result {
        ...
        if output.execution_time_ms > 0 {
            if let Some(f) = embed.get_mut("footer") {
                *f = json!({
        ...
        if let Some(ref username) = self.username {
            if let Some(u) = payload.get_mut("username") {
                *u = json!(username);
            }
        }

        if let Some(ref avatar_url) = self.avatar_url {
            if let Some(a) = payload.get_mut("avatar_url") {
                *a = json!(avatar_url);
            }
        }
    ...
    pub fn with_identity(mut self, username: &str, avatar_url: Option<&str>) -> Self {
        ...
        self.avatar_url = avatar_url.map(ToString::to_string);
        self
    }
...
impl TelegramChannel {
    ...
    fn format_for_telegram(output: &ToolOutput) -> String {
        let status_emoji = if output.success { "✅" } else { "❌" };
        ...
        if response.status().is_success() {
            ...
            result
                ...
                .map_or_else(
                    ...
                    |message_id| Ok(format!("tg_msg_{message_id}")),
                )
        } else {
            let status = response.status();
            let error_text = response
...
impl NotificationChannel for WebhookChannel {
    async fn send_notification(&self, output: &ToolOutput) -> Result<String> {
        ...
        let client = reqwest::Client::new();
        let mut request = client.post(&self.url).timeout(Duration::from_secs(30));
...
mod tests {
    ...
    async fn test_notification_router() {
        ...
        let processed = router
            .process(output)
            .await
            .expect("Failed to process notification router output");
        ...
        let results = processed
            ...
            .unwrap()
            .as_array()
            .expect("notification_results should be an array");
    ...
    async fn test_discord_formatting() {
        ...
        assert!(formatted.get("embeds").unwrap().is_array());
        assert!(formatted.get("username").unwrap() == "RiglrBot");
        assert!(formatted.get("avatar_url").unwrap() == "https://example.com/avatar.png");

        let embed = formatted.get("embeds").unwrap().get(0).unwrap();
        assert!(embed
            .get("title")
            .unwrap()
            .as_str()
        ...
        assert!(embed.get("color").unwrap() == 0x0000_ff00); // Green for success
    }

    #[tokio::test]
    async fn test_telegram_formatting() {
        let _channel = TelegramChannel::new("bot_token", "chat_id");
        let output = utils::error_output("trading_bot", "Insufficient balance");

        let formatted = TelegramChannel::format_for_telegram(&output);
    ...
    fn test_notification_router_name_and_config() {
        ...
        assert_eq!(config.get("name").unwrap(), "NotificationRouter");
        assert_eq!(config.get("type").unwrap(), "notifier");
        assert_eq!(config.get("default_channel").unwrap(), "console");
        assert_eq!(config.get("routing_rules").unwrap(), 0);
    }
    ...
    fn test_determine_routes_with_default_channel() {
        ...
        let output = utils::success_output("test", json!({}));
        let channels = router.determine_routes(&output);

        assert_eq!(channels.len(), 1);
        assert_eq!(channels.first().unwrap(), "console");
    }
    ...
    async fn test_send_notifications_channel_not_found() {
        let router = NotificationRouter::default();
        let output = utils::success_output("test", json!({}));
        let channels = vec!["nonexistent".to_string()];

        let results = router
            .send_notifications(&output, &channels)
            .await
            .expect("Failed to send notifications");

        assert_eq!(results.len(), 1);
        assert!(!results.first().unwrap().success);
        assert_eq!(results.first().unwrap().channel, "nonexistent");
        assert!(results
            .first()
            .unwrap()
            .error
            .as_ref()
            .expect("error should be present for nonexistent channel")
            .contains("Channel 'nonexistent' not found"));
    }

    #[test]
    fn test_determine_priority() {
        let _router = NotificationRouter::default();

        // Test error case
        let error_output = utils::error_output("test", "error");
        assert_eq!(
            NotificationRouter::determine_priority(&error_output),
            NotificationPriority::High
        );
        ...
        assert_eq!(
            NotificationRouter::determine_priority(&trading_output),
            NotificationPriority::Normal
        );
        ...
        assert_eq!(
            NotificationRouter::determine_priority(&swap_output),
            NotificationPriority::Normal
        );
        ...
        assert_eq!(
            NotificationRouter::determine_priority(&other_output),
            NotificationPriority::Low
        );
    }
    ...
    fn test_discord_title_case() {
        let _channel = DiscordChannel::new("https://discord.com/api/webhooks/test");

        assert_eq!(DiscordChannel::title_case("hello_world"), "Hello World");
        assert_eq!(DiscordChannel::title_case("test"), "Test");
        assert_eq!(DiscordChannel::title_case(""), "");
        assert_eq!(DiscordChannel::title_case("a"), "A");
        assert_eq!(DiscordChannel::title_case("one_two_three"), "One Two Three");
    }
    ...
    fn test_discord_format_with_metadata() {
        ...
        let embed = formatted.get("embeds").unwrap().get(0).unwrap();
        assert!(embed.get("fields").unwrap().is_array());
        assert!(embed
            .get("footer")
            .unwrap()
            .get("text")
            .unwrap()
            .as_str()
            .expect("footer text should be a string")
            .contains("1500ms"));
    }
    ...
    fn test_telegram_escape_markdown() {
        ...
        let escaped = TelegramChannel::escape_markdown(text);
    ...
    async fn test_console_channel_send_notification_success_with_colors() {
        ...
        let result = channel
            ...
            .await
            .expect("Failed to send console notification");
    ...
    fn test_notification_result_serialization() {
        ...
        let serialized =
            serde_json::to_string(&result).expect("Failed to serialize NotificationResult");
        let deserialized: NotificationResult =
            serde_json::from_str(&serialized).expect("Failed to deserialize NotificationResult");
    ...
    async fn test_notification_router_process_complete_flow() {
        ...
        assert_eq!(
            processed
                .routing_info
                .as_ref()
                .expect("routing_info should be present")
                .priority,
            NotificationPriority::Low
        );
