...
/// - User interaction fails (Input/Select dialogs)
/// - Recursive demo call fails with invalid parameters
...
pub async fn run_demo(context: Arc<ApplicationContext>, query: String) -> Result<()> {
    ...
    if query.len() >= 2 {
            ...
            token_results.iter().enumerate()
        {
                ...
                || symbol.to_lowercase().contains(&query.to_lowercase())
            {
                ...
                println!("      Market Cap: ${market_cap:.0}");
                let color = if change_24h >= 0.0 { "green" } else { "red" };
                println!(
                    "      24h Change: {}%",
                    match color {
                        "green" => format!("{change_24h:+.1}").bright_green(),
                        _ => format!("{change_24h:+.1}").bright_red(),
                    }
                );
                println!();
            }
    ...
    for (i, &(text, username, likes, retweets)) in sample_tweets.iter().enumerate() {
        println!("   {}. {}", i.saturating_add(1), text);
        println!("      {username} | {likes} ❤️ {retweets} 🔄");
        println!();
    }
    ...
    match selection {
        0 => {
            ...
            Box::pin(run_demo(context, new_query)).await?;
        }
        1 => {
            ...
            for (i, &(name, symbol, price, change)) in trending_tokens.iter().enumerate() {
                ...
                println!(
                    "      24h: {}%",
                    match color {
                        "green" => format!("{change:+.1}").bright_green(),
                        _ => format!("{change:+.1}").bright_red(),
                    }
                );
                println!();
            }
        ...
        3 => {
            ...
            println!("   Simulating analysis for {symbol} across multiple data sources...");
            sleep(Duration::from_millis(800)).await;
...
mod tests {
    ...
    fn test_token_search_logic_when_query_matches_name_should_display() {
        ...
        for &(name, symbol, _address, _price, _market_cap, _change_24h) in &token_results {
    ...
    fn test_trending_tokens_data_integrity() {
        ...
        // Test first token
        let (name, symbol, price, change) = *trending_tokens
            .first()
            .expect("Should have at least one trending token");
    ...
    fn test_latest_news_data_integrity() {
        ...
        let (title, source, time) = *latest_news
            .first()
            .expect("Should have at least one news item");
        assert!(!title.is_empty());
        assert!(!source.is_empty());
