...
use riglr_graph_memory::RawTextDocument;
#[cfg(test)]
...
fn create_test_config() -> Config {
    // Use ConfigBuilder since Config is marked #[non_exhaustive]
    Config::builder()
...
/// - Interactive prompts fail due to terminal issues
/// - Dialog selection interactions fail
...
pub async fn run_demo(
    ...
    // Neo4j client would be initialized here in a real implementation
    // let _neo4j_client = Neo4jClient::new(&config.neo4j_url).await?;
    /*
    ...
    */
    ...
    if init || graph_memory.is_none() {
        ...
        if graph_memory.is_some() {
            ...
            for (i, data) in sample_data.iter().enumerate() {
                let display_index = i.saturating_add(1);
                println!("   {}. {}", display_index, truncate_text(data, 100));
            }
        }
        pb.finish_and_clear();
    }
    ...
    let default_index = options.len().saturating_sub(1);
...
mod tests {
    ...
    fn test_get_sample_blockchain_data_should_return_ten_entries() {
        ...
        assert!(data
            .first()
            .is_some_and(|d| d.contains("Wallet 0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045")));
        assert!(data.first().is_some_and(|d| d.contains("100 SOL")));
        assert!(data.first().is_some_and(|d| d.contains("Jupiter protocol")));

        assert!(data.get(1).is_some_and(|d| d.contains("500 ETH")));
        assert!(data.get(1).is_some_and(|d| d.contains("Ethereum mainnet")));

        assert!(data.get(2).is_some_and(|d| d.contains("Uniswap V3")));
        assert!(data.get(2).is_some_and(|d| d.contains("WETH/USDC")));
    }
    ...
    fn test_sample_data_entries_have_expected_content() {
        ...
        assert!(data
            .first()
            .is_some_and(|d| d.contains("0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045")));
    ...
    fn test_sample_data_specific_entries_content() {
        ...
        assert!(data.get(2).is_some_and(|d| d.contains("Uniswap V3 pool")));
        assert!(data.get(2).is_some_and(|d| d.contains("WETH/USDC")));
        assert!(data.get(2).is_some_and(|d| d.contains("0.05%")));

        // Test entry 4 (index 3) - Compound liquidation
        assert!(data
            .get(3)
            .is_some_and(|d| d.contains("Compound protocol liquidation")));
        assert!(data.get(3).is_some_and(|d| d.contains("10 WBTC")));
        assert!(data.get(3).is_some_and(|d| d.contains("0x789GHI")));

        // Test entry 7 (index 6) - Flash loan attack
        assert!(data.get(6).is_some_and(|d| d.contains("flash loan attack")));
        assert!(data.get(6).is_some_and(|d| d.contains("2M USDC")));
        assert!(data.get(6).is_some_and(|d| d.contains("0x111AAA")));

        // Test entry 8 (index 7) - Staking rewards
        assert!(data.get(7).is_some_and(|d| d.contains("Staking rewards")));
        assert!(data.get(7).is_some_and(|d| d.contains("1000 validators")));
        assert!(data.get(7).is_some_and(|d| d.contains("Solana")));
    }
    ...
    fn test_sample_data_no_empty_strings() {
        ...
        for (index, entry) in data.iter().enumerate() {
            assert!(
                !entry.is_empty(),
                "Entry at index {index} should not be empty"
            );
            assert!(
                !entry.trim().is_empty(),
                "Entry at index {index} should not be only whitespace"
            );
        }
    }
}
