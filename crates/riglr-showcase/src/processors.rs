...
#[path = "processors/distiller.rs"]
pub mod distiller;
#[path = "processors/formatter.rs"]
pub mod formatter;
#[path = "processors/notifier.rs"]
pub mod notifier;
...
pub use formatter::{Html, Json, Markdown, MultiFormatProcessor};

// Re-export distiller types
pub use distiller::{DistillationProcessor, Smart};

pub use distiller::Mock;
...
impl Debug for ProcessorPipeline {
...
impl ProcessorPipeline {
    ...
    /// # Errors
    ///
    /// Returns an error if any processor in the pipeline fails to process the output.
    pub async fn process(&self, mut output: ToolOutput) -> Result<ProcessedOutput> {
        ...
        for processor in &self.processors {
            if processor.can_process(&output) {
                ...
                output.result = mem::take(&mut current.processed_result);
...
pub mod utils {
        ...
        output.error.as_ref().map_or_else(
            ...
            |error| {
                ...
                if error.contains("connection") {
                    ...
                    return "Access denied. Please check your authentication.".to_string();
    ...
    pub fn with_timing(mut output: ToolOutput, start_time: SystemTime) -> ToolOutput {
        if let Ok(duration) = start_time.elapsed() {
            output.execution_time_ms = u64::try_from(duration.as_millis()).unwrap_or(u64::MAX);
        }
        output
    }
}
...
mod tests {
    ...
    impl OutputProcessor for TestProcessor {
        async fn process(&self, input: ToolOutput) -> Result<ProcessedOutput> {
            if self.should_error {
                Err(anyhow::anyhow!("Test processor error"))
            } else {
                Ok(ProcessedOutput {
    ...
    fn test_processed_output_creation() {
        ...
        let routing = processed
            .routing_info
            .expect("routing_info should be Some in test");
    ...
    fn test_pipeline_info() {
        ...
        assert_eq!(info.first().unwrap().get("name").unwrap(), "processor1");
        assert_eq!(info.first().unwrap().get("type").unwrap(), "test_processor");
        assert_eq!(info.get(1).unwrap().get("name").unwrap(), "processor2");
        assert_eq!(info.get(1).unwrap().get("type").unwrap(), "test_processor");
    }
    ...
    async fn test_pipeline_process_empty() {
        ...
        let processed = pipeline
            ...
            .await
            .expect("pipeline processing should succeed in test");
    ...
    async fn test_pipeline_process_with_skip() {
        ...
        assert_eq!(
            processed.processed_result.get("processed_by").unwrap(),
            "process_processor"
        );
        assert_eq!(
            processed.summary,
    ...
    async fn test_pipeline_process_error() {
        ...
        let error = result.expect_err("pipeline should return error in test");
        assert!(error.to_string().contains("Test processor error"));
    }
    ...
    async fn test_pipeline_composition() {
        let pipeline = ProcessorPipeline::default()
            .add_processor(Mock::default())
            .add_processor(Markdown::default());
    ...
    async fn test_output_processor_defaults() {
        ...
        assert_eq!(config.get("type").unwrap(), "test_processor");
        assert_eq!(config.get("custom_field").unwrap(), "test_value");
    }
    ...
    fn test_serialization_tool_output() {
        let output = utils::success_output("test", serde_json::json!({"data": "test"}));
        let serialized =
            serde_json::to_string(&output).expect("serialization should succeed in test");
        let deserialized: ToolOutput =
            serde_json::from_str(&serialized).expect("deserialization should succeed in test");
    ...
    fn test_serialization_processed_output() {
        ...
        let serialized =
            serde_json::to_string(&processed).expect("serialization should succeed in test");
        let deserialized: ProcessedOutput =
            serde_json::from_str(&serialized).expect("deserialization should succeed in test");
    ...
    fn test_serialization_output_format() {
        ...
        for format in formats {
            let serialized =
                serde_json::to_string(&format).expect("serialization should succeed in test");
    ...
    fn test_serialization_notification_priority() {
        ...
        for priority in priorities {
            let serialized =
                serde_json::to_string(&priority).expect("serialization should succeed in test");
    ...
    fn test_serialization_routing_info() {
        ...
        let serialized =
            serde_json::to_string(&routing_info).expect("serialization should succeed in test");
        let deserialized: RoutingInfo =
            serde_json::from_str(&serialized).expect("deserialization should succeed in test");
