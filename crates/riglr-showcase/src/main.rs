...
use riglr_solana_tools::clients::Clients;
use riglr_web_tools::client::{HttpConfig, WebClient};
...
async fn main() -> Result<()> {
    ...
    let api_clients = Arc::new(Clients::new(&config.providers));
    context.set_extension(api_clients);

    // 4. Web Client
    let web_client = Arc::new(WebClient::with_config(HttpConfig::default())?);
    ...
    match cli.command {
        ...
        Commands::Interactive => {
            commands::interactive::run_chat(&context)?;
        }
    }
...
fn init_logging(verbose: bool) {
    ...
    let filter_string = format!("riglr_showcase={level},riglr_core={level},riglr_solana_tools={level},riglr_evm_tools={level},riglr_web_tools={level},riglr_graph_memory={level}");
...
mod tests {
    ...
    fn test_cli_parsing_with_solana_command() {
        ...
        let cli = Cli::try_parse_from(args).expect("Failed to parse CLI args in test");

        assert!(!cli.verbose);
        assert_eq!(cli.config, ".env");

        if let Commands::Solana { address } = cli.command {
            assert_eq!(
                address,
                Some("11111111111111111111111111111111".to_string())
            );
        } else {
            panic!("Expected Solana command, got: {:?}", cli.command);
        }
    }
    ...
    fn test_cli_parsing_with_evm_command_full_args() {
        ...
        if let Commands::Evm { address, chain_id } = cli.command {
            assert_eq!(
                address,
            ...
            panic!("Expected Evm command, got: {:?}", cli.command);
        }
    }
    ...
    fn test_cli_parsing_with_web_command() {
        ...
        if let Commands::Web { query } = cli.command {
            assert_eq!(query, "blockchain analysis");
        } else {
            panic!("Expected Web command, got: {:?}", cli.command);
        }
    }
    ...
    fn test_cli_parsing_with_graph_command_init_only() {
        ...
        if let Commands::Graph { init, query } = cli.command {
            assert!(init);
            assert_eq!(query, None);
        } else {
            panic!("Expected Graph command, got: {:?}", cli.command);
        }
    }
    ...
    fn test_cli_parsing_with_cross_chain_command() {
        ...
        if let Commands::CrossChain { token } = cli.command {
            assert_eq!(token, "USDC");
        } else {
            panic!("Expected CrossChain command, got: {:?}", cli.command);
        }
    }
    ...
    fn test_cli_parsing_with_agents_command_default_scenario() {
        ...
        if let Commands::Agents { scenario } = cli.command {
            assert_eq!(scenario, "trading"); // default value
        } else {
            panic!("Expected Agents command, got: {:?}", cli.command);
        }
    }
    ...
    fn test_cli_parsing_with_interactive_command() {
        ...
        if matches!(cli.command, Commands::Interactive) {
            ...
            panic!("Expected Interactive command, got: {:?}", cli.command);
        }
    }
