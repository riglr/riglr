...
impl ProviderConfig for MagicConfig {
    fn validate(&self) -> Result<(), AuthError> {
        if self.publishable_key.is_empty() {
            return Err(AuthError::ConfigError("Magic publishable key required".to_string()));
        }
        if self.secret_key.is_empty() {
            return Err(AuthError::ConfigError("Magic secret key required".to_string()));
        }
        Ok(())
    }
    ...
    pub fn new(config: MagicConfig) -> Self {
        let client = reqwest::Client::builder()
            .default_headers({
                ...
                headers.insert(
                    "X-Magic-Secret-Key",
                    config.secret_key.parse().unwrap(),
                );
                headers
            })
            .build()
...
impl SignerFactory for MagicProvider {
    async fn create_signer(
        ...
        let _token = auth_data.credentials.get("token")
