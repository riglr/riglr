...
async fn test_parallel_client_configurations() {
    ...
    println!("Successfully created {} clients in parallel", successful_configs.len());
...
async fn test_thread_safe_signer_isolation() {
    ...
    for (task_id, key_valid, has_signer, addr) in results {
        println!("Task {}: key_valid={}, has_signer={}, addr={:?}", 
...
async fn test_parallel_resource_cleanup() {
    ...
    println!("Resource cleanup test: {} {} {} clients created", 
...
async fn test_concurrent_error_isolation() {
    ...
    let operations = vec![
        ("valid_balance", "0x0000000000000000000000000000000000000000"),
        ("invalid_address", "invalid_address_format"),
        ("another_valid", "0x0000000000000000000000000000000000000001"),
        ("empty_address", ""),
    ...
    let invalid_ops: Vec<_> = results.iter()
...
async fn test_high_concurrency_load() {
    ...
    println!("High concurrency test: {} completed, {} timed out, {} successful", 
...
async fn test_client_state_consistency() {
    ...
    for (i, client) in clients.into_iter().enumerate() {
        let handle = tokio::spawn(async move {
            ...
            let consistent = chain_id1 == chain_id2 
                && rpc_url1 == rpc_url2 
                && has_signer1 == has_signer2;
    ...
    for (task_id, consistent, chain_id, has_signer) in &results {
        println!("  Task {}: consistent={}, chain_id={}, has_signer={}", 
                task_id, consistent, chain_id, has_signer);
    }
    
    // All clients should have consistent state
    assert!(all_consistent, "Client state was not consistent across concurrent access");
