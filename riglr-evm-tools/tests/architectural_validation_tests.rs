...
use riglr_evm_tools::{
    balance::get_eth_balance, client::EvmClient, error::EvmToolError, transaction::transfer_eth,
};
...
async fn test_client_cloning_thread_safety() {
    ...
    if let Ok(client) = client_result {
        ...
        assert!(
            client1.provider().get_chain_id().await.is_ok()
                || client1.provider().get_chain_id().await.is_err()
        );
        assert!(
            client2.provider().get_chain_id().await.is_ok()
                || client2.provider().get_chain_id().await.is_err()
        );
    }
}
...
async fn test_signer_requirement_enforcement() {
    let client = match EvmClient::mainnet().await {
        Ok(client) => client,
        Err(_) => match EvmClient::anvil().await {
            Ok(client) => client,
            Err(_) => {
    ...
    assert!(
        error_msg.contains("signer") || error_msg.contains("Client requires signer configuration"),
...
async fn test_multi_chain_operations() {
    ...
    if successful_clients.len() >= 2 {
        ...
        println!(
            "Multi-chain operations completed: {:?}, {:?}",
            result1.is_ok(),
            result2.is_ok()
        );
    }
    ...
    println!(
        "Multi-chain test completed with {} successful clients",
...
async fn test_error_handling_independence() {
    ...
    println!(
        "Another client creation result: {}",
        another_client_result.is_ok()
    );
}
