...
use riglr_graph_memory::{
    document::{Document, DocumentMetadata, DocumentChunk},
    graph::{GraphMemory, Node, Edge, RelationType},
    vector_store::{InMemoryVectorStore, SimilarityMetric},
    extractor::{EntityExtractor, Entity, EntityType},
...
fn vector_store_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("store_creation", |b| {
        b.iter(|| {
            InMemoryVectorStore::new(black_box(384))
        })
    });
...
fn error_handling_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("error_database_error", |b| {
        b.iter(|| {
            GraphMemoryError::DatabaseError(black_box("Connection failed".to_string()))
        })
    });
    ...
    group.bench_function("error_not_found", |b| {
        b.iter(|| {
            GraphMemoryError::NotFound(black_box("Document not found".to_string()))
        })
    });
...
fn throughput_benchmarks(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("create_nodes", size),
            size,
            |b, &size| {
                b.iter(|| {
