...
fn test_entity_extraction_error() {
    ...
    assert_eq!(
        error.to_string(),
        "Entity extraction error: Failed to parse text"
    );

    let error2 = GraphMemoryError::EntityExtraction("No entities found".to_string());
    assert_eq!(
        error2.to_string(),
        "Entity extraction error: No entities found"
    );
}
...
fn test_generic_error() {
    ...
    assert_eq!(
        error2.to_string(),
        "Graph memory error: Operation cancelled"
    );
}
...
fn test_http_error_conversion() {
    ...
    let result = runtime
        .block_on(async { reqwest::get("http://invalid-domain-graph-test-12345.com").await });
...
fn test_error_chain() {
    ...
    fn outer_operation() -> Result<()> {
        inner_operation().map_err(|e| GraphMemoryError::Generic(format!("Operation failed: {}", e)))
    }
...
fn test_error_variants_display() {
    ...
    assert!(entity_err
        .to_string()
        .starts_with("Entity extraction error:"));
